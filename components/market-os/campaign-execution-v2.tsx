...
function Button({ href, children, onClick, kind = "soft" }: { href?: string; children: React.ReactNode; onClick?: () => void; kind?: "primary" | "dark" | "soft" | "danger" }) {
  ...
  const styles = kind === "primary" ? "bg-emerald-600 text-slate-950 shadow-emerald-100" : kind === "dark" ? "bg-white text-slate-950" : kind === "danger" ? "bg-rose-600 text-slate-950" : "border border-slate-200 bg-white text-slate-800"
...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="mb-2 block text-xs font-black uppercase tracking-[0.18em] text-slate-9500">{label}</span>{children}</label> }
const inputClass = "w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-950 outline-none placeholder:text-slate-500 focus:border-emerald-700 focus:ring-4 focus:ring-emerald-100"

function Shell({ children }: { children: React.ReactNode }) {
  return <main data-market-os-root className="min-h-screen bg-slate-50 text-slate-950"><div className="mx-auto max-w-[1850px] space-y-6 p-4 lg:p-8">{children}</div></main>
}
...
function Metric({ label, value, note }: { label: string; value: string | number; note: string }) { return <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500">{label}</p><p className="mt-2 text-3xl font-black text-slate-950">{value}</p><p className="mt-1 text-sm font-bold text-slate-9500">{note}</p></div> }
...
export function MainCampaignBoard() {
  ...
  return <Shell>
    ...
    <Panel><Header title="Execution log" subtitle="Every real action writes to this log for traceability." /> <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{store.logs.slice(0,8).map(log=><div key={log.id} className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">{log.action}</p><p className="mt-2 text-sm font-bold text-slate-700">{log.detail}</p><p className="mt-1 text-xs text-slate-500">{new Date(log.at).toLocaleString()}</p></div>)}</div></Panel>
  </Shell>
}
