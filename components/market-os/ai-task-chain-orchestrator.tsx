    ...
    <main data-market-os-root className="min-h-screen bg-slate-50 p-6 text-slate-950">
      <section className="mx-auto max-w-7xl space-y-6">
        <div className="rounded-[2rem] bg-white p-8 text-slate-950">
          ...
          <p className="mt-4 max-w-3xl text-slate-600">Executable task-chain control: create, start, progress, block, complete and delete chains.</p>
          <div className="mt-8 grid gap-3 md:grid-cols-4">
            <div className="rounded-3xl bg-white/10 p-4"><p className="text-xs uppercase text-slate-600">Running</p><p className="mt-2 text-3xl font-black">{running}</p></div>
            ...
            <div className="rounded-3xl bg-white/10 p-4"><p className="text-xs uppercase text-slate-600">Avg Progress</p><p className="mt-2 text-3xl font-black">{avgProgress}%</p></div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <button onClick={createChain} className="mt-4 rounded-2xl bg-white px-5 py-3 text-sm font-bold text-slate-950">Create Chain</button>
        </div>
        ...
        <div className="grid gap-5">
          {filtered.map((chain) => (
            <article key={chain.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p className="mt-1 text-sm text-slate-9500">Linked: {chain.linked_objective || "—"}</p>
              <div className="mt-5 rounded-2xl border border-slate-200 p-4"><div className="mb-2 flex justify-between text-sm"><span className="font-bold">Progress</span><span>{Number(chain.progress || 0)}%</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-white" style={{ width: `${Number(chain.progress || 0)}%` }} /></div></div>
              <div className="mt-5 grid gap-2 md:grid-cols-4">{[chain.step_1, chain.step_2, chain.step_3, chain.step_4].map((step, i) => <div key={i} className="rounded-2xl border border-slate-200 p-3 text-sm"><p className="text-xs font-bold uppercase text-slate-9500">Step {i + 1}</p><p className="mt-1">{step || "Not defined"}</p></div>)}</div>
              <div className="mt-5 flex flex-wrap gap-2">
                <button onClick={() => updateChain(chain, { status: "running", progress: Math.max(Number(chain.progress || 0), 10) }, "Start Chain")} disabled={loadingId === chain.id} className="rounded-2xl bg-white px-4 py-2 text-sm font-bold text-slate-950 disabled:opacity-50">Start Chain</button>
