...
export function Button({ children, href, onClick, kind="soft", type="button" }:{children:React.ReactNode; href?:string; onClick?:()=>void; kind?:"primary"|"soft"|"danger"|"success"|"dark"; type?:"button"|"submit"}) {
  const cls = kind==="primary" ? "bg-emerald-500 text-slate-950 shadow-lg shadow-emerald-200 hover:bg-emerald-400" : kind==="danger" ? "bg-rose-600 text-white hover:bg-rose-700" : kind==="success" ? "bg-emerald-700 text-white hover:bg-emerald-800" : kind==="dark" ? "bg-slate-950 text-white hover:bg-slate-800" : "border border-slate-200 bg-white text-slate-900 hover:bg-slate-50"
...
export function DarkPanel({ children, className="" }:{children:React.ReactNode; className?:string}) { return <section className={`rounded-3xl border border-emerald-400/20 bg-slate-950 text-white shadow-2xl shadow-emerald-950/20 ${className}`}>{children}</section> }
...
export function Badge({ children, tone="slate" }:{children:React.ReactNode; tone?:"slate"|"emerald"|"amber"|"rose"|"blue"}) {
  ...
  return <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-black uppercase ${c}`}>{children}</span>
}
...
export function Metric({ label, value, sub, tone="slate" }:{label:string; value:string; sub:string; tone?:"slate"|"emerald"|"rose"|"amber"}) {
  return <Panel className="p-5"><p className="text-xs font-black uppercase tracking-wider text-slate-400">{label}</p><p className="mt-2 text-3xl font-black text-slate-950">{value}</p><p className="mt-1 text-sm font-bold text-slate-500">{sub}</p></Panel>
}
export function Shell({ children }:{children:React.ReactNode}) { return <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#064e3b_0,#f8fafc_28%,#f8fafc_100%)] text-slate-950 selection:bg-emerald-200 selection:text-slate-950">{children}</div> }
