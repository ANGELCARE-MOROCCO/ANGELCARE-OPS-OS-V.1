...
function defaultStore(): Store {
  return {
    ...
    tasks: seedTasks(),
    logs: [{ id: uid(), at: new Date().toLocaleString(), action: "Workspace initialized", detail: "Campaign cockpit seed data loaded." }],
  }
}
...
function PrimaryButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button {...props} className={`rounded-2xl bg-emerald-700 px-5 py-3 text-sm font-black text-white shadow-lg transition hover:bg-emerald-800 ${props.className || ""}`} />
}

function DarkButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button {...props} className={`rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-slate-800 ${props.className || ""}`} />
}
...
function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...props} className={`min-h-[110px] w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-950 outline-none placeholder:text-slate-400 focus:border-emerald-700 focus:ring-4 focus:ring-emerald-100 ${props.className || ""}`} />
}

function StatCard({ label, value, note, tone }: { label: string; value: string | number; note: string; tone: string }) {
  return (
    <div className={`rounded-3xl p-5 text-white shadow-lg ${tone}`}>
      <p className="text-xs font-black uppercase tracking-[0.2em] text-white/70">{label}</p>
      <p className="mt-3 text-4xl font-black">{value}</p>
      <p className="mt-2 text-sm font-bold text-white/80">{note}</p>
    </div>
  )
}
...
export default function CampaignExecutionV2() {
  ...
  function commit(next: Store, action: string, detail: string) {
    const withLog = {
      ...next,
      logs: [{ id: uid(), at: new Date().toLocaleString(), action, detail }, ...next.logs].slice(0, 60),
    }
    setStore(withLog)
    writeStore(withLog)
  }
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950 selection:bg-emerald-200 selection:text-slate-950">
      <div className="mx-auto max-w-[1800px] space-y-6 p-4 lg:p-8">
        <section className="overflow-hidden rounded-[2.2rem] bg-gradient-to-br from-slate-950 via-emerald-950 to-black p-7 text-white shadow-2xl lg:p-10">
          <div className="grid gap-8 xl:grid-cols-[1.3fr_.7fr]">
            <div>
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <Link href="/market-os" className="rounded-2xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-black text-white">← Market-OS</Link>
                ...
                <button onClick={() => setActivePanel("create")} className="rounded-2xl bg-emerald-500 px-5 py-3 text-sm font-black text-white shadow-lg">+ Create campaign</button>
                <button onClick={() => selected && updateCampaign(selected.id, { stage: "live", readiness: 100 }, "Campaign launched")} className="rounded-2xl border border-white/20 bg-white/10 px-5 py-3 text-sm font-black text-white">Launch selected</button>
              </div>
            </div>
          ...
          {gateways.map(gateway => (
            <button
              ...
              className={`rounded-[1.75rem] border p-5 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-xl ${
                activePanel === gateway.key ? "border-emerald-700 bg-emerald-950 text-white" : "border-slate-200 bg-white text-slate-950"
              }`}
            >
              <div className="text-3xl">{gateway.icon}</div>
              <h3 className={`mt-4 text-lg font-black ${activePanel === gateway.key ? "text-white" : "text-slate-950"}`}>{gateway.title}</h3>
              ...
              <span className={`mt-4 inline-flex rounded-xl px-3 py-2 text-xs font-black ${activePanel === gateway.key ? "bg-white text-emerald-950" : "bg-slate-950 text-white"}`}>
                {gateway.action}
              </span>
            </button>
          ))}
        ...
        <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
                  ...
                  <div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      <div className="rounded-2xl bg-slate-50 p-4">
                        <p className="text-xs font-black uppercase text-slate-400">Leads</p>
                        <p className="text-2xl font-black">{campaign.leads}</p>
                      </div>
                      <div className="rounded-2xl bg-slate-50 p-4">
                        <p className="text-xs font-black uppercase text-slate-400">ROI</p>
                        <p className="text-2xl font-black">{campaign.spentMad ? Math.round((campaign.revenueMad / campaign.spentMad) * 10) / 10 : 0}x</p>
                      </div>
                    </div>
                  </div>
                  <div className="grid gap-2">
                    <Link href={`/market-os/campaign-lifecycle/${campaign.id}`} className="rounded-2xl bg-slate-950 px-4 py-3 text-center text-sm font-black text-white">Open command page</Link>
                    ...
                    <button onClick={() => deleteCampaign(campaign.id)} className="rounded-2xl bg-rose-600 px-4 py-3 text-sm font-black text-white">Delete campaign</button>
                  </div>
                </div>
              </Card>
            ))}
          ...
          <aside className="space-y-6">
            <Card className="bg-slate-950 text-white">
              ...
              <h2 className="mt-2 text-3xl font-black text-white">{selected?.name || "No campaign selected"}</h2>
              {selected ? (
              ...
              <div className="mt-4 space-y-3">
                {selectedTasks.map(task => (
                  <div key={task.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <button onClick={() => deleteTask(task.id)} className="mt-3 rounded-xl bg-rose-600 px-3 py-2 text-xs font-black text-white">Delete task</button>
                  </div>
                ))}
  ...
  return (
    <section className="min-h-screen bg-slate-50 p-4 text-slate-950 lg:p-8">
      <div className="mx-auto max-w-[1600px] space-y-6">
        <div className="rounded-[2rem] bg-gradient-to-br from-slate-950 via-emerald-950 to-black p-7 text-white shadow-2xl">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-4xl text-sm font-semibold leading-7 text-white/75">{subtitle}</p>
            </div>
