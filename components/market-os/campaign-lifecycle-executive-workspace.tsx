...
const labelClass = 'text-xs font-black uppercase tracking-[0.18em] text-slate-500'
      ...
      <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="mb-4 flex flex-wrap gap-2">
            <span className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-white/80">Market-OS</span>
            <span className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-black uppercase tracking-[0.2em] text-white/80">Campaign Lifecycle</span>
        ...
        <div className="grid min-w-[280px] gap-3 rounded-3xl border border-white/10 bg-white/10 p-4 backdrop-blur">
          ...
          <Link href="/market-os/campaign-lifecycle/launch-control" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/15 px-5 py-3 text-sm font-black text-white"><Rocket className="h-4 w-4" /> Launch control</Link>
          <p className="text-center text-xs font-bold text-slate-300">Current room: {mode}</p>
        </div>
      </div>
    </section>
  )
    ...
    <article className={`rounded-[2rem] border bg-white p-5 shadow-sm transition ${selected ? 'border-slate-950 ring-4 ring-slate-100' : 'border-slate-200'}`}>
      <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">{campaign.strategy} · Owner: {campaign.owner} · {campaign.funnelStage}</p>
        </div>
        <div className="grid gap-2 sm:grid-cols-2 xl:w-[360px]">
          ...
          <div className="rounded-2xl bg-slate-50 p-4"><p className={labelClass}>Budget</p><p className="mt-1 text-2xl font-black">{formatMad(campaign.budgetMad)}</p><p className="text-xs font-bold text-slate-500">Spent {formatMad(campaign.spentMad)}</p></div>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-4">
        <div><div className="mb-2 flex justify-between text-xs font-black uppercase text-slate-500"><span>Readiness</span><span>{campaign.readiness}%</span></div><Bar value={campaign.readiness} /></div>
        <div><div className="mb-2 flex justify-between text-xs font-black uppercase text-slate-500"><span>Confidence</span><span>{campaign.confidence}%</span></div><Bar value={campaign.confidence} /></div>
        <div><div className="mb-2 flex justify-between text-xs font-black uppercase text-slate-500"><span>Tasks</span><span>{calculateTaskCompletion(campaign)}%</span></div><Bar value={calculateTaskCompletion(campaign)} /></div>
        <div><div className="mb-2 flex justify-between text-xs font-black uppercase text-slate-500"><span>Budget burn</span><span>{calculateBudgetBurn(campaign)}%</span></div><Bar value={calculateBudgetBurn(campaign)} /></div>
      </div>
      <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">Launch blockers / warnings</p>
        <div className="mt-2 flex flex-wrap gap-2">{warnings.map((warning) => <span key={warning} className="rounded-full border border-amber-200 bg-white px-3 py-1 text-xs font-black text-amber-700">{warning}</span>)}</div>
      </div>
      <div className="mt-5 flex flex-wrap gap-2">
        <Link href={`/market-os/campaign-lifecycle/${campaign.id}`} className="inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white"><Eye className="h-4 w-4" /> Open control room</Link>
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_180px_180px]">
          <div className="relative"><Search className="absolute left-4 top-3.5 h-4 w-4 text-slate-400" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search campaign, owner, offer, city, audience..." className={`${inputClass} pl-11`} /></div>
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          <div className="rounded-2xl bg-slate-50 p-4"><p className="font-black">Average executive score</p><p className="mt-2 text-3xl font-black">{avgScore}%</p><p className="mt-1 text-sm font-semibold text-slate-500">Combines readiness, confidence, tasks, budget and risk.</p></div>
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-4xl text-sm font-semibold leading-6 text-slate-500">{campaign.hook}</p>
          </div>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm"><p className={labelClass}>Strategy brief</p><div className="mt-4 grid gap-4">{[['Objective', campaign.objective], ['Audience', campaign.audience], ['Persona', campaign.persona], ['Offer', campaign.offer], ['Service line', campaign.serviceLine], ['Funnel', campaign.funnelStage], ['CTA', campaign.cta], ['Landing path', campaign.landingPath], ['Decision needed', campaign.decisionNeeded], ['Compliance', campaign.complianceNote]].map(([a,b]) => <div key={a} className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">{a}</p><p className="mt-1 text-sm font-bold text-slate-800">{b}</p></div>)}</div></div>
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm"><p className={labelClass}>Launch warnings</p><div className="mt-4 grid gap-3">{campaignReadinessWarnings(campaign).map((warning) => <div key={warning} className="flex items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-black text-amber-800"><AlertTriangle className="h-4 w-4" />{warning}</div>)}</div></div>
      </section>
      <section className="grid gap-5 lg:grid-cols-2"><Panel title="Execution tasks" items={campaign.tasks.map((task) => ({title: task.title, meta: `${task.department} · ${task.owner} · Due ${task.due}`, status: task.status, note: task.impact}))} /><Panel title="Campaign assets" items={campaign.assets.map((asset) => ({title: asset.name, meta: `${asset.type} · ${asset.owner}`, status: asset.status, note: 'Required for campaign quality and launch governance.'}))} /></section>
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm"><p className={labelClass}>Experiment room</p><div className="mt-4 grid gap-4">{campaign.experiments.map((experiment) => <div key={experiment.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-black">{experiment.name}</p><span className={`rounded-full border px-3 py-1 text-xs font-black ${badgeClass(experiment.status)}`}>{experiment.status}</span></div><p className="mt-2 text-sm font-semibold text-slate-500">{experiment.hypothesis}</p><div className="mt-3 grid gap-3 md:grid-cols-3"><div className="rounded-xl bg-slate-50 p-3 text-sm font-bold">A: {experiment.variantA}</div><div className="rounded-xl bg-slate-50 p-3 text-sm font-bold">B: {experiment.variantB}</div><div className="rounded-xl bg-slate-50 p-3 text-sm font-bold">Metric: {experiment.decisionMetric}</div></div></div>)}</div></section>
    </div>
  )
}

function Panel({ title, items }: { title: string; items: { title: string; meta: string; status: string; note: string }[] }) {
  return <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm"><p className={labelClass}>{title}</p><div className="mt-4 grid gap-3">{items.map((item) => <div key={item.title} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-black text-slate-950">{item.title}</p><p className="mt-1 text-xs font-bold text-slate-500">{item.meta}</p></div><span className={`rounded-full border px-3 py-1 text-xs font-black ${badgeClass(item.status)}`}>{item.status}</span></div><p className="mt-2 text-sm font-semibold text-slate-600">{item.note}</p></div>)}</div></section>
}
