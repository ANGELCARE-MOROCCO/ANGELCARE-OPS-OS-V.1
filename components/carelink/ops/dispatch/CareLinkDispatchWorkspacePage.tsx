...
export function CareLinkDispatchWorkspacePage({ view, initialPayload }: { view: DispatchWorkspaceView; initialPayload?: DispatchPayload }) {
  ...
  const cities = useMemo(() =>
       Array.from(new Set([...data.missions.map((m) => m.city || ''), ...data.sectors.map((s) => s.city_name || '')].filter(Boolean))).sort(), [data.missions, data.sectors])
  const statuses = useMemo(() => Array.from(new Set(data.missions.map((m) => String(m.status || '')).filter(Boolean))).sort(), [data.missions])

  const refresh = useCallback(async () => {
    setBusy(true)
    try {
      const endpoint = view === 'dispatch-board' ? '/api/carelink/ops/live-missions' : `/api/carelink/ops/live-missions/${view}`
  ...
  const runAction = useCallback(async (body: Record<string, unknown>) => {
    setBusy(true)
    try {
      const res = await fetch('/api/carelink/ops/live-missions/workspace-action', {
        method: 'POST',
