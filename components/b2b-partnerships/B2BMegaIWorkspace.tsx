...
export default function B2BMegaIWorkspace({ mode }: { mode: Mode }) {
  ...
  return (
    <main className={styles.workspace}>
      ...
      {mode === 'campaigns' && <CampaignsView campaigns={campaigns} sequences={sequences} advanceCampaign={advanceCampaign} busy={busy} />}
...
function ImportsView({ imports, promoteImport, busy }: { imports: AnyRow[]; promoteImport: (id:string)=>void; busy:boolean }) {
  return <section className={styles.grid}><div className={styles.panel}><div className={styles.panelHeader}><div><h2>Import batches</h2><p>Validation, erreurs, lignes promues et staging avant CRM.</p></div></div><div className={styles.tableWrap}><table><thead><tr><th>Batch</th><th>Status</th><th>Rows</th><th>Valid</th><th>Promoted</th><th>Action</th></tr></thead><tbody>{imports.map((row)=><tr key={row.id}><td><strong>{row.name}</strong><small>{row.segment || row.source}</small></td><td><span className={styles.badge}>{row.status}</span></td><td>{row.total_rows}</td><td>{row.valid_rows}</td><td>{row.promoted_rows}</td><td><button className={styles.secondaryButton} disabled={busy} onClick={()=>promoteImport(row.id)}>Promote</button></td></tr>)}{!imports.length && <tr><td colSpan={6}>Aucun import. Créez votre premier batch hôtel / clinique.</td></tr>}</tbody></table></div></div><aside className={styles.panel}><div className={styles.panelHeader}><div><h2>CSV standards</h2><p>Colonnes recommandées pour accélérer l’import.</p></div></div><div className={styles.cards}><div className={styles.card}><h3>Required</h3><p>name, sector, city, phone, email</p></div><div className={styles.card}><h3>Decision maker</h3><p>decision_maker_name, decision_maker_role, decision_maker_email</p></div><div className={styles.card}><h3>Commercial fit</h3><p>priority_score, potential_service_fit, opportunity_description</p></div></div></aside></section>
}
function CampaignsView({ campaigns, sequences, advanceCampaign, busy }: { campaigns: AnyRow[]; sequences: AnyRow[]; advanceCampaign:(id:string)=>void; busy:boolean }) {
  return <section className={styles.grid}><div className={styles.panel}><div className={styles.panelHeader}><div><h2>Campaign execution</h2><p>Avancer les campagnes et générer les prochaines tâches de séquence.</p></div></div><div className={styles.tableWrap}><table><thead><tr><th>Campaign</th><th>Status</th><th>Enrolled</th><th>Objective</th><th>Action</th></tr></thead><tbody>{campaigns.map((row)=><tr key={row.id}><td><strong>{row.name}</strong><small>{row.segment}</small></td><td><span className={styles.badge}>{row.status}</span></td><td>{row.enrolled_count || 0}</td><td>{row.objective || '—'}</td><td><button className={styles.secondaryButton} disabled={busy} onClick={()=>advanceCampaign(row.id)}>Advance</button></td></tr>)}{!campaigns.length && <tr><td colSpan={5}>Aucune campagne. Créez une séquence multi-touch.</td></tr>}</tbody></table></div></div><aside className={styles.panel}><div className={styles.panelHeader}><div><h2>Sequences</h2><p>Email, WhatsApp, call, LinkedIn par ordre d’exécution.</p></div></div><div className={styles.cards}>{sequences.map(seq=><div className={styles.card} key={seq.id}><h3>{seq.name}</h3><p>{seq.objective || seq.segment}</p></div>)}{!sequences.length && <div className={styles.card}><h3>Recommended 14-day rhythm</h3><p>Day 0 email, Day 2 WhatsApp, Day 4 call, Day 7 recap, Day 10 LinkedIn, Day 14 reactivation.</p></div>}</div></aside></section>
}
