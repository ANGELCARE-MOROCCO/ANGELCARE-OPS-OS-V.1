  ...
  return (
    <aside className="space-y-4">
      <div className="rounded-[2rem] border border-emerald-400/40 bg-black p-5 font-mono shadow-2xl shadow-emerald-950/40">
        ...
        {['SMTP route armed', 'IMAP signal nominal', 'Revenue linker active', 'Access matrix synced', 'Bulk engine standing by', 'Provider setup guard active'].map((line, i) => (
      ...
      <div className="rounded-[2rem] border border-slate-700/80 bg-[#0b1220] p-5">
        <h3 className="text-lg font-black text-white">Recent local actions</h3>
        <p className="mt-1 text-xs leading-5 text-slate-400">These actions persist in browser storage so you can validate UI execution without external email credentials.</p>
        <div className="mt-4 space-y-2">
          {(storedActions.length ? storedActions : ['No local action yet. Click Run/Save buttons to validate execution state.']).map((op) => (
      ...
      <div className="rounded-[2rem] border border-amber-400/30 bg-amber-400/10 p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-black text-white">True production boundary</h3>
            <p className="mt-2 text-sm leading-6 text-amber-100/90">This pack includes backend-ready API stubs and local state validation. Real send/receive requires Google/Microsoft/SMTP credentials and database connection values in your environment.</p>
          </div>
        </div>
      </div>
    </aside>
