...
const initialState: AppState = {
  mailboxes: [
    { id: 'mbx-ops', name: 'Operations', address: 'operations@angelcare.ma', department: 'Operations', owner: 'Operations Lead', provider: 'Menara SMTP/IMAP', status: 'Needs setup', inbound: 184, outbound: 92, unresolved: 38, slaRisk: 13, sendEnabled: true, receiveEnabled: true, signature: 'AngelCare Operations', routingRule: 'Route care schedule cases to Operations' },
    { id: 'mbx-family', name: 'Family Support', address: 'families@angelcare.ma', department: 'Care', owner: 'Care Experience', provider: 'Menara SMTP/IMAP', status: 'Needs setup', inbound: 132, outbound: 74, unresolved: 25, slaRisk: 10, sendEnabled: true, receiveEnabled: true, signature: 'AngelCare Family Support', routingRule: 'Family issues escalate if critical' },
    { id: 'mbx-billing', name: 'Billing', address: 'billing@angelcare.ma', department: 'Finance', owner: 'Billing Manager', provider: 'Menara SMTP/IMAP', status: 'Warning', inbound: 96, outbound: 41, unresolved: 22, slaRisk: 9, sendEnabled: true, receiveEnabled: true, signature: 'AngelCare Billing', routingRule: 'Invoice and payment messages to Finance' },
    { id: 'mbx-sales', name: 'Sales', address: 'sales@angelcare.ma', department: 'Revenue', owner: 'Sales Director', provider: 'Menara SMTP/IMAP', status: 'Needs setup', inbound: 119, outbound: 151, unresolved: 18, slaRisk: 6, sendEnabled: true, receiveEnabled: true, signature: 'AngelCare Sales', routingRule: 'Leads and proposals to revenue board' },
    { id: 'mbx-legal', name: 'Legal', address: 'legal@angelcare.ma', department: 'Legal', owner: 'Legal Admin', provider: 'Menara SMTP/IMAP', status: 'Restricted', inbound: 31, outbound: 17, unresolved: 6, slaRisk: 2, sendEnabled: false, receiveEnabled: true, signature: 'AngelCare Legal Notice', routingRule: 'Block send without approval' },
    { id: 'mbx-hr', name: 'HR', address: 'hr@angelcare.ma', department: 'HR', owner: 'HR Manager', provider: 'Menara SMTP/IMAP', status: 'Needs setup', inbound: 51, outbound: 28, unresolved: 7, slaRisk: 2, sendEnabled: true, receiveEnabled: true, signature: 'AngelCare HR', routingRule: 'Candidate/staff records to HR' },
  ],
  conversations: [
...
async function emailOsOptionalRequest(path: string, payload?: Record<string, unknown>, method = 'POST') {
  try {
    return await emailOsRequest(path, payload, method);
  } catch (error: any) {
    return { ok: false, error: error?.message || String(error), skipped: true };
  }
}

function mapBackendSnapshotToState(snapshot: any, fallback: AppState): AppState {
  const accounts = Array.isArray(snapshot?.accounts) ? snapshot.accounts : [];
  const messages = Array.isArray(snapshot?.messages) ? snapshot.messages : [];
  const drafts = Array.isArray(snapshot?.drafts) ? snapshot.drafts : [];
  const jobs = Array.isArray(snapshot?.syncJobs) ? snapshot.syncJobs : [];
  const audit = Array.isArray(snapshot?.audit) ? snapshot.audit : [];

  const mailboxes: Mailbox[] = accounts.length
    ? accounts.map((a: any) => ({
        id: String(a.id || a.slug || a.email_address),
        name: a.mailbox_name || a.name || a.slug || a.email_address,
        address: a.email_address,
        department: a.department || 'Email',
        owner: a.owner_label || a.owner || 'Mailbox Owner',
        provider: 'Menara SMTP/IMAP',
        status:
          a.status === 'connected'
            ? 'Connected'
            : a.status === 'warning'
              ...
              : a.status === 'restricted'
                ? 'Restricted'
                : 'Needs setup',
        inbound: messages.filter((m: any) => m.account_id === a.id && m.direction === 'inbound').length,
        outbound: messages.filter((m: any) => m.account_id === a.id && m.direction === 'outbound').length,
        unresolved: messages.filter((m: any) => m.account_id === a.id && !['resolved', 'archived', 'sent'].includes(String(m.status || '').toLowerCase())).length,
        slaRisk: messages.filter((m: any) => m.account_id === a.id && ['critical', 'urgent', 'high'].includes(String(m.priority || '').toLowerCase())).length,
        sendEnabled: Boolean(a.send_enabled),
        receiveEnabled: Boolean(a.receive_enabled),
        signature: a.signature_name || a.mailbox_name || a.email_address,
        routingRule: a.routing_rule || 'Standard routing',
      }))
    : fallback.mailboxes;

  const conversations: Conversation[] = messages.length
    ? messages.slice(0, 200).map((m: any) => {
        const account = accounts.find((a: any) => a.id === m.account_id);
        return {
          id: String(m.id || m.message_id || `MSG-${Date.now()}`),
          subject: m.subject || 'No subject',
          from: m.from_name || m.from_email || 'Unknown sender',
          fromEmail: m.from_email || '',
          mailbox: account?.mailbox_name || account?.email_address || 'Mailbox',
          owner: account?.owner_label || 'Mailbox Owner',
          department: account?.department || m.business_context || 'Email',
          status: m.status === 'sent' ? 'Resolved' : m.priority === 'critical' ? 'Escalated' : 'Assigned',
          priority: m.priority === 'critical' ? 'Critical' : m.priority === 'high' ? 'High' : 'Normal',
          sla: m.priority === 'critical' ? 'Immediate' : 'Open',
          revenueLink: m.business_context || 'Email OS',
          partnerLink: account?.routing_rule || 'Mailbox routing',
          client: m.from_email || (Array.isArray(m.to_emails) ? m.to_emails[0] : '') || 'Contact',
          tags: [m.business_context || 'email', m.direction || 'message'].filter(Boolean),
          lastMessage: m.snippet || String(m.body_text || '').slice(0, 180) || 'Open message to read content.',
          body: m.body_text || m.snippet || '',
          notes: ['Loaded from email_messages database'],
          attachments: [],
        };
      })
    : fallback.conversations;

  const mappedDrafts: Draft[] = drafts.length
    ? drafts.map((d: any) => {
        const account = accounts.find((a: any) => a.id === d.account_id);
        return {
          id: String(d.id),
          mailbox: account?.email_address || account?.mailbox_name || 'Mailbox',
          to: Array.isArray(d.to_emails) ? d.to_emails.join(', ') : '',
          subject: d.subject || '(no subject)',
          body: d.body_text || '',
          status: d.status === 'queued' ? 'Queued' : d.status === 'approval_required' ? 'Approval required' : d.status === 'sent' ? 'Sent blocked' : 'Draft',
          linkedThread: d.thread_id || undefined,
        };
      })
    : fallback.drafts;

  const queueJobs: QueueJob[] = jobs.length
    ? jobs.map((j: any) => {
        const account = accounts.find((a: any) => a.id === j.account_id);
        return {
          id: String(j.id),
          job: j.job_type || 'Mailbox sync',
          mailbox: account?.email_address || account?.mailbox_name || 'Mailbox',
          state: j.state === 'failed' ? 'Failed' : j.state === 'completed' ? 'Completed' : j.state === 'running' ? 'Running' : 'Queued',
          owner: 'Email Engine',
          retry: Number(j.retry || j.attempts || 0),
          impact: j.error || `${j.message_count || 0} messages processed`,
        };
      })
    : fallback.queueJobs;
  ...
  return {
    ...
    drafts: mappedDrafts,
    queueJobs,
    audit: audit.length ? audit.map((a: any) => `${new Date(a.created_at).toLocaleTimeString()} · ${a.action}`).slice(0, 80) : fallback.audit,
  };
}

async function loadEmailOsLiveState(fallback: AppState): Promise<AppState> {
  const snapshot = await emailOsRequest('/api/email-os/state', {}, 'GET');
  return mapBackendSnapshotToState(snapshot, fallback);
}
...
function MetricCard({ label, value, icon: Icon, helper, tone = 'cyan', onClick }: { label: string; value: string | number; icon: LucideIcon; helper: string; tone?: Tone; onClick?: () => void }) {
  return (
    <button type="button" onClick={onClick} className="rounded-3xl border border-slate-700/80 bg-[#0b1220] p-5 text-left shadow-xl shadow-black/20 transition hover:-translate-y-0.5 hover:border-cyan-300/50 hover:bg-[#0f172a]">
...
function ShellHeader({ title, activeNav, mailboxes, selectedMailbox, setSelectedMailbox, query, setQuery, act, router, reset }: { title: string; activeNav?: typeof nav[number]; mailboxes: Mailbox[]; selectedMailbox: string; setSelectedMailbox: (v: string) => void; query: string; setQuery: (v: string) => void; act: (v: string, updater?: (s: AppState) => AppState) => void; router: ReturnType<typeof useRouter>; reset: () => void }) {
  return (
...
{mailboxes.map((m, index) => (
  <option key={`${m.id || m.address}-${index}`} value={m.address}>
    {m.name} · {m.address}
  </option>
))}
          </select>
          <ActionButton icon={RefreshCw} onClick={() => { act('Workspace pulse requested for ' + selectedMailbox); emailOsRequest('/api/email-os/sync', { email: selectedMailbox, limit: 25 }).then((result: any) => act('Backend mailbox sync completed: ' + JSON.stringify(result))).catch((error: any) => act('Backend mailbox sync failed: ' + (error?.message || String(error)))); }}>Refresh pulse</ActionButton>
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[1fr_auto_auto]">
        <div className="flex items-center gap-3 rounded-2xl border border-slate-700/80 bg-slate-950/80 px-4 py-3">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search thread, mailbox, deal, contact, note, failure log, template..." className="w-full bg-transparent text-sm text-white outline-none placeholder:text-slate-400" />
        </div>
...
function RightRail({ state, act, openEmail, router }: { state: AppState; act: (v: string, updater?: (s: AppState) => AppState) => void; openEmail: (id: string) => void; router: ReturnType<typeof useRouter> }) {
  ...
  const actions = [
    ...
    { label: 'Sync mailboxes', icon: RefreshCw, tone: 'cyan' as Tone, run: () => { act('Started mailbox sync sweep'); emailOsRequest('/api/email-os/sync', { limit: 25 }).then((result: any) => act('Backend sync sweep completed: ' + JSON.stringify(result))).catch((error: any) => act('Backend sync sweep failed: ' + (error?.message || String(error)))); } },
    { label: 'Retry failures', icon: Zap, tone: 'amber' as Tone, run: () => { act('Retried all failed jobs', s => ({ ...s, queueJobs: s.queueJobs.map(j => j.state === 'Failed' ? { ...j, state: 'Queued', retry: j.retry + 1 } : j) })); emailOsRequest('/api/email-os/outbox/retry', { limit: 10 }).then((result: any) => act('Backend outbox retry completed: ' + JSON.stringify(result))).catch((error: any) => act('Backend outbox retry failed: ' + (error?.message || String(error)))); } },
  ...
  return (
    <aside className="space-y-4">
      <div className="rounded-[2rem] border border-cyan-300/30 bg-[#07111f] p-5 shadow-2xl shadow-cyan-950/30">
        ...
        <div className="mt-5 grid gap-3"><div className="rounded-3xl border border-slate-700/80 bg-slate-950/70 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-300">Critical command</p><div className="mt-1 text-3xl font-black text-white">{critical.length}</div><p className="mt-2 text-xs leading-5 text-slate-300">{critical[0]?.subject || 'No critical thread raised.'}</p></div><div className="rounded-3xl border border-slate-700/80 bg-slate-950/70 p-4"><p className="text-xs font-bold uppercase tracking-widest text-slate-300">Provider blockers</p><div className="mt-1 text-3xl font-black text-white">{blockers}</div><p className="mt-2 text-xs leading-5 text-slate-300">Missing credentials, failed jobs or restricted send controls.</p></div></div>
      </div>
      <div className="rounded-[2rem] border border-cyan-300/25 bg-[#0b1220] p-5"><h3 className="text-lg font-black text-white">Clickable command grid</h3><p className="mt-1 text-xs leading-5 text-slate-300">Every tile performs an action or navigates.</p><div className="mt-4 grid grid-cols-2 gap-2">{actions.map(a => <button key={a.label} type="button" onClick={a.run} className="rounded-2xl border border-slate-700/80 bg-slate-950/70 p-3 text-left transition hover:border-cyan-300/60 hover:bg-cyan-300/10"><a.icon className="h-4 w-4 text-cyan-200" /><div className="mt-2 text-xs font-black text-white">{a.label}</div></button>)}</div></div>
      <div className="rounded-[2rem] border border-slate-700/80 bg-[#0b1220] p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="text-lg font-black text-white">Command memory</h3><p className="mt-1 text-xs leading-5 text-slate-300">Latest: <span className="text-cyan-200">{state.audit[0] || 'No action yet'}</span></p></div><button type="button" onClick={() => setShowLog(!showLog)} className="rounded-2xl border border-slate-600 bg-slate-950 px-3 py-2 text-xs font-bold text-slate-200 hover:border-cyan-300/50">{showLog ? 'Hide' : 'Show'} log</button></div>{showLog ? <div className="mt-4 space-y-2">{(state.audit.length ? state.audit : ['No actions yet']).slice(0, 8).map((op) => <div key={op} className="rounded-2xl border border-slate-700/70 bg-slate-950/70 p-3 text-sm text-slate-200"><CheckCircle2 className="mr-2 inline h-4 w-4 text-emerald-300" />{op}</div>)}</div> : <div className="mt-4 rounded-2xl border border-cyan-300/20 bg-cyan-300/10 p-4 text-sm leading-6 text-cyan-100">Audit is summarized until you open it.</div>}</div>
    </aside>
  );
}
...
export function EmailOSV12Shell({ mode = 'command' }: { mode?: V12Mode }) {
  ...
  const { state, setState, act, reset } = useEmailOsState();
  ...
  useEffect(() => {
    let cancelled = false;
    loadEmailOsLiveState(state)
      .then((liveState) => {
        if (cancelled) return;
        setState((current) => addAudit({ ...liveState, audit: current.audit }, 'Loaded live Email OS database state'));
        const firstMailbox = liveState.mailboxes[0]?.address;
        if (firstMailbox && !selectedMailbox) setSelectedMailbox(firstMailbox);
      })
      .catch((error: any) => {
        act('Live Email OS database state failed to load: ' + (error?.message || String(error)));
        if (!selectedMailbox && state.mailboxes[0]?.address) setSelectedMailbox(state.mailboxes[0].address);
      });
    return () => { cancelled = true; };
    // run once on module mount only
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (!selectedMailbox && state.mailboxes[0]?.address) {
      setSelectedMailbox(state.mailboxes[0].address);
    }
  }, [selectedMailbox, state.mailboxes]);
  ...
  const createDraft = (thread?: Conversation) => {
    const accountEmail = selectedMailbox || state.mailboxes[0]?.address || '';
    const draft: Draft = {
      id: 'DRF-' + Date.now(),
      mailbox: accountEmail || thread?.mailbox || 'Mailbox',
      to: thread?.fromEmail || 'client@example.com',
      subject: draftSubject,
      body: draftBody,
      status: thread?.mailbox === 'Legal' ? 'Approval required' : 'Draft',
      linkedThread: thread?.id,
    };

    act('Created draft ' + draft.id, s => ({ ...s, drafts: [draft, ...s.drafts] }));

    if (accountEmail) {
      emailOsRequest('/api/email-os/drafts', {
        account_email: accountEmail,
        to: [draft.to],
        subject: draft.subject,
        body_text: draft.body,
        approval_required: draft.status === 'Approval required',
        queue: false,
      })
        .then((result: any) => act('Backend draft saved: ' + (result?.id || draft.id)))
        .catch((error: any) => act('Backend draft save failed: ' + (error?.message || String(error))));
    }
  };

  return (
    <div className="email-os-v12-scope min-h-screen bg-[#020617] text-white [&_*]:opacity-100 [&_h1]:!text-white [&_h2]:!text-white [&_h3]:!text-white [&_h4]:!text-white [&_p]:!text-slate-200 [&_label]:!text-slate-200 [&_input]:!text-white [&_input::placeholder]:!text-slate-400 [&_select]:!text-white [&_option]:!bg-slate-950 [&_option]:!text-white">
      ...
      <div className="relative flex min-h-screen">
        ...
        <main className="flex-1 p-4 md:p-8">
          <ShellHeader title={title} activeNav={activeNav} mailboxes={state.mailboxes} selectedMailbox={selectedMailbox} setSelectedMailbox={setSelectedMailbox} query={query} setQuery={setQuery} act={act} router={router} reset={reset} />
          ...
          <section className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_0.9fr]"><div className="space-y-6">
            ...
            {mode === 'composer' && <ComposerStudio state={state} selectedMailbox={selectedMailbox} setSelectedMailbox={setSelectedMailbox} selectedConversationObjects={selectedConversationObjects} draftSubject={draftSubject} setDraftSubject={setDraftSubject} draftBody={draftBody} setDraftBody={setDraftBody} createDraft={createDraft} act={act} />}
            ...
            {mode === 'mailboxes' && <MailboxRegistry state={state} setState={setState} act={act} />}
...
function CommandDashboard({ state, act, router, openEmail }: { state: AppState; act: (v: string, updater?: (s: AppState) => AppState) => void; router: ReturnType<typeof useRouter>; openEmail: (id: string) => void }) {
  ...
  const widgets = [
    ...
    { label: 'Retry failed jobs', icon: RefreshCw, run: () => act('Retried failed jobs from command home', s => ({ ...s, queueJobs: s.queueJobs.map(j => j.state === 'Failed' ? { ...j, state: 'Queued', retry: j.retry + 1 } : j) })) },
...
function ThreadDossiers({ conversations, selectedThreads, toggleThread, updateThreads, openEmail }: { conversations: Conversation[]; selectedThreads: string[]; toggleThread: (id: string) => void; updateThreads: (ids: string[], updater: (c: Conversation) => Conversation, label: string) => void; openEmail: (id: string) => void }) {
  ...
  return <Panel title="Thread dossier command" icon={MessageSquare} desc="Open, inspect, note, link, resolve or escalate each conversation."><div className="grid gap-5 lg:grid-cols-[0.85fr_1.15fr]"><div className="space-y-3">{conversations.map(c => <button key={c.id} onClick={() => toggleThread(c.id)} className={cx('w-full rounded-3xl border p-4 text-left', active?.id === c.id ? 'border-cyan-300/70 bg-cyan-300/10' : 'border-slate-700/80 bg-[#08111f]')}><h4 className="font-black text-white">{c.subject}</h4><p className="mt-1 text-sm text-slate-300">{c.from} · {c.mailbox}</p><div className="mt-2 flex gap-2"><StatusPill value={c.priority}/><StatusPill value={c.status}/></div></button>)}</div>{active ? <div className="rounded-[2rem] border border-slate-700/80 bg-[#08111f] p-5"><h3 className="text-2xl font-black text-white">{active.subject}</h3><p className="mt-2 text-slate-300">{active.body}</p><div className="mt-4 flex flex-wrap gap-2"><StatusPill value={active.priority}/><StatusPill value={active.status}/><Pill tone="emerald">{active.client}</Pill></div><div className="mt-5 grid gap-3 md:grid-cols-2"><ActionButton icon={Eye} onClick={() => openEmail(active.id)}>Open reader</ActionButton><ActionButton icon={Link2} onClick={() => updateThreads([active.id], c=>({...c, notes:[`Linked business context at ${new Date().toLocaleTimeString()}`,...c.notes]}),'Linked context')}>Link context</ActionButton><ActionButton icon={CheckCircle2} tone="emerald" onClick={() => updateThreads([active.id], c=>({...c,status:'Resolved'}),'Resolved thread')}>Resolve</ActionButton><ActionButton icon={Bell} tone="rose" onClick={() => updateThreads([active.id], c=>({...c,status:'Escalated',priority:'Critical'}),'Escalated thread')}>Escalate</ActionButton></div><div className="mt-5 space-y-2">{active.notes.map(n => <div key={n} className="rounded-2xl bg-slate-950 p-3 text-sm text-slate-300">{n}</div>)}</div></div> : null}</div></Panel>;
}

function ComposerStudio({ state, selectedMailbox, setSelectedMailbox, selectedConversationObjects, draftSubject, setDraftSubject, draftBody, setDraftBody, createDraft, act }: { state: AppState; selectedMailbox: string; setSelectedMailbox: (v: string) => void; selectedConversationObjects: Conversation[]; draftSubject: string; setDraftSubject: (v:string)=>void; draftBody: string; setDraftBody: (v:string)=>void; createDraft: (thread?: Conversation)=>void; act: (v: string, updater?: (s: AppState) => AppState) => void }) {
  return <Panel title="Composer Studio" icon={Send} desc="Draft, improve, save, approve and queue send records." actions={<><ActionButton icon={Save} onClick={() => createDraft(selectedConversationObjects[0])}>Save draft</ActionButton><ActionButton icon={Wand2} onClick={() => { setDraftBody(draftBody + '\n\n[Quality check completed: clear next step added.]'); act('Improved draft body with assistant'); }}>Improve</ActionButton></>}><div className="grid gap-5 lg:grid-cols-[1fr_0.9fr]"><div className="space-y-4"><select value={selectedMailbox} onChange={(e) => { setSelectedMailbox(e.target.value); act('Composer mailbox selected: ' + e.target.value); }} className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-white">{state.mailboxes.map((m, index)=><option key={`${m.id || m.address}-${index}`} value={m.address}>{m.name} · {m.address}</option>)}</select><input value={draftSubject} onChange={e=>setDraftSubject(e.target.value)} className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-white"/><textarea value={draftBody} onChange={e=>setDraftBody(e.target.value)} rows={12} className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-white"/><div className="flex flex-wrap gap-2"><ActionButton icon={Paperclip} tone="slate" onClick={() => act('Attachment placeholder added to draft')}>Attach</ActionButton><ActionButton icon={BadgeCheck} tone="amber" onClick={() => act('Approval requested for current draft')}>Request approval</ActionButton></div></div><div className="rounded-[2rem] border border-slate-700 bg-white p-5 text-slate-950"><b>Live preview</b><h4 className="mt-4 text-xl font-black">{draftSubject}</h4><pre className="mt-4 whitespace-pre-wrap font-sans text-sm leading-6">{draftBody}</pre></div></div></Panel>;
}

function AccessMatrix({ state, act }: { state: AppState; act: (v: string, updater?: (s: AppState) => AppState) => void }) { return <Panel title="Access Matrix" icon={UserCog} desc="Toggle permissions and persist them."><div className="space-y-3">{state.permissions.map(p => <div key={p.id} className="rounded-2xl border border-slate-700 bg-[#08111f] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><b className="text-white">{p.user}</b><p className="text-sm text-slate-300">{p.role} · {p.mailbox}</p></div><div className="flex flex-wrap gap-2">{(['read','send','approve','admin'] as const).map(k => <ActionButton key={k} icon={p[k] ? CheckCircle2 : Lock} tone={p[k] ? 'emerald':'rose'} onClick={() => act(`Toggled ${k} for ${p.user}`, s=>({...s, permissions:s.permissions.map(x=>x.id===p.id?{...x,[k]:!x[k]}:x)}))}>{k}: {String(p[k])}</ActionButton>)}</div></div></div>)}</div></Panel>; }
function EngineCenter({ state, act }: { state: AppState; act: (v: string, updater?: (s: AppState) => AppState) => void }) { return <Panel title="Engine Center" icon={Server} desc="Retry, pause, complete or delete jobs." actions={<ActionButton icon={RefreshCw} onClick={() => act('Retried all failed jobs', s=>({...s,queueJobs:s.queueJobs.map(j=>j.state==='Failed'?{...j,state:'Queued',retry:j.retry+1}:j)}))}>Retry failed</ActionButton>}><div className="space-y-3">{state.queueJobs.map(j => <div key={j.id} className="rounded-2xl border border-slate-700 bg-[#08111f] p-4"><div className="flex flex-wrap justify-between gap-3"><div><b className="text-white">{j.job}</b><p className="text-sm text-slate-300">{j.id} · {j.impact}</p></div><StatusPill value={j.state}/></div><div className="mt-3 flex flex-wrap gap-2"><ActionButton icon={RefreshCw} onClick={() => act('Retried '+j.id, s=>({...s,queueJobs:s.queueJobs.map(x=>x.id===j.id?{...x,state:'Queued',retry:x.retry+1}:x)}))}>Retry</ActionButton><ActionButton icon={PauseCircle} tone="amber" onClick={() => act('Paused '+j.id, s=>({...s,queueJobs:s.queueJobs.map(x=>x.id===j.id?{...x,state:'Paused'}:x)}))}>Pause</ActionButton><ActionButton icon={CheckCircle2} tone="emerald" onClick={() => act('Completed '+j.id, s=>({...s,queueJobs:s.queueJobs.map(x=>x.id===j.id?{...x,state:'Completed'}:x)}))}>Complete</ActionButton><ActionButton icon={Trash2} tone="rose" onClick={() => act('Deleted '+j.id, s=>({...s,queueJobs:s.queueJobs.filter(x=>x.id!==j.id)}))}>Delete</ActionButton></div></div>)}</div></Panel>; }
...
function ConfigurationCenter({ state, act }: { state: AppState; act: (v: string, updater?: (s: AppState) => AppState) => void }) {
  ...
  async function runConfigurationAction(label: string, action: () => Promise<void> | void) {
    setBusyAction(label);
    try {
      await action();
    } catch (error: any) {
      act(`${label} failed: ${error?.message || String(error)}`);
    } finally {
      setBusyAction(null);
    }
  }
  ...
  function saveConfiguration() {
    return runConfigurationAction('Save configuration', async () => {
      act('Saved Email OS configuration locally', (s) => ({ ...s, config: form }));

      const result = await emailOsOptionalRequest('/api/email-os/configuration', form);
      if (result?.skipped) {
        act('Configuration API not available yet; local configuration saved safely');
      } else {
        act('Backend configuration saved');
      }
    });
  }

  function testSelectedMailbox() {
    return runConfigurationAction('Test selected mailbox provider', async () => {
      const mailbox = state.mailboxes[0];
      if (!mailbox?.address) {
        act('No mailbox available for provider test');
        return;
      }

      const result = await emailOsRequest('/api/email-os/accounts/test', {
        email: mailbox.address,
        mode: 'both',
      });

      act('Provider test completed for ' + mailbox.address + ': ' + JSON.stringify(result));
    });
  }

  function syncAllMailboxes() {
    return runConfigurationAction('Sync all mailboxes', async () => {
      const result = await emailOsRequest('/api/email-os/sync', { limit: 25 });
      act('Backend sync all completed: ' + JSON.stringify(result));
    });
  }

  function retryOutbox() {
    return runConfigurationAction('Retry outbox', async () => {
      const result = await emailOsRequest('/api/email-os/outbox/retry', { limit: 10 });
      act('Backend outbox retry completed: ' + JSON.stringify(result));
    });
  }

  function seedAccounts() {
    return runConfigurationAction('Seed mailbox registry', async () => {
      const result = await emailOsOptionalRequest('/api/email-os/accounts/seed-from-env', {});
      if (result?.skipped) {
        act('Seed route not available yet; use Mailbox Registry create/edit instead');
      } else {
        act('Mailbox registry seed completed: ' + JSON.stringify(result));
      }
    });
  }

  const controls = [
    { label: 'Save configuration', icon: Save, tone: 'emerald' as Tone, run: saveConfiguration, helper: 'Persists provider mode, hosts, ports, SLA and approval policy.' },
    { label: 'Test provider', icon: RefreshCw, tone: 'cyan' as Tone, run: testSelectedMailbox, helper: 'Runs the real mailbox health endpoint against the first live mailbox.' },
    { label: 'Sync all', icon: Server, tone: 'cyan' as Tone, run: syncAllMailboxes, helper: 'Calls the backend sync route and records the result in command memory.' },
    { label: 'Retry outbox', icon: Zap, tone: 'amber' as Tone, run: retryOutbox, helper: 'Calls the backend outbox retry route.' },
    { label: 'Seed registry', icon: Database, tone: 'slate' as Tone, run: seedAccounts, helper: 'Optional: imports preconfigured accounts when the backend seed route exists.' },
  ];
  ...
  return (
    <Panel
      ...
      icon={Settings}
      desc="Production controls for Menara SMTP/IMAP, routing, SLA, approval policy, sync and delivery recovery."
      actions={
        <>
          <ActionButton icon={Save} tone="emerald" onClick={saveConfiguration}>Save configuration</ActionButton>
          <ActionButton icon={RefreshCw} onClick={testSelectedMailbox}>Test provider</ActionButton>
        </>
      }
    >
      <div className="grid gap-5 xl:grid-cols-[1fr_0.85fr]">
        <div className="rounded-[2rem] border border-slate-700 bg-[#08111f] p-5">
          <h4 className="text-lg font-black text-white">Provider settings</h4>
          <p className="mt-1 text-sm leading-6 text-slate-300">These values stay local immediately and call backend configuration when that route exists.</p>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {(Object.keys(form) as Array<keyof typeof form>).map((k) => (
              <label key={k} className="block">
                <span className="mb-2 block text-sm font-bold text-slate-200">{k}</span>
                <input
                  value={form[k]}
        ...
        <div className="space-y-3">
          <div className="rounded-[2rem] border border-cyan-300/25 bg-[#07111f] p-5">
            <h4 className="text-lg font-black text-white">Production action panel</h4>
            <p className="mt-1 text-sm leading-6 text-slate-300">Every button either calls a real backend route or records the exact safe fallback.</p>
            <div className="mt-4 grid gap-3">
              {controls.map((control) => (
                <button
                  ...
                  type="button"
                  onClick={control.run}
                  disabled={busyAction === control.label}
                  className="rounded-2xl border border-slate-700 bg-slate-950/70 p-4 text-left transition hover:border-cyan-300/60 hover:bg-cyan-300/10 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  <div className="flex items-center justify-between gap-3">
                    <span className="flex items-center gap-3">
                      <control.icon className="h-5 w-5 text-cyan-200" />
                      <span className="font-black text-white">{busyAction === control.label ? 'Running...' : control.label}</span>
                    </span>
                    <ChevronRight className="h-4 w-4 text-slate-300" />
                  </div>
                  <p className="mt-2 text-xs leading-5 text-slate-300">{control.helper}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-[2rem] border border-slate-700 bg-[#08111f] p-5">
            <h4 className="text-lg font-black text-white">Current live mailbox source</h4>
            <div className="mt-3 space-y-2">
              {state.mailboxes.slice(0, 6).map((m, index) => (
                <div key={`${m.id || m.address}-${index}`} className="rounded-2xl border border-slate-700 bg-slate-950/70 p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="text-sm font-black text-white">{m.name}</span>
                    <StatusPill value={m.status} />
                  </div>
                  <p className="mt-1 text-xs text-slate-300">{m.address} · send {String(m.sendEnabled)} · receive {String(m.receiveEnabled)}</p>
                </div>
              ))}
            </div>
          </div>
...
function MailboxRegistry({ state, setState, act }: { state: AppState; setState: React.Dispatch<React.SetStateAction<AppState>>; act: (v: string, updater?: (s: AppState) => AppState) => void }) { return <Panel title="Mailbox Registry" icon={Mail} actions={<ActionButton icon={Plus} tone="emerald" onClick={()=>act('Created mailbox record',s=>({...s,mailboxes:[{...initialState.mailboxes[0],id:'mbx-'+Date.now(),name:'New Mailbox',address:'new@angelcare.ma',status:'Needs setup'},...s.mailboxes]}))}>Create mailbox</ActionButton>}><div className="space-y-3">{state.mailboxes.map(m=><div key={m.id} className="rounded-2xl border border-slate-700 bg-[#08111f] p-4"><div className="flex flex-wrap justify-between gap-3"><div><b className="text-white">{m.name}</b><p className="text-sm text-slate-300">{m.address} · {m.owner}</p></div><StatusPill value={m.status}/></div><div className="mt-3 flex flex-wrap gap-2"><ActionButton icon={CheckCircle2} tone="emerald" onClick={()=>act('Marked mailbox connected: '+m.name,s=>({...s,mailboxes:s.mailboxes.map(x=>x.id===m.id?{...x,status:'Connected'}:x)}))}>Connect</ActionButton><ActionButton icon={Edit3} onClick={()=>act('Edited mailbox owner: '+m.name,s=>({...s,mailboxes:s.mailboxes.map(x=>x.id===m.id?{...x,owner:'Updated Owner'}:x)}))}>Edit</ActionButton><ActionButton icon={Trash2} tone="rose" onClick={()=>act('Deleted mailbox: '+m.name,s=>({...s,mailboxes:s.mailboxes.filter(x=>x.id!==m.id)}))}>Delete</ActionButton></div></div>)}</div></Panel>; }
function TemplateCenter({ state, act, setDraftSubject, setDraftBody, router }: { state: AppState; act: (v: string, updater?: (s: AppState) => AppState) => void; setDraftSubject:(v:string)=>void; setDraftBody:(v:string)=>void; router: ReturnType<typeof useRouter> }) { return <Panel title="Template Center" icon={FileText} actions={<ActionButton icon={Plus} tone="emerald" onClick={()=>act('Created template',s=>({...s,templates:[{id:'TPL-'+Date.now(),name:'New template',category:'General',subject:'New subject',approval:'No approval',quality:80,body:'Hello,'},...s.templates]}))}>Create template</ActionButton>}><div className="grid gap-3 md:grid-cols-2">{state.templates.map(t=><div key={t.id} className="rounded-3xl border border-slate-700 bg-[#08111f] p-4"><h4 className="font-black text-white">{t.name}</h4><p className="mt-1 text-sm text-slate-300">{t.subject}</p><div className="mt-3 flex flex-wrap gap-2"><ActionButton icon={Copy} onClick={()=>{setDraftSubject(t.subject);setDraftBody(t.body);act('Loaded template into composer: '+t.name);router.push('/email-os/composer/v12')}}>Use</ActionButton><ActionButton icon={Edit3} onClick={()=>act('Improved template quality: '+t.name,s=>({...s,templates:s.templates.map(x=>x.id===t.id?{...x,quality:Math.min(100,x.quality+1)}:x)}))}>Improve</ActionButton><ActionButton icon={Trash2} tone="rose" onClick={()=>act('Deleted template: '+t.name,s=>({...s,templates:s.templates.filter(x=>x.id!==t.id)}))}>Delete</ActionButton></div></div>)}</div></Panel>; }
function AutomationCenter({ state, act }: { state: AppState; act: (v: string, updater?: (s: AppState) => AppState) => void }) { return <Panel title="Automation Center" icon={Workflow}><div className="space-y-3">{state.automations.map(r=><div key={r.id} className="rounded-2xl border border-slate-700 bg-[#08111f] p-4"><b className="text-white">{r.name}</b><p className="text-sm text-slate-300">{r.trigger} → {r.action}</p><div className="mt-3 flex gap-2"><StatusPill value={r.status}/><ActionButton icon={PauseCircle} onClick={()=>act('Toggled automation: '+r.name,s=>({...s,automations:s.automations.map(x=>x.id===r.id?{...x,status:x.status==='Active'?'Paused':'Active'}:x)}))}>{r.status==='Active'?'Pause':'Activate'}</ActionButton></div></div>)}</div></Panel>; }
function AuditCenter({ state }: { state: AppState }) { return <Panel title="Audit Trail" icon={Shield}>{(state.audit.length?state.audit:['No audit event yet.']).map(a=><div key={a} className="mb-2 rounded-2xl border border-slate-700 bg-[#08111f] p-3 text-sm text-slate-200">{a}</div>)}</Panel>; }
...
function DraftLine({ d, act }: { d: Draft; act: (v: string, updater?: (s: AppState) => AppState) => void }) { return <div className="mb-3 rounded-2xl border border-slate-700 bg-[#08111f] p-4"><div className="flex flex-wrap justify-between gap-3"><div><b className="text-white">{d.subject}</b><p className="text-sm text-slate-300">{d.to} · {d.mailbox}</p></div><StatusPill value={d.status}/></div><div className="mt-3 flex flex-wrap gap-2"><ActionButton icon={BadgeCheck} tone="emerald" onClick={()=>act('Approved draft '+d.id,s=>({...s,drafts:s.drafts.map(x=>x.id===d.id?{...x,status:'Draft'}:x)}))}>Approve</ActionButton><ActionButton icon={Send} onClick={()=>act('Queued draft '+d.id,s=>({...s,drafts:s.drafts.map(x=>x.id===d.id?{...x,status:'Queued'}:x),queueJobs:[{id:'JOB-'+Date.now(),job:'Send '+d.subject,mailbox:d.mailbox,state:'Queued',owner:'Outbox',retry:0,impact:'Queued outbound draft'},...s.queueJobs]}))}>Queue</ActionButton><ActionButton icon={Trash2} tone="rose" onClick={()=>act('Deleted draft '+d.id,s=>({...s,drafts:s.drafts.filter(x=>x.id!==d.id)}))}>Delete</ActionButton></div></div>; }
function AttachmentCenter({ state, act }: { state: AppState; act: (v: string, updater?: (s: AppState) => AppState) => void }) { return <Panel title="Attachments" icon={Paperclip}>{state.conversations.map(c=><div key={c.id} className="mb-3 rounded-2xl border border-slate-700 bg-[#08111f] p-4"><b className="text-white">{c.id} · {c.subject}</b><p className="mt-2 text-sm text-slate-300">{c.attachments.length?c.attachments.join(', '):'No attachments'}</p><ActionButton icon={Paperclip} onClick={()=>act('Attached proof file to '+c.id,s=>({...s,conversations:s.conversations.map(x=>x.id===c.id?{...x,attachments:[...x.attachments,'proof-'+Date.now()+'.pdf']}:x)}))}>Add proof</ActionButton></div>)}</Panel>; }
function SyncCenter({ state, act }: { state: AppState; act: (v: string, updater?: (s: AppState) => AppState) => void }) { return <Panel title="Sync Center" icon={RefreshCw} actions={<ActionButton icon={RefreshCw} onClick={()=>{ act('Created sync jobs for all mailboxes',s=>({...s,queueJobs:[...s.mailboxes.map(m=>({id:'JOB-'+Date.now()+m.id,job:'Sync '+m.name,mailbox:m.address,state:'Queued' as const,owner:'Sync Engine',retry:0,impact:'Manual sync requested'})),...s.queueJobs]})); emailOsRequest('/api/email-os/sync', { limit: 25 }).then((result)=>act('Backend sync all completed: '+JSON.stringify(result))).catch((error)=>act('Backend sync all failed: '+(error?.message || String(error)))); }}>Sync all</ActionButton>}>{state.queueJobs.map(j=><div key={j.id} className="mb-2 text-slate-300">{j.id} · {j.job} · {j.state}</div>)}</Panel>; }
