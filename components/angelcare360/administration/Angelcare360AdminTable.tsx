...
import type { Angelcare360AdminColumnDefinition, Angelcare360AdminRowActionDefinition } from '@/types/angelcare360/administration'

type Angelcare360AdminTableProps = {
  columns: Angelcare360AdminColumnDefinition[]
  rows: Array<Record<string, unknown>>
  actions?: Angelcare360AdminRowActionDefinition[]
  onRowAction?: (action: Angelcare360AdminRowActionDefinition, row: Record<string, unknown>) => void
  emptyFallback?: React.ReactNode
}

function formatValue(kind: Angelcare360AdminColumnDefinition['kind'], value: unknown) {
  ...
  if (kind === 'date' || kind === 'datetime') {
    ...
    return date.toLocaleString('fr-FR', {
      dateStyle: kind === 'date' ? 'medium' : 'medium',
...
function getStatusStyle(status: string) {
  ...
  if (['active', 'actif', 'published', 'published', 'sent', 'open', 'completed', 'approved', 'enrolled'].includes(normalized)) {
    return { background: '#dcfce7', color: '#166534', border: '1px solid #bbf7d0' }
  }
  if (['planned', 'pending', 'draft', 'new', 'open', 'qualification', 'qualification'].includes(normalized)) {
    return { background: '#eff6ff', color: '#1d4ed8', border: '1px solid #bfdbfe' }
  }
  if (['inactive', 'archived', 'closed', 'suspended', 'cancelled', 'rejected'].includes(normalized)) {
    return { background: '#fef2f2', color: '#b91c1c', border: '1px solid #fecaca' }
  }
  return { background: '#f8fafc', color: '#334155', border: '1px solid #e2e8f0' }
}

export default function Angelcare360AdminTable({ columns, rows, actions = [], onRowAction, emptyFallback }: Angelcare360AdminTableProps) {
  if (!rows.length) {
