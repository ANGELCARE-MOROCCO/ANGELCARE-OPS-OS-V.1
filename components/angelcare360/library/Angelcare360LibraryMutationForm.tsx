...
export type Angelcare360LibraryMutationField = {
  name: string
  label: string
...
type Props = {
  ...
  operation: string
  submitLabel: string
  fields: Angelcare360LibraryMutationField[]
...
export default function Angelcare360LibraryMutationForm({
  title,
  description,
  entity,
  operation,
  ...
  const [values, setValues] = useState<Record<string, string>>(() => {
    ...
    for (const field of fields) {
      next[field.name] = String(initialValues?.[field.name] ?? '')
    }
    return next
  })
  ...
  async function submit() {
    setError(null)
    setMessage(null)
    const response = await fetch('/api/angelcare360/library', {
      method: 'POST',
