...
type Props = {
  items: Angelcare360InventoryNavigationItem[]
}

export default function Angelcare360InventoryNavigation({ items }: Props) {
  const pathname = usePathname() || '/angelcare-360-command-center/inventaire'
  return (
    <nav style={navStyle} aria-label="Navigation inventaire AngelCare 360">
      {items.map((item) => {
        const isActive = pathname === item.href || pathname.startsWith(`${item.href}/`)
        return (
          <Link key={item.key} href={item.href} aria-current={isActive ? 'page' : undefined} style={{ ...linkStyle, ...(isActive ? activeLinkStyle : null) }} title={item.summary}>
...
const badgeStyle: React.CSSProperties = { flexShrink: 0, borderRadius: 999, padding: '5px 9px', background: '#dbeafe', color: '#1d4ed8', fontSize: 11, fontWeight: 900 }
