...
type Angelcare360AcademicNavigationProps = {
  items: Angelcare360AcademicNavigationItem[]
}

export default function Angelcare360AcademicNavigation({ items }: Angelcare360AcademicNavigationProps) {
  const pathname = usePathname() || '/angelcare-360-command-center/academique'

  return (
    <nav style={navStyle} aria-label="Navigation académique AngelCare 360">
      {items.map((item) => {
