...
try {
  ...
  const browser = createCorporateBrowser({
    app: { getPath: () => temporary, getVersion: () => "1.7.2" },
  ...
  assert.equal(state.acPlus.enabled, false, "Locked operating mode must keep AC+ disabled");
  assert.equal(state.split.enabled, false, "Locked operating mode must keep split disabled");
  ...
  assert.equal(state.acPlus.enabled, false);
  assert.equal(state.split.enabled, false);
  assert.equal(state.split.mode, 1, "Relocking must close split without closing the tabs");
  assert.equal(state.acPlusTabCount, 2, "Relocking must preserve opened AC+ tabs");

  browser.destroy();
  console.log("MZ13_OPERATING_MODE_CONTROLS_SMOKE_PASSED");
} finally {
