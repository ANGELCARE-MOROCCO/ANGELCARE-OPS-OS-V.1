...
if(!["1.5.0","1.5.1","1.5.2","1.6.0", "1.7.3"].includes(packageJson.version))throw new Error("Mega ZIP 6 requires ANGELCARE Desktop version 1.5.0 or a supported cumulative version.");
if(!String(packageJson.scripts?.verify||"").includes("verify-unified-release-1.7.3.mjs"))throw new Error("Unified 1.7.3 verifier is not wired into npm run verify.");
const defaults=JSON.parse(read("config/defaults.json"));
if(!["6.0.0","9.0.0","11.3.0"].includes(defaults.desktopContractVersion))throw new Error("Desktop contract version must preserve the Mega ZIP 6 contract or a supported cumulative contract.");
