...
let manifest = { schemaVersion: 1, product: "ANGELCARE Desktop", version: packageJson.version, buildNumber: Number(process.env.ANGELCARE_DESKTOP_BUILD_NUMBER || 172), channel: process.env.ANGELCARE_DESKTOP_RELEASE_CHANNEL || "stable", releaseDate: new Date().toISOString(), minimumSupportedVersion: process.env.ANGELCARE_DESKTOP_MINIMUM_VERSION || "1.5.0", mandatory: process.env.ANGELCARE_DESKTOP_MANDATORY_UPDATE === "1", releaseNotes: process.env.ANGELCARE_DESKTOP_RELEASE_NOTES || "Unified Windows/macOS 1.7.2 release with AC+ workspaces, split operations, dormant WhatsApp activation and Fleet Lifecycle command compatibility.", platforms };
