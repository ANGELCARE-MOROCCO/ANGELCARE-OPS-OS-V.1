...
export async function GET(request: NextRequest) {
  ...
  const account = result.ok ? result.account || null : null
...
export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => null) as { token?: string; mode?: string; password?: string; passwordConfirmation?: string; action?: string; code?: string } | null
  ...
  const result = body.action === 'mfa.confirm'
    ...
    : await completeTenantAccessToken({ token: String(body.token || ''), mode: body.mode === 'reset' ? 'reset' : 'invite', password: String(body.password || ''), passwordConfirmation: String(body.passwordConfirmation || '') })
