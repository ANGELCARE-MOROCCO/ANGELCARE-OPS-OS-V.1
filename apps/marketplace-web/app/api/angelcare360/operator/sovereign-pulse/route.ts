...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Sovereign Pulse indisponible.' }, { status: 403 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Action Sovereign Pulse refusée.' }, { status: 400 })
  }
}
