...
import { guardAc360Action } from '@/lib/ac360/guard'
...
export async function POST(request: Request) {
  ...
  const policy = await resolveAc360PolicySafety({
    ...
    actionKey,
    routePath: body.routePath || body.route_path || '/api/ac360/guard/check',
    httpMethod: 'POST',
    quantity: Number(body.quantity || 1),
    metadata: { ...(body.metadata && typeof body.metadata === 'object' ? body.metadata : {}), source: 'api.ac360.guard.check.POST', phase: 'phase_1e' },
  })
  if (!policy.allowed) return json(ac360PolicyToGuardResult(policy, actionKey), { status: 402 })

  const result = await guardAc360Action({
    orgId: body.orgId || body.org_id,
    actionKey,
    quantity: Number(body.quantity || 1),
