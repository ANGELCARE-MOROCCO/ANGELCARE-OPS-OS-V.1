...
import { getAuthorizedStudioSource,searchStudioSource } from '@/angelcare-marketplace/studio-source-registry/resolver'
import { searchCanonicalCatalogForStudio } from '@/angelcare-marketplace/studio-universal/catalog-source-gateway'
import type { StudioSourceSearchInput } from '@/angelcare-marketplace/studio-source-registry/types'
...
export async function GET(request:Request,{params}:{params:Promise<{sourceId:string}>}){const id=requestId(request);try{const context=await requireMarketplaceApiContext('marketplace.cms.view');const{sourceId}=await params;const url=new URL(request.url);const locale=url.searchParams.get('locale');const decoded=decodeURIComponent(sourceId),input:StudioSourceSearchInput={query:(url.searchParams.get('q')||undefined)?.slice(0,120),limit:int(url.searchParams.get('limit')),cursor:url.searchParams.get('cursor'),filters:filters(url),context:{locale:locale==='fr'||locale==='en'||locale==='ar'?locale:context.locale,territoryId:url.searchParams.get('territoryId')||context.territoryId,audienceId:url.searchParams.get('audienceId')}};getAuthorizedStudioSource(decoded,context);return apiSuccess(decoded==='catalog.items'?await searchCanonicalCatalogForStudio(input,context):await searchStudioSource(decoded,input,context),{requestId:id})}catch(error){return apiFailure(error,id)}}
