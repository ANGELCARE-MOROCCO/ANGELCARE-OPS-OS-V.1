...
export default async function Page(){const context=await requireMarketplacePageContext('marketplace.journeys.view');const data=await getJourneyAdminSummary(context);const allowed:string[]=['b2b_quotation', 'hospitality_programme', 'corporate_benefit'];const selected=allowed.length?data.journeys.filter(j=>allowed.includes(j.journey_type)):data.journeys;return <JourneyAdminCommand data={{...data,journeys:selected,total:selected.length}}/> }
