...
export default async function Page({params}:{params:Promise<{locale:string}>}){const {locale:raw}=await params;const locale=raw==='ar'?'ar':raw==='en'?'en':'fr';const snapshot=await publicVerticalSnapshot('health_partner');return <PublicHealthExperience locale={locale} mode="maternity" activePrograms={snapshot.activePrograms} organizations={snapshot.organizations}/>}
