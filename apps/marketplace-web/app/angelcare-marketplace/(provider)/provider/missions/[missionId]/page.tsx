...
export default async function Page({params}:{params:Promise<{missionId:string}>}){const context=await requireMarketplacePageContext('marketplace.providers.missions.view');const {missionId}=await params;return <MissionDossier data={await missionDossier(missionId,context)} canManage={hasMarketplacePermission(context,'marketplace.operations.missions.manage')}/>}
