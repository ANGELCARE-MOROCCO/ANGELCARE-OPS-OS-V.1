...
export default async function TeacherPortalLayout({ children }: { children: React.ReactNode }) {
  const identity = await requirePortalIdentity('teacher')
  const identityName = String(identity.person.full_name || identity.appUser.full_name || identity.appUser.email || 'Utilisateur SANILA')
  return <RolePortalShell kind="teacher" schoolName={String(identity.school.name || 'Établissement SANILA')} academicYear={String(identity.academicYear?.label || 'Année active')} identityName={identityName}>{children}</RolePortalShell>
}
