...
export function cmsBlocksToPuckData(blocks:CmsBlock[],root?:Record<string,unknown>):Data{
  const content=blocks.filter(block=>block.status!=='archived').sort((a,b)=>a.sort_order-b.sort_order).map((block,index)=>{
    const visualAlias=visualAliasFromMetadata(block.settings,block.content)
    ...
    const publicContent={...(block.content||{})} as Record<string,unknown>
    delete publicContent.__studioPuck
    delete publicContent.__studioVisualAlias
    const type=visualAlias||string(block.block_type)
    const props:Record<string,unknown>={id:block.block_key||stableStudioId(type,index),...publicContent,sourceDesign:(block.settings as any)?.sourceDesign||{},responsive:(block.settings as any)?.responsive||{mobileVisible:true,tabletVisible:true,desktopVisible:true},hidden:block.status==='hidden',locked:Boolean((block.settings as any)?.locked)}
    if(visualAlias){props.__studioVisualAlias=visualAlias;props.__studioVisualCategory=studioVisualExperience(visualAlias)?.categoryKey||'';props.__studioVisualVariant=studioVisualExperience(visualAlias)?.variant||''}
...
export function puckDataToCmsBlocks(data:Data){
  ...
  return content.map((component,index)=>{
    ...
    if(visual)publicContent.__studioVisualAlias=visual.key
    else delete publicContent.__studioVisualAlias
    return {blockKey:id,blockType:canonicalType,sortOrder:index,content:publicContent,settings:{sourceDesign:props.sourceDesign||{},responsive:props.responsive||{},locked:Boolean(props.locked),studioFormat:'angelcare-puck-v2',...(visual?{studioVisualAlias:visual.key,studioVisualCategory:visual.categoryKey,studioVisualVariant:visual.variant,studioCanonicalType:visual.canonicalType}:{})},status:props.hidden===true?'hidden':'active'}
  })
}
