...
export function assertStudioPolicy(report:StudioPolicyReport){if(report.decision==='BLOCK')throw new MarketplaceError('NOT_READY',`Politique P11 bloquante: ${report.blockers.map(x=>x.ruleId).join(', ')}`)}
