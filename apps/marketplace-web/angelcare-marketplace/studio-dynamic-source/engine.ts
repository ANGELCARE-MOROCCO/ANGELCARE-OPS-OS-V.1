...
const materialize=(entity:StudioSourceEntity)=>({title:entity.title,label:entity.subtitle||'',body:entity.badges.slice(0,3).map(row=>row.value||row.label).filter(Boolean).join(' · '),description:entity.subtitle||'',mediaUrl:entity.image?.url||'',mediaAlt:entity.image?.alt||entity.title,value:entity.id,__studioSourceReference:entity.canonicalRef,...(itemAction(entity)?{action:itemAction(entity),actionLabel:'Découvrir'}:{})})
