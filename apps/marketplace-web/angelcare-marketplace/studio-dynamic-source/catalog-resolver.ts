...
export async function resolveCatalogDynamicSource(recipe:StudioDynamicSourceReference,context:StudioDynamicResolveContext):Promise<StudioSourceEntity[]>{
  ...
  if(['merchandising_popular','merchandising_best_pick','merchandising_new_arrival'].includes(recipe.strategy)){
    ...
    const result=await searchDiscovery({locale:context.locale,territoryCode:territory,limit:120});const map=new Map(result.items.map(item=>[item.id,item]));return ids.map(id=>map.get(id)).filter(Boolean).slice(0,limit).map(entity)
  }
  if(recipe.strategy==='collection_items'){
    const ids=await collectionIds(recipe);if(!ids.length)return[];const result=await searchDiscovery({locale:context.locale,territoryCode:territory,limit:240});const map=new Map(result.items.map(item=>[item.id,item]));return ids.map(id=>map.get(id)).filter(Boolean).slice(0,limit).map(entity)
  }
