...
import { createWebPresenceDraft, getWebPresenceSnapshot, publishWebPresence, publishWebPresenceOneClick, rollbackWebPresence, updateWebPresenceDraft, validateWebPresenceDraft, webPresenceHistory } from './repository'
...
function failure(error:unknown,rid:string){if(error instanceof WebPresenceInputError)return NextResponse.json({error:{code:error.code,message:error.message,...(error.field?{field:error.field}:{})},requestId:rid},{status:error.status});console.error(`[web-presence:${rid}]`,error);return NextResponse.json({error:{code:'PUBLICATION_BLOCKED',message:`Erreur serveur Web Presence. Aucune publication n’a été confirmée. Référence ${rid}.`},requestId:rid},{status:500})}
...
export async function postPublishNow(request:Request){const rid=requestId(request);try{const payload=await body(request),ctx=await contextFor('marketplace.web_presence.publish');if(!hasMarketplacePermission(ctx,'marketplace.web_presence.manage'))throw new WebPresenceInputError('PERMISSION_DENIED','Les permissions Web Presence manage + publish sont requises.',403);const scope=parseScope(payload.scope);return NextResponse.json({data:await publishWebPresenceOneClick(scope,payload.configuration,Number(payload.expectedCurrentRevision||0),String(payload.changeSummary||''),ctx,rid,request),requestId:rid})}catch(error){return failure(error,rid)}}
