...
export function validateConfiguration(configuration: WebPresenceConfiguration, assets: WebPresenceMediaAsset[]): ValidationResult {
  ...
  const logo=configuration.icons.organizationLogo.assetKey?byKey.get(configuration.icons.organizationLogo.assetKey):null;if(logo&&logo.metadata.official!==true&&logo.metadata.approved_official_logo!==true)warning('OFFICIAL_LOGO_METADATA_MISSING','icons.organizationLogo','Le média sélectionné sera utilisé comme logo Organization; son marquage officiel dans la Media Library reste recommandé.')
  const effectiveSocialAssetKey=configuration.social.defaultImageAssetKey||configuration.icons.organizationLogo.assetKey||configuration.icons.highResolution.assetKey||configuration.icons.favicon.assetKey;requireAsset('social.defaultImageAssetKey',effectiveSocialAssetKey,['image/png','image/jpeg','image/webp']);if(!configuration.social.defaultImageAssetKey&&effectiveSocialAssetKey)warning('SOCIAL_IMAGE_FALLBACK','social.defaultImageAssetKey','Aucune image sociale dédiée: le média de marque sélectionné est utilisé comme fallback.');const social=effectiveSocialAssetKey?byKey.get(effectiveSocialAssetKey):null;if(social&&social.width&&social.height&&Math.abs(social.width/social.height-1200/630)>.08)warning('SOCIAL_RATIO','social.defaultImageAssetKey','Le ratio recommandé est environ 1.91:1 (1200×630).')
