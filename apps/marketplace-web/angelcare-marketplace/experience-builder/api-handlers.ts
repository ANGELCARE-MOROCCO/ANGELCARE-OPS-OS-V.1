...
export async function handlePageTransition(request:Request,pageId:string){const id=requestId(request);try{const context=await requireMarketplaceApiContext('marketplace.cms.publish');const body=await parseJsonObject(request);const target=cleanText(body.target,30) as CmsPageStatus;if(!pageStatuses.has(target))throw new MarketplaceError('VALIDATION_ERROR','Statut cible invalide.');return apiSuccess(await transitionPage({pageId,target,reason:requireText(body.reason,'reason','Motif',1000),context,requestId:id}),{requestId:id})}catch(error){return apiFailure(error,id)}}
