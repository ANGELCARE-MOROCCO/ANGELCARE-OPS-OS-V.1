...
export function FeaturesPage() {
  ...
  return <>
    ...
    <section className={`${styles.section} ${styles.sectionSoft}`}><SectionHeading index="02" eyebrow="CARTE DES RESPONSABILITÉS" title="Lire SANILA comme un atlas d’exploitation, pas comme un mur de fonctionnalités." body="Chaque capacité existe parce qu’une responsabilité réelle doit être exécutée, contrôlée ou comprise."/><CapabilityAtlasVisual/></section>
    <section className={styles.section}><SectionHeading index="03" eyebrow="DOMAINES MAJEURS" title="Entrer par le problème qui doit réellement avancer." /><div className={styles.solutionTriptych}>{PRODUCT_DOMAINS.slice(0,3).map((d)=><Link href={sanilaHref(d.slug)} key={d.slug}><span>{d.eyebrow}</span><h2>{d.nav}</h2><p>{d.outcome}</p><ul>{d.features.slice(0,5).map(f=><li key={f}>{f}</li>)}</ul><strong>Explorer <SanilaIcon name="arrow" size={14}/></strong></Link>)}</div></section>
