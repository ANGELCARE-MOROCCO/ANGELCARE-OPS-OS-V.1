...
/**
 * react-leaflet v5's runtime props are correct here, but the unified repository's
 * installed React 19/Leaflet declaration graph currently narrows these component
 * props incorrectly. Keep runtime components untouched and isolate the declaration
 * incompatibility at this boundary instead of weakening the application's data types.
 */
...
export function LiveVisitorGeoMap({points,commercePoints,mode,selected,onSelect,layer,display,playbackAt}:Props){
 ...
 return <LeafletMapContainer center={mode==='morocco'?[32.8,-6.5]:[20,0]} zoom={mode==='morocco'?5:2} minZoom={2} maxZoom={18} scrollWheelZoom className="ac-live-leaflet"><LeafletTileLayer attribution="&copy; OpenStreetMap contributors" url={process.env.NEXT_PUBLIC_OSM_TILE_URL||'https://tile.openstreetmap.org/{z}/{x}/{y}.png'}/><Fit points={rendered} mode={mode}/>{rendered.map(point=><LeafletCircleMarker key={point.id} center={[point.lat,point.lng]} radius={display==='heat'?Math.min(34,10+Math.sqrt(point.count)*8):Math.min(18,6+Math.sqrt(point.count)*3)} pathOptions={{color:point.color,fillColor:point.color,fillOpacity:display==='heat'?0.18:selected===point.id?0.95:0.68,weight:selected===point.id?3:display==='heat'?1:1.5}} eventHandlers={{click:()=>{const target=point.members[0];onSelect(target.id)}}}><LeafletTooltip direction="top"><strong>{point.title}</strong><br/>{point.subtitle}<br/>{point.count>1?`${point.count} signaux · `:''}{point.precision}{point.amount?` · ${point.amount.toLocaleString('fr-FR')} Dh`:''}</LeafletTooltip></LeafletCircleMarker>)}</LeafletMapContainer>}
