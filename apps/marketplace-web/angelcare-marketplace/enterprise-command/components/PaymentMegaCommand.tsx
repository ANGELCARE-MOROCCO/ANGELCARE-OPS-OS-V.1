...
export function PaymentMegaCommand({ initial }: { initial: AdminPaymentDossier }) {
  ...
  async function act(action: string) {
    ...
    try {
      const response = await fetch(`/api/angelcare-marketplace/admin/payments/${payment.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ action, reason }) })
  ...
  async function refund() {
    ...
    try {
      const response = await fetch(`/api/angelcare-marketplace/admin/payments/${payment.id}/refund`, {
        method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ amount: Number(refundAmount), reason: refundReason, idempotencyKey: `mega-refund:${payment.id}:${refundAmount}:${Date.now()}` }),
      })
  ...
  return <div className={styles.command}>
    <section className={styles.hero}><div className={styles.panelTitle}><div><div className={styles.eyebrow}>Finance Object Dossier</div><h1 className={styles.title}>{payment.public_reference}</h1><p className={styles.lead}>{payment.customer_name} · {payment.order_reference || payment.canonical_object_type || 'Transaction autonome'} · {payment.provider_key || payment.selected_method || 'Méthode non définie'}</p></div><div className={styles.toolbar}><button className={styles.button} onClick={pdf}><Download size={14} />PDF</button><Link className={styles.buttonSecondary} href="/angelcare-marketplace/admin/payments"><RefreshCcw size={14} />Registre</Link></div></div></section>
    ...
    <div className={styles.grid2}>
      <div className={styles.panel}>
        ...
        <div className={styles.rowActions} style={{ marginTop: 12 }}><button className={styles.button} disabled={busy || ['captured', 'refunded', 'partially_refunded'].includes(payment.status)} onClick={() => act('capture')}>Capturer</button><button className={styles.buttonSecondary} disabled={busy || ['captured', 'refunded', 'partially_refunded', 'failed', 'cancelled'].includes(payment.status)} onClick={() => act('failed')}>Marquer échec</button><button className={styles.buttonDanger} disabled={busy || ['captured', 'refunded', 'partially_refunded', 'failed', 'cancelled'].includes(payment.status)} onClick={() => act('cancelled')}>Annuler</button></div>
      ...
      <div className={styles.panel}>
        ...
        <button className={styles.buttonDanger} style={{ marginTop: 12 }} disabled={busy || !refundable || !refundAmount || Number(refundAmount) <= 0 || Number(refundAmount) > refundable} onClick={refund}><RotateCcw size={14} />Rembourser</button>
