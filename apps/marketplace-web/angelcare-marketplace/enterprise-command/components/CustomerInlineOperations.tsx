    ...
    {section === 'family' ? <>
      <div className={styles.grid2}>
        <section className={styles.panel}><div className={styles.panelTitle}><h3><UsersRound size={16} /> Responsables familiaux</h3><span className={styles.chip}>{data.guardians.length}</span></div>
          ...
          <div className={styles.inlineCreate}><div className={styles.grid3}><F label="Nouveau nom"><input className={styles.input} value={guardianName} onChange={e => setGuardianName(e.target.value)} /></F><F label="Relation"><select className={styles.select} value={guardianRelation} onChange={e => setGuardianRelation(e.target.value)}><option value="parent">Parent</option><option value="mother">Mère</option><option value="father">Père</option><option value="guardian">Tuteur</option><option value="relative">Proche autorisé</option></select></F><F label="Email"><input className={styles.input} value={guardianEmail} onChange={e => setGuardianEmail(e.target.value)} /></F></div><button className={styles.buttonSecondary} disabled={busy || !familyId || !guardianName} onClick={() => void call(`/api/angelcare-marketplace/admin/families/${familyId}/guardians`, 'POST', { fullName: guardianName, relationship: guardianRelation, email: guardianEmail, customerAccountId: txt(c, 'id') }, 'Responsable ajouté.')}><UserPlus size={14} />Ajouter responsable</button></div>
        </section>
...
function GuardianEditor({ row, familyId, customerId, busy, execute }: { row: Row; familyId: string; customerId: string; busy: boolean; execute: (u: string, m: string, b: Row, ok: string) => Promise<void> }) {
  ...
  return <ExpandRow title={name} subtitle={`${relationship} · ${email || phone || 'contact non renseigné'}`} status={status}><div className={styles.grid3}><F label="Nom"><input className={styles.input} value={name} onChange={e => setName(e.target.value)} /></F><F label="Relation"><select className={styles.select} value={relationship} onChange={e => setRelationship(e.target.value)}><option value="parent">Parent</option><option value="mother">Mère</option><option value="father">Père</option><option value="guardian">Tuteur</option><option value="relative">Proche autorisé</option></select></F><F label="Email"><input className={styles.input} value={email} onChange={e => setEmail(e.target.value)} /></F><F label="Téléphone"><input className={styles.input} value={phone} onChange={e => setPhone(e.target.value)} /></F><F label="Statut"><select className={styles.select} value={status} onChange={e => setStatus(e.target.value)}><option value="active">Active</option><option value="inactive">Inactive</option><option value="archived">Archived</option></select></F><F label="Principal"><label className={styles.checkRow}><input type="checkbox" checked={primary} onChange={e => setPrimary(e.target.checked)} /> Contact principal</label></F></div><button className={styles.button} disabled={busy || !familyId || !name} onClick={() => void execute(`/api/angelcare-marketplace/admin/families/${familyId}/guardians/${txt(row, 'id')}`, 'PATCH', { fullName: name, relationship, email, phone, isPrimary: primary, status, customerAccountId: customerId }, 'Responsable mis à jour.')}><PencilLine size={14} />Enregistrer</button></ExpandRow>
}
...
function ChildEditor({ row, customerId, busy, execute }: { row: Row; customerId: string; busy: boolean; execute: (u: string, m: string, b: Row, ok: string) => Promise<void> }) {
  ...
  return <ExpandRow title={firstName} subtitle={[birthDate, age, school].filter(Boolean).join(' · ')} status={status}><div className={styles.grid3}><F label="Prénom"><input className={styles.input} value={firstName} onChange={e => setFirstName(e.target.value)} /></F><F label="Naissance"><input className={styles.input} type="date" value={birthDate} onChange={e => setBirthDate(e.target.value)} /></F><F label="Tranche d’âge"><select className={styles.select} value={age} onChange={e => setAge(e.target.value)}><option value="0-2">0–2 ans</option><option value="3-5">3–5 ans</option><option value="6-9">6–9 ans</option><option value="10-12">10–12 ans</option><option value="13+">13+ ans</option></select></F><F label="Niveau scolaire"><input className={styles.input} value={school} onChange={e => setSchool(e.target.value)} /></F><F label="Statut"><select className={styles.select} value={status} onChange={e => setStatus(e.target.value)}><option value="active">Active</option><option value="inactive">Inactive</option><option value="archived">Archived</option></select></F></div><button className={styles.button} disabled={busy || !firstName} onClick={() => void execute(`/api/angelcare-marketplace/admin/customers/${customerId}/family/children/${txt(row, 'id')}`, 'PATCH', { first_name: firstName, birth_date: birthDate || null, age_group: age || null, school_level: school || null, status }, 'Enfant mis à jour.')}><Save size={14} />Enregistrer</button></ExpandRow>
}
