...
export function ProductMasterRegistry({ snapshot, openCreate = false }: { snapshot: CommerceProductAtelierSnapshot; openCreate?: boolean }) {
  ...
  const [create, setCreate] = useState(openCreate), [minimized, setMinimized] = useState<string[]>([]), [action, setAction] = useState<BulkAction | null>(null)
  ...
  async function execute() {
    ...
    for (const item of selected) {
      ...
      try {
        await atelierApi(`/api/angelcare-marketplace/admin/commerce/catalog-items/${item.id}/${action}`, { method: 'POST', body: JSON.stringify({}) })
        next.push({ itemKey: item.itemKey, id: item.id, previousState: item.status, newState: action === 'publish' ? 'published' : action === 'unpublish' ? 'paused' : 'archived', result: 'success' })
      } catch (error) {
    ...
    setResults(next); setNotice(`${next.filter(x => x.result === 'success').length} succès · ${next.filter(x => x.result === 'blocked').length} bloqués · ${next.filter(x => x.result === 'failed').length} échecs`); setBusy(false); setAction(null)
  }
  ...
  return <main className={styles.canvas}>
    ...
    {action ? <div className={styles.confirmBackdrop}><section className={styles.confirmCard}><h3>Confirm {action}</h3><p>{selected.length} product(s) will be processed individually; blocked products remain unchanged.</p><div className={styles.filterRow}><button type="button" className={styles.buttonSecondary} onClick={() => setAction(null)}>Cancel</button><button type="button" className={styles.button} disabled={busy} onClick={() => void execute()}>Confirm</button></div></section></div> : null}
  </main>
}
