...
function fields(pickers:StudioPickerData){return {
 ...
 mediaAssetKey:{type:'custom' as const,label:'Media Vault',render:({value,onChange}:any)=><MediaField pickers={pickers} value={typeof value==='string'?value:''} onChange={onChange}/>},mediaUrl:{type:'text' as const,label:'URL média externe · secours'},mediaAlt:{type:'text' as const,label:'Texte alternatif'},
 categoryKey:{type:'custom' as const,label:'Catégorie réelle',render:({value,onChange}:any)=><RecordField label="Catégorie" rows={pickers.categories} value={typeof value==='string'?value:''} onChange={onChange}/>},collectionKey:{type:'custom' as const,label:'Collection réelle',render:({value,onChange}:any)=><RecordField label="Collection" rows={pickers.collections} value={typeof value==='string'?value:''} onChange={onChange}/>},
