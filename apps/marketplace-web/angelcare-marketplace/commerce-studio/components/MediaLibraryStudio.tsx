...
const MEDIA_ACCEPT='image/jpeg,image/png,image/webp,image/avif,image/svg+xml,video/mp4,video/webm,application/pdf'
...
export function MediaLibraryStudio({ initialMedia, mode = 'library', canManage = false }: { initialMedia: MediaAsset[]; mode?: string; canManage?: boolean }) {
  ...
  async function replaceAsset(){
    const file=replacementFiles[0];if(!canManage||!file||!selected)return
    ...
    if(result){setSelected(result);setMedia((current)=>current.map((asset)=>asset.id===result.id?result:asset));setReplacementFiles([])}
    setProgress({})
  }
