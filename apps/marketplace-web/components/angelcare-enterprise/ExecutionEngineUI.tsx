...
export type ExecTone = 'blue' | 'green' | 'amber' | 'red' | 'purple' | 'cyan' | 'slate'

const tones: Record<ExecTone, { bg: string; text: string; border: string; glow: string; dark: string }> = {
  ...
  return (
    <div
      style={{
        ...
        padding: 2,
        background:
          'radial-gradient(circle at top left, rgba(37,99,235,.14), transparent 34%), radial-gradient(circle at top right, rgba(220,38,38,.10), transparent 28%), linear-gradient(180deg,#f8fafc,#eef2ff)',
      }}
    >
      {children}
    </div>
...
export function ExecHero({
  ...
  subtitle: string
  actions?: ReactNode
  stats?: { label: string; value: ReactNode; detail?: string }[]
  tone?: ExecTone
}) {
  const t = tones[tone]
  return (
    <section
      style={{
        ...
        color: 'white',
        marginBottom: 22,
        background: `linear-gradient(135deg,#020617 0%,${t.dark} 54%,#0f766e 100%)`,
        boxShadow: '0 34px 100px rgba(2,6,23,.30)',
      }}
    >
      <div
        style={{
          ...
          inset: 0,
          background:
            'radial-gradient(circle at 18% 12%, rgba(255,255,255,.18), transparent 25%), radial-gradient(circle at 82% 22%, rgba(34,211,238,.19), transparent 25%)',
          pointerEvents: 'none',
        }}
      />
      <div
      ...
      >
        <div>
          ...
          <h1 style={{ margin: '10px 0', fontSize: 42, lineHeight: 1.02, letterSpacing: -1.2 }}>
            {title}
          </h1>
          <p style={{ color: '#dbeafe', margin: 0, fontWeight: 760, lineHeight: 1.6, maxWidth: 980 }}>
            {subtitle}
          </p>
          ...
          {(stats || []).slice(0, 5).map((item) => (
            ...
            >
              <div>
                ...
                {item.detail ? (
                  <div style={{ color: '#dbeafe', fontSize: 11, fontWeight: 700, marginTop: 2 }}>
                    {item.detail}
                  </div>
                ) : null}
              </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ paddingLeft: 5, marginBottom: 15 }}>
        ...
        {subtitle ? (
          <p style={{ color: '#64748b', margin: '5px 0 0', fontWeight: 760, lineHeight: 1.45 }}>
            {subtitle}
          </p>
        ) : null}
      </div>
...
export function ExecNav({ items }: { items: { label: string; href: string; tone?: ExecTone }[] }) {
  return (
