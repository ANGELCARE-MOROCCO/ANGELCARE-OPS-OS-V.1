...
const entityRoutes: Record<string, string> = {
  ...
  artifact: "/ac-capital-os/artifacts",
};

type InspectorKind = "workflow" | "agent" | "event" | "integrity" | "approval" | "doctrine" | "metric" | "artifact" | "dead-letter" | "notification";
...
export function OrchestratorPage({ actor }: { actor: CapitalActor }) {
  ...
  const deadLetters = rows(data.deadLetters);
  const schedules = rows(data.schedules);
  const agentOutputs = rows(data.agentOutputs);
  const commandResults = rows(data.commandResults);
  ...
  const unreadNotifications = notifications.filter((item) => text(item.status) === "unread");
  const openDeadLetters = deadLetters.filter((item) => text(item.status) === "open");
  ...
  const recommendation = useMemo(() => {
    ...
    if (openDeadLetters[0]) return { type: "dead-letter" as const, row: openDeadLetters[0], title: "Recover a terminal department failure", detail: text(openDeadLetters[0].error_message) || text(openDeadLetters[0].event_type), action: "Open dead letter" };
  ...
  }, [activeWorkflows, criticalIssues, events, openDeadLetters, pendingApprovals, queuedEvents]);
        ...
        : inspector?.kind === "integrity" ? text(selected.title) || "Integrity issue"
          : inspector?.kind === "approval" ? text(selected.approval_type) || "Founder approval"
            : inspector?.kind === "doctrine" ? "Compiled capital doctrine"
              : inspector?.kind === "artifact" ? text(selected.title) || "Capital artifact"
                : inspector?.kind === "dead-letter" ? text(selected.event_type) || "Dead letter"
                  : inspector?.kind === "notification" ? text(selected.title) || "Notification"
        ...
        : inspector?.kind === "integrity" ? "Broken-chain prevention"
          : inspector?.kind === "approval" ? "Version-bound founder authority"
            : inspector?.kind === "doctrine" ? "Executable institutional policy"
              : inspector?.kind === "artifact" ? "Versioned document and export control"
                : inspector?.kind === "dead-letter" ? "Terminal failure recovery"
                  : inspector?.kind === "notification" ? "Governed attention signal"
  ...
  return (
    ...
    >
      ...
      <section className={styles.ultraSystems}>
        <article>
          <header><div><span>DOCUMENT FACTORY</span><h3>Versioned capital artifacts</h3><p>PDF, DOCX, XLSX, CSV, JSON and complete ZIP packs.</p></div><FileArchive size={22}/></header>
          <strong>{artifacts.length}</strong>
          <div>{artifacts.slice(0,4).map((artifact)=><button key={text(artifact.id)} onClick={()=>setInspector({kind:"artifact",row:artifact})}><span>{text(artifact.title)}</span><em>{text(artifact.approval_status)} · v{Number(artifact.current_version||1)}</em><ChevronRight size={14}/></button>)}</div>
          <button className={styles.systemLink} onClick={()=>router.push("/ac-capital-os/artifacts")}>Open document factory <ArrowUpRight size={14}/></button>
        </article>
        <article>
          <header><div><span>AUTONOMOUS SCHEDULES</span><h3>Durable agent cadence</h3><p>Schedules, next execution, health and controlled activation.</p></div><CalendarClock size={22}/></header>
          <strong>{schedules.filter((item)=>item.enabled===true).length}</strong>
          <div>{schedules.slice(0,5).map((schedule)=><button key={text(schedule.id)} onClick={()=>navigateToWorkspace("ai-operations")}><span>{text(schedule.agent_key)}</span><em>{schedule.enabled===true?"enabled":"paused"} · {text(schedule.frequency_key)}</em><ChevronRight size={14}/></button>)}</div>
          <button className={styles.systemLink} onClick={()=>router.push("/ac-capital-os/ai-control")}>Open AI Operations <ArrowUpRight size={14}/></button>
        </article>
        <article data-alert={openDeadLetters.length>0?"true":"false"}>
          <header><div><span>EXCEPTIONS & SIGNALS</span><h3>Notifications and dead letters</h3><p>Recover terminal failures and inspect every governed attention signal.</p></div><BellRing size={22}/></header>
          <strong>{unreadNotifications.length + openDeadLetters.length}</strong>
          <div>{openDeadLetters.slice(0,2).map((item)=><button key={text(item.id)} onClick={()=>setInspector({kind:"dead-letter",row:item})}><span>{text(item.event_type)}</span><em>dead letter · {text(item.error_code)}</em><ChevronRight size={14}/></button>)}{unreadNotifications.slice(0,3).map((item)=><button key={text(item.id)} onClick={()=>setInspector({kind:"notification",row:item})}><span>{text(item.title)}</span><em>{text(item.severity)} · unread</em><ChevronRight size={14}/></button>)}</div>
          <span className={styles.systemMeta}>{agentOutputs.length} agent output(s) · {commandResults.length} command result(s)</span>
        </article>
      </section>
      ...
      >
        ...
        {inspector?.kind === "workflow" ? (
          <div className={styles.drawerStack}>
            ...
            <section className={styles.connectedGrid}>
              ...
              ].map(([label, id, route]) => <button key={String(label)} disabled={!id} onClick={() => router.push(String(route))}><Link2 size={17} /><span>{text(label)}</span><strong>{id ? text(id).slice(0, 12) : "Not created"}</strong><ChevronRight size={14} /></button>)}
            </section>
        ...
        {inspector?.kind === "artifact" ? (
          <div className={styles.drawerStack}>
            <section className={styles.approvalHero}><FileArchive size={26}/><div><span>{text(selected.approval_status)} · v{Number(selected.current_version||1)}</span><h3>{text(selected.title)}</h3><p>{text(selected.artifact_type).replaceAll("-"," ")} · {text(selected.confidentiality)}</p></div></section>
            <section className={styles.drawerCommands}><a href={`/api/ac-capital-os/artifacts/${text(selected.id)}/download?format=pdf`}><FileArchive size={16}/> Download PDF</a><a href={`/api/ac-capital-os/artifacts/${text(selected.id)}/download?format=docx`}><FileArchive size={16}/> Download DOCX</a><a href={`/api/ac-capital-os/artifacts/${text(selected.id)}/download?format=zip`}><Download size={16}/> Full ZIP</a><button onClick={()=>router.push("/ac-capital-os/artifacts")}><ArrowUpRight size={16}/> Open factory</button></section>
            <details open className={styles.jsonDetails}><summary>Content snapshot</summary><pre>{pretty(selected.content_snapshot)}</pre></details>
            <details className={styles.jsonDetails}><summary>Source snapshot</summary><pre>{pretty(selected.source_snapshot)}</pre></details>
          </div>
        ) : null}

        {inspector?.kind === "dead-letter" ? (
          <div className={styles.drawerStack}>
            <section className={styles.integrityHero} data-severity="critical"><AlertTriangle size={24}/><div><span>TERMINAL FAILURE · {text(selected.error_code)}</span><h3>{text(selected.event_type)}</h3><p>{text(selected.error_message)}</p></div></section>
            <section className={styles.factGrid}><article><span>Attempts</span><strong>{count(selected.attempts)}</strong></article><article><span>Status</span><strong>{text(selected.status)}</strong></article><article><span>Entity</span><strong>{text(selected.entity_type)}</strong></article><article><span>Last failure</span><strong>{formatDate(selected.last_failed_at)}</strong></article></section>
            <label className={styles.noteField}><span>Recovery note</span><textarea value={decisionNote} onChange={(event)=>setDecisionNote(event.target.value)} placeholder="Record why this terminal event can be retried."/></label>
            <section className={styles.drawerCommands}><button onClick={()=>void act("dead-letter-retry",{deadLetterId:selected.id,note:decisionNote},"Dead letter requeued")} disabled={Boolean(busy)}><RotateCcw size={16}/> Requeue event</button></section>
            <details open className={styles.jsonDetails}><summary>Original payload</summary><pre>{pretty(selected.payload)}</pre></details>
          </div>
        ) : null}

        {inspector?.kind === "notification" ? (
          <div className={styles.drawerStack}>
            <section className={styles.doctrineHero}><BellRing size={26}/><div><span>{text(selected.severity)} · {text(selected.status)}</span><h3>{text(selected.title)}</h3><p>{text(selected.message)}</p></div></section>
            <section className={styles.factGrid}><article><span>Workspace</span><strong>{text(selected.workspace_key)}</strong></article><article><span>Entity</span><strong>{text(selected.entity_type)}</strong></article><article><span>Created</span><strong>{formatDate(selected.created_at)}</strong></article><article><span>Recipient</span><strong>{text(selected.recipient_role||selected.recipient_id)||"All authorized"}</strong></article></section>
            <section className={styles.drawerCommands}>{text(selected.action_href)?<button onClick={()=>router.push(text(selected.action_href))}><ArrowUpRight size={16}/> Open action</button>:null}</section>
            <details className={styles.jsonDetails}><summary>Notification metadata</summary><pre>{pretty(selected.metadata)}</pre></details>
          </div>
        ) : null}
