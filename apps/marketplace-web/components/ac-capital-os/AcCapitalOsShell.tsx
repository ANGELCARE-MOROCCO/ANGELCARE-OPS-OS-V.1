...
export function AcCapitalOsShell({
  children,
  activeWorkspaceKey = 'data-room',
  zipLabel = 'Mega ZIP 08',
  subtitle = 'AngelCare internal capital intelligence module',
      ...
      <aside className={styles.sidebar} aria-label="AC CAPITAL OS workspaces">
        ...
        <nav className={styles.workspaceNav}>
          {AC_CAPITAL_OS_WORKSPACES.map((workspace) => {
            ...
            const isReady = workspace.status === 'foundation-ready' || workspace.status === 'activated-mz2' || workspace.status === 'activated-mz3' || workspace.status === 'activated-mz4' || workspace.status === 'activated-mz5' || workspace.status === 'activated-mz6' || workspace.status === 'activated-mz7' || workspace.status === 'activated-mz8';
            return (
              <a
                key={workspace.key}
      ...
      <section className={styles.mainSurface}>
        <header className={styles.topbar}>
          ...
          <div className={styles.topbarActions}>
            ...
            <a className={styles.apiBadge} href="/api/ac-capital-os/foundation">Foundation API</a>
            <a className={styles.apiBadge} href="/api/ac-capital-os/executive-cockpit">Cockpit API</a>
            <a className={styles.apiBadge} href="/api/ac-capital-os/capital-radar">Radar API</a>
            <a className={styles.apiBadge} href="/api/ac-capital-os/qualification-engine">Qualification API</a>
            <a className={styles.apiBadge} href="/api/ac-capital-os/funder-intelligence">Funder API</a>
            <a className={styles.apiBadge} href="/api/ac-capital-os/capital-doctrine">Doctrine API</a>
            <a className={styles.apiBadge} href="/api/ac-capital-os/case-builder">Case Builder API</a>
            <a className={styles.apiBadge} href="/api/ac-capital-os/data-room">Data Room API</a>
          </div>
        </header>
        {children}
      </section>
