...
export default function AmbassadorPayoutsRoute({ snapshot, loading, refreshing, error, success, onRefresh }: Props) {
  ...
  const readyToPay = enriched.filter((item) => ["payment_prepared", "to_pay"].includes(item.status))
  ...
  const rejected = enriched.filter((item) => item.status === "rejected")
  const blocked = enriched.filter((item) => ["blocked", "disputed", "correction"].includes(item.status) || Boolean(item.meta.exception?.type))
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-5 py-5 text-slate-950 xl:px-7">
      <div className="mx-auto w-full max-w-none space-y-5">
        <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950">Droits financiers & paiements</h1><p className="mt-1 text-[11px] font-black uppercase tracking-[0.14em] text-blue-700">Financial Entitlement & Payment Operations</p>
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => openModal("create")} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-blue-200 hover:bg-blue-700"><Plus size={17} /> Créer un droit financier</button>
              ...
              <button onClick={() => openModal("payment")} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black"><Banknote size={17} /> Autoriser un paiement</button>
        ...
        <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm">
          <div className="grid xl:grid-cols-[0.72fr_1.28fr]">
            <div className="bg-[#082b4d] p-6 text-white">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#a9cce9]">Posture d’engagement financier</p>
              <p className="mt-4 text-sm font-bold text-[#d9e7f3]">Exposition contrôlée hors dossiers rejetés</p>
              <p className="mt-2 text-4xl font-black tabular-nums text-white">{money(exposure)}</p>
              <div className="mt-6 grid grid-cols-2 gap-4 border-t border-white/15 pt-5">
                <div><p className="text-[10px] font-black uppercase tracking-[0.12em] text-[#a9cce9]">Bénéficiaires concernés</p><p className="mt-2 text-2xl font-black tabular-nums text-white">{new Set(enriched.map((item) => String(item.ambassador_id || ""))).size}</p></div>
                <div><p className="text-[10px] font-black uppercase tracking-[0.12em] text-[#a9cce9]">Exceptions ouvertes</p><p className="mt-2 text-2xl font-black tabular-nums text-white">{exceptions.length}</p></div>
              </div>
            </div>
            <div className="p-5 sm:p-6">
              <div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-black text-slate-950">Balance de règlement</h2><p className="mt-1 text-xs font-semibold text-slate-500">Montants backend répartis par état financier réellement présent.</p></div><CircleDollarSign size={20} className="text-blue-700" /></div>
              <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                {[{ label: "À contrôler", rows: pending, tone: "border-amber-200 bg-amber-50 text-amber-900" }, { label: "Approuvé", rows: approved, tone: "border-blue-200 bg-blue-50 text-blue-800" }, { label: "Prêt au paiement", rows: readyToPay, tone: "border-cyan-200 bg-cyan-50 text-cyan-800" }, { label: "Payé période", rows: paid, tone: "border-emerald-200 bg-emerald-50 text-emerald-800" }, { label: "Rejeté", rows: rejected, tone: "border-rose-200 bg-rose-50 text-rose-800" }].map((item) => <div key={item.label} className={`rounded-2xl border p-4 ${item.tone}`}><p className="text-[9px] font-black uppercase tracking-[0.13em]">{item.label}</p><p className="mt-2 text-xl font-black tabular-nums">{money(item.rows.reduce((sum, row) => sum + row.amount, 0))}</p><p className="mt-1 text-[10px] font-bold opacity-80">{item.rows.length} dossier(s)</p></div>)}
              </div>
              {blocked.length ? <button type="button" onClick={() => setStatusFilter("blocked")} className="mt-4 flex w-full items-center justify-between rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-left text-rose-800"><span className="flex items-center gap-2 text-xs font-black"><ShieldAlert size={15} /> Registre d’exception nécessitant une action</span><span className="font-black tabular-nums">{blocked.length}</span></button> : null}
            </div>
          </div>
        </section>
        ...
        <Panel>
          ...
          <div className="overflow-x-auto">
            ...
            {loading && <div className="flex min-h-[180px] items-center justify-center gap-2 text-sm font-black"><Loader2 className="animate-spin" size={18} /> Chargement du registre financier…</div>}
          </div>
        </Panel>
      ...
      {modal === "create" && <ModalFrame title="Créer un droit financier contrôlé" subtitle="Source de revenu réelle, attribution, base éligible, ajustements et preuve avant approbation financière." icon={FileCheck2} onClose={() => setModal(null)} footer={<div className="flex items-center justify-between gap-4"><div className="text-sm font-black">Contrôles {createReadiness.filter((item) => item.ok).length}/{createReadiness.length} · Commission nette {money(netDue)}{duplicateRisk ? " · Doublon détecté" : ""}</div><div className="flex gap-2"><button onClick={() => setModal(null)} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black">Annuler</button><button disabled={busy || createScore < 100} onClick={() => submitCreate("draft")} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black disabled:opacity-40">Enregistrer brouillon</button><button disabled={busy || createScore < 100} onClick={() => submitCreate("pending")} className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-black text-white disabled:opacity-40">{busy ? "Synchronisation..." : "Créer et soumettre"}</button></div></div>}>
      ...
      {modal === "approve" && <ModalFrame title="Approbation financière contrôlée" subtitle="Contrôlez attribution, preuves, calcul 10%, risques et décision pour un ou plusieurs incentives." icon={ShieldCheck} onClose={() => setModal(null)} footer={<div className="flex items-center justify-between gap-4"><div className="text-sm font-black">{approvalDraft.ids.length} dossier(s) · {money(enriched.filter((item) => approvalDraft.ids.includes(String(item.id))).reduce((sum, item) => sum + item.amount, 0))}</div><div className="flex gap-2"><button onClick={() => setModal(null)} className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-black">Fermer</button><button disabled={busy || !approvalDraft.ids.length || !approvalDraft.note.trim()} onClick={submitApproval} className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-black text-white disabled:opacity-40">{busy ? "Traitement..." : "Décider & journaliser"}</button></div></div>}>
        <div className="grid gap-5 xl:grid-cols-[0.95fr_1.1fr_0.85fr]">
          ...
          <div className="space-y-5"><Panel><h3 className="text-lg font-black">Contrôles de validation</h3><div className="mt-4"><ReadinessList items={[{ label: "Dossier(s) sélectionné(s)", ok: approvalDraft.ids.length > 0 }, { label: "Approbateur identifié", ok: Boolean(approvalDraft.reviewer.trim()) }, { label: "Note de décision", ok: Boolean(approvalDraft.note.trim()) }, { label: "Taux contractuel contrôlé", ok: approvalDraft.ids.every((id) => number(enriched.find((item) => String(item.id) === id)?.commissionRate) === COMMISSION_RATE) }, { label: "Aucun dossier déjà payé", ok: approvalDraft.ids.every((id) => !["paid", "reconciled"].includes(enriched.find((item) => String(item.id) === id)?.status || "")) }]} /></div></Panel>{feedback && <div className={`rounded-2xl p-4 text-sm font-black ${feedback.type === "error" ? "bg-rose-50 text-rose-800" : "bg-emerald-50 text-emerald-800"}`}>{feedback.text}</div>}</div>
        </div>
      </ModalFrame>}
