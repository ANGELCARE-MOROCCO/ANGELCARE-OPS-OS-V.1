      ...
      <section className="overflow-hidden rounded-[2rem] border border-slate-900 bg-[radial-gradient(circle_at_top_right,rgba(244,63,94,0.30),transparent_30%),linear-gradient(135deg,#020617,#0f172a_52%,#312e81)] text-slate-950 shadow-2xl shadow-slate-300">
        <div className="grid gap-8 p-6 lg:grid-cols-[1.5fr_.8fr] lg:p-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
            <div className="rounded-3xl border border-slate-200 bg-slate-100 p-5"><p className="text-xs font-black uppercase tracking-widest text-slate-600">Workspace readiness</p><p className="mt-2 text-4xl font-black">{averageReadiness}%</p><div className="mt-4"><Meter value={averageReadiness} /></div></div>
            <div className="grid grid-cols-3 gap-3"><div className="rounded-2xl border border-slate-200 bg-slate-100 p-4"><p className="text-xs font-black uppercase text-slate-600">Urgent</p><p className="mt-2 text-2xl font-black">{urgent.length}</p></div><div className="rounded-2xl border border-slate-200 bg-slate-100 p-4"><p className="text-xs font-black uppercase text-slate-600">Review</p><p className="mt-2 text-2xl font-black">{reviewQueue.length}</p></div><div className="rounded-2xl border border-slate-200 bg-slate-100 p-4"><p className="text-xs font-black uppercase text-slate-600">Ready</p><p className="mt-2 text-2xl font-black">{publishable.length}</p></div></div>
          </div>
        </div>
      </section>
