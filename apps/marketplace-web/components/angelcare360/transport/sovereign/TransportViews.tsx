...
import { EmptyState, StatusPill, formatDate, formatMoney, formatTime } from './TransportPrimitives'
...
function InPageNav({items}:{items:Array<{href:string;label:string}>}){
  return <nav className={styles.inPageNav} aria-label="Navigation du dossier">{items.map(item=><a key={item.href} className={styles.anchorChip} href={item.href}>{item.label}</a>)}</nav>
}
...
export function RoutesCommand({snapshot}:{snapshot:TransportSnapshot}){
  return <><section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Route Command</div><h2 className={styles.sectionTitle}>Circuits et préparation</h2><p className={styles.sectionCopy}>Chaque circuit articule direction, type, véhicule, chauffeur, arrêts et population affectée.</p></div></div>
    ...
    <TransportDrawer trigger="Créer / modifier un circuit" title="Route Design Studio"><RouteStudio routes={snapshot.routes} vehicles={snapshot.vehicles} drivers={snapshot.drivers}/></TransportDrawer></>
}
...
export function RouteOperationsChamber({snapshot,route,stops,assignments,runs}:{snapshot:TransportSnapshot;route:TransportRoute;stops:TransportStop[];assignments:TransportAssignment[];runs:TransportRun[]}){
  return <><section id="route-overview" className={styles.dossierHero}><div className={styles.identity}><div className={styles.sectionKicker}>Route Operations Chamber</div><h2>{route.code} · {route.label}</h2><p className={styles.sectionCopy}>Circuit {route.direction} · {route.routeType}</p><div className={styles.identityGrid}>
    ...
    <InPageNav items={[{href:'#route-overview',label:'Vue opérationnelle'},{href:'#route-stops',label:'Arrêts & élèves'},{href:'#route-runs',label:'Courses & historique'}]}/>
    <section id="route-stops" className={styles.section}><div className={styles.gridTwo}><div className={styles.panel}><div className={styles.sectionHeader}><div><h2 className={styles.sectionTitle}>Séquence des arrêts</h2><p className={styles.sectionCopy}>Heures planifiées; aucune ETA live.</p></div></div>{stops.length?<div className={styles.timeline}>{stops.map(st=><div className={styles.timelineItem} key={st.id}><div className={styles.time}>{formatTime(st.plannedTime)}</div><div className={styles.rail}/><div className={styles.timelineBody}><strong>{st.order}. {st.label}</strong><p>{st.zone||st.address||'Localisation textuelle non renseignée'} · {st.studentCount} élève(s)</p></div></div>)}</div>:<EmptyState title="Aucun arrêt" copy="Ajoutez les arrêts avant la mise en service du circuit."/ >}</div>
...
export function StopsCommand({snapshot}:{snapshot:TransportSnapshot}){
  return <><section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Stop Sequence Control</div><h2 className={styles.sectionTitle}>Arrêts planifiés</h2><p className={styles.sectionCopy}>Adresse, zone, ordre et heure planifiée. Les coordonnées servent uniquement à la cartographie de planification.</p></div></div>
    ...
    <TransportDrawer trigger="Créer / ordonnancer un arrêt" title="Stop Sequence Control"><StopStudio routes={snapshot.routes} stops={snapshot.stops}/></TransportDrawer></>
}
...
export function FleetReadiness({snapshot}:{snapshot:TransportSnapshot}){
  return <><section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Fleet Readiness Command</div><h2 className={styles.sectionTitle}>Flotte et horizons de conformité</h2></div></div>
    ...
    <TransportDrawer trigger="Créer / modifier un véhicule" title="Fleet Readiness Studio"><VehicleStudio vehicles={snapshot.vehicles}/></TransportDrawer></>
}
...
export function VehicleDossier({snapshot,vehicle,routes,runs,safety}:{snapshot:TransportSnapshot;vehicle:TransportVehicle;routes:TransportRoute[];runs:TransportRun[];safety:TransportSnapshot['safetyChecks']}){
  return <><section id="vehicle-overview" className={styles.dossierHero}><div className={styles.identity}><div className={styles.sectionKicker}>Vehicle Readiness Dossier</div><h2>{vehicle.code} · {vehicle.label}</h2><p className={styles.sectionCopy}>{vehicle.vehicleType} · {vehicle.plateNumber||'Immatriculation non renseignée'}</p><div className={styles.identityGrid}><div className={styles.identityCell}><span>État</span><StatusPill value={vehicle.status} tone={tone(vehicle.status)}/></div><div className={styles.identityCell}><span>Capacité</span><strong>{vehicle.capacity}</strong></div><div className={styles.identityCell}><span>Ceintures</span><strong>{vehicle.seatbeltCount||'Non renseigné'}</strong></div><div className={styles.identityCell}><span>Assurance</span><strong className={expired(vehicle.insuranceExpiry)?styles.bad:''}>{formatDate(vehicle.insuranceExpiry)}</strong></div><div className={styles.identityCell}><span>Inspection</span><strong className={expired(vehicle.inspectionExpiry)?styles.bad:''}>{formatDate(vehicle.inspectionExpiry)}</strong></div><div className={styles.identityCell}><span>Circuits actifs</span><strong>{vehicle.routeCount}</strong></div></div></div>
    ...
    <InPageNav items={[{href:"#vehicle-overview",label:"Readiness"},{href:"#vehicle-relationships",label:"Circuits & sécurité"},{href:"#vehicle-history",label:"Historique courses"}]}/>
    <section id="vehicle-relationships" className={styles.section}><div className={styles.gridTwo}><div className={styles.panel}><h2 className={styles.sectionTitle}>Circuits affectés</h2>{routes.length?routes.map(r=><div className={styles.attentionItem} key={r.id}><div className={styles.num}>{r.assignmentCount}</div><div><Link href={`${BASE}/circuits/${r.id}`}><strong>{r.code} · {r.label}</strong></Link><p>{r.direction} · {r.stopCount} arrêts</p></div></div>):<EmptyState title="Aucun circuit" copy="Ce véhicule n’est affecté à aucun circuit actif."/ >}</div><div className={styles.panel}><h2 className={styles.sectionTitle}>Contrôles sécurité récents</h2>{safety.length?safety.slice(0,10).map(c=><div className={styles.attentionItem} key={c.id}><div className={styles.num}>•</div><div><strong>{c.checkType} · {c.result}</strong><p>{formatDate(c.checkedAt,true)} · {c.notes||'Sans note'}</p></div></div>):<EmptyState title="Aucun contrôle enregistré" copy="Le véhicule n’a pas encore de contrôle sécurité dans l’autorité avancée."/ >}</div></div></section>
    <section id="vehicle-history" className={styles.section}><h2 className={styles.sectionTitle}>Courses</h2><RunsTable runs={runs}/></section>
  </>
}
...
export function DriversCommand({snapshot}:{snapshot:TransportSnapshot}){
  ...
  return <><section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Driver Readiness</div><h2 className={styles.sectionTitle}>Chauffeurs</h2></div></div><div className={styles.cardGrid}>{snapshot.drivers.map(d=><article className={styles.card} key={d.id}><div className={styles.cardTop}><span className={styles.cardCode}>{d.code}</span><StatusPill value={d.status} tone={tone(d.status)}/></div><h3 className={styles.cardTitle}>{d.fullName}</h3><div className={styles.cardMeta}>{d.phone||'Téléphone non renseigné'}<br/>Permis {d.licenseNumber||'—'} · expire {formatDate(d.licenseExpiry)}</div><div className={styles.cardFooter}><div className={styles.metricPair}><strong>{d.routeCount}</strong><span>circuits</span></div><div className={styles.metricPair}><strong>{d.runCountToday}</strong><span>courses jour</span></div><div className={styles.metricPair}><strong className={expired(d.licenseExpiry)?styles.bad:''}>{expired(d.licenseExpiry)?'EXP':'OK'}</strong><span>permis</span></div></div></article>)}</div></section><TransportDrawer trigger="Gérer les chauffeurs" title="Driver Readiness"><DriverStudio drivers={snapshot.drivers} staff={snapshot.staff}/></TransportDrawer></>
}
...
export function AssignmentsMatrix({snapshot}:{snapshot:TransportSnapshot}){
  return <><section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Student Mobility Matrix</div><h2 className={styles.sectionTitle}>Affectations élèves</h2><p className={styles.sectionCopy}>Circuit, arrêt, direction et période de service. Le montant affiché est un montant Transport enregistré, pas une facture Finance.</p></div></div>
    ...
    <TransportDrawer trigger="Affecter un élève" title="Student Mobility Assignment"><AssignmentStudio students={snapshot.students} routes={snapshot.routes} stops={snapshot.stops}/></TransportDrawer></>
}
...
export function RunsBoard({snapshot}:{snapshot:TransportSnapshot}){
  ...
  return <><section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Daily Movement Board</div><h2 className={styles.sectionTitle}>Courses et exécution</h2><p className={styles.sectionCopy}>État enregistré de l’opération; aucun véhicule en mouvement n’est simulé.</p></div></div><RunsTable runs={[...snapshot.runs].sort((a,b)=>(b.runDate+b.startedAt).localeCompare(a.runDate+a.startedAt))}/></section>
    {snapshot.authority==='advanced'?<TransportDrawer trigger="Ouvrir une course" title="Daily Movement Board"><RunStudio routes={snapshot.routes} vehicles={snapshot.vehicles} drivers={snapshot.drivers} locked={locked}/></TransportDrawer>:<div className={styles.truthBox}><strong>Course avancée non disponible sur l’autorité historique.</strong></div>}</>
}
...
function FieldRunConsoles({snapshot,mode}:{snapshot:TransportSnapshot;mode:'pickup'|'dropoff'}){
  const locked=snapshot.authority!=='advanced'||!snapshot.integrity.installed||!snapshot.integrity.safeForOperations
  const runs=snapshot.runs.filter(r=>r.runDate===today()&&r.runType===mode&&['planned','started','in_progress','incident'].includes(r.status))
  if(snapshot.authority!=='advanced')return <div className={styles.truthBox}><strong>Console d’exécution indisponible sur l’autorité historique.</strong><span>Le plan d’affectation reste consultable sans simuler une course.</span></div>
  return <section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Field Operations Mode</div><h2 className={styles.sectionTitle}>Consoles de course actives</h2><p className={styles.sectionCopy}>Actions terrain enregistrées: arrivée à l’arrêt, montée, absence, dépôt, retard, incident et clôture. Aucun GPS live.</p></div><TransportDrawer trigger="Ouvrir une nouvelle course" title="Daily Movement Board"><RunStudio routes={snapshot.routes} vehicles={snapshot.vehicles} drivers={snapshot.drivers} locked={locked}/></TransportDrawer></div>
    {runs.length?<div className={styles.fieldRunGrid}>{runs.map(run=>{
      const assignments=snapshot.assignments.filter(a=>a.routeId===run.routeId&&a.status==='active')
      const students=assignments.map(a=>snapshot.students.find(st=>st.id===a.studentId)).filter(Boolean) as TransportSnapshot['students']
      const stops=snapshot.stops.filter(st=>st.routeId===run.routeId&&st.status!=='archived').sort((a,b)=>a.order-b.order)
      return <div className={styles.fieldRunCard} key={run.id}><div><span>{run.routeCode}</span><strong>{run.routeLabel}</strong><p>{run.driverName||'Chauffeur non affecté'} · {run.vehicleLabel||'Véhicule non affecté'}</p></div><StatusPill value={run.status} tone={tone(run.status)}/><TransportDrawer trigger={`Console ${run.routeCode}`} title={`${mode==='pickup'?'Ramassage':'Dépôt'} · ${run.routeCode}`}><RunEventConsole run={run} students={students} stops={stops} locked={locked}/></TransportDrawer></div>
    })}</div>:<EmptyState title="Aucune course active aujourd’hui" copy="Ouvrez une course uniquement lorsque l’autorité avancée et le contrôle de sécurité l’autorisent."/>}
  </section>
}
export function PickupControl({snapshot}:{snapshot:TransportSnapshot}){const rows=operationRows(snapshot,'pickup');return <><OperationControl title="Pickup Operations" copy="Ramassage quotidien fondé sur les événements réellement enregistrés." rows={rows} /><FieldRunConsoles snapshot={snapshot} mode="pickup"/><PlannedRoster snapshot={snapshot} mode="pickup" /></>}
export function DropoffControl({snapshot}:{snapshot:TransportSnapshot}){const rows=operationRows(snapshot,'dropoff');return <><OperationControl title="Dropoff Operations" copy="Dépôt quotidien, distinct du ramassage, avec preuve d’événement enregistrée." rows={rows} /><FieldRunConsoles snapshot={snapshot} mode="dropoff"/><PlannedRoster snapshot={snapshot} mode="dropoff" /></>}
function PlannedRoster({snapshot,mode}:{snapshot:TransportSnapshot;mode:'pickup'|'dropoff'}){
  const planned=snapshot.assignments.filter(a=>a.status==='active'&&(a.serviceDirection==='round_trip'||a.serviceDirection===mode))
  return <section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Plan opérationnel</div><h2 className={styles.sectionTitle}>{mode==='pickup'?'Élèves attendus au ramassage':'Élèves attendus au dépôt'}</h2><p className={styles.sectionCopy}>Cette liste provient des affectations actives. Elle ne prétend pas qu’une course ou un événement de montée/dépôt a déjà été enregistré.</p></div></div>{planned.length?<div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Élève</th><th>Circuit</th><th>Arrêt</th><th>Direction</th><th>État source</th></tr></thead><tbody>{planned.map(a=><tr key={a.id}><td><strong>{a.studentName}</strong><span className={styles.subtle}>{a.studentCode}</span></td><td>{a.routeCode}<span className={styles.subtle}>{a.routeLabel}</span></td><td>{a.stopLabel||'—'}</td><td>{a.serviceDirection}</td><td><StatusPill value="Planifié" tone="neutral"/></td></tr>)}</tbody></table></div>:<EmptyState title="Aucune affectation active" copy="Aucun élève n’est actuellement attendu sur ce sens de service."/>}</section>
}
function OperationControl({title,copy,rows}:{title:string;copy:string;rows:any[]}){return <section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>{title}</div><h2 className={styles.sectionTitle}>{title==='Pickup Operations'?'Ramassage':'Dépôt'} aujourd’hui</h2><p className={styles.sectionCopy}>{copy}</p></div></div>{rows.length?<div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Élève</th><th>Circuit</th><th>Arrêt</th><th>État</th><th>Heure enregistrée</th><th>Course</th></tr></thead><tbody>{rows.map(({run,a,state,time})=><tr key={`${run.id}-${a.id}`}><td><strong>{a.studentName}</strong><span className={styles.subtle}>{a.studentCode}</span></td><td>{a.routeCode}</td><td>{a.stopLabel||'—'}</td><td><StatusPill value={state} tone={state==='confirmed'?'good':state==='absent'?'bad':'warn'}/></td><td>{formatTime(time)}</td><td><span>{run.runType==='dropoff'?'Dépôt':'Ramassage'}</span></td></tr>)}</tbody></table></div>:<EmptyState title="Aucune opération active" copy="Aucune course correspondante n’est actuellement ouverte pour aujourd’hui."/>}</section>}
...
export function SafetyDepartureGate({snapshot}:{snapshot:TransportSnapshot}){
  ...
  return <><section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Safety Departure Gate</div><h2 className={styles.sectionTitle}>Préparation avant mouvement</h2><p className={styles.sectionCopy}>Le contrôle sécurité est un registre opérationnel; « blocked » et « failed » ne sont jamais maquillés en readiness.</p></div></div>
    ...
    {snapshot.authority==='advanced'?<TransportDrawer trigger="Enregistrer un contrôle sécurité" title="Safety Departure Gate"><SafetyStudio vehicles={snapshot.vehicles} drivers={snapshot.drivers} locked={locked}/></TransportDrawer>:null}
...
export function IncidentCommand({snapshot}:{snapshot:TransportSnapshot}){
  ...
  return <section className={styles.section}><div className={styles.sectionHeader}><div><div className={styles.sectionKicker}>Incident Truth</div><h2 className={styles.sectionTitle}>Incidents et exceptions réelles</h2><p className={styles.sectionCopy}>Aucune table « incident » n’est inventée. Cette chambre consolide courses en incident, événements incident et contrôles sécurité en échec réellement présents.</p></div></div>
    <div className={styles.gridThree}><div className={styles.panel}><h2 className={styles.sectionTitle}>Courses incident</h2>{incidentRuns.length?incidentRuns.map(r=><div className={styles.attentionItem} key={r.id}><div className={styles.num}>!</div><div><Link href={`${BASE}/${r.runType==='dropoff'?'depot':'ramassage'}`}><strong>{r.routeCode}</strong></Link><p>{formatDate(r.runDate)} · {r.driverName||'—'}</p></div></div>):<EmptyState title="Aucune course en incident" copy="Aucun état incident n’est actuellement enregistré."/ >}</div>
