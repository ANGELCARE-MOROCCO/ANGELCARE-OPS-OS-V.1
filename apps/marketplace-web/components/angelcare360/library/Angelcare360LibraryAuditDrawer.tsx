...
export default function Angelcare360LibraryAuditDrawer({ events }: Props) {
  return (
    <section style={cardStyle}>
      <h3 style={titleStyle}>Audit bibliothèque</h3>
      {events.length > 0 ? (
        <ul style={listStyle}>
          {events.map((event) => (
            <li key={event.id} style={itemStyle}>{event.module} · {event.action} · {event.severity}</li>
          ))}
        </ul>
      ) : (
        <p style={emptyStyle}>Aucun événement d’audit bibliothèque pour le moment.</p>
      )}
    </section>
  )
}
