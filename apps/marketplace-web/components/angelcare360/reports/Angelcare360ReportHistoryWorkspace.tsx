...
export default function Angelcare360ReportHistoryWorkspace({ history }: Props) {
  return (
    <div style={stackStyle}>
      {history.length ? history.map((entry) => (
        <article key={`${entry.history_type}-${entry.id}`} style={cardStyle}>
          <div style={rowStyle}>
            <div>
              <div style={eyebrowStyle}>{entry.history_type === 'request' ? 'Demande' : 'Export'}</div>
              <h3 style={titleStyle}>{entry.label || entry.entity_code}</h3>
              <p style={metaStyle}>{entry.status}</p>
            </div>
            <span style={statusStyle}>{entry.export_format || entry.status}</span>
          </div>
          <div style={gridStyle}>
      ...
      )) : <div style={emptyStyle}>Aucun historique de rapport réel n’est encore disponible.</div>}
    </div>
  )
}
