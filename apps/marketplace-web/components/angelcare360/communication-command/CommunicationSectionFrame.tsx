...
export default function CommunicationSectionFrame({active,title,description,actions,children}:{active:string;title:string;description:string;actions?:React.ReactNode;children:React.ReactNode}){return <div className={styles.page}><nav className={styles.nav}>{links.map(([href,label,Icon])=><Link key={href} href={href} className={styles.navLink} data-active={active===href?'true':'false'}><Icon/>{label}</Link>)}</nav><main className={styles.sectionPage}><header className={styles.sectionHero}><div><p className={styles.eyebrow}>SANILA · Communication Command</p><h1>{title}</h1><p>{description}</p></div>{actions?<div className={styles.actions}>{actions}</div>:null}</header>{children}</main></div>}
