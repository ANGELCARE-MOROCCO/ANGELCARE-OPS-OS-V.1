...
type Angelcare360AdmissionsStatusChangeDrawerProps = {
  open: boolean
  title: string
...
export default function Angelcare360AdmissionsStatusChangeDrawer({
  open,
  title,
  entity,
  id,
  ...
  onSaved,
  disabledReason,
}: Angelcare360AdmissionsStatusChangeDrawerProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok || !result?.ok) {
        throw new Error(result?.error || 'Le changement de statut a échoué.')
      }
      onSaved()
    } catch (err) {
    ...
    <div style={overlayStyle} role="presentation" onClick={onClose}>
      <div style={drawerStyle} onClick={(event) => event.stopPropagation()}>
        ...
        <label style={fieldShellStyle}>
          <span style={labelStyle}>Nouveau statut</span>
        ...
        <label style={fieldShellStyle}>
          ...
          <textarea value={note} onChange={(event) => setNote(event.target.value)} style={{ ...inputStyle, minHeight: 96, resize: 'vertical' }} placeholder="Motif ou contexte de la décision" />
        </label>
