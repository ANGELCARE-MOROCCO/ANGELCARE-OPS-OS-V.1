...
import type { Angelcare360AdmissionsRowActionDefinition } from '@/types/angelcare360/admissions'

type ColumnDefinition = { key: string; label: string; kind?: 'text' | 'status' | 'date' | 'datetime' | 'number' | 'boolean' | 'chips'; width?: string }

type Angelcare360AdmissionsTableProps = {
  columns: ColumnDefinition[]
  rows: Array<Record<string, unknown>>
  actions?: Angelcare360AdmissionsRowActionDefinition[]
  onRowAction?: (action: Angelcare360AdmissionsRowActionDefinition, row: Record<string, unknown>) => void
  emptyFallback?: React.ReactNode
  detailHrefKey?: string
}

function formatValue(kind: ColumnDefinition['kind'], value: unknown) {
...
function getStatusStyle(status: string) {
  ...
  if (['active', 'actif', 'open', 'approved', 'converted', 'recu', 'validé', 'enrolled', 'complete'].includes(normalized)) {
    return { background: '#dcfce7', color: '#166534', border: '1px solid #bbf7d0' }
  }
  if (['planned', 'pending', 'draft', 'new', 'contacted', 'in_review', 'documents_en_attente', 'requis', 'en_attente'].includes(normalized)) {
    return { background: '#eff6ff', color: '#1d4ed8', border: '1px solid #bfdbfe' }
  }
  if (['warning', 'waitlisted'].includes(normalized)) {
    return { background: '#fff7ed', color: '#c2410c', border: '1px solid #fed7aa' }
  }
  if (['inactive', 'archived', 'closed', 'suspended', 'rejected', 'refuse', 'expired', 'expire', 'critical'].includes(normalized)) {
    return { background: '#fef2f2', color: '#b91c1c', border: '1px solid #fecaca' }
  }
  return { background: '#f8fafc', color: '#334155', border: '1px solid #e2e8f0' }
}

export default function Angelcare360AdmissionsTable({
  columns,
  rows,
  actions = [],
  onRowAction,
  emptyFallback,
  detailHrefKey,
}: Angelcare360AdmissionsTableProps) {
  if (!rows.length) {
