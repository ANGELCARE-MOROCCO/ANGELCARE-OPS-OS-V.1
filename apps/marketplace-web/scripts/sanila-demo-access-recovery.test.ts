...
const sessionAuthority = readFileSync(new URL('../lib/auth/session.ts', import.meta.url), 'utf8')
const changePasswordPage = readFileSync(new URL('../app/angelcare-360-access/change-password/page.tsx', import.meta.url), 'utf8')
const mfaPage = readFileSync(new URL('../app/angelcare-360-access/mfa/page.tsx', import.meta.url), 'utf8')
const schoolLoginPage = readFileSync(new URL('../app/angelcare-360-access/login/page.tsx', import.meta.url), 'utf8')
const portalAuth = readFileSync(new URL('../lib/angelcare360/portal/auth.ts', import.meta.url), 'utf8')
...
assert.match(sessionAuthority, /if \(\(user as any\)\.__demo === true\) return user/)
assert.ok(changePasswordPage.includes("if ((user as { __demo?: boolean }).__demo === true) redirect('/angelcare-360-command-center')"))
assert.ok(changePasswordPage.includes("if ((current as { __demo?: boolean }).__demo === true) redirect('/angelcare-360-command-center')"))
assert.ok(mfaPage.includes("if ((user as { __demo?: boolean } | null)?.__demo === true) redirect('/angelcare-360-command-center')"))
assert.match(page, /store\.set\(APP_SESSION_COOKIE, '', expired\)/)
assert.match(page, /angelcare360_support_access/)
assert.match(schoolLoginPage, /cookieStore\.set\(DEMO_COOKIE, ''/)
assert.match(portalAuth, /store\.set\(DEMO_COOKIE,''/)
assert.match(sessionAuthority, /__mfaRequired\) redirect\('\/angelcare-360-access\/mfa'\)/)
assert.match(sessionAuthority, /must_change_password\)\) redirect\('\/angelcare-360-access\/change-password'\)/)
console.log('DEMO_PIN_BYPASSES_PASSWORD_AND_MFA_POLICY=PASS')
console.log('DEMO_PASSWORD_MUTATION_BLOCKED=PASS')
console.log('DEMO_MFA_CHALLENGE_BLOCKED=PASS')
console.log('DEMO_PRINCIPAL_SWITCH_CLEARS_NORMAL_CONTEXT=PASS')
console.log('NORMAL_LOGIN_CLEARS_DEMO_CONTEXT=PASS')
console.log('NORMAL_PASSWORD_AND_MFA_POLICY_UNCHANGED=PASS')
