...
for(const marker of ['writeMarketplaceAudit','tenantId','territoryId','assertFulfillmentTransition','Au moins une preuve validée est requise','Une réconciliation approuvée est requise','finance_handover','dispute_open','approval_status','Litige introuvable dans votre périmètre','Réconciliation introuvable dans votre périmètre','replace(/[%_,.()]/g'])check(`repository ${marker}`,repository.includes(marker))
...
const contextNav=text('angelcare-marketplace/shells/AdminWorkspaceContextNav.tsx')
check('canonical Operations navigation mounted',nav.includes("href:'/angelcare-marketplace/admin/operations'"))
check('Fulfillment specialist exposed',contextNav.includes("['Fulfillment','/operations/mission-control']")&&fs.existsSync(file('app/angelcare-marketplace/(protected)/admin/operations/mission-control/page.tsx')))
check('Mission Control preserved',fs.existsSync(file('app/angelcare-marketplace/(protected)/admin/operations/missions/page.tsx'))&&contextNav.includes("['Missions','/operations/missions']"))
