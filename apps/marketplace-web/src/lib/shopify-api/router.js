...
function getShopifyUrl(env, pathname = '') {
  return `${getPublicAppUrl(env)}${getShopifyBasePath(env)}${pathname}`;
}
  ...
  try {
    ...
    return sendHtml(
      ...
      [
        ...
        `<p><a href="${escapeHtml(getShopifyUrl(env, '/dashboard'))}">Open the dashboard</a></p>`,
        '</body>',
        '</html>',
      ].join(''),
    );
...
function handleInstall(req, res, env) {
  ...
  installUrl.searchParams.set('redirect_uri', getShopifyUrl(env, '/api/auth/callback'));
...
async function handleDashboardSummary(req, res, url, env) {
  try {
    ...
    return sendJson(res, 502, {
      ...
      installUrl: getShopifyUrl(env, '/install'),
    });
  }
}
...
function formatEndpointList(env) {
  return {
    dashboard: getShopifyUrl(env, '/dashboard'),
    install: getShopifyUrl(env, '/install'),
    callback: getShopifyUrl(env, '/api/auth/callback'),
    summary: getShopifyUrl(env, '/api/dashboard/summary'),
    products: getShopifyUrl(env, '/api/products'),
    orders: getShopifyUrl(env, '/api/orders'),
    customers: getShopifyUrl(env, '/api/customers'),
    checkouts: getShopifyUrl(env, '/api/checkouts'),
    payments: getShopifyUrl(env, '/api/payments'),
    analytics: getShopifyUrl(env, '/api/analytics'),
    trafficSummary: getShopifyUrl(env, '/api/traffic/summary'),
    trafficCollect: getShopifyUrl(env, '/api/traffic/collect'),
    trafficScript: getShopifyUrl(env, '/api/traffic/script.js'),
    revenueMetrics: getShopifyUrl(env, '/api/metrics/revenue'),
    inventoryAdjust: getShopifyUrl(env, '/api/inventory/adjust'),
    webhooks: getShopifyUrl(env, '/api/webhooks/shopify'),
  };
}
