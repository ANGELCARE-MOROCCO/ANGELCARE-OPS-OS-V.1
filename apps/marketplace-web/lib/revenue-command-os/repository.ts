...
function rowToWorkspace(row: Row): RevenueOsWorkspaceDefinition {
  ...
  return {
    ...
    href: fallback?.href || row.href || `/revenue-command-os/${row.workspace_key}`,
    icon: fallback?.icon || row.icon || row.icon_name || 'Circle',
    order: Number(fallback?.order ?? row.display_order ?? 999),
    permission: fallback?.permission || row.permission_key || row.required_permission || 'revenue_os.view',
    status: fallback?.status || row.maturity_status || row.status || 'planned',
