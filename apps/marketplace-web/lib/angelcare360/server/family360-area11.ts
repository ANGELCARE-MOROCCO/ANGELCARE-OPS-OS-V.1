...
import { preparePortalInvitation } from '@/lib/angelcare360/server/portal-invitations'
import type {
...
async function mutate(client:Client,schoolId:string,actorId:string,request:Angelcare360Area11MutationRequest,payload:Row):Promise<{data?:Row|null;message:string;deepLink?:string|null;subjectId?:string}> {
  ...
  if(operation==='portal_access.invite') {
    const personId=subjectKind==='person'?subjectId:payloadText(payload,'personId'); const invitation=await preparePortalInvitation({kind:'parent',personId,email:payloadOptional(payload,'email'),reason:payloadOptional(payload,'reason')||'Activation Espace Famille'}); const {data,error}=await client.from(AREA11_TABLES.portal).insert({school_id:schoolId,family_id:payloadOptional(payload,'familyId'),person_id:personId,portal_app_user_id:null,access_state:invitation.state==='smtp_accepted'?'activation_pending':'invitation_prepared',visibility_json:jsonValue(payload.visibility),effective_from:now,status:'active',created_by_user_id:actorId,metadata_json:{invitation_id:invitation.id,delivery_state:invitation.state}}).select('*').single();if(error)throw new Error(error.message);return{data:data as Row,message:invitation.message}
  }
