...
function unavailable(section: string, error: unknown): AnyRecord {
  return {
    ok: false,
    section,
...
async function optionalSection(section: string, task: () => Promise<unknown>): Promise<AnyRecord> {
  try {
    return {
      ok: true,
      section,
...
export function buildPendingAnalytics() {
  return {
...
export function emptyCheckoutAnalytics() {
  return {
    totalShown: 0,
    openCount: null,
    openValue: 0,
...
export function emptyPaymentAnalytics() {
  return {
    currentBalance: null,
...
export function emptyFunnelAnalytics() {
  return {
    paidOrders: 0,
...
export async function fetchCustomerAnalytics(env: AppEnv, first = 25) {
  const query = `
...
function normalizeCheckout(checkout: AnyRecord) {
  return {
...
export async function fetchCheckoutAnalytics(env: AppEnv, limit = 25) {
...
export async function fetchPaymentAnalytics(env: AppEnv, first = 10) {
  const query = `
...
export async function fetchAdminAnalytics(env: AppEnv, options: AnyRecord = {}) {
