...
export default function MarketingLiveSyncPanel() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch('/api/marketing/live-snapshot', { cache: 'no-store' })
  ...
  if (error) return <div style={boxStyle}>Marketing sync: {error}</div>
  if (!snapshot) return <div style={boxStyle}>Loading Marketing live synchronization...</div>

  return (
    <section style={panelStyle}>
      <header style={headerStyle}>
        <div>
          <h2 style={titleStyle}>Live Marketing Synchronization</h2>
