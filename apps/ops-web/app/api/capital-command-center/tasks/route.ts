...
export async function POST(request: NextRequest) {
  try {
    ...
    const guarded = await runOperationalWiredAction('capital.tasks.create', async () => {
      ...
      quantity,
      idempotencyKey: body.idempotencyKey || body.idempotency_key || buildOperationalIdempotencyKey('capital.tasks.create', `${body?.title || body?.starter || 'task'}:${quantity}`),
