...
export async function POST(request: NextRequest) {
  ...
  if (result.device?.id) await context.supabase.from("whatsapp_desktop_devices").update({ last_whatsapp_lease_renewal_at: new Date().toISOString() }).eq("id", result.device.id)
  return ok(result)
}
