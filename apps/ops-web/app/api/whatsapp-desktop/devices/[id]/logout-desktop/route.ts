...
import { DEVICE_ACTION_PERMISSIONS, lifecycleError, logoutDesktop } from "@/lib/whatsapp-desktop/device-lifecycle"
...
export async function POST(request: NextRequest, routeContext: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  try {
    ...
    return ok(await logoutDesktop(context, id, String(body.reason || "Déconnexion Desktop administrative")))
