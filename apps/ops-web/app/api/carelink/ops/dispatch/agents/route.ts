...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, source: payload.source, generatedAt: payload.generatedAt, data: (payload as any)['agents'] || [] }, { headers: { 'Cache-Control': 'no-store, no-cache, must-revalidate' } })
}

export async function POST(request: Request) {
  const body = await request.json().catch(() => ({}))
  const result = await runCareLinkDispatchWorkspaceAction({ action: 'create_agent' as any, payload: body })
