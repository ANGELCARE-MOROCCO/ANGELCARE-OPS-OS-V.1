...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await db.from('b2b_communication_templates').update({ archived_at: new Date().toISOString(), is_active: false, updated_by: actor.id }).eq('id', id).select('*').single()
