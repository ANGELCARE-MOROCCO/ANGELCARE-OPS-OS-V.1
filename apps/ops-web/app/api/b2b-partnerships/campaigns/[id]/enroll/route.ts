...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const rows = prospectIds.map((prospect_id: string) => ({ campaign_id: id, prospect_id, status: 'Enrolled', current_step_order: 0 }))
    ...
    await db.from('b2b_campaigns').update({ enrolled_count: prospectIds.length, updated_at: new Date().toISOString() }).eq('id', id)
