...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from('b2b_marketplace_quote_notes').insert({
    quote_request_id: id,
    note,
    note_type: String(body.noteType || 'internal'),
    created_by: guard.user?.id || null,
    created_by_name: guard.user?.name || guard.user?.email || 'AngelCare Admin',
  }).select('*').single()
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 400 })
  await supabase.from('b2b_marketplace_quote_activity_logs').insert({ quote_request_id: id, activity_type: 'note_added', title: 'Note interne ajoutée', description: note, actor_user_id: guard.user?.id || null, actor_name: guard.user?.name || guard.user?.email || 'AngelCare Admin' })
