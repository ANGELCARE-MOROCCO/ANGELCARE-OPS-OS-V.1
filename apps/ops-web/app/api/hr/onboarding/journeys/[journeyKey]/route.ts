import { NextRequest } from "next/server";
import { archiveJourney, getOnboardingWorkspace, updateJourney } from "@/lib/hr-onboarding/server";
import { noStoreJson, onboardingErrorResponse } from "@/lib/hr-onboarding/http";

type Context = { params: Promise<{ journeyKey: string }> };
export const dynamic = "force-dynamic";
export const revalidate = 0;

export async function GET(_request: NextRequest, context: Context) {
  try {
    const { journeyKey } = await context.params;
    return noStoreJson({ ok: true, workspace: await getOnboardingWorkspace(journeyKey) });
  } catch (error) {
    return onboardingErrorResponse(error);
  }
}

export async function PATCH(request: NextRequest, context: Context) {
  try {
    const { journeyKey } = await context.params;
    const body = await request.json() as Record<string, unknown>;
    const result = await updateJourney(journeyKey, body);
    return noStoreJson({ ...result, workspace: await getOnboardingWorkspace(journeyKey) });
  } catch (error) {
    return onboardingErrorResponse(error);
  }
}

export async function DELETE(request: NextRequest, context: Context) {
  try {
    const { journeyKey } = await context.params;
    const body = await request.json() as Record<string, unknown>;
    const result = await archiveJourney(journeyKey, body);
    return noStoreJson({ ...result, workspace: await getOnboardingWorkspace() });
  } catch (error) {
    return onboardingErrorResponse(error);
  }
}
