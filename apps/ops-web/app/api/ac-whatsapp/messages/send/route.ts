...
export async function POST(request: NextRequest) {
  ...
  if (!internalNote && messageType !== 'text' && (!MEDIA_TYPES.has(messageType) || !media || (!media.url && !media.base64))) return fail('VALID_MEDIA_REQUIRED', 422)
  if (typeof media?.base64 === 'string' && media.base64.length > 42000000) return fail('MEDIA_TOO_LARGE', 413)
  ...
  const message = await context.supabase.from('ac_whatsapp_messages').insert({
    ...
    caption: messageType === 'text' ? null : caption || null,
    status: 'queued',
    sender_user_id: context.user.id,
    sender_display_name_snapshot: senderName,
    sender_role_snapshot: senderRole,
  ...
  try {
    ...
    const preview = messageType === 'text' ? text : `${messageType.toUpperCase()}${caption ? `: ${caption}` : ''}`
    await Promise.all([
    ...
    await audit(context, { action: 'message.send', entityType: 'message', entityId: message.data.id, newState: { externalMessageId: external, conversationId, messageType, senderName, senderRole } })
