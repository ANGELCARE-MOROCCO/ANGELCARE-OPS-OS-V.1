...
import { evaluateInboundAutomation } from '@/lib/ac-whatsapp/automation'
...
export async function POST(request:NextRequest){
 ...
 try{
  ...
  if(eventType.startsWith('session.')){
   ...
   if(!normalized.fromMe){try{await evaluateInboundAutomation({supabase,account,conversation:{...conv.data,message_count:Number(conv.data.message_count||0)+1},contact,inboundMessageId:insert.data.id,inboundText:String(normalized.body||normalized.caption||''),isNewConversation})}catch(automationCause){await supabase.from('ac_whatsapp_security_events').insert({severity:'medium',event_type:'automation.evaluation_failed',title:'Évaluation automatique non terminée',description:automationCause instanceof Error?automationCause.message:String(automationCause),account_id:account.id,metadata:{conversation_id:conv.data.id,message_id:insert.data.id}})}}
  }
