...
export async function POST(request:NextRequest){
 ...
 try{
  ...
  if(eventType.startsWith('session.')){
   ...
   const existingContact=await supabase.from('ac_whatsapp_contacts').select('*').eq('whatsapp_id',contactWa).maybeSingle();if(existingContact.error)throw existingContact.error
   const resolvedContactName=existingContact.data?.display_name||normalized.senderName||normalized.message?.contact?.pushName||normalized.message?.contact?.name||phone||'Contact non identifié'
   const contactUpsert=await supabase.from('ac_whatsapp_contacts').upsert({whatsapp_id:contactWa,phone_number_e164:phone?`+${phone}`:existingContact.data?.phone_number_e164||null,display_name:resolvedContactName,organization_name:existingContact.data?.organization_name||null,contact_type:existingContact.data?.contact_type&&existingContact.data.contact_type!=='unknown'?existingContact.data.contact_type:'unqualified',preferred_language:existingContact.data?.preferred_language||'fr',lead_stage:existingContact.data?.lead_stage||'new',priority:existingContact.data?.priority||'normal',tags:existingContact.data?.tags||[],last_contact_at:normalized.timestamp,last_response_at:normalized.fromMe?existingContact.data?.last_response_at||null:normalized.timestamp},{onConflict:'whatsapp_id'}).select('*').single();if(contactUpsert.error)throw contactUpsert.error
   ...
   const senderDisplayName=normalized.fromMe?(account.name||'Compte WhatsApp AngelCare'):(contactUpsert.data.display_name||'Contact non identifié')
   const senderType=normalized.fromMe?'whatsapp_account':'contact'
   if(!conv.data){const created=await supabase.from('ac_whatsapp_conversations').insert({account_id:account.id,contact_id:contactUpsert.data.id,remote_chat_id:remote,queue_id:queueId,status:normalized.fromMe?'waiting_customer':'new',priority:'normal',unread_count:normalized.fromMe?0:1,message_count:1,last_message_preview:(normalized.body||`[${normalized.type}]`).slice(0,240),last_message_direction:normalized.fromMe?'outbound':'inbound',last_message_at:normalized.timestamp,last_message_sender_display_name_snapshot:senderDisplayName,last_message_sender_type:senderType,sla_first_response_due_at:new Date(Date.now()+15*60000).toISOString(),sla_resolution_due_at:new Date(Date.now()+240*60000).toISOString()}).select('*').single();if(created.error)throw created.error;conv=created}else{const updates:any={last_message_preview:(normalized.body||`[${normalized.type}]`).slice(0,240),last_message_direction:normalized.fromMe?'outbound':'inbound',last_message_at:normalized.timestamp,last_message_sender_display_name_snapshot:senderDisplayName,last_message_sender_type:senderType,message_count:(conv.data.message_count||0)+1,status:normalized.fromMe?'waiting_customer':['closed','resolved','archived'].includes(conv.data.status)?'reopened':conv.data.status,unread_count:normalized.fromMe?conv.data.unread_count||0:(conv.data.unread_count||0)+1};await supabase.from('ac_whatsapp_conversations').update(updates).eq('id',conv.data.id)}
