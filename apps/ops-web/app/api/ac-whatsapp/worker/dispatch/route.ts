...
export async function POST(request:NextRequest){
 const supplied=request.headers.get('x-ac-whatsapp-worker-secret');const expected=process.env.AC_WHATSAPP_WORKER_SECRET;if(!expected||supplied!==expected)return fail('WORKER_UNAUTHORIZED',401)
 const supabase=await createServiceClient();const body=await request.json().catch(()=>({}));const workerId=String(body.workerId||'angelcare-worker');const limit=Math.max(1,Math.min(Number(body.limit||25),100))
 await supabase.rpc('ac_whatsapp_release_stale_outbox',{p_age_minutes:10});const claimed=await supabase.rpc('ac_whatsapp_claim_outbox',{p_worker_id:workerId,p_limit:limit});if(claimed.error)return fail(claimed.error.message,500)
 const results=[] as any[]
 for(const item of claimed.data||[]){
  const account=await supabase.from('ac_whatsapp_accounts').select('*').eq('id',item.account_id).maybeSingle();const startedAt=new Date().toISOString()
  try{
   if(!account.data?.openwa_session_id)throw new Error('ACCOUNT_SESSION_NOT_CONFIGURED');if(account.data.outbound_enabled===false)throw new Error('ACCOUNT_OUTBOUND_PAUSED')
   const sent:any=item.message_type==='text'?await openwa.sendText(account.data.openwa_session_id,item.chat_id,item.body||''):await openwa.sendMedia(account.data.openwa_session_id,item.message_type,item.chat_id,item.media_payload||{},item.body||'')
   const external=externalId(sent);const sentAt=new Date().toISOString()
   await Promise.all([
    ...
    item.conversation_id?supabase.from('ac_whatsapp_messages').update({status:'sent',external_message_id:external,sent_at:sentAt}).eq('client_message_id',item.client_message_id):Promise.resolve(),
   ]);results.push({id:item.id,status:'sent',external})
  }catch(cause){
   const message=cause instanceof Error?cause.message:String(cause);const terminal=item.attempt_count>=item.max_attempts
   await Promise.all([
