...
import { acContext, audit, fail, normalizeOpenWAAccountStatus, ok, scopeAccounts } from '@/lib/ac-whatsapp/server'
import { openwa } from '@/lib/ac-whatsapp/openwa-client'

function sessionName(value:string){const base=value.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'').slice(0,32)||'angelcare';return `${base}-${crypto.randomUUID().slice(0,6)}`.slice(0,42)}
...
export async function POST(request:NextRequest){
 ...
 if(body.auto_start!==false){try{const started:any=await openwa.startSession(session.id);await context.supabase.from('ac_whatsapp_accounts').update({status:normalizeOpenWAAccountStatus(started.status,'starting'),runtime_metadata:started}).eq('id',account.data.id)}catch(cause){await context.supabase.from('ac_whatsapp_accounts').update({status:'error',last_error:cause instanceof Error?cause.message:'OPENWA_START_FAILED'}).eq('id',account.data.id)}}
