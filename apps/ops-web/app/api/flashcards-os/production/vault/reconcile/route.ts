...
export async function POST(){const access=await assertFlashcardsApiAccess('flashcards_os.view_vault');if(!access.ok)return NextResponse.json({error:access.message},{status:access.status});try{return NextResponse.json({result:await reconcileVault(actorFromUser(access.user))})}catch(error){return NextResponse.json({error:error instanceof Error?error.message:'Vault reconciliation failed.'},{status:503})}}
