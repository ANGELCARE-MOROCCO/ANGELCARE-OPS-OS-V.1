...
async function loadOverview(): Promise<Overview> {
  ...
  try {
    ...
    return {
      source: 'fallback',
      warnings: [`SaaS Factory server helper unavailable: ${error?.message || 'unknown error'}`],
      counts: {},
      modules: [],
      optionGroups: [],
      options: [],
      ...
      actions: [],
      apis: [],
      auditEvents: [{ event_type: 'saas_factory.configuration.fallback', title: 'Configuration API served safe fallback state', actor: 'system', severity: 'warning', created_at: now }],
    }
  }
}
  ...
  try {
    ...
    const groups = (overview.optionGroups || []).map((group: any, index: number) => {
      ...
      return {
        ...
        status: group.is_enabled === false ? 'disabled' : disabled > 0 ? 'attention' : 'healthy',
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        enabledOptions: options.filter((option: any) => option.enabled).length,
        disabledOptions: options.filter((option: any) => !option.enabled).length,
        featureFlags: flags.length,
        pendingWarnings: warnings.length,
        readinessScore: Math.max(60, 100 - warnings.length * 8 - groups.filter((group: any) => group.status !== 'healthy').length * 3),
    ...
    return NextResponse.json({ ok: false, source: 'error', error: error?.message || 'Configuration API failed', generatedAt: new Date().toISOString(), summary: {}, groups: [], options: [], flags: [], policies: [], recommendations: [], auditEvents: [], warnings: [error?.message || 'Configuration API failed'] }, { status: 500 })
  }
}
...
export async function POST(request: Request) {
  ...
  let result: any = { ok: true, source: 'fallback', data: { id: `preview-${Date.now()}`, ...(body.payload || {}) }, warning: 'Safe preview returned because live persistence helper is unavailable.' }
  try {
    const server = await import('@/lib/saas-factory/server')
    if (type === 'group') result = await server.saveFactoryOptionGroup(body.payload || {})
    else if (type === 'flag') result = await server.saveFactoryFeatureFlag(body.payload || {})
    else result = await server.saveFactoryOption(body.payload || {})
  } catch {}
  await writeAudit({ event_type: 'saas_factory.configuration.save', title: `Configuration ${type} saved`, severity: result?.ok === false ? 'warning' : 'info', metadata_json: { type, payload: body.payload, result } })
