...
import { listFactoryApis, saveFactoryApi } from '@/lib/saas-factory/server'

export const dynamic = 'force-dynamic'

export async function GET(request: Request) {
  try {
    const result = await listFactoryApis()
    return NextResponse.json({ ok: true, apis: result.data, source: result.source, error: result.error })
  } catch (error: any) {
...
export async function POST(request: Request) {
  try {
    ...
    const result = await saveFactoryApi(body)
