...
import { listFactoryModules, saveFactoryModule } from '@/lib/saas-factory/server'

export const dynamic = 'force-dynamic'

export async function GET(request: Request) {
  try {
    const result = await listFactoryModules()
    return NextResponse.json({ ok: true, modules: result.data, source: result.source, error: result.error })
  } catch (error: any) {
...
export async function POST(request: Request) {
  try {
    ...
    const result = await saveFactoryModule(body)
