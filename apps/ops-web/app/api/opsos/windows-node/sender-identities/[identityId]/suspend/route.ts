...
import { suspendSenderIdentity } from "@/lib/email-os-core/sender-identity"
...
export async function POST(request: Request, context: { params: Promise<{ identityId: string }> }) {
  ...
  try {
    ...
    if (!reason) return senderIdentityError(new Error("Suspension reason is required."), 400)
    return senderIdentityOk(await suspendSenderIdentity(identityId, actorFromSenderIdentityAuth(request, auth), reason))
  } catch (error) {
