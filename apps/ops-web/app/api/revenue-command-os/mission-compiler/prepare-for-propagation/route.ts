...
export async function POST(request:Request){const user=await getCurrentUser();if(!user)return compilerError('UNAUTHENTICATED','Authentification requise.',401);try{const parsed=preparePropagationSchema.safeParse(await request.json());if(!parsed.success)return compilerError('INVALID_PROPAGATION_PREP',parsed.error.message,422);const tenantId=tenantOf(user);return NextResponse.json({ok:true,data:await prepareForPropagation({tenantId,runId:parsed.data.runId,reason:parsed.data.reason,actor:actorOf(user,tenantId)}),mode:'live',externalActions:true})}catch(error){return compilerError('PROPAGATION_PREP_FAILED',error instanceof Error?error.message:String(error),500)}}
