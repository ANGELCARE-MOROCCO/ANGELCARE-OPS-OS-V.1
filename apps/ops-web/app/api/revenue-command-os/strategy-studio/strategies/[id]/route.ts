...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){const user=await getCurrentUser();if(!user)return studioError('UNAUTHENTICATED','Authentification requise.',401);try{const {id}=await params;return NextResponse.json({ok:true,data:await loadStudioDossier(id,tenantOf(user)),mode:'live',externalActions:true})}catch(error){return studioError('STUDIO_DOSSIER_FAILED',error instanceof Error?error.message:String(error),500)}}
