...
export async function GET(){const user=await getCurrentUser();if(!user)return studioError('UNAUTHENTICATED','Authentification requise.',401);try{return NextResponse.json({ok:true,data:await listStudioStrategies(tenantOf(user)),mode:'live',externalActions:true})}catch(error){return studioError('STUDIO_LIST_FAILED',error instanceof Error?error.message:String(error),500)}}
