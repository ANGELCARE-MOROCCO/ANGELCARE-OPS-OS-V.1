...
export async function GET(){const user=await getCurrentUser();if(!user)return NextResponse.json({ok:false,error:'Authentification requise.'},{status:401});const permissions=new Set(Array.isArray((user as any).permissions)?(user as any).permissions.map(String):[]);return NextResponse.json({ok:true,release:COMMANDS_2000_RELEASE,count:cumulative.length,coverage,domainCoverage,governance,commands:cumulative,posture:'live',externalActionsEnabled:true})}
