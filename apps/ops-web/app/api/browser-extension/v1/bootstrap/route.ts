...
export async function GET(req:NextRequest){
  ...
  const production=await loadProductionStatus(auth.db,auth.context.device)
  return NextResponse.json({ok:true,user:{id:auth.context.user.id,name:auth.context.user.full_name||auth.context.user.name||auth.context.user.email||'AngelCare operator',email:auth.context.user.email||null,role:auth.context.user.role||auth.context.user.role_key||null},device:{id:auth.context.device.id,installationId:auth.context.device.installation_id,status:auth.context.device.status,extensionVersion:auth.context.device.extension_version,releaseChannel:auth.context.device.desired_release_channel||auth.context.device.release_channel||'pilot',healthStatus:auth.context.device.health_status||'unknown'},accessVersion:Number(access.profile?.access_version||0),modules,capabilities:access.capabilities.map((r:any)=>r.capability_key),adapters:access.adapters.map((r:any)=>r.adapter_key),autonomy:access.autonomy.map((r:any)=>({actionPattern:r.action_pattern,mode:r.mode})),approvals:access.approvals.map((r:any)=>({commandPattern:r.command_pattern,approvalLevel:r.approval_level,approverRole:r.approver_role})),scopes,production,features:{b2bAccountIntelligence:true,b2bPartnerLifecycle:true,b2bManagementCommand:true,productionHardening:true,contractVersion:'mega7-v1',commands:[...B2B_INTELLIGENCE_COMMANDS,...B2B_EXECUTION_COMMANDS,...B2B_DEAL_COMMANDS,...B2B_PARTNER_COMMANDS,...B2B_MANAGEMENT_COMMANDS].map((x)=>x.commandKey)},issuedAt:new Date().toISOString()})
}
