...
export async function GET(){ return NextResponse.json({ok:true,service:'ANGELCARE Revenue Command Extension Gateway',version:'0.7.0',contractVersion:B2B_EXTENSION_CONTRACT.version,b2bCapabilities:B2B_EXTENSION_CONTRACT.capabilityCount,modules:BROWSER_EXTENSION_MODULES.map((m:any)=>m.key),timestamp:new Date().toISOString()}) }
