...
function isAllowedAction(action: string): action is MessageAction {
  return ["delete", "archive", "restore", "spam", "mark_read", "mark_unread", "star", "unstar", "tag", "label", "move_folder", "permanent_delete"].includes(action)
}
...
function actionPatch(action: MessageAction, payload: any = {}, source: SourceType = "inbox") {
  ...
  if (action === "permanent_delete") {
    patch.status = "permanently_deleted"
    patch.folder = "deleted_forever"
