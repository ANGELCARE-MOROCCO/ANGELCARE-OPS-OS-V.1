...
export async function POST(request: Request) {
  ...
  const guarded = await runOperationalWiredAction('email_os.ai_assist', async () => {
    ...
    quantity: 1,
    idempotencyKey: body.idempotencyKey || body.idempotency_key || buildOperationalIdempotencyKey('email.ai-assist', `${body.subject || 'untitled'}:${body.tone || 'professional'}`),
