...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'Impossible de charger l’autorité financière.' }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error instanceof Angelcare360AccessError) return NextResponse.json({ ok: false, error: error.message }, { status: error.status })
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'Échec de la commande financière.' }, { status: 500 })
  }
}
