...
import {
  ...
  pickerCopy, previewCopyCsv, recordCopyUsage, rejectCopyVersion, submitCopyVersion,
  updateCopyCategory,
...
export async function POST(request:Request,context:RouteContext){
  ...
  try{
    ...
    if(route==="import/commit")return socialOk(await commitCopyCsv(cleanString(body.csv,2_000_000),jsonObject(body.mapping) as CopyVaultImportMapping,cleanString(body.filename,260),auth.actor,{duplicatePolicy:cleanString(body.duplicatePolicy,20)==="import"?"import":"skip"}))
