...
export async function POST(request: Request) {
  ...
  const title = String(body.title || 'CSA follow-up')
  const source = String(body.source || 'csa')
  const priority = String(body.priority || 'medium')

  try {
    const supabase = await createClient()
    const { data, error } = await supabase.from('csa_followups').insert([
      {
        title,
    ...
    return NextResponse.json({ ok: true, mode: 'live-write', followup: data })
  } catch (error: any) {
