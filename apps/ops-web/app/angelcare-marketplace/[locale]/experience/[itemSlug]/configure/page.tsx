...
export default async function Page({params}:{params:Promise<{locale:string;itemSlug:string}>}){const {locale,itemSlug}=await params;const data=await getAdaptiveExperience({locale:categoryNativeLocale(locale),slug:itemSlug});if(!data){notFound()}return <AdaptiveExperience data={data} focusConfigurator/>}
