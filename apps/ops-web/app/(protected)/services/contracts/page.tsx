...
export default function Page() {
  ...
  return (
    <AppShell title="Contracts" subtitle="ServiceOS enterprise layer" breadcrumbs={[{ label: 'Services', href: '/services' }, { label: 'Contracts' }]}>
      <ServiceOSHeader title="Contracts" subtitle="Real synchronized Contracts layer connected to shared AngelCare ServiceOS blueprints, rules, pricing, deployments and missions." />
      <ServiceOSPanel>
        <div className="mb-4">
          <h2 className="text-xl font-black text-slate-950">Contracts and B2B operating layer</h2>
          <p className="mt-1 text-sm text-slate-600">Institution, school, hotel and corporate service contracts with SLA and billing logic.</p>
        </div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:14}}>{['School SLA','Hotel Concierge Care','Corporate Emergency Care','Clinic Partner Support'].map((x: any)=><div key={x} style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:20,padding:16}}><h3>{x}</h3><p>Includes SLA, dedicated pool, report cadence, renewal and billing profile.</p></div>)}</div></ServiceOSPanel>
    </AppShell>
  )
}
