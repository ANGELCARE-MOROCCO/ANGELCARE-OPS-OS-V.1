...
import MediaVaultPicker from "./MediaVaultPicker"
...
export default function BulkOrchestrator({open,onClose,assets,campaigns,capabilities,onUpload,onRefresh,setPulse}:{open:boolean;onClose:()=>void;assets:SocialMediaAsset[];campaigns:SocialCampaign[];capabilities:{facebookPublish:boolean;facebookStory:boolean;instagramPublish:boolean};onUpload:(files:File[])=>Promise<SocialMediaAsset[]>;onRefresh:()=>Promise<void>;setPulse:(p:PulseState|null)=>void}){
  ...
  const useBulkCopy=(selection:CopyVaultSelection)=>{setBulkCopy(selection);setBulkCaption(selection.body);if(selection.hashtags.length)setBulkTags(selection.hashtags.map(tag=>`#${tag}`).join(" "))}
  const useSlotCopy=(index:number,selection:CopyVaultSelection)=>{setSlotCopies(v=>({...v,[index]:selection}));patchSlot(index,{caption:selection.body,hashtags:selection.hashtags})}
  ...
  const submit=async()=>{
    ...
    try{
      ...
      let usageWarning="";try{const publications=Array.isArray(apply.data.publications)?apply.data.publications:[];for(let i=0;i<slots.length;i++){const selection=slotCopies[i]||bulkCopy;if(!selection)continue;const publicationId=publications[i]?.id;await fetch("/api/social-command/copy-vault/usage",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({itemId:selection.itemId,versionNo:selection.versionNo,surface:slotCopies[i]?"bulk_slot":"bulk_apply_all",publicationId:publicationId||null,bulkPlanId:create.data.id,contentSnapshot:slots[i].caption,customized:slots[i].caption.trim()!==selection.body.trim(),context:{slotNo:slots[i].slotNo,format:slots[i].format,channels:slots[i].channels}})}).then(async r=>{const p=await r.json().catch(()=>({ok:false,error:`HTTP ${r.status}`}));if(!r.ok||p?.ok===false)throw new Error(p?.error||`HTTP ${r.status}`)})}}catch(e){usageWarning=e instanceof Error?e.message:String(e)}await onRefresh();setPulse({id:crypto.randomUUID(),label:`${apply.data.done} créations planifiées`,status:apply.data.failed?"failed":"completed",progress:100,step:apply.data.failed?"Terminé avec anomalies":usageWarning?"Planifié · suivi Copy Vault à vérifier":"Synchronisé avec Temporal Command",completed:apply.data.done,total:slots.length,failed:apply.data.failed,detail:apply.data.failed?`${apply.data.failed} slot(s) nécessitent une correction`:usageWarning||"Calendrier et file d’exécution mis à jour"})
  ...
  return <SovereignModal open={open} onClose={onClose} title="Mass Publishing Orchestrator" kicker="STUDIO · SOVEREIGN WORKSPACE · BULK INTELLIGENCE">
    ...
    <div className={styles.bulkArchitecture}>
      ...
      <main className={styles.slotStage}>
        ...
        <div className={styles.slotMatrix}>{slots.map((slot,i)=>{const asset=assets.find(a=>a.id===slot.assetIds[0]);const d=new Date(slot.scheduledAt);return <article key={slot.slotNo} className={`${styles.slotRow} ${slot.assetIds.length?styles.slotReady:""}`}>
          ...
          {expanded===i?<div className={styles.slotEditor}>
            ...
            <div className={styles.span2}><CopyVaultPicker compact surface="bulk_slot" format={slot.format} channels={slot.channels} campaignId={campaignId||undefined} copyTypes={slot.format==="story"?["story_text","promotional_message","opening_hook","cta"]:slot.format==="reel"?["reel_caption","promotional_message","opening_hook","cta"]:["post_caption","promotional_message","offer_text","opening_hook","cta"]} onSelect={(selection)=>useSlotCopy(i,selection)}/></div>{slot.format!=="story"?<label className={styles.span2}>Légende commune<textarea rows={3} value={slot.caption} onChange={e=>patchSlot(i,{caption:e.target.value})}/></label>:<label className={styles.span2}>Brief interne Story<textarea rows={2} value={slot.caption} onChange={e=>patchSlot(i,{caption:e.target.value})} placeholder="Note créative interne — non incrustée automatiquement dans le média"/></label>}
      ...
      <aside className={styles.bulkInspector}>
        ...
        <CopyVaultPicker surface="bulk_apply_all" format={format} channels={channels} campaignId={campaignId||undefined} copyTypes={format==="story"?["story_text","promotional_message","opening_hook","cta"]:format==="reel"?["reel_caption","promotional_message","opening_hook","cta"]:["post_caption","promotional_message","offer_text","opening_hook","cta"]} onSelect={useBulkCopy}/>{bulkCopy?<div className={styles.capabilityWarning}>Copy Vault · {bulkCopy.code} · v{bulkCopy.versionNo} · approved source</div>:null}<label>Légende commune<textarea className={styles.field} rows={2} value={bulkCaption} onChange={e=>setBulkCaption(e.target.value)} placeholder="Appliquer à tous les slots"/></label>
