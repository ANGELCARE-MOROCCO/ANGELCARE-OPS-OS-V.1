...
const visual: Record<string, { concept: string; description: string; tone: string; layout: Layout; icon: any }> = {
  ...
  'Packages MZ13': { concept: 'Departure Terminal', description: 'Les packages attendent autorité, readiness et fenêtre de départ.', tone: 'from-blue-950 to-slate-950', layout: 'terminal', icon: PackageCheck },
  ...
  'Validation MZ14': { concept: 'Operational Clearance Certificate', description: 'Readiness, coverage et Shadow guarantees convergent vers une certification.', tone: 'from-emerald-950 to-slate-950', layout: 'certificate', icon: ShieldCheck },
}
...
function ClearanceDesk({ rows }: { rows: Action[] }) { return <div className="space-y-6"><div><SChip tone="amber"><BadgeCheck size={11} /> Operational Clearance Desk</SChip><h2 className="mt-4 text-4xl font-black tracking-[-.055em]">Sensitive actions wait for human authority before movement.</h2></div><section className="grid gap-6 xl:grid-cols-[300px_1fr]"> <aside className="rounded-[38px] bg-slate-950 p-6 text-white"><SIcon icon={BadgeCheck} tone="amber" /><p className="mt-5 text-4xl font-black">{rows.length}</p><p className="mt-1 text-[9px] font-black uppercase tracking-[.14em] text-slate-500">Clearance requests</p><p className="mt-5 text-[10px] leading-5 text-slate-300">L’autorité, la portée et la raison doivent rester explicites dans le backend; cette page ne fabrique aucune décision.</p></aside><div className="space-y-4">{rows.map((action) => <article key={action.id} className="grid gap-4 rounded-[32px] border border-amber-200 bg-white p-5 shadow-[0_18px_50px_rgba(146,64,14,.06)] md:grid-cols-[auto_1fr_auto] md:items-center"><SIcon icon={BadgeCheck} tone="amber" /><div><h3 className="text-sm font-black">{action.actionType}</h3><p className="mt-1 text-[10px] text-slate-500">{action.adapterCode} · {action.target.module} · priority {action.priority}</p></div><Status value={action.status} /></article>)}</div></section>{!rows.length ? <SEmpty title="Clearance desk vide" description="Aucune action n’attend actuellement d’approbation." icon={BadgeCheck} /> : null}</div> }
...
function DarkFact({ label, value }: { label: string; value: string }) { return <div className="rounded-[22px] border border-white/10 bg-white/7 p-4 text-center backdrop-blur"><p className="text-2xl font-black">{value}</p><p className="mt-1 text-[8px] font-black uppercase tracking-[.12em] text-slate-500">{label}</p></div> }
function TinyFact({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl border border-slate-200 bg-white/80 p-3"><p className="text-[8px] font-black uppercase text-slate-500">{label}</p><p className="mt-1 truncate text-[10px] font-black text-slate-700">{value}</p></div> }
