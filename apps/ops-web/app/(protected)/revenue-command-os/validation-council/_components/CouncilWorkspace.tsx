  ...
  return (
    <div className="space-y-7">
      ...
      <CouncilHero
        ...
        warning={error || 'Conseil facultatif en exécution live — l’opérateur peut adopter, modifier ou ignorer chaque recommandation.'}
      />
      <div className="mt-4 rounded-[26px] border border-violet-200 bg-white p-5 shadow-sm">
        <label className="block"><span className="text-[9px] font-black uppercase tracking-[.15em] text-slate-500">Identifiant du dossier stratégique</span><input value={strategyId} onChange={(event) => setStrategyId(event.target.value)} placeholder="UUID de la stratégie persistée" className="mt-3 h-13 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm font-bold text-slate-950 outline-none placeholder:text-slate-500 focus:border-violet-400 focus-visible:ring-2 focus-visible:ring-violet-200" /></label>
      </div>
      </>
      ...
      <section className="relative min-h-[880px] overflow-hidden rounded-[52px] border border-violet-200 bg-white p-5 shadow-[0_35px_110px_rgba(76,29,149,.09)] sm:p-8">
        ...
        <div className="relative grid min-h-[820px] place-items-center">
          ...
          {agents.map(([name, scope, tone], index) => {
            ...
            return <article key={name} className={`absolute w-52 rounded-[28px] border bg-white/94 p-4 shadow-[0_18px_50px_rgba(15,23,42,.08)] backdrop-blur transition ${review ? 'border-emerald-200' : 'border-slate-200'}`} style={{ left: `calc(50% + ${Math.cos(angle) * radiusX}px - 104px)`, top: `calc(50% + ${Math.sin(angle) * radiusY}px - 90px)` }}><div className="flex items-center justify-between"><SIcon icon={index === 7 ? Swords : index === 9 ? UserRoundCheck : BrainCircuit} tone={tone} className="h-9 w-9 rounded-xl" /><span className={`h-2.5 w-2.5 rounded-full ${review ? 'bg-emerald-500 shadow-[0_0_14px_rgba(16,185,129,.65)]' : 'bg-slate-300'}`} /></div><p className="mt-4 text-[8px] font-black uppercase tracking-[.12em] text-slate-500">Siège {String(index + 1).padStart(2, '0')}</p><h3 className="mt-1 text-xs font-black">{name}</h3><p className="mt-1 text-[9px] leading-4 text-slate-500">{scope}</p><p className={`mt-3 line-clamp-3 text-[9px] font-bold ${review ? 'text-emerald-700' : 'text-slate-500'}`}>{review?.verdict || 'Analyse disponible sur lancement'}</p></article>
          })}
        </div>
      </section>

      {data ? <div className="grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
        <section className="rounded-[42px] border border-slate-200 bg-white p-6 shadow-[0_25px_75px_rgba(15,23,42,.07)]"><div className="flex items-center gap-4"><SIcon icon={BadgeCheck} tone="emerald" /><div><SChip tone="emerald">Synthèse consultative</SChip><h2 className="mt-2 text-3xl font-black tracking-[-.05em]">{data.classification?.classification || 'Analyse à lancer'}</h2></div></div><div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4">{Object.entries(scores).map(([key, value]) => <Score key={key} label={key} value={key === 'confidence' ? Math.round(Number(value) * 100) : Number(value)} />)}</div><div className="mt-6"><STraceLink traceId={data.traceId || data.councilRunId || strategyId} /></div></section>
...
function Score({ label, value }: { label: string; value: number }) { const safe = Number.isFinite(value) ? value : 0; return <div className="rounded-[22px] border border-slate-200 p-4"><div className="flex items-center justify-between"><p className="truncate text-[8px] font-black uppercase text-slate-500">{label}</p><p className="text-sm font-black">{safe}</p></div><div className="mt-3 h-1.5 rounded-full bg-slate-100"><div className="h-full rounded-full bg-emerald-500" style={{ width: `${Math.max(2, Math.min(100, safe))}%` }} /></div></div> }
