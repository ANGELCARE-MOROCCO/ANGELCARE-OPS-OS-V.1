...
import LiveEntityActions from '../_components/live-operations/LiveEntityActions'
import OperationalEntityDrawer from '../_components/operational-depth/OperationalEntityDrawer'
...
const evidence = [
  ...
  ['Doctrine', 'Règles, preuves, cas et SOP consultables', ShieldCheck, 'emerald'],
] as const
...
export default async function StrategyEnginePage() {
  ...
  return (
    <main data-revenue-route="strategy-engine" data-revenue-workspace="strategy-engine" data-revenue-surface="light" className="min-h-screen bg-[#f6f8fc] px-4 py-6 text-slate-950 sm:px-6 lg:px-8">
      <StrategiesHero
        state={truthState(operations.sourceState, strategies.length)}
        posture="Production analytique · dossier persisté"
        authority="Autorité opérateur complète · décision et exécution directes"
        summary={top
          ? `${top.code} · ${top.thesis}`
        ...
        metrics={[
          { label: 'Stratégies persistées', value: strategies.length, note: `${operations.counts.strategiesReadyForCouncil} directement exploitable(s)`, tone: 'violet' },
          { label: 'Approuvées', value: approved, note: 'Décision immédiate par l’opérateur', tone: 'emerald' },
      ...
      <div className="mt-7 grid gap-6 2xl:grid-cols-[330px_1fr_390px]">
        <aside className="space-y-4">
          <div><SChip tone="blue">Plan des preuves</SChip><h2 className="mt-3 text-2xl font-black tracking-[-.04em]">Contexte réel disponible pour construire et exécuter la stratégie.</h2></div>
        ...
        <section className="relative min-h-[760px] overflow-hidden rounded-[48px] border border-violet-200 bg-white p-7 shadow-[0_32px_100px_rgba(76,29,149,.08)]">
          ...
          <div className="relative mt-14 rounded-[34px] bg-slate-950 p-6 text-white"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-[9px] font-black uppercase tracking-[.15em] text-violet-300">Posture active</p><h3 className="mt-2 text-xl font-black">Sortie analytique persistée et versionnée</h3><p className="mt-2 max-w-2xl text-[10px] leading-5 text-slate-300">Les actions sont disponibles immédiatement. Email OS et WhatsApp conservent leurs mécanismes de canal existants; chaque résultat reste audité.</p></div><SChip tone="emerald">LIVE</SChip></div></div>
        </section>
      ...
      <section className="mt-7 rounded-[44px] border border-slate-200 bg-white p-7"><div className="flex items-start gap-4"><StrategyStaticIcon icon={Layers3} tone="blue" /><div><SChip tone="blue">Chaîne stratégique opérationnelle</SChip><h2 className="mt-3 text-3xl font-black tracking-[-.05em]">Objectif → Stratégies → Décision opérateur → Compilation → Exécution</h2><p className="mt-2 max-w-4xl text-xs leading-6 text-slate-500">Cette page expose les stratégies persistées et permet leur passage direct vers le Studio, la compilation et l’exécution. Le Conseil demeure un appui facultatif.</p></div></div></section>
    </main>
  )
}
...
function StrategyCard({ strategy, index }: { strategy: RevenueOsOperationalStrategy; index: number }) {
  ...
  return <article className={`relative overflow-hidden rounded-[34px] border p-6 shadow-[0_20px_55px_rgba(15,23,42,.07)] ${tone}`}>
    ...
    <div className="mt-4 flex flex-wrap gap-2"><LiveEntityActions entityType="strategy" entityId={strategy.id} compact /><OperationalEntityDrawer entityType="strategy" entityId={strategy.id} title={strategy.title} compact /></div><div className="mt-5 flex items-center justify-between border-t border-slate-200/70 pt-4"><span className="text-[10px] font-bold text-slate-500">Version {strategy.version}</span><Link href="/revenue-command-os/strategy-studio" className="inline-flex items-center gap-1 text-[10px] font-black text-blue-700">Décider <ArrowRight size={13} /></Link></div>
  </article>
}
