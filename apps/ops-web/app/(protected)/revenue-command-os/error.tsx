...
export default function RevenueCommandOsError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div data-revenue-route="error-boundary" data-revenue-surface="light" className="min-h-[calc(100vh-86px)] bg-[#f4f7fb] p-4 sm:p-6">
      <section className="mx-auto grid min-h-[720px] max-w-[1560px] overflow-hidden rounded-[52px] border border-rose-200 bg-white shadow-[0_40px_130px_rgba(15,23,42,.13)] lg:grid-cols-[.78fr_1.22fr]">
        <div data-revenue-surface="dark" className="relative overflow-hidden bg-gradient-to-br from-rose-950 via-red-950 to-slate-950 p-8 text-white sm:p-12">
          ...
          <div className="relative flex h-full flex-col justify-between">
            <div><span className="grid h-16 w-16 place-items-center rounded-[24px] border border-rose-300/30 bg-rose-500/20"><ShieldAlert size={30} /></span><p className="mt-8 text-[10px] font-black uppercase tracking-[.24em] text-rose-100">Interruption contrôlée</p><h1 className="mt-3 text-5xl font-black tracking-[-.065em] text-white">La chaîne de vérité a été interrompue.</h1><p className="mt-5 max-w-lg text-sm font-semibold leading-7 text-rose-50">Revenue Command OS refuse de présenter une donnée incertaine comme une vérité opérationnelle.</p></div>
