      ...
      <div className="mt-8 overflow-x-auto pb-5">
        <div className="flex min-w-max gap-5">
          {objectives.map((objective, index) => <article key={objective.id} className={`relative w-[370px] overflow-hidden rounded-[32px] border bg-white p-6 shadow-[0_22px_60px_rgba(15,23,42,.07)] ${index === 0 ? 'border-blue-300' : 'border-slate-200'}`}>
            ...
            <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4"><SChip tone="violet">{objective.executionMode}</SChip><span className="inline-flex items-center gap-1 text-xs font-black text-slate-500">Dossier consolidé dans cette vue <BadgeCheck size={14} /></span></div>
          </article>)}
        </div>
      </div>
  ...
  return <div className="min-h-screen bg-[#f5f7fa] px-4 py-7 sm:px-7 lg:px-10 xl:px-12">
    <section className="mx-auto max-w-[1740px]">
      ...
      <div className="mt-8 grid gap-5 xl:grid-cols-[minmax(0,1fr)_430px]">
        ...
        <aside className="sticky top-6 self-start rounded-[34px] border border-slate-200 bg-slate-950 p-7 text-white shadow-[0_28px_80px_rgba(15,23,42,.2)]"><p className="text-[10px] font-black uppercase tracking-[.18em] text-blue-300">Causal investigation</p><h2 className="mt-3 text-2xl font-black">Chaîne complète de l’événement</h2><div className="mt-7 space-y-1">{['Événement persisté', 'Acteur identifié', 'Ressource enregistrée', 'Résultat enregistré'].map((step, index) => <div key={step} className="flex gap-3"><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full text-[10px] font-black ${events.length ? 'bg-emerald-400 text-emerald-950' : 'bg-white/10 text-slate-500'}`}>{index + 1}</span><div className="flex-1 border-b border-white/10 py-2.5"><p className="text-sm font-bold">{step}</p><p className="mt-1 font-mono text-[9px] text-slate-500">trace.segment.{index + 1}</p></div></div>)}</div><div className="mt-7 rounded-2xl border border-white/10 bg-white/5 p-4"><p className="text-[9px] font-black uppercase tracking-[.15em] text-slate-500">Trace sélectionnée</p><p className="mt-2 break-all font-mono text-xs text-blue-200">{events[0]?.eventId || 'Aucune trace sélectionnée'}</p></div></aside>
      </div>
    </section>
  </div>
}
