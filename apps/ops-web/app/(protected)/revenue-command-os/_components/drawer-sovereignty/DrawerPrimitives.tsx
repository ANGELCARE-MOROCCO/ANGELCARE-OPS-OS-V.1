...
export function SovereignDrawerPanel({ children, width = 'max-w-[820px]', dataId }: { children: ReactNode; width?: string; dataId: string }) {
  ...
  return <aside data-revenue-component="drawer-panel" data-revenue-surface="light" className={`relative flex h-full w-full ${width} flex-col overflow-hidden border-l border-white/10 bg-white shadow-[-34px_0_110px_rgba(2,6,23,.32)] ${styles.panel}`} data-drawer-id={dataId} data-drawer-width={widthPx}>{children}</aside>
}

export function DrawerCloseButton({ onClose, inverted = false }: { onClose: () => void; inverted?: boolean }) {
  return <button data-revenue-component="drawer-close" type="button" onClick={onClose} aria-label="Fermer le panneau" className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl border transition focus-visible:outline-none focus-visible:ring-4 ${inverted ? 'border-white/15 bg-white/10 text-white hover:bg-white/15 focus-visible:ring-white/20' : 'border-slate-200 bg-white text-slate-700 shadow-sm hover:bg-slate-50 focus-visible:ring-blue-100'}`}><X size={19} /></button>
}

export function DrawerBadge({ children, tone = 'slate', inverted = false }: { children: ReactNode; tone?: Tone; inverted?: boolean }) {
  return <span data-revenue-component="drawer-badge" data-tone={tone} data-inverted={inverted ? 'true' : 'false'} className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[9px] font-black uppercase tracking-[.11em] ${inverted ? 'border-white/15 bg-white/10 text-white' : tones[tone].badge}`}>{children}</span>
}
...
export function DrawerSection({ title, eyebrow, icon: Icon, tone = 'blue', children, action, dark = false }: { title: string; eyebrow?: string; icon?: ElementType; tone?: Tone; children: ReactNode; action?: ReactNode; dark?: boolean }) {
  return <section data-revenue-component="drawer-section" data-revenue-surface={dark ? 'dark' : 'light'} data-tone={tone} className={`overflow-hidden rounded-[28px] border ${dark ? 'border-white/10 bg-slate-950 text-white' : 'border-slate-200 bg-white shadow-[0_16px_45px_rgba(15,23,42,.045)]'}`}>
...
export function DrawerMetric({ label, value, note, icon: Icon, tone = 'blue', dark = false }: { label: string; value: ReactNode; note?: string; icon?: ElementType; tone?: Tone; dark?: boolean }) {
  return <div data-revenue-component="drawer-metric" data-revenue-surface={dark ? 'dark' : 'light'} data-tone={tone} className={`rounded-[22px] border p-4 ${dark ? 'border-white/10 bg-white/[.06]' : 'border-slate-200 bg-white shadow-[0_10px_30px_rgba(15,23,42,.035)]'}`}>
...
export function DrawerExecutiveBrief({ children, tone = 'blue', dark = false }: { children: ReactNode; tone?: Tone; dark?: boolean }) {
  return <div data-revenue-component="drawer-brief" data-revenue-surface={dark ? 'dark' : 'light'} data-tone={tone} className={`rounded-[26px] border p-5 ${dark ? 'border-white/10 bg-white/[.07]' : tones[tone].soft}`}>
...
export function DrawerActionFooter({ children, note }: { children: ReactNode; note?: string }) {
  return <footer data-revenue-component="drawer-footer" data-revenue-surface="light" className={`border-t border-slate-200 bg-white/97 px-5 py-4 shadow-[0_-18px_45px_rgba(15,23,42,.07)] backdrop-blur-xl sm:px-7 ${styles.actionFooter}`} data-drawer-action-footer="true">
