...
export function AngelCareHeroBrand({ inverted = false }: { inverted?: boolean }) {
  return <div className={styles.brand} data-hero-component="brand" data-inverted={inverted ? 'true' : 'false'}>
...
export function HeroTruthBadge({ state, label }: { state: HeroTruthState; label?: string }) {
  return <span data-hero-component="truth-badge" className={`${styles.truthBadge} ${truthTone[state]}`} aria-label={`État opérationnel: ${label || state}`}><span className={styles.truthDot} />{label || state}</span>
}

export function HeroAuthorityBadge({ children, inverted = false }: { children: React.ReactNode; inverted?: boolean }) {
  return <span data-hero-component="authority-badge" className={styles.authorityBadge} data-inverted={inverted ? 'true' : 'false'}><ShieldCheck size={13} aria-hidden="true" />{children}</span>
}

export function HeroMetricGrid({ metrics, dark = false, className = '' }: { metrics: HeroMetric[]; dark?: boolean; className?: string }) {
  return <div data-hero-component="metric-grid" className={`${styles.metricGrid} ${className}`}>{metrics.map((metric) => <div key={`${metric.label}-${metric.value}`} className={`${styles.metric} ${metricTone[metric.tone || 'slate']}`} data-hero-card={dark ? 'dark' : 'light'} data-dark={dark ? 'true' : 'false'}>
...
export function HeroActionRail({ actions = [], dark = false }: { actions?: HeroAction[]; dark?: boolean }) {
  ...
  return <div data-hero-component="actions" className={styles.actionRail}>{actions.map((action) => {
...
export function HeroExecutiveBrief({ label = 'Intelligence exécutive', children, dark = false }: { label?: string; children: React.ReactNode; dark?: boolean }) {
  return <div data-hero-card={dark ? 'dark' : 'light'} className={styles.executiveBrief} data-dark={dark ? 'true' : 'false'}><span>{label}</span><p>{children}</p></div>
}

export function HeroWarning({ children, dark = false }: { children?: React.ReactNode; dark?: boolean }) {
  if (!children) return null
  return <div data-hero-card={dark ? 'dark' : 'light'} className={styles.warning} data-dark={dark ? 'true' : 'false'}>{children}</div>
}
