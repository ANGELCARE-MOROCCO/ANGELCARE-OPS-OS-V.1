...
const initialState = {
  ...
  executionMode: 'live' as RevenueOsExecutionMode,
}
...
const chapters = [
  ...
  ['07', 'Exécution', 'Mode live permanent'],
  ['08', 'Activation', 'Disponibilité immédiate'],
] as const
      ...
      <form onSubmit={submit} className="grid max-h-[94vh] w-full max-w-[1380px] overflow-hidden rounded-[44px] border border-white/20 bg-white shadow-[0_50px_180px_rgba(2,6,23,.48)] xl:grid-cols-[310px_minmax(0,1fr)_340px]">
        ...
        <main className="overflow-y-auto p-6 sm:p-8 lg:p-10">
          <div className="flex items-start justify-between gap-6">
            <div><p className="text-[10px] font-black uppercase tracking-[.2em] text-blue-700">Mandate Ledger</p><h1 className="mt-2 text-3xl font-black tracking-[-.045em] text-slate-950">Créer un objectif stratégique live</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Le mandat est enregistré immédiatement et devient disponible pour l’intelligence, les commandes, la compilation et l’exécution.</p></div>
          ...
          <div className="mt-8 space-y-7">
            ...
            <Field icon={ShieldCheck} label="Exécution Revenue OS" number="07"><div className="rounded-[24px] border border-emerald-300 bg-emerald-50 p-5"><p className="text-xs font-black text-emerald-950">LIVE · autorité opérateur complète</p><p className="mt-1 text-[10px] leading-5 text-emerald-800">Le mandat est immédiatement disponible pour être lancé, modifié, compilé, planifié ou exécuté.</p></div></Field>
          </div>
        ...
        <aside className="overflow-y-auto border-l border-slate-100 bg-slate-50/80 p-6 sm:p-8">
          ...
          <div className="mt-5 rounded-[28px] border border-emerald-200 bg-emerald-50 p-5"><div className="flex gap-3"><ShieldCheck size={18} className="shrink-0 text-emerald-700" /><div><p className="text-xs font-black text-emerald-950">Activation immédiate</p><p className="mt-1 text-[10px] leading-5 text-emerald-800">Le mandat devient immédiatement exploitable par les workspaces Revenue OS. Les canaux Email OS et WhatsApp conservent leur fonctionnement actuel.</p></div></div></div>
          <div className="mt-6 grid gap-3"><button disabled={busy} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-6 py-3.5 text-sm font-black text-white shadow-xl disabled:opacity-60">{busy ? <Loader2 className="animate-spin" size={17} /> : <ArrowRight size={17} />}Créer et activer</button><button type="button" onClick={onClose} className="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50">Annuler</button></div>
        </aside>
      </form>
    </div>
  )
