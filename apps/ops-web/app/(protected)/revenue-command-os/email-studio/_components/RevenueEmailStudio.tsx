    ...
    <section className="mx-auto max-w-[1760px]">
      ...
      <div className="mt-6 grid gap-6 2xl:grid-cols-[330px_minmax(0,1fr)]">
        <aside className="rounded-[34px] border border-slate-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,.06)]">
          <div className="flex items-center justify-between"><div><p className="text-[9px] font-black uppercase tracking-[.16em] text-slate-500">Périmètre autorisé</p><h2 className="mt-2 text-xl font-black">Mailboxes assignées</h2></div><button onClick={() => void loadAssignments()} disabled={busy} className="rounded-xl border border-slate-200 p-2 text-slate-600 hover:bg-slate-50 disabled:opacity-50" aria-label="Actualiser"><RefreshCw size={16} className={busy ? 'animate-spin' : ''} /></button></div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_440px]">
            ...
            <aside className="rounded-[36px] border border-slate-200 bg-slate-950 p-6 text-white shadow-[0_26px_76px_rgba(15,23,42,.18)]"><div className="flex items-center justify-between"><div><p className="text-[9px] font-black uppercase tracking-[.16em] text-blue-300">Traçabilité</p><h2 className="mt-2 text-2xl font-black">Derniers envois</h2></div><Clock3 size={21} className="text-blue-300" /></div><div className="mt-6 space-y-3">{outbox.slice(0, 10).map((row) => <article key={String(row.id)} className="rounded-[22px] border border-white/10 bg-white/5 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-xs font-black text-white">{row.subject || '(Sans objet)'}</p><p className="mt-1 truncate text-[10px] text-slate-500">{row.to_email || row.recipient || 'Destinataire indisponible'}</p></div><span className={`rounded-full border px-2 py-1 text-[8px] font-black uppercase ${statusTone(row.status)}`}>{row.status || 'unknown'}</span></div><div className="mt-3 flex justify-between text-[9px] font-bold text-slate-500"><span>{displayDate(row.sent_at || row.scheduled_at || row.created_at)}</span><span>{Number(row.open_count || row.opens_count || 0)} ouverture(s)</span></div>{row.last_error ? <p className="mt-2 rounded-xl bg-rose-500/10 p-2 text-[9px] font-bold text-rose-200">{row.last_error}</p> : null}</article>)}</div>{!outbox.length ? <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-5 text-center"><Send className="mx-auto text-slate-500" /><p className="mt-3 text-sm font-black">Aucun envoi dans cette mailbox</p><p className="mt-2 text-xs leading-5 text-slate-500">Les futurs envois et leurs résultats apparaîtront ici.</p></div> : null}<Link href="/email-os" className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-xs font-black text-white hover:bg-white/15">Ouvrir Email OS complet <ArrowUpRight size={15} /></Link></aside>
          </div>
        </div>
      </div>
    </section>
