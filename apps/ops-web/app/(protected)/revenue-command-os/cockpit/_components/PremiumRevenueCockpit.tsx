...
const zones: Array<{ key: CockpitZoneKey | 'overview'; label: string; short: string; icon: ReactNode }> = [
  ...
  { key: 'approvals-governance', label: 'Décisions & commandes', short: 'Décisions', icon: <BadgeCheck size={17} /> },
]
...
function CommandHeader({ dashboard, busy, actionBusy, roleView, setRoleView, onRefresh, onExport }: { dashboard: CockpitDashboard | null; busy: boolean; actionBusy: boolean; roleView: CockpitRoleView; setRoleView: (value: CockpitRoleView) => void; onRefresh: () => void; onExport: () => void }) {
  ...
  return <>
    <CockpitHero
      ...
      posture={`Mode ${dashboard?.executionMode || 'live'}`}
      ...
      metrics={[
        ...
        { label: 'Autorité', value: 'LIVE', note: 'Tous opérateurs authentifiés', tone: 'emerald' },
      ]}
      actions={[
      ...
      warning={dashboard?.counts.externalActionsExecuted ? `${dashboard.counts.externalActionsExecuted} action(s) externe(s) confirmée(s) dans la source actuelle.` : 'Tous les opérateurs authentifiés disposent de l’autorité live. Email OS et WhatsApp conservent leurs canaux actuels.'}
    />
...
function Overview({ dashboard, onOpenZone, onSelectException }: { dashboard: CockpitDashboard; onOpenZone: (zone: CockpitZoneKey) => void; onSelectException: (exception: RevenueException) => void }) {
  return <div className="space-y-5">
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <CommandMetric label="Exécution réussie" value={String(dashboard.execution.succeeded)} detail={`${dashboard.execution.queued} en queue · ${dashboard.execution.failed} échec(s)`} progress={ratio(dashboard.execution.succeeded, dashboard.execution.succeeded + dashboard.execution.failed + dashboard.execution.queued)} tone="violet" icon={<Rocket />} />
      ...
      <Panel title="Ce qui a changé" eyebrow="Causalité live" action={<button onClick={() => onOpenZone('command-runs')} className="text-xs font-black text-blue-700">Chronologie complète <ChevronRight className="inline" size={14} /></button>}>
        <div className="space-y-2">
          {dashboard.timeline.slice(0, 8).map((event, index) => <div key={event.id} className="grid grid-cols-[36px_minmax(0,1fr)_auto] gap-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5">
            ...
            <span className="text-[10px] font-bold text-slate-500">{timeAgo(event.occurredAt)}</span>
          </div>)}
    ...
    <Panel title="Santé des treize zones" eyebrow="Operating system integrity">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{dashboard.zoneHealth.map((health) => <button key={health.zone} onClick={() => onOpenZone(health.zone)} className="rounded-2xl border border-slate-200 bg-white p-4 text-left transition hover:-translate-y-0.5 hover:shadow-lg">
        <div className="flex items-center justify-between"><span className={`h-2.5 w-2.5 rounded-full ${health.status === 'healthy' ? 'bg-emerald-500' : health.status === 'blocked' ? 'bg-red-500' : health.status === 'stale' ? 'bg-amber-500' : 'bg-slate-300'}`} /><span className="text-[10px] font-black uppercase tracking-[.12em] text-slate-500">{health.status}</span></div><p className="mt-4 text-sm font-black text-slate-950">{zoneLabel(health.zone)}</p><p className="mt-1 text-xs text-slate-500">{health.records} objet(s) · {health.criticalItems} critique(s)</p><p className="mt-3 line-clamp-2 text-[11px] leading-5 text-slate-500">{health.message}</p>
      </button>)}</div>
    </Panel>
  </div>
}
  ...
  return <div className="space-y-5">
    <Panel title={objective.title} eyebrow={`${objective.code} · version ${objective.version}`} action={<Freshness value={objective.freshness.state} text={objective.freshness.message} />}>
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,.75fr)]">
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><p className="text-xs font-black uppercase tracking-[.14em] text-slate-500">Périmètre d’exécution</p><Definition label="Business units" value={objective.businessUnits.join(', ') || 'Non défini'} /><Definition label="Territoires" value={objective.territories.join(', ') || 'Non défini'} /><Definition label="Segments" value={objective.segments.join(', ') || 'Non défini'} /><Definition label="Exécution" value="LIVE" /><Definition label="Prochain jalon" value={objective.nextMilestone || 'À déterminer'} /></div>
      </div>
    </Panel>
    <div className="grid gap-5 lg:grid-cols-2"><Panel title="Points opérationnels" eyebrow="Intervention directe">{objective.currentBlockers.length ? <ul className="space-y-2">{objective.currentBlockers.map((blocker) => <li key={blocker} className="flex gap-3 rounded-2xl border border-red-100 bg-red-50 p-3 text-sm font-semibold text-red-900"><AlertTriangle className="shrink-0" size={17} />{blocker}</li>)}</ul> : <EmptyState icon={<CheckCircle2 />} title="Aucun point critique déclaré" text="Le système surveille les signaux, la capacité et la trajectoire." positive />}</Panel><Panel title="Ce que le moteur cherche à gagner" eyebrow="Executive mandate"><p className="text-lg font-black leading-8 text-slate-950">Atteindre {formatDh(objective.revenueTarget)} avec une marge ≥ {objective.marginTarget}% sur {objective.territories.join(', ') || 'les territoires sélectionnés'}.</p><p className="mt-4 text-sm leading-7 text-slate-500">La progression est mesurée contre les revenus réalisés, le pipeline qualifié, la capacité et les résultats et la capacité.</p></Panel></div>
  </div>
}
...
function DecisionsZone({ approvals }: { approvals: ApprovalGovernanceSummary[] }) {
  return <Panel title={`Décisions & commandes · ${approvals.length}`} eyebrow="Exécution directe"><div className="space-y-3">{approvals.map((approval) => <div key={approval.id} className="grid gap-4 rounded-3xl border border-slate-200 p-5 lg:grid-cols-[minmax(0,1fr)_220px_160px]"><div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-amber-50 px-2 py-1 text-[10px] font-black uppercase text-amber-700">{approval.approvalType}</span>{approval.reversible ? <span className="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-black uppercase text-emerald-700">Réversible</span> : <span className="rounded-full bg-red-50 px-2 py-1 text-[10px] font-black uppercase text-red-700">Irréversible</span>}</div><h3 className="mt-3 text-base font-black text-slate-950">{approval.title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{approval.businessConsequence}</p>{approval.conditions.length ? <div className="mt-3 flex flex-wrap gap-2">{approval.conditions.map((condition) => <Tag key={condition} text={condition} />)}</div> : null}</div><div className="rounded-2xl bg-slate-50 p-4"><Definition label="Opérateur" value={approval.requestedBy || approval.requiredRole} /><Definition label="Décision" value={approval.status} /><Definition label="Expiration" value={approval.expiresAt ? new Date(approval.expiresAt).toLocaleDateString('fr-FR') : 'Non définie'} /></div><div className="flex flex-col items-stretch justify-center gap-2"><StatusPill value={approval.status} /><Link href="/revenue-command-os/approvals" className="rounded-xl bg-slate-950 px-4 py-2.5 text-center text-xs font-black text-white">Ouvrir les décisions</Link></div></div>)}{!approvals.length ? <EmptyState icon={<BadgeCheck />} title="Aucune décision enregistrée" text="Les décisions directes et leurs résultats apparaîtront ici." /> : null}</div></Panel>
}
...
function ExceptionDrawer({ exception, busy, onClose, onAcknowledge }: { exception: RevenueException; busy: boolean; onClose: () => void; onAcknowledge: () => void }) {
  ...
  return <SovereignDrawerOverlay onClose={onClose} label={`Executive Intervention Dossier — ${exception.code}`}>
    <SovereignDrawerPanel width="max-w-[900px]" dataId="MZ23-DRAWER-01-EXCEPTION">
      <header data-drawer-surface="dark" className="relative overflow-hidden bg-[linear-gradient(135deg,#1b0b0d_0%,#4c1018_48%,#111827_100%)] px-5 py-6 text-white sm:px-7 sm:py-7">
...
function StrategyCompact({ strategy, council }: { strategy: StrategyAssemblySummary; council: CockpitDashboard['council'] }) { return <div><div className="flex items-center justify-between"><Tag text={strategy.archetype} /><span className="text-2xl font-black text-blue-700">{strategy.confidence}%</span></div><h3 className="mt-4 text-lg font-black text-slate-950">{strategy.title}</h3><p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-600">{strategy.thesis}</p><div className="mt-4 rounded-2xl bg-slate-50 p-4"><p className="text-[10px] font-black uppercase tracking-[.14em] text-slate-500">Council classification</p><p className="mt-2 text-sm font-black text-slate-950">{council?.classification || 'Non évaluée'}</p><p className="mt-1 text-xs text-slate-500">{council?.blockingFindings || 0} blocage(s) · {council?.contradictions || 0} contradiction(s)</p></div></div> }
function ProgramCompact({ program }: { program: RevenueProgramSummary }) { return <div className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between"><div><p className="text-sm font-black text-slate-950">{program.title}</p><p className="mt-1 text-xs text-slate-500">{formatDh(program.pipelineContribution)} pipeline</p></div><span className="text-xl font-black text-emerald-700">{program.progressPercent}%</span></div><div className="mt-3 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-emerald-500" style={{ width: `${Math.min(100, program.progressPercent)}%` }} /></div><div className="mt-3 flex justify-between text-[10px] font-bold text-slate-500"><span>{program.activeCampaigns} campagnes</span><span>{program.tasksBlocked} bloquées</span></div></div> }
...
function ScoreCard({ label, value }: { label: string; value: number }) { return <div className="rounded-2xl border border-slate-200 p-4"><div className="flex items-center justify-between"><span className="text-[10px] font-black uppercase text-slate-500">{label}</span><span className={`text-xl font-black ${value >= 70 ? 'text-emerald-700' : value >= 50 ? 'text-amber-700' : 'text-red-700'}`}>{value}</span></div><div className="mt-3 h-1.5 rounded-full bg-slate-100"><div className={`h-1.5 rounded-full ${value >= 70 ? 'bg-emerald-500' : value >= 50 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${value}%` }} /></div></div> }
function ScoreMini({ label, value }: { label: string; value: number }) { return <div className="rounded-xl bg-white p-3 text-center shadow-sm"><p className="text-lg font-black text-slate-950">{value}</p><p className="text-[9px] font-black uppercase text-slate-500">{label}</p></div> }
...
function Definition({ label, value }: { label: string; value: string }) { return <div className="border-b border-slate-200 py-3 last:border-0"><p className="text-[9px] font-black uppercase tracking-[.13em] text-slate-500">{label}</p><p className="mt-1 text-sm font-bold leading-6 text-slate-800">{value}</p></div> }
...
function Freshness({ value, text }: { value: string; text: string }) { const color = value === 'live' || value === 'fresh' ? 'bg-emerald-500' : value === 'stale' ? 'bg-red-500' : 'bg-amber-500'; return <span className="inline-flex items-center gap-2 text-[10px] font-black uppercase text-slate-500"><span className={`h-2 w-2 rounded-full ${color}`} />{text}</span> }
...
function EmptyState({ icon, title, text, positive }: { icon: ReactNode; title: string; text: string; positive?: boolean }) { return <div className="grid place-items-center rounded-3xl border border-dashed border-slate-200 py-12 text-center"><span className={`grid h-12 w-12 place-items-center rounded-2xl ${positive ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-50 text-slate-500'}`}>{icon}</span><p className="mt-4 text-sm font-black text-slate-800">{title}</p><p className="mt-1 max-w-md text-xs leading-5 text-slate-500">{text}</p></div> }
