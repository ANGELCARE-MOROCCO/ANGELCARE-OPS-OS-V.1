...
import { BookOpenCheck, Copy, Filter, Search, Play } from 'lucide-react'
...
export default function CommandCatalogueExperience({ data, commands, loading, warnings, selectCommand }: CommandRouteContext) {
  ...
  const filtered = useMemo(() => commands.filter((command) => {
    ...
    return (family === 'all' || command.family === family) && (status === 'all' || command.status === status) && haystack.includes(query.trim().toLowerCase())
  }), [commands, family, query, status])
  ...
  return <div className="space-y-6" data-command-experience="canonical-command-registry">
    <CommandRouteMasthead
      ...
      subtitle="Rechercher, filtrer et ouvrir chaque doctrine opérationnelle avec sa version, son contexte, ses sorties et son exécution live."
    ...
    >
      <div className="grid grid-cols-3 gap-2"><CommandStat label="Visibles" value={filtered.length} tone="violet" /><CommandStat label="Actives" value={commands.filter((command) => command.status === 'approved').length} tone="emerald" /><CommandStat label="Exécutables" value={commands.length} tone="cyan" /></div>
    </CommandRouteMasthead>
    ...
    <CommandPanel title="Recherche institutionnelle" eyebrow="Filtres actifs" icon={Filter} tone="blue">
      <div className="grid gap-3 lg:grid-cols-[1fr_280px_240px]">
        ...
        <select value={status} onChange={(event) => setStatus(event.target.value)} className="h-12 rounded-2xl border border-slate-300 bg-white px-4 text-xs font-black text-slate-900 outline-none focus:border-violet-500"><option value="all">Tous les statuts</option>{[...new Set(commands.map((command) => command.status))].sort().map((value) => <option key={value} value={value}>{value}</option>)}</select>
      </div>
    </CommandPanel>
    ...
    <section className="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_24px_70px_rgba(15,23,42,.06)]">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1120px] border-collapse">
          <thead className="bg-slate-950 text-white"><tr>{['Commande', 'Doctrine', 'Famille', 'Mode', 'Version', 'Statut', 'Action'].map((label) => <th key={label} className="px-4 py-4 text-left text-[9px] font-black uppercase tracking-[.13em]">{label}</th>)}</tr></thead>
          <tbody className="divide-y divide-slate-100">
            {filtered.map((command) => <tr key={command.id} className="group transition hover:bg-violet-50/55">
              ...
              <td className="px-4 py-4"><SChip tone="emerald">LIVE</SChip></td>
              ...
              <td className="px-4 py-4"><div className="flex gap-2"><button type="button" onClick={() => selectCommand(command)} className="inline-flex items-center gap-1 rounded-xl bg-slate-950 px-3 py-2 text-[9px] font-black uppercase tracking-[.1em] text-white"><Play size={12}/>Exécuter</button><button type="button" onClick={() => void navigator.clipboard?.writeText(command.commandCode)} className="grid h-9 w-9 place-items-center rounded-xl border border-slate-200 text-slate-600" aria-label={`Copier ${command.commandCode}`}><Copy size={15} /></button></div></td>
            </tr>)}
          </tbody>
        </table>
      </div>
