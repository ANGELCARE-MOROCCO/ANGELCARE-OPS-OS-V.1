...
import { notificationTone } from '@/lib/notificationCommandEngine'
...
export function NotificationCard({ notification, children }: { notification: any; children?: React.ReactNode }) {
  const tone = notificationTone(notification.severity)
  return (
    <article style={cardStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <Badge tone={tone}>{String(notification.severity || 'info').toUpperCase()}</Badge>
          <Badge tone="#64748b">{notification.status || 'unread'}</Badge>
          <Badge tone="#7c3aed">IMPACT {notification.impact_score || 0}</Badge>
        </div>
        <small style={{ color: '#64748b', fontWeight: 800 }}>{formatDate(notification.created_at)}</small>
      </div>
      <h3 style={cardTitleStyle}>{notification.title}</h3>
      <p style={cardTextStyle}>{notification.message || 'No message.'}</p>
      <p style={metaStyle}>{notification.notification_type} • {notification.source}</p>
      {notification.action_url ? <ActionLink href={notification.action_url} variant="light">{notification.action_label || 'Open'}</ActionLink> : null}
      {children}
    </article>
  )
}
...
const metaStyle: React.CSSProperties = { margin: 0, color: '#475569', fontSize: 12, fontWeight: 850 }
