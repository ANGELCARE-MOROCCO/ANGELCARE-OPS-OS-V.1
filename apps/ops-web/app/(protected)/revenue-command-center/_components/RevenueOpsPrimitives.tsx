...
export function EmptyState({
  title = 'Aucune donnée',
  text = 'Aucun élément disponible pour le moment.',
  children,
}: {
  title?: string
  text?: string
...
export function KpiCard({
  ...
  const displaySub = sub || subtitle || delta

  return (
    <div style={kpiStyle}>
      <span>{displayTitle}</span>
      <strong style={{ color: tone }}>{safeValue}</strong>
      {displaySub ? <small>{displaySub}</small> : null}
    </div>
  )
}
...
export function WorkspaceHero({
  ...
  return (
    <section style={heroStyle}>
      ...
      {children || action || actions ? <div style={heroActionStyle}>{children}{action}{actions}</div> : null}
    </section>
  )
}
...
export function TaskCard({
  ...
  const assigned = assigneeName || ownerName || task?.assigneeName || task?.ownerName || 'Non assigné'
  ...
  const body = (
    <article style={taskCardStyle}>
      ...
      <div style={taskMetaGridStyle}>
        <small style={taskMetaStyle}>Owner: {assigned}</small>
        <small style={taskMetaStyle}>
          {task?.related_type || 'general'} {task?.due_at ? `• due ${new Date(task.due_at).toLocaleDateString('fr-FR')}` : ''}
        </small>
      </div>

      {badge ? <div>{badge}</div> : null}
      {children}
      {action ? <div style={footerStyle}>{action}</div> : null}
      {footer ? <div style={footerStyle}>{footer}</div> : null}
    </article>
  )
...
const badgeLightStyle: React.CSSProperties = {
  display: 'inline-flex',
  padding: '5px 9px',
  borderRadius: 999,
  background: '#f1f5f9',
