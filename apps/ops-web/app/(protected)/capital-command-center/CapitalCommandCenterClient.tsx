...
function trainingPreviewUrl(record: AnyRecord) {
  return record?.id ? `/capital-command-center/training-pages/${record.id}/preview` : ''
}

function openTrainingPreview(record: AnyRecord) {
  const url = trainingPreviewUrl(record)
  if (!url) {
    alert('Save the training HTML page first, then open the rendered page.')
    return
  }
...
function TrainingForm({ record, locked, update, updateData, data, tab }: any) {
  ...
  return <FormShell>
    ...
    {tab === 'content' && <>
      <div className="span2 html-preview-cta"><div><b>Rendered training page</b><small>After saving, staff should open the rendered browser page, not the HTML source code.</small></div><button type="button" onClick={() => openTrainingPreview(record)} disabled={!record.id}>Open rendered HTML in new tab</button></div>
      <TextArea label="HTML training page content" value={record.html_content} onChange={(v) => update('html_content', v)} locked={locked} wide placeholder="Paste the complete production HTML page here. This field is saved in Supabase and will render as a real browser page from the Open rendered HTML action after save." />
...
function ContextActions({ entity, record, setModal, modal }: any) { return <div className="summary-card"><p className="eyebrow">Quick operations</p>{entity === 'trainings' ? <button onClick={() => openTrainingPreview(record)} disabled={!record.id}>Open rendered HTML</button> : null}<button onClick={() => setModal({ ...modal, mode: 'edit' })}>Edit current record</button><button onClick={() => setModal({ ...modal, tab: 'audit' })}>Open audit tab</button><button onClick={() => navigator.clipboard?.writeText(record.reference_number || '')}>Copy reference</button><button onClick={() => window.print()}>Print screen view</button></div> }
...
.html-preview-cta{display:flex!important;align-items:center;justify-content:space-between;gap:14px;border:1px solid #bfdbfe;background:linear-gradient(135deg,#eff6ff,#ffffff);border-radius:22px;padding:16px 18px}
.html-preview-cta b,.html-preview-cta small{display:block}
.html-preview-cta b{font-size:15px;font-weight:950;color:#0f172a}
.html-preview-cta small{font-size:12px;font-weight:750;color:#64748b;margin-top:4px}
.html-preview-cta button:disabled{opacity:.45;cursor:not-allowed}
