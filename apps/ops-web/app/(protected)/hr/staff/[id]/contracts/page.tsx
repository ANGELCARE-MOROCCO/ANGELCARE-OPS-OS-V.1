...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = (data['contracts'] || data.docs || data.reviews || []).slice(0,100)
  const fields = fieldMap['contracts'] || ['title','status','created_at']
  return <AppShell><StaffShell>
    <StaffHero eyebrow="Staff 360" title="Staff Contracts" subtitle={staff?.full_name || staff?.name || 'Staff record'} actions={<><StaffButton href={`/hr/staff/${id}`}>Back to 360</StaffButton><StaffButton href={`/hr/staff/${id}/edit`} variant="light">Edit profile</StaffButton></>} />
    <StaffPanel title="Staff Contracts" subtitle="Synchronized staff records.">
