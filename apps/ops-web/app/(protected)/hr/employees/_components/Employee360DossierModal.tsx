...
import { Employee360CommandSurface } from './Employee360NativeCommand'
import {
...
function EmployeeMegaHRWorkspace({
  ...
  useEffect(() => {
    ...
    async function loadWorkspaceFromProduction() {
      ...
      setActions([])
      setSelectedActionId('')
      setWorkspaceMessage('Aucun cas RH sauvegardé. Les modèles opérationnels restent disponibles comme commandes de création, sans générer de faux enregistrements.')
    }
  ...
  function deleteAction(actionId: string) {
    ...
    if (deletedActionKey) {
      setDeletedHRWorkspaceKeys((current) => current.includes(deletedActionKey) ? current : [...current, deletedActionKey])
    }
    persist(next, 'Cas archivé dans le workspace. Sauvegardez pour confirmer l’archivage en production.')
          ...
          {selectedAction ? (
            <div className="mt-4 grid gap-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button type="button" onClick={() => deleteAction(selectedAction.id)} className="col-span-2 rounded-2xl border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-black text-rose-700 shadow-sm transition hover:-translate-y-0.5 hover:bg-rose-100 hover:shadow-md">Archiver le cas</button>
              </div>
            </div>
          ) : (
          ...
          <div className="max-h-[94vh] w-full max-w-7xl overflow-auto rounded-[36px] border border-white/80 bg-white shadow-[0_50px_170px_rgba(15,23,42,0.45)] ring-1 ring-slate-200/80">
            <div className="sticky top-0 z-10 flex flex-col gap-3 border-b border-slate-200 bg-white/95 p-5 backdrop-blur-xl xl:flex-row xl:items-start xl:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => { deleteAction(selectedAction.id); setAttestationModalOpen(false) }} className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-black text-rose-700 shadow-sm transition hover:-translate-y-0.5 hover:bg-rose-100 hover:shadow-md">Archiver le cas</button>
          ...
          <div className="max-h-[94vh] w-full max-w-7xl overflow-auto rounded-[36px] border border-white/80 bg-white shadow-[0_50px_170px_rgba(15,23,42,0.45)] ring-1 ring-slate-200/80">
            <div className="sticky top-0 z-10 flex flex-col gap-3 border-b border-slate-200 bg-white/95 p-5 backdrop-blur-xl xl:flex-row xl:items-start xl:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" onClick={() => saveSelectedActionToProduction(selectedAction)} className="rounded-2xl bg-violet-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-violet-100 transition hover:-translate-y-0.5 hover:bg-violet-700">Save</button>
                <button type="button" onClick={() => printAction(selectedAction)} className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-black text-white shadow-lg shadow-slate-200 transition hover:-translate-y-0.5 hover:bg-slate-800">Print A4</button>
                <button type="button" onClick={() => { deleteAction(selectedAction.id); setContractModalOpen(false) }} className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-black text-rose-700 shadow-sm transition hover:-translate-y-0.5 hover:bg-rose-100 hover:shadow-md">Archiver le cas</button>
...
export default function Employee360DossierModal({
  ...
  const [nativeCommandOpen, setNativeCommandOpen] = useState(false)
  ...
  async function save() {
    ...
    try {
      ...
      const updatedEmployee = (payload?.employee || payload?.data || { ...form, ...normalizePatch(form) }) as EmployeeRecord
  ...
  async function archiveEmployee() {
    ...
    try {
      ...
      setError(err?.message || 'Impossible d’archiver ce dossier.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[2147483000] isolate overflow-hidden bg-slate-950/76 p-2 backdrop-blur-md sm:p-4 print:static print:bg-white print:p-0">
      <style>{`
        @media print {
      ...
      <div className="relative z-[2147483001] mx-auto flex h-[calc(100dvh-16px)] w-[calc(100vw-16px)] max-w-[1880px] flex-col overflow-hidden rounded-[34px] sm:h-[calc(100dvh-32px)] sm:w-[calc(100vw-32px)] sm:rounded-[40px] border border-white/80 bg-white shadow-[0_60px_220px_rgba(15,23,42,0.55)] ring-1 ring-slate-200/70 print:hidden">
        <div className="no-print border-b border-slate-100 bg-gradient-to-r from-white via-violet-50/80 to-cyan-50/80 px-6 py-5">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="grid gap-2 sm:grid-cols-2 xl:flex xl:flex-wrap xl:justify-end">
              <button onClick={() => setNativeCommandOpen(true)} className="rounded-2xl bg-gradient-to-r from-violet-700 to-indigo-700 px-4 py-3 text-sm font-black text-white shadow-lg shadow-violet-200 transition hover:-translate-y-0.5">
                <Activity className="mr-2 inline h-4 w-4" />
                Commandement natif 360
              </button>
              ...
              <button disabled={saving} onClick={archiveEmployee} className={confirmDelete ? 'rounded-2xl bg-rose-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-rose-200' : 'rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-black text-rose-700 shadow-sm transition hover:bg-rose-100'}>
                <Trash2 className="mr-2 inline h-4 w-4" />
                {confirmDelete ? 'Confirmer l’archivage' : 'Archiver le dossier'}
              </button>
      ...
      {nativeCommandOpen && id ? (
        <Employee360CommandSurface
          employeeId={id}
          initialEmployee={form}
          mode="modal"
          onClose={() => setNativeCommandOpen(false)}
          onSaved={(updated) => {
