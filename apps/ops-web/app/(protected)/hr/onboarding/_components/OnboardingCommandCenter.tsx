...
export default function OnboardingCommandCenter({
  ...
  const [selectedId, setSelectedId] = useState(initialData.journeys[0]?.id ?? "");
  ...
  useEffect(() => {
    ...
    setTasksLive(initialData.tasks || []);
    setDocsLive(initialData.documents || []);
    setFeed(initialData.activity || []);
    setSelectedId((current) => initialData.journeys.some((journey) => journey.id === current) ? current : (initialData.journeys[0]?.id ?? ""));
  }, [initialData]);
  ...
  const allTasks = tasksLive.filter(
    (task) => String(task.journey_id || "") === String(selected?.id),
  );
    ...
    return (
      <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#eef2ff,transparent_34%),#f7f8fc] text-slate-950">
        ...
        <main className="min-w-0 p-5">
          <header className="sticky top-0 z-40 -mx-5 -mt-5 mb-5 border-b border-slate-200/80 bg-white/90 px-6 py-4 backdrop-blur-xl">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <div className="text-xs font-black tracking-[0.24em] text-violet-600">HR / CANDIDATE ONBOARDING</div>
                <h1 className="text-3xl font-black tracking-tight">Onboarding Command Center</h1>
                <p className="text-sm font-semibold text-slate-500">Le commandement visuel historique est préservé. Les parcours, tâches, documents, checklists, notes et preuves sont désormais alimentés exclusivement par Supabase.</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <Link href="/hr/onboarding/checklists" className="btn-lite"><ClipboardCheck className="h-4 w-4" />Checklists</Link>
                <button onClick={() => setModal("journey")} className="btn-primary"><Plus className="h-4 w-4" />New Journey</button>
              </div>
            </div>
          </header>

          <section className="mb-5 grid gap-4 md:grid-cols-4">
            <Kpi label="Active journeys" value={0} icon={Users} />
            <Kpi label="Tasks completed" value="0/0" icon={CheckCircle2} />
            <Kpi label="Documents" value={0} icon={FileCheck2} />
            <Kpi label="Avg. progress" value="0%" icon={Zap} />
          </section>

          <section className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)] 2xl:grid-cols-[320px_minmax(0,1fr)_370px]">
            <div className="card p-4">
              <div className="grid grid-cols-2 rounded-2xl bg-slate-100 p-1 text-sm font-black">
                <button className="rounded-xl bg-white py-2 text-violet-700 shadow-sm">All Onboardings</button>
                <button disabled className="rounded-xl py-2 text-slate-400">My Onboardings</button>
              </div>
              <div className="mt-4 flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-slate-400">
                <Search className="h-4 w-4" /><span className="text-sm font-semibold">Search candidate, position...</span>
              </div>
              <div className="mt-4 grid min-h-[520px] place-items-center rounded-[26px] border border-dashed border-violet-200 bg-gradient-to-br from-violet-50/70 via-white to-cyan-50/70 p-6 text-center">
                <div><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-white text-violet-700 shadow-lg ring-1 ring-violet-100"><Users className="h-6 w-6" /></div><h2 className="mt-4 text-lg font-black">Aucun parcours enregistré</h2><p className="mt-2 text-xs font-bold leading-5 text-slate-500">Aucun collaborateur fictif n’est injecté. Créez le premier parcours réel pour activer le dossier.</p><button onClick={() => setModal("journey")} className="btn-primary mt-5"><Plus className="h-4 w-4" />Créer le premier parcours</button></div>
              </div>
            </div>
            ...
            <div className="min-w-0 space-y-4">
              <div className="card p-5">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div><div className="text-[10px] font-black uppercase tracking-[0.2em] text-violet-600">Lifecycle cockpit</div><h2 className="mt-2 text-2xl font-black">Parcours onboarding prêt à être activé</h2><p className="mt-2 text-sm font-semibold text-slate-500">La même architecture de phases, tâches, documents, notes et activité reste disponible dès la création d’un parcours.</p></div>
                  <span className="rounded-full bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">Supabase authority ready</span>
                </div>
                <div className="relative mt-7 grid grid-cols-7 gap-2">
                  <div className="absolute left-8 right-8 top-5 h-1 rounded-full bg-slate-100" />
                  {phases.map((phase, index) => <div key={phase} className="relative text-center"><div className="relative z-10 mx-auto grid h-10 w-10 place-items-center rounded-full bg-slate-200 text-sm font-black text-slate-500">{index + 1}</div><div className="mt-3 text-[11px] font-black text-slate-500">{phase}</div><div className="mt-1 text-[10px] font-black text-slate-400">Pending</div></div>)}
                </div>
              </div>

              <div className="card overflow-hidden">
                <div className="flex gap-1 overflow-x-auto border-b border-slate-100 bg-white p-2">
                  {tabs.map((tab) => <button key={tab} disabled className="rounded-xl px-4 py-2 text-xs font-black text-slate-400">{tab}</button>)}
                </div>
                <div className="grid min-h-[410px] place-items-center p-8 text-center">
                  <div className="max-w-lg"><div className="mx-auto grid h-16 w-16 place-items-center rounded-[22px] bg-violet-50 text-violet-700"><ClipboardCheck className="h-7 w-7" /></div><h3 className="mt-5 text-xl font-black">Commandement opérationnel vide</h3><p className="mt-2 text-sm font-semibold leading-6 text-slate-500">Les tâches, documents, checklists, notes et événements apparaîtront ici depuis leurs tables natives. Aucun exemple local ou faux historique n’est affiché.</p><button onClick={() => setModal("journey")} className="btn-primary mt-5"><Plus className="h-4 w-4" />Lancer un parcours</button></div>
                </div>
              </div>
            </div>

            <aside className="hidden space-y-4 2xl:block">
              <SummaryPanel progress={0} completed={0} total={0} />
              <div className="card p-5"><div className="flex items-center justify-between"><h3 className="font-black">Important Documents</h3><FileCheck2 className="h-4 w-4 text-violet-600" /></div><div className="mt-5 rounded-2xl border border-dashed border-slate-200 p-5 text-center text-xs font-bold leading-5 text-slate-500">Aucune exigence documentaire avant l’affectation d’une checklist publiée.</div></div>
              <div className="card p-5"><div className="flex items-center justify-between"><h3 className="font-black">Activity Feed</h3><Activity className="h-4 w-4 text-violet-600" /></div><div className="mt-5 rounded-2xl border border-dashed border-slate-200 p-5 text-center text-xs font-bold leading-5 text-slate-500">Aucune activité enregistrée.</div></div>
            </aside>
          </section>
        </main>
        {modal === "journey" ? (
          <ExecutionModal
            modal="journey"
            selected={emptyJourney}
            isPending={isPending}
            onClose={() => setModal(null)}
            onCreateJourney={(journey: Journey) => {
              setJourneys([journey]);
  ...
  function toggleTask(task: Row) {
    ...
    startTransition(() => {
      void (async () => {
        const result = await updateOnboardingTask(String(task.id), { status: next });
        if (!result?.ok) {
          setToast(`Mise à jour impossible · ${result?.error || result?.message || "erreur"}`);
          return;
        }
  ...
  function deleteTask(task: Row) {
    startTransition(() => {
      void (async () => {
        const result = await deleteOnboardingTask(String(task.id));
        if (!result?.ok) {
          setToast(`Archivage tâche impossible · ${result?.error || result?.message || "erreur"}`);
          return;
        }
        log(`Task archived: ${task.title}`);
        router.refresh();
      })();
    });
  }
  function completePhase() {
    ...
    startTransition(() => {
      void (async () => {
        const result = await advanceOnboardingJourney(selected.id);
        if (!result?.ok) {
          setToast(`Avancement bloqué · ${result?.error || result?.message || "gates incomplets"}`);
          return;
        }
        log("Phase avancée avec validation des gates");
        router.refresh();
      })();
    });
  }
  ...
  function deleteJourney() {
    startTransition(() => {
      void (async () => {
        const result = await deleteOnboardingJourney(selected.id);
        if (!result?.ok) {
          setToast(`Archivage impossible · ${result?.error || result?.message || "erreur"}`);
          return;
        }
        log("Parcours onboarding archivé");
        router.refresh();
      })();
    });
  }
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#eef2ff,transparent_34%),#f7f8fc] text-slate-950">
      ...
      <main className="min-w-0 p-5">
          ...
          <section className="mb-5 grid gap-4 md:grid-cols-4">
            ...
            <Kpi
              ...
              icon={CheckCircle2}
            />
            <Kpi label="Documents" value={docs.length} icon={FileCheck2} />
      ...
      {modal && (
        <ExecutionModal
          ...
          onCreateDoc={() => {
            setModal(null);
            log("Document persisted");
            router.refresh();
          }}
          onNote={() => {
            setModal(null);
            log("Activity persisted");
            router.refresh();
          }}
          onReassign={() => {
            setModal(null);
...
function DocumentsPanel({ docs, setModal, setDocsLive }: any) {
  async function del(x: any) {
    const result = await deleteOnboardingDocument(String(x.id));
    if (!result?.ok) return;
    setDocsLive((rows: Row[]) => rows.filter((row) => row.id !== x.id));
  }
  return (
    <div className="card p-5">
      ...
      {docs.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-200 p-5 text-center text-xs font-bold text-slate-500">Aucun document enregistré pour ce parcours.</div>
      ) : null}
      {docs.map((doc: any) => (
        <div
          key={doc.id}
        ...
        >
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {
                const url = String(doc.file_url || doc.fileUrl || "");
                if (url) window.open(url, "_blank", "noopener,noreferrer");
              }}
              disabled={!String(doc.file_url || doc.fileUrl || "")}
              title={String(doc.file_url || doc.fileUrl || "") ? "Ouvrir le document" : "Aucun fichier téléchargeable n’est encore attaché"}
              className="rounded-xl bg-slate-50 p-2 disabled:cursor-not-allowed disabled:opacity-35"
            >
...
function ActivityPanel({ feed, selected, setModal }: any) {
  return (
    <div className="card p-5">
      ...
      {feed.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-200 p-5 text-center text-xs font-bold text-slate-500">Aucune activité enregistrée pour ce parcours.</div>
      ) : null}
      {feed.slice(0, 5).map((a: any) => (
          <div
            key={a.id || a.title}
...
function ExecutionModal({
  ...
  async function pushEvent(title: string, body: string, type = "activity") {
    const result = await addOnboardingNote({
      journey_id: selected.id,
      title,
      body,
      type,
    });
    if (!result?.ok) throw new Error(result?.error || result?.message || "Activity persistence failed");
    router.refresh();
    return result;
  }
  async function save() {
    if (isJourney) {
      ...
      // The canonical create operation instantiates the published checklist, tasks and document requirements atomically.
    ...
    if (isDocument) {
      ...
      const documentResult = await createOnboardingDocument(doc);
      if (!documentResult?.ok) {
        alert(`Document save failed: ${documentResult?.error || documentResult?.message || "unknown error"}`);
        return;
      }
      await addOnboardingNote({
        journey_id: selected.id,
    ...
    if (isReminder) {
      ...
      const reminderResult = await createOnboardingReminder(reminder);
      if (!reminderResult?.ok) {
        alert(`Reminder save failed: ${reminderResult?.error || reminderResult?.message || "unknown error"}`);
        return;
      }
      onNote(reminder);
    ...
    if (isTask) {
      ...
      const taskResult = await createOnboardingTask(t);
      if (!taskResult?.ok) {
        alert(`Task save failed: ${taskResult?.error || taskResult?.message || "unknown error"}`);
        return;
      }
      onCreateTask(t);
      return;
    ...
    const noteResult = await addOnboardingNote(n);
    if (!noteResult?.ok) {
      alert(`Timeline save failed: ${noteResult?.error || noteResult?.message || "unknown error"}`);
      return;
    }
    onNote(n);
  }
            ...
            <div className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-[28px] border border-slate-200 bg-white p-4">
                  <div className="mb-3 text-sm font-black">Persisted evidence trail</div>
                  <p className="text-sm font-semibold leading-6 text-slate-500">Every timeline note, decision, blocker, comment and escalation is written to the canonical onboarding activity authority. The refreshed activity feed is the only operational source of truth.</p>
                </div>
              </div>
            </div>
          ) : isQuickAction ? (
