...
export default async function DetailPage({ params }: any) {
  const row = await getHRRecord('hr_onboarding_steps', params.id)
  if (!row) return <AppShell title="Not found" subtitle="Missing record." breadcrumbs={[{label:'HR',href:'/hr'}]}><div>Not found.</div></AppShell>
  return <AppShell title={row.title || 'Record'} subtitle="Restored operational detail page." breadcrumbs={[{label:'HR',href:'/hr'},{label:'Onboarding',href:'/hr/onboarding'},{label:row.title || 'Record'}]} actions={<PageAction href="/hr/onboarding" variant="light">Back</PageAction>}>
    ...
    <HRPanel title="Edit main status" subtitle="Safe quick update.">
      <form action={updateRestore} style={{display:'grid',gap:12}}>
        <input type="hidden" name="table" value="hr_onboarding_steps" />
        <input type="hidden" name="id" value={row.id} />
        <input type="hidden" name="returnPath" value="/hr/onboarding" />
