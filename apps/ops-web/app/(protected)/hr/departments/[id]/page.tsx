...
export default async function DetailPage({ params }: any) {
  const row = await getHRRecord('hr_departments', params.id)
  if (!row) return <AppShell title="Not found" subtitle="Missing record." breadcrumbs={[{label:'HR',href:'/hr'}]}><div>Not found.</div></AppShell>
  return <AppShell title={row.name || 'Record'} subtitle="Restored operational detail page." breadcrumbs={[{label:'HR',href:'/hr'},{label:'Departments',href:'/hr/departments'},{label:row.name || 'Record'}]} actions={<PageAction href="/hr/departments" variant="light">Back</PageAction>}>
    <HRHero title={row.name || 'Record'} subtitle={`Status: ${row.status || 'active'} • Stage: ${row.stage || 'active'}`} />
    <HRPanel title="Edit main status" subtitle="Safe quick update.">
      <form action={updateRestore} style={{display:'grid',gap:12}}>
        <input type="hidden" name="table" value="hr_departments" />
        <input type="hidden" name="id" value={row.id} />
        <input type="hidden" name="returnPath" value="/hr/departments" />
