...
export default async function Page() {
  ...
  const [rowsRes, openingsRes, candidatesRes, staffRes, tasksRes] = await Promise.all([
    supabase.from('hr_tasks').select('*').order('created_at', { ascending:false }).limit(80),
  ...
  return <AppShell title="HR Command Center" subtitle="One place to execute urgent HR work, synchronize tasks, and monitor operational risk." breadcrumbs={[{label:'HR',href:'/hr'},{label:'HR Command Center'}]} actions={<><PageAction href="/hr" variant="light">HR Home</PageAction><PageAction href="/hr/command-center">Command Center</PageAction></>}>
    <HrV2Hero title="HR Command Center" subtitle="One place to execute urgent HR work, synchronize tasks, and monitor operational risk." actions={<><HrAction href="/hr/tasks">Create HR task</HrAction><HrAction href="/hr/analytics" variant="light">Analytics</HrAction></>} />
