...
export default async function Angelcare360AttendanceLatePage({
  searchParams,
}: {
  ...
  const rows = await listAngelcare360LateRecords({
  ...
  if (!Array.isArray(rows)) {
    return (
      <Angelcare360EmptyState
        title="Retards indisponibles"
        description="La liste des retards n’a pas pu être chargée."
  ...
  return (
    ...
    >
      <form style={toolbarStyle} method="get">
        ...
        <Link href="/angelcare-360-command-center/presences/retards" style={secondaryButtonStyle}>
          Réinitialiser
        </Link>
      </form>
      ...
      {rows.length === 0 ? (
        <Angelcare360EmptyState
          title="Aucun retard"
          description="Aucun retard ne correspond aux filtres sélectionnés."
          actionLabel="Réinitialiser"
          actionHref="/angelcare-360-command-center/presences/retards"
        />
      ) : (
