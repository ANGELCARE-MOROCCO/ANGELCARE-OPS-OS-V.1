...
import { Badge } from '@refferq/components/ui/badge';
...
import { Skeleton } from '@refferq/components/ui/skeleton';
import {
  Table,
  TableBody,
  TableCell,
  ...
  TableHeader,
  TableRow,
} from '@refferq/components/ui/table';
import {
  Dialog,
  DialogContent,
  DialogDescription,
  ...
  DialogHeader,
  DialogTitle,
} from '@refferq/components/ui/dialog';
...
export default function AffiliateDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/market-os/refferq/affiliate/profile');
      ...
      if (data.success) {
        setStats({
          ...
          referralLink: `${window.location.origin}/market-os/ambassadors/refferq/r/${data.affiliate?.referralCode}`,
  ...
  const handleSubmitLead = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/market-os/refferq/affiliate/referrals', {
        method: 'POST',
  ...
  const handleGenerateCode = async () => {
    try {
      const response = await fetch('/api/market-os/refferq/affiliate/generate-code', { method: 'POST' });
      const data = await response.json();
      if (data.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Commission Banner */}
      <div>
        <Card className="overflow-hidden border-0 bg-gradient-to-br from-emerald-600 via-teal-600 to-cyan-700 text-white shadow-lg">
          <CardContent className="relative flex items-center justify-between p-6">
            <div className="flex items-center gap-5">
              ...
              <div>
                <p className="text-sm font-medium tracking-wide text-white/90">Earn 20% commission on all paid customers</p>
                <p className="mt-1 text-xl font-bold tracking-tight">Start referring today and grow your wealth!</p>
              </div>
            </div>
            <Button
              variant="secondary"
              onClick={() => setShowSubmitModal(true)}
              className="hidden gap-2 border-0 bg-white text-emerald-700 shadow-md transition hover:scale-105 hover:bg-emerald-50 active:scale-95 sm:flex"
            >
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div key={i} className="transition-transform hover:-translate-y-1">
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {referrals.length > 5 && (
            <Button variant="ghost" size="sm" asChild>
              <a href="/market-os/ambassadors/refferq/affiliate/referrals" className="gap-1">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => window.location.href = '/market-os/ambassadors/refferq/affiliate/referrals'}>
        ...
        <Card className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => window.location.href = '/market-os/ambassadors/refferq/affiliate/reports'}>
          <CardContent className="p-5 flex items-center gap-3">
        ...
        <Card className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => window.location.href = '/market-os/ambassadors/refferq/affiliate/resources'}>
          <CardContent className="p-5 flex items-center gap-3">
