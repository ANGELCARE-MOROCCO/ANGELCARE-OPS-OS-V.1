...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const registerRes = await fetch('/api/market-os/refferq/auth/register', {
        method: 'POST',
      ...
      // Step 2: Send OTP
      const otpRes = await fetch('/api/market-os/refferq/auth/send-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/market-os/refferq/auth/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      if (res.ok && data.success) {
        ...
        setTimeout(() => {
          ...
          if (user.role === 'ADMIN') {
            router.push('/market-os/ambassadors/refferq/admin');
          } else {
            router.push('/market-os/ambassadors/refferq/affiliate');
          }
        }, 2000);
      } else {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const res = await fetch('/api/market-os/refferq/auth/send-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email }),
      });
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        {step !== 'success' && (
          <p className="text-center text-sm text-muted-foreground">
            ...
            <Link href="/market-os/ambassadors/refferq/login" className="font-medium text-primary hover:underline">
              Sign in
            </Link>
          </p>
        )}
