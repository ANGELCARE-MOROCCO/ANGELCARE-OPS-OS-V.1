...
import { Avatar, AvatarFallback } from '@refferq/components/ui/avatar';
import { Separator } from '@refferq/components/ui/separator';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@refferq/components/ui/tabs';
import {
  Table,
  TableBody,
  TableCell,
...
import { Checkbox } from '@refferq/components/ui/checkbox';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import {
  ...
  SelectTrigger,
  SelectValue,
} from '@refferq/components/ui/select';
...
export default function PartnerDetailPage() {
  ...
  useEffect(() => {
    if (!authLoading && (!user || user.role !== 'ADMIN')) {
      router.push('/market-os/ambassadors/refferq/login');
      return;
    }
    if (user && partnerId) {
      fetchPartnerData();
  ...
  const fetchPartnerData = async () => {
    try {
      const res = await fetch('/api/market-os/refferq/admin/affiliates');
      if (res.ok) {
  ...
  const fetchCustomers = async () => {
    try {
      const res = await fetch('/api/market-os/refferq/admin/referrals');
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchCommissions = async () => {
    try {
      const res = await fetch(`/api/market-os/refferq/admin/transactions?affiliateId=${partnerId}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchPayouts = async () => {
    try {
      const res = await fetch(`/api/market-os/refferq/admin/payouts?affiliateId=${partnerId}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleCreatePayout = async () => {
    ...
    try {
      const res = await fetch('/api/market-os/refferq/admin/payouts', {
        method: 'POST',
  ...
  if (!partner) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <Button className="mt-6" onClick={() => router.push('/market-os/ambassadors/refferq/admin/partners')}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-3">
          <Button variant="ghost" size="sm" className="-ml-2" onClick={() => router.push('/market-os/ambassadors/refferq/admin/partners')}>
                  ...
                  <TableBody>
                    {customers.map((customer) => (
                      <TableRow key={customer.id}>
                        ...
                        <TableCell>
                          <Button variant="ghost" size="sm" onClick={() => router.push(`/market-os/ambassadors/refferq/admin/customers/${customer.id}`)}>
