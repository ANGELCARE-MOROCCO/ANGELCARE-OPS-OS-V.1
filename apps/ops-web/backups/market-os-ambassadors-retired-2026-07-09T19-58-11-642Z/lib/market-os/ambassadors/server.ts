...
async function tryDbList(entity: EntityKey) {
  ...
  const { data, error } = await supabase.from(table).select("*").order("updated_at", { ascending: false }).limit(1000)
...
function localRows(entity: EntityKey) {
  const store = readLocalStore()
  return store.tables[ENTITY_CONFIG[entity].table] || []
}
...
function rowsForReport(snapshot: AmbassadorWorkspaceSnapshot, reportType: string): { headers: string[]; rows: unknown[][] } {
  ...
  if (reportType.includes("mission")) return { headers: ["Mission", "Ambassador", "City", "Status", "Due", "Completed"], rows: snapshot.missions.map((item) => [item.title, item.ambassador_id, item.city, item.status, item.due_date, item.completed_at]) }
