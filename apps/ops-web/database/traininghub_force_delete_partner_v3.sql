-- TrainingHub force delete partner v3
-- Schema-safe version: does NOT assume core_organizations has legal_name/display_name/email/etc.
-- Run this in Supabase SQL editor.

create or replace function public.traininghub_force_delete_partner_v3(
...
declare
  ...
  v_parent_table text;
  v_parent_has_org boolean := false;
  v_parent_has_partner boolean := false;
begin
  select to_jsonb(o)
    into v_org_json
    from public.core_organizations o
   where o.id::text = p_organization_id
   limit 1;

  if v_org_json is null then
  ...
  v_org_name := coalesce(
    v_org_json->>'name',
    v_org_json->>'legal_name',
    v_org_json->>'display_name',
    v_org_json->>'title',
    p_organization_id
  );

  v_haystack := lower(coalesce(v_org_json::text, ''));
  ...
  if not v_is_smoke and not p_allow_non_smoke then
    return jsonb_build_object(
      ...
      'isSmoke', v_is_smoke,
      'message', 'Hard delete refused: this partner is not detected as smoke/test/demo. Use archive/suspend for real partners.'
    );
  end if;

  -- Multi-pass cleanup: children first, then parent rows.
  for v_pass in 1..12 loop

    -- A) Delete rows that reference child records linked to this organization.
    for r in
    ...
    loop
      ...
      if v_parent_table is not null and to_regclass('public.' || v_parent_table) is not null then
        select exists (
          ...
          where table_schema = 'public' and table_name = v_parent_table and column_name = 'organization_id'
        ) into v_parent_has_org;

        select exists (
          select 1 from information_schema.columns
          where table_schema = 'public' and table_name = v_parent_table and column_name = 'partner_id'
        ) into v_parent_has_partner;

        if v_parent_has_org then
          begin
            v_sql := format(
          ...
          exception when others then
            v_errors := v_errors || jsonb_build_object('pass', v_pass, 'table', r.table_name, 'column', r.column_name, 'via', v_parent_table || '.organization_id', 'error', SQLERRM);
          end;
        end if;

        if v_parent_has_partner then
          begin
            v_sql := format(
          ...
          exception when others then
            v_errors := v_errors || jsonb_build_object('pass', v_pass, 'table', r.table_name, 'column', r.column_name, 'via', v_parent_table || '.partner_id', 'error', SQLERRM);
          end;
        end if;
      end if;
    end loop;

    -- B) Delete any row directly carrying this organization id.
    for r in
      select table_schema, table_name, column_name
      from information_schema.columns
      where table_schema = 'public'
    ...
    loop
      ...
      exception when others then
        v_errors := v_errors || jsonb_build_object('pass', v_pass, 'table', r.table_name, 'column', r.column_name, 'via', 'direct_org_column', 'error', SQLERRM);
      end;
    end loop;
  ...
  exception when others then
    return jsonb_build_object(
      ...
      'errors', v_errors,
      'finalError', SQLERRM,
      'message', 'Organization still blocked after dynamic cleanup. See finalError/errors.'
    );
  end;
  ...
  return jsonb_build_object(
    ...
    'deletedLog', v_deleted_log,
    'errors', v_errors,
    'message', 'Smoke/test/demo partner permanently deleted by schema-safe RPC v3.'
  );
end;
$$;

grant execute on function public.traininghub_force_delete_partner_v3(text, boolean) to authenticated;
grant execute on function public.traininghub_force_delete_partner_v3(text, boolean) to service_role;
