...
export function copyVaultPermissions(actor: SocialCommandActor): CopyVaultPermissions {
  ...
  if(!enforced){return {view:true,use:true,create:true,edit:true,editOwn:true,editAll:true,submit:true,import:true,approve:true,reject:true,manageCategories:true,archive:true,governance:true,rbacEnforced:false,actorRole:role,actorId:actor.id}}
  ...
  return {
    ...
    archive:govern||has("copy.archive"), governance:govern||has("copy.manage_governance"), rbacEnforced:true, actorRole:role, actorId:actor.id,
  }
}
...
export async function listCopyVault(input: Record<string,unknown>, actor: SocialCommandActor): Promise<CopyVaultBootstrap> {
  ...
  const db=await socialDb(); const {data,error}=await db.from("social_command_copy_items").select("*").order("updated_at",{ascending:false}).limit(600)
  if(error) throw missingTableMessage(error)
  const all=await hydrateItems(data||[])
  let items=all
  const q=normalizeText(input.q); const status=cleanString(input.status,60); const language=cleanString(input.language,40).toLowerCase(); const businessUnit=cleanString(input.businessUnit,120).toLowerCase(); const type=cleanString(input.copyType,100); const categoryId=cleanString(input.categoryId,120); const approvedOnly=Boolean(input.approvedOnly)
  ...
  if(status) items=items.filter(item=>status==="approved"?item.approved_version?.status==="approved":item.current_version?.status===status)
  ...
  const categories=await readCategories()
  const stats={items:all.filter(i=>i.lifecycle_status==="active").length,approved:all.filter(i=>i.approved_version?.status==="approved"&&i.lifecycle_status==="active").length,inReview:all.filter(i=>i.current_version?.status==="in_review").length,drafts:all.filter(i=>i.current_version?.status==="draft").length,rejected:all.filter(i=>i.current_version?.status==="rejected").length,archived:all.filter(i=>i.lifecycle_status==="archived").length,categories:categories.filter(c=>c.status==="active").length,usageEvents:all.reduce((n,i)=>n+Number(i.usage_count||0),0)}
...
async function replaceCategoryLinks(itemId:string,categoryIds:string[]){const db=await socialDb();await db.from("social_command_copy_category_links").delete().eq("item_id",itemId);if(categoryIds.length){const {error}=await db.from("social_command_copy_category_links").insert(categoryIds.map(category_id=>({item_id:itemId,category_id})));if(error)throw error}}
...
export async function createCopyItem(input:Record<string,unknown>,actor:SocialCommandActor){
  ...
  const requestedStatus=cleanString(input.status,60)||"draft"; const status=requestedStatus==="approved"&&copyVaultPermissions(actor).approve?"approved":requestedStatus==="in_review"?"in_review":"draft"
  ...
  if(status==="in_review"||status==="approved")await db.from("social_command_copy_approval_events").insert({id:crypto.randomUUID(),item_id:id,version_no:1,action:status==="approved"?"approved":"submitted",stage:version.approval_policy,note:"",actor_user_id:actor.id,actor_role:normalizedRole(actor),created_at:now})
...
export async function createCopyRevision(itemId:string,input:Record<string,unknown>,actor:SocialCommandActor){
  const current=await getCopyItem(itemId,actor); requireItemEdit(actor,current); if(current.lifecycle_status==="archived")throw new Error("Archived copy cannot be revised")
...
export async function updateCopyCategory(categoryId:string,input:Record<string,unknown>,actor:SocialCommandActor){requirePermission(actor,"manageCategories");const db=await socialDb();const patch:any={updated_at:nowIso()};if(input.name!==undefined){patch.name=cleanString(input.name,160);patch.slug=slugify(patch.name)}if(input.description!==undefined)patch.description=cleanString(input.description,1000);if(input.parentId!==undefined||input.parent_id!==undefined)patch.parent_id=cleanString(input.parentId??input.parent_id,120)||null;if(input.status!==undefined)patch.status=cleanString(input.status,30)==="archived"?"archived":"active";if(input.sortOrder!==undefined||input.sort_order!==undefined)patch.sort_order=Number((input.sortOrder ?? input.sort_order) || 0);const {data,error}=await db.from("social_command_copy_categories").update(patch).eq("id",categoryId).select("*").single();if(error)throw error;await auditSocial(actor.id,"copy_vault.category_updated","copy_vault_category",categoryId,{patch:Object.keys(patch)});return data as CopyVaultCategory}
...
export async function commitCopyCsv(csv:string,mapping:CopyVaultImportMapping|undefined,filename:string,actor:SocialCommandActor,options?:{duplicatePolicy?:"skip"|"import"}){
  ...
  const {error:jobError}=await db.from("social_command_copy_import_jobs").insert({id:jobId,filename:cleanString(filename,260)||"copy-vault.csv",status:"processing",row_count:preview.total,valid_count:preview.valid,error_count:preview.invalid,duplicate_count:preview.duplicates,skipped_duplicate_count:duplicateSkipped,imported_count:0,mapping:preview.mapping,created_by:actor.id,created_at:now,completed_at:null})
  ...
  for(const row of preview.rows){
    ...
    if(row.valid&&row.duplicate&&duplicatePolicy==="skip"){
      ...
      errors.push("Exact duplicate skipped by import policy")
    }else if(row.valid){
      try{
        const normalized=row.normalized as Record<string,unknown>
        const categoryIds=await ensureCategoryPath([cleanString(normalized.category,160),cleanString(normalized.subcategory,160),cleanString(normalized.collectionName,160)],actor)
        const requestedStatus=cleanString(normalized.status,60)
        const permittedStatus=requestedStatus==="approved"&&copyVaultPermissions(actor).approve?"approved":requestedStatus==="in_review"?"in_review":"draft"
        const result=await createCopyItem({...normalized,categoryIds,status:permittedStatus,metadata:{import_job_id:jobId,import_row_no:row.rowNo,notes:cleanString(normalized.notes,2000)}},actor)
        itemId=result.item.id;versionNo=result.item.current_version_no;imported++;rowStatus="imported"
      }catch(error){failed++;errors.push(error instanceof Error?error.message:String(error));rowStatus="failed"}
    }else failed++
    const {error:rowError}=await db.from("social_command_copy_import_rows").insert({id:crypto.randomUUID(),job_id:jobId,row_no:row.rowNo,status:rowStatus,error_messages:errors,raw:row.normalized,item_id:itemId,version_no:versionNo,created_at:nowIso()})
    if(rowError)throw rowError
  }
  const {error:finishError}=await db.from("social_command_copy_import_jobs").update({status:failed?"completed_with_errors":"completed",imported_count:imported,error_count:failed,skipped_duplicate_count:skippedDuplicates,completed_at:nowIso()}).eq("id",jobId)
  if(finishError)throw finishError
  await auditSocial(actor.id,"copy_vault.csv_imported","copy_vault_import",jobId,{rows:preview.total,imported,failed,duplicates:preview.duplicates,skippedDuplicates,duplicatePolicy})
  return {jobId,imported,failed,skippedDuplicates,duplicatePolicy,preview}
}
...
export async function bulkCopyAction(input:Record<string,unknown>,actor:SocialCommandActor){const ids=stringArray(input.itemIds??input.item_ids).slice(0,200);const action=cleanString(input.action,40);if(!ids.length)throw new Error("No Copy Vault items selected");const results:any[]=[];for(const id of ids){try{const item=await getCopyItem(id,actor);const version=item.current_version_no;if(action==="submit")results.push({id,ok:true,data:await submitCopyVersion(id,version,actor)});else if(action==="approve")results.push({id,ok:true,data:await approveCopyVersion(id,version,cleanString(input.note,1000),actor)});else if(action==="archive")results.push({id,ok:true,data:await archiveCopyItem(id,actor)});else throw new Error("Unsupported bulk action")}catch(error){results.push({id,ok:false,error:error instanceof Error?error.message:String(error)})}}return {results,ok:results.filter(r=>r.ok).length,failed:results.filter(r=>!r.ok).length}}
