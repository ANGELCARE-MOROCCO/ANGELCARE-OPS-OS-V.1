...
function normalizeOpportunity(raw: unknown, sourceByUrl: Map<string, ExternalResearchSource>): ExternalResearchOpportunity | null {
  ...
  return {
    ...
    strategicValueScore: Math.max(0, Math.min(100, number(row.strategicValueScore, number(row.relevanceScore, 50)))),
...
export async function executeOpenRouterReport(input: {
  ...
  const sanitizedContext = sanitizeForExternalAi(input.context) as JsonRecord;
  const prompt = [
...
// AC_CAPITAL_OS_ULTRA_DEPARTMENT_FINAL_09
// External-provider privacy boundary: provider prompts receive only sanitized,
// size-bounded context. Raw files, credentials and directly identifying records
// are never forwarded by the institutional runtime.
const externalAiBlockedKey = /(?:password|secret|credential|api[_-]?key|access[_-]?token|refresh[_-]?token|encrypted|file[_-]?reference|storage[_-]?(?:path|key)|raw[_-]?(?:file|document|content|bytes)|binary|attachment[_-]?(?:data|blob)|bank[_-]?account|account[_-]?number|iban|passport|(?:^|[_-])cin(?:$|[_-])|national[_-]?id|personal[_-]?data|customer[_-]?data|client[_-]?data|employee[_-]?data|child[_-]?data|parent[_-]?data)/i;

function sanitizeForExternalAi(value: unknown, depth = 0): unknown {
  if (depth > 8) return "[CONTEXT_DEPTH_LIMIT]";
  if (value == null || typeof value === "number" || typeof value === "boolean") return value;
  if (typeof value === "string") return value.length > 12000 ? `${value.slice(0, 12000)}…[TRUNCATED]` : value;
  if (Array.isArray(value)) return value.slice(0, 100).map((item) => sanitizeForExternalAi(item, depth + 1));
  if (typeof value !== "object") return String(value);

  const output: JsonRecord = {};
  for (const [key, item] of Object.entries(value as JsonRecord).slice(0, 180)) {
    if (externalAiBlockedKey.test(key)) {
      output[key] = "[REDACTED_BY_AC_CAPITAL_POLICY]";
      continue;
    }
    output[key] = sanitizeForExternalAi(item, depth + 1);
  }
  return output;
}

// Generic, doctrine-bound structured capability executor used by every institutional agent.
export async function executeOpenRouterCapability(input: {
  agentKey: string;
  capability: string;
  systemPrompt: string;
  prompt: string;
  ...
  if (agentDay >= number(agentQuota.maxRunsPerDay, 20)) throw new Error(`AC_CAPITAL_${input.agentKey.toUpperCase().replaceAll("-", "_")}_DAILY_RUN_LIMIT`);
  if (agentWeek >= number(agentQuota.maxRunsPerWeek, 100)) throw new Error(`AC_CAPITAL_${input.agentKey.toUpperCase().replaceAll("-", "_")}_WEEKLY_RUN_LIMIT`);
  if (agentMonth >= number(agentQuota.maxRunsPerMonth, 400)) throw new Error(`AC_CAPITAL_${input.agentKey.toUpperCase().replaceAll("-", "_")}_MONTHLY_RUN_LIMIT`);
  if (providerDay + 1 > number(providerLimits.maxRequestsPerDay, 40)) throw new Error("AC_CAPITAL_OPENROUTER_DAILY_INTERNAL_LIMIT");
  if (providerMonth + 1 > number(providerLimits.maxRequestsPerMonth, 1000)) throw new Error("AC_CAPITAL_OPENROUTER_MONTHLY_INTERNAL_LIMIT");
  ...
  const runResult = await supabase.from("ac_capital_ai_agent_runs").insert({
    ...
    status: "running",
    phase: "analysis",
    research_query: `${input.capability}: ${input.prompt.slice(0, 1200)}`,
    search_provider_key: null,
    analysis_provider_key: "openrouter",
    configuration_snapshot: {
      ...
      provider: { provider_key: provider.provider_key, model_code: model },
      contextKeys: Object.keys(sanitizedContext),
      privacyBoundary: "sanitized-external-provider-context",
    },
    actor_id: input.actorId || null,
    started_at: now(),
  }).select("*").single();
  ...
  try {
    const result = await fetchJson(provider.endpoint, {
      ...
      body: JSON.stringify({
        model,
        messages: [
          { role: "system", content: `${input.systemPrompt}\nUse only supplied evidence. Never invent facts. Distinguish facts, inference and missing evidence. All external actions remain human-controlled.` },
        ...
        response_format: {
          type: "json_schema",
          json_schema: { name: `ac_capital_${input.capability}`.slice(0, 60), strict: true, schema: input.schema },
        },
      }),
    }, number(config.timeoutMs, 90000));
    const choice = object((Array.isArray(result.payload.choices) ? result.payload.choices[0] : null));
    ...
    await ledger(supabase, {
      ...
      provider_key: "openrouter",
      agent_key: agent.agent_key,
      command_code: `AC_CAPITAL_${input.capability.toUpperCase()}`.slice(0, 120),
      request_count: 1,
      credits_consumed: 0,
      input_tokens: inputTokens,
      output_tokens: outputTokens,
      ...
      provider_request_id: responseId,
      selected_model: selectedModel,
      metadata: { capability: input.capability, workflowId: input.workflowId, eventId: input.eventId },
    });
    await updateRun(supabase, runId, {
      status: "completed",
      phase: "completed",
      ...
      input_tokens: inputTokens,
      output_tokens: outputTokens,
      internal_actions: { capability: input.capability, externalActions: { allowed: false, executed: 0, status: "permanently-locked" } },
      result_payload: parsed,
      finished_at: now(),
    });
    await supabase.from("ac_capital_ai_agents").update({ last_run_at: now(), last_success_at: now(), consecutive_failures: 0, updated_at: now() }).eq("id", agent.id);
    ...
    await updateRun(supabase, runId, { status: "failed", phase: "failed", error_code: message.slice(0, 180), error_message: message.slice(0, 3000), finished_at: now() }).catch(() => null);
    await ledger(supabase, {
      run_id: runId, provider_key: "openrouter", agent_key: agent.agent_key,
      command_code: `AC_CAPITAL_${input.capability.toUpperCase()}`.slice(0, 120), request_count: 1,
      credits_consumed: 0, input_tokens: 0, output_tokens: 0, http_status: status,
      outcome: "failed", error_code: message.slice(0, 180), metadata: { capability: input.capability, message: message.slice(0, 1000) },
    });
    await supabase.from("ac_capital_ai_runtime_incidents").insert({
      incident_key: `capability-${input.capability}-${runId}`,
      provider_key: "openrouter",
      agent_key: agent.agent_key,
      severity: status === 401 || status === 403 ? "critical" : "warning",
      status: "open",
      title: `${input.capability} execution failed`,
      description: message.slice(0, 2000),
      evidence: { runId, status, workflowId: input.workflowId, eventId: input.eventId },
    });
    throw error;
  }
}
