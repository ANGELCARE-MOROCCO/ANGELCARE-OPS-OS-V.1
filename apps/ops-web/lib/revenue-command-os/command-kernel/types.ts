...
export type RevenueCommandRunMode = 'live'|'shadow'|'simulation'|'recommend'|'approval-gated'
...
export type RevenueCommandPlanStep = { order:number; commandCode:string; version:string; mode:RevenueCommandRunMode; status:'ready'|'running'|'completed'|'blocked'|'awaiting-approval'|'simulation-only'; dependsOn:string[]; intendedTools:string[]; expectedOutput:string[]; reasons:string[] }
...
export type RevenueCommandKernelBootstrap = { contractVersion:string; releaseCode:string; moduleVersion:string; executionPosture:'live'; externalActionsEnabled:boolean; generatedAt:string; storageMode:'supabase-overlay'|'canonical-only'|'degraded'; dataMode:'live'|'degraded'|'canonical-fallback'; expectedCount:number; persistedCount:number; missingCount:number; driftCount:number; families:RevenueCommandFamily[]; commands:RevenueCommandDefinition[]; versions:RevenueCommandVersion[]; triggers:RevenueCommandTrigger[]; schedules:RevenueCommandSchedule[]; graphs:RevenueCommandGraph[]; runs:RevenueCommandRun[]; issues:RevenueCommandKernelIssue[]; readiness:RevenueCommandKernelReadiness; counters:Record<string,number> }
