...
export type RevenueCommandKernelBootstrap = { contractVersion:string; releaseCode:string; moduleVersion:string; executionPosture:'shadow'; externalActionsEnabled:false; generatedAt:string; storageMode:'supabase'|'kernel-fallback'; families:RevenueCommandFamily[]; commands:RevenueCommandDefinition[]; versions:RevenueCommandVersion[]; triggers:RevenueCommandTrigger[]; schedules:RevenueCommandSchedule[]; graphs:RevenueCommandGraph[]; runs:RevenueCommandRun[]; issues:RevenueCommandKernelIssue[]; readiness:RevenueCommandKernelReadiness; counters:Record<string,number> }
