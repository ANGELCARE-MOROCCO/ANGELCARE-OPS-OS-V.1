...
export type MarketingAiPermission = 'view' | 'manage' | 'run' | 'import' | 'schedule' | 'govern'
...
export async function requireMarketingAiUser(permission: MarketingAiPermission) {
  ...
  const allowed = privilegedRoles.has(role) || explicit || (permission === 'view' && marketingRoles.has(role)) || (['run', 'schedule'].includes(permission) && role === 'marketing_director')
...
export function apiErrorResponse(error: unknown) {
  ...
  const status = message === 'UNAUTHENTICATED' ? 401
    ...
    : message.includes('MISSING') || message.includes('DISABLED') ? 503
