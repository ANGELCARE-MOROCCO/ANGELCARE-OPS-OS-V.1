...
import { getGlobalOperatingDefaults } from './production-operations-service'
...
const OPPORTUNITY_INACTIVE_STATUSES=new Set(['deferred','rejected','expired','converted','archived','cancelled','closed','superseded'])
...
function publicOpportunity(signal:MarketSignal){const profile=profileFrom(signal)||derivedProfile(signal);const inactive=OPPORTUNITY_INACTIVE_STATUSES.has(clean(signal.status).toLowerCase())||['deferred','rejected','expired','converted'].includes(profile.decision.status);return {id:signal.id,code:signal.code,title:signal.title,summary:signal.summary,status:signal.status,operationalState:inactive?'history':'active',confidence:Number(signal.confidence||0),urgency:Number(signal.urgency||0),opportunityScore:Number(signal.opportunity_score||0),sourceLabel:signal.source_label,sourceUrl:signal.source_url,services:signal.services||[],audiences:signal.audiences||[],cities:signal.cities||[],detectedAt:signal.detected_at,updatedAt:signal.updated_at,profile}}
...
export async function getOpportunityIntelligenceSnapshot(){
  ...
  const active=opportunities.filter(item=>item.operationalState==='active')
  const history=opportunities.filter(item=>item.operationalState==='history')
...
export async function runOpportunityIntelligence(input:{actorId:string;actorName:string;reason?:string;continuationMode?:RuntimeContinuationMode;scanWeb?:boolean}){
  ...
  const operatingDefaults=await getGlobalOperatingDefaults()
  const payload={today:new Date().toISOString().slice(0,10),timezone:operatingDefaults.timezone,planningHorizons:[7,30,60,90,180],existingSignals:current.signals.slice(0,40).map(item=>({title:item.title,summary:item.summary,status:item.status,services:item.services,audiences:item.audiences,cities:item.cities})),existingContent:current.dossiers.slice(0,60).map(item=>({title:item.title,service:item.service_label,audience:item.audience,channel:item.channel,status:item.status})),requiredSignalClasses:['seasonal','periodic','event-driven','market','consumer','search','channel','format','competitor','service-demand','reputation','internal-performance-gap']}
...
export async function detachOpportunityFromStrategy(input:{actorId:string;actorName:string;signalId:string;strategyId:string;reason:string}){
 const signal=await requireSignal(input.signalId)
 if(clean(input.reason).length<8)throw new ContentCommandActionError({code:'REASON_REQUIRED',message:'Expliquez pourquoi cette opportunité doit être détachée de la stratégie.',status:400,kind:'validation',recovery:[{key:'complete_reason',label:'Compléter le motif'}]})
 const supabase=await createServiceClient() as any
 const result=await supabase.from('market_content_strategies').select('*').eq('id',input.strategyId).maybeSingle()
 if(result.error)throw result.error
 if(!result.data)throw new ContentCommandActionError({code:'STRATEGY_NOT_FOUND',message:'La stratégie liée n’existe plus.',status:404,kind:'not_found'})
 const strategy=result.data as JsonRecord
 const current=arr(strategy.signal_ids)
 if(!current.includes(signal.id))return {detached:false,strategyId:input.strategyId,reason:'ALREADY_DETACHED'}
 const status=clean(strategy.status).toLowerCase()
 const protectedStatuses=new Set(['approved','active','completed','validated','published','closed'])
 if(protectedStatuses.has(status))throw new ContentCommandActionError({code:'RECORD_PROTECTED',message:`La stratégie ${clean(strategy.code||strategy.title||strategy.id)} est ${status}. Elle doit être amendée, supersédée ou rouverte avant de détacher le signal.`,status:409,kind:'protected_record',details:{strategy:{id:strategy.id,code:strategy.code,title:strategy.title,status}},recovery:[{key:'open_strategy',label:'Ouvrir la stratégie',href:`/market-os/content-command-center/strategies?strategyId=${encodeURIComponent(input.strategyId)}`},{key:'governance',label:'Ouvrir Record Governance',href:`/market-os/content-command-center/record-governance?entityType=strategy&entityId=${encodeURIComponent(input.strategyId)}`}]})
 const next=current.filter(id=>id!==signal.id)
 const update=await supabase.from('market_content_strategies').update({signal_ids:next,updated_at:now()}).eq('id',input.strategyId).select('*').single()
 if(update.error)throw update.error
 await auditContentHeadquarters({actorId:input.actorId,actorName:input.actorName,action:'content_opportunity.strategy_detached',entityType:'signal',entityId:signal.id,detail:{strategyId:input.strategyId,reason:input.reason,previousSignalIds:current,nextSignalIds:next}})
 return {detached:true,strategyId:input.strategyId,strategy:update.data}
}
