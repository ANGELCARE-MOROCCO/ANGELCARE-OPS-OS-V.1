-- TrainingHub Partner Portal Blueprint Layer
-- Optional migration for partner requests, documents, notifications and activity.
-- Safe to run in Supabase SQL editor before real production use.
...
create table if not exists public.partner_request_comments (
  id uuid primary key default gen_random_uuid(),
...
alter table public.partner_request_comments enable row level security;
alter table public.partner_documents enable row level security;
alter table public.partner_notifications enable row level security;
alter table public.partner_activity_events enable row level security;

-- Policies intentionally conservative: service role/admin can manage; partner user access should be
-- governed by existing TrainingHub scoped clients / app-level session logic where applicable.
-- If your project has helper functions for current app user organization, replace these with stricter policies.
