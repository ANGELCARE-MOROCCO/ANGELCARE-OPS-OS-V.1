-- ATTENDANCE REAL DATA SYNC BRIDGE — SQL FIX
-- Fixes: column s.profile_id does not exist
-- Safe version: does not reference hr_staff_profiles.profile_id.

-- 1) Diagnostic view for HR attendance identity mapping.
...
select
  ...
  case
    ...
    when a.profile_id is not null then 'profile_id_present_but_staff_profiles_has_no_profile_id_column'
    else 'unmapped'
...
-- 2) Diagnostic view for raw overhead punch logs.
...
-- 3) Safe backfill attendance.staff_id from attendance.user_id.
...
-- 4) Safe backfill logs.staff_id from logs.user_id.
