...
export default function Angelcare360LibraryToolbar({ createHref, auditHref, lockedReason }: Props) {
  return (
    <section style={cardStyle}>
      <div style={rowStyle}>
        {createHref ? <Link href={createHref} style={primaryLinkStyle}>Créer une opération</Link> : null}
        {auditHref ? <Link href={auditHref} style={secondaryLinkStyle}>Audit bibliothèque</Link> : null}
        <span style={lockedStyle}>{lockedReason || 'Les relances et exports restent verrouillés.'}</span>
      </div>
    </section>
  )
}
