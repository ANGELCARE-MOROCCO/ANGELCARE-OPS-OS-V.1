...
export default function Angelcare360DocumentGovernanceWorkspace({ readiness }: Props) {
  return (
    <section style={cardStyle}>
      <h2 style={titleStyle}>Gouvernance documentaire</h2>
      <p style={textStyle}>{readiness.reason}</p>
      <ul style={listStyle}>
        <li>Templates: {readiness.templateReady ? 'prêts' : 'verrouillés'}</li>
        <li>Stockage: {readiness.storageReady ? 'prêt' : 'verrouillé'}</li>
        <li>Rétention: {readiness.retentionReady ? 'prête' : 'verrouillée'}</li>
        <li>Audit: {readiness.auditReady ? 'ok' : 'manquant'}</li>
        <li>Export: {readiness.exportReady ? 'prêt' : 'verrouillé'}</li>
