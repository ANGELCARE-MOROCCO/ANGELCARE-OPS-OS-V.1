      ...
      <section className="mx-auto max-w-7xl space-y-6">
            ...
            {executiveKpis.map((kpi) => (
              <div key={kpi.id} className="rounded-3xl bg-white/10 p-4">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-xs uppercase text-slate-600">{kpi.label}</p>
                ...
                <p className="mt-1 text-xs text-slate-600">{kpi.trend}</p>
                <p className="mt-3 text-xs leading-5 text-slate-600">{kpi.interpretation}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="rounded-3xl border border-slate-200 bg-white p-5">
            <p className="text-xs font-bold uppercase text-slate-9500">Decision Impact</p>
            <p className="mt-2 text-2xl font-black">{formatMad(totalImpact)}</p>
          </div>
          <div className="rounded-3xl border border-slate-200 bg-white p-5">
            <p className="text-xs font-bold uppercase text-slate-9500">Pending Decisions</p>
            <p className="mt-2 text-2xl font-black">{pendingDecisions}</p>
          </div>
          <div className="rounded-3xl border border-slate-200 bg-white p-5">
            <p className="text-xs font-bold uppercase text-slate-9500">High Risk Items</p>
            ...
            <article key={decision.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div className="rounded-2xl bg-slate-50 p-4 text-right">
                  <p className="text-xs font-bold uppercase text-slate-9500">Financial Impact</p>
              ...
              <div className="mt-5 grid gap-4 lg:grid-cols-3">
                <div className="rounded-2xl border border-slate-200 p-4">
                  <p className="text-xs font-bold uppercase text-slate-9500">Diagnosis</p>
                  <p className="mt-2 text-sm text-slate-700">{decision.diagnosis}</p>
                </div>
                <div className="rounded-2xl border border-slate-200 p-4">
                  <p className="text-xs font-bold uppercase text-slate-9500">Recommended Decision</p>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                <MarketActionButton moduleKey="investor" engine="data" actionKey="approve_decision" actionLabel="Approve Decision" className="rounded-2xl bg-white px-4 py-2 text-sm font-bold text-slate-950">Approve Decision</MarketActionButton>
