...
const views: Array<{ key: MarketingAiView; label: string; description: string; href: string; icon: React.ReactNode }> = [
  ...
  { key: 'research-control', label: 'Contrôle Recherche IA', description: 'Tavily, OpenRouter, agents et quotas', href: `${AI_BASE}/research-control`, icon: <FileSearch /> },
  ...
  { key: 'settings', label: 'Configuration', description: 'AI provider, sécurité et limites', href: `${AI_BASE}/settings`, icon: <Settings2 /> },
  ...
  return (
    <section className={styles.hero}>
      ...
      <div className={styles.heroStatus}>
        <div><span>AI provider</span><strong>{loading ? 'Vérification…' : ready ? 'Configuré' : 'Configuration requise'}</strong></div>
        <div><span>Modèle</span><strong>{snapshot?.provider.model || 'AI provider via .env.local'}</strong></div>
...
function AiNav({ active: _active }: { active: MarketingAiView }) {
  return null
}
...
function DashboardView() {
  ...
  async function health() {
    setHealthMessage('Test AI provider en cours…')
        ...
        <section className={styles.executiveGrid}>
          <article className={styles.primaryPanel}>
            ...
            <div className={styles.commandLanes}>
              ...
              <Link href={`${AI_BASE}/learning`}><Sparkles /><div><strong>Développement mensuel</strong><p>Mettre à jour AI provider et les ressources marketing avec recherche sourcée et validation humaine.</p></div><ChevronRight /></Link>
...
function CommandsView() {
  ...
  async function execute() {
    ...
    setNotice('AI provider exécute la commande…')
...
function LearningView() {
  ...
  async function updateResources() {
    setNotice('Recherche AI provider et ressources marketing en cours…')
...
TAVILY_API_KEY=YOUR_SECRET
MARKETING_AI_PRIMARY_MODEL=openrouter/auto
MARKETING_AI_FALLBACK_MODEL=
...
export default function MarketingAiDirectorWorkspace({ view }: { view: MarketingAiView }) {
  ...
  if (view === 'commands') workspace = <CommandsView />
  else if (view === 'skills') workspace = <SkillsView />
  else if (view === 'schedules') workspace = <SchedulesView />
  else if (view === 'missions') workspace = <MissionsView />
  else if (view === 'runs') workspace = <RunsView />
  else if (view === 'learning') workspace = <LearningView />
  else if (view === 'doctrine') workspace = <DoctrineView />
  else if (view === 'settings') workspace = <SettingsView />
  else workspace = <DashboardView />
