...
export function dossierRequirements(dossier: DossierViewModel): DossierRequirement[] {
  ...
  const briefValues = [dossier.brief.objective, dossier.brief.audience, dossier.brief.userProblem, dossier.brief.coreMessage, dossier.brief.format, dossier.brief.tone, dossier.brief.version]
  const hasBrief = briefValues.every((value) => Boolean(value) && !/non défini|non définie|non documenté|non documentée|à constituer|à sélectionner/i.test(value)) && dossier.brief.channels.length > 0
  ...
  if (!hasReviewer) requirements.push({ id: "reviewer", label: "Autorité de révision manquante", detail: "Le prochain gate de revue ou validation ne peut pas être attribué.", owner: dossier.owner, blocking: ["ai_review", "human_review", "validated"].includes(dossier.currentStage), resolved: false, actionLabel: "Affecter la gouvernance", href: "/market-os/content-command-center/briefs" })
  if (!hasBrief) requirements.push({ id: "brief", label: "Brief lié incomplet", detail: "Objectif, audience, problème, message, format, canal, ton et version initiale doivent être persistés dans ce dossier.", owner: dossier.owner, blocking: ["brief", "scope_locked", "planned"].includes(dossier.currentStage), resolved: false, actionLabel: "Ouvrir le Brief Recovery Desk", href: "/market-os/content-command-center/briefs" })
