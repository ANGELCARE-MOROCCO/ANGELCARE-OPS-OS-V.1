...
function ProductionAssetsStage({ dossier }: { dossier: DossierViewModel }) {
  ...
  return <div className={styles.productionStudio}>
    ...
    <div className={styles.productionHero}>
      <div className={styles.productionPreview}>{dossier.assets[0] ? <ContentMediaPreview source={{ id: dossier.assets[0].id, title: dossier.assets[0].title, url: dossier.assets[0].url, filename: dossier.assets[0].title, contentType: dossier.assets[0].type, sourceLabel: "Dossier · Production" }} mode="inspector" fit="contain"/> : <><ImageIcon/><span>Aperçu de version non disponible</span></>}</div>
    ...
    {dossier.assets.length ? <div className={styles.assetShelf}>{dossier.assets.slice(0, 8).map((asset, index) => <article key={asset.id}><div><ContentMediaPreview source={{ id: asset.id, title: asset.title, url: asset.url, filename: asset.title, contentType: asset.type }} mode="compact" interactive={false} showLabel={false}/><span>V{index + 1}</span></div><strong>{asset.title}</strong><small>{asset.type} · {humanStatus(asset.status)}</small></article>)}</div> : <EmptyStage icon={ImageIcon} title="Aucun asset lié" detail="Le dossier ne contient aucun asset ou exemple généré dans le snapshot actuel."/>}
  </div>
}
