    ...
    <section className={styles.evidenceTheatre}>
      ...
      <article className={styles.primaryEvidenceStage}>
        ...
        {item && view === "proof" ? <div className={styles.evidencePreview}>
          {item.latestEvidence ? <ContentMediaPreview source={{ id: item.latestEvidence.id, title: item.latestEvidence.title || item.dossier.title, url: item.latestEvidence.preview_url, bridgeFileId: item.latestEvidence.bridge_file_id, storageKey: item.latestEvidence.storage_key, contentType: item.latestEvidence.content_type, filename: item.latestEvidence.filename, sizeBytes: item.latestEvidence.size_bytes, sourceLabel: "Evidence Lab" }} mode="inspector" fit="contain"/> : <div className={styles.previewFallback}><FileImage/><strong>Aucune preuve visualisable</strong><p>Joignez la version ou l’export contrôlé. Aucune image de démonstration ne remplace la preuve réelle.</p></div>}
