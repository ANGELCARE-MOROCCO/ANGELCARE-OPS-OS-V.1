...
function statusKey(lead: AnyRow) {
  ...
  if (["hot", "chaud"].includes(raw)) return "hot"
...
export default function AmbassadorLeadsRoute({ snapshot, loading, refreshing, error, success, onRefresh }: Props) {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leads.filter((lead) => {
      ...
      return new Date(text(b.created_at, b.updated_at, 0)).getTime() - new Date(text(a.created_at, a.updated_at, 0)).getTime()
    })
  ...
  const metrics = useMemo(() => {
    ...
    const hot = leads.filter((item) => statusKey(item) === "hot").length
  ...
  const todayQueue = useMemo(() => leads
    ...
    .sort((a, b) => new Date(text(a.next_followup_at, "2999-12-31")).getTime() - new Date(text(b.next_followup_at, "2999-12-31")).getTime())
    .slice(0, 8), [leads])

  const commandMetrics = useMemo(() => {
    const actionable = leads.filter((item) => !["converted", "disqualified", "archived"].includes(statusKey(item)))
    const startToday = new Date(); startToday.setHours(0, 0, 0, 0)
    const endToday = new Date(startToday); endToday.setDate(endToday.getDate() + 1)
    const dueToday = actionable.filter((item) => { const raw = text(item.next_followup_at); if (!raw) return false; const due = new Date(raw); return !Number.isNaN(due.getTime()) && due >= startToday && due < endToday }).length
    const overdue = actionable.filter((item) => { const raw = text(item.next_followup_at); if (!raw) return false; const due = new Date(raw); return !Number.isNaN(due.getTime()) && due < startToday }).length
    const uncontacted = actionable.filter((item) => statusKey(item) === "new").length
    const qualified = leads.filter((item) => ["qualified", "hot", "negotiating", "ready_to_convert"].includes(statusKey(item))).length
    const ready = leads.filter((item) => statusKey(item) === "ready_to_convert").length
    const converted = leads.filter((item) => statusKey(item) === "converted").length
    return { dueToday, overdue, uncontacted, qualified, ready, converted }
  }, [leads])
  ...
  return (
    <div data-ambassador-leads-route="enterprise-real-sync" className="min-h-screen bg-[#f6f8fc] text-slate-950">
      <div className="mx-auto w-full max-w-none px-5 pb-12 pt-5 2xl:px-7">
        <header className="rounded-[28px] border border-slate-200 bg-white px-6 py-5 shadow-sm shadow-slate-200/60">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950">Leads & conversion</h1>
              <p className="mt-1 text-[10px] font-black uppercase tracking-[0.17em] text-blue-700">Ambassador Commercial Conversion Operations</p>
              <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-600">Centralisez chaque prospect, contrôlez l’attribution ambassadeur, sécurisez la source, orchestrez les relances et préparez les conversions sans rupture de données.</p>
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <SignalPill tone="blue">Handoff conversion réel</SignalPill>
              </div>
            </div>
        ...
        <section className="mt-4 grid gap-4 xl:grid-cols-[1.05fr_1.45fr_0.8fr]">
          <div className="relative overflow-hidden rounded-[28px] border border-slate-800 bg-slate-950 p-5 text-white shadow-[0_22px_55px_rgba(15,23,42,0.18)]">
            <div className="absolute -right-12 -top-12 h-48 w-48 rounded-full bg-blue-600/20 blur-2xl" />
            <div className="relative"><div className="flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-[0.17em] text-blue-200">Commande de relance</span><Phone size={18} className="text-blue-300" /></div><div className="mt-5 grid grid-cols-3 gap-3"><RevenueCommandDatum label="À traiter aujourd’hui" value={commandMetrics.dueToday} /><RevenueCommandDatum label="En retard" value={commandMetrics.overdue} danger /><RevenueCommandDatum label="Non contactés" value={commandMetrics.uncontacted} warning /></div><button type="button" onClick={() => { resetFilters(); setOnlyDue(true) }} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-3.5 py-2 text-xs font-black text-slate-950">Ouvrir la file de relance <ArrowRight size={14} /></button></div>
          </div>
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Pipeline commercial réel</p><h2 className="mt-1 text-lg font-black text-slate-950">De la capture à la conversion</h2></div><span className="rounded-full bg-blue-50 px-3 py-1.5 text-xs font-black text-blue-700">{formatNumber(metrics.received)} reçus</span></div>
            <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-4"><PipelineDatum label="À qualifier" value={Math.max(0, metrics.received - commandMetrics.qualified - commandMetrics.converted)} /><PipelineDatum label="Qualifiés" value={commandMetrics.qualified} /><PipelineDatum label="Prêts à convertir" value={commandMetrics.ready} /><PipelineDatum label="Convertis" value={commandMetrics.converted} success /></div>
            <div className="mt-5 flex h-2 overflow-hidden rounded-full bg-slate-100"><span className="bg-blue-600" style={{ width: `${metrics.received ? Math.max(3, Math.round((commandMetrics.qualified / metrics.received) * 100)) : 0}%` }} /><span className="bg-emerald-500" style={{ width: `${metrics.received ? Math.round((commandMetrics.converted / metrics.received) * 100) : 0}%` }} /></div>
          </div>
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><ShieldCheck size={19} className="text-blue-700" /><span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-black text-amber-700">Exceptions</span></div><div className="mt-5 text-4xl font-black tabular-nums text-slate-950">{metrics.withoutFollowup}</div><p className="mt-1 text-sm font-black text-slate-700">Sans prochaine action</p><div className="mt-4 space-y-2 text-xs font-bold text-slate-500"><div className="flex justify-between"><span>Échéance ≤ 48 h</span><strong className="text-amber-700">{metrics.expiring}</strong></div><div className="flex justify-between"><span>Doublons détectés</span><strong className="text-rose-700">{duplicateIds.size}</strong></div><div className="flex justify-between"><span>Coût moyen renseigné</span><strong className="text-slate-800">{formatMoney(metrics.costPerLead)}</strong></div></div></div>
        </section>

        <section className="mt-4 rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.25fr_repeat(4,minmax(130px,0.55fr))_auto]">
                ...
                <table className="w-full min-w-[1180px] text-left text-sm">
                  <thead className="bg-slate-50 text-[10px] font-black uppercase tracking-[0.13em] text-slate-500">
                    <tr>
                      ...
                      <th className="px-3 py-3">Lead</th><th>Contact</th><th>Ville</th><th>Source</th><th>Ambassadeur</th><th>Besoin / qualité</th><th>Statut</th><th>Prochain suivi</th><th>Âge</th><th className="pr-4 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedLeads.map((lead) => {
                      ...
                      const leadMeta = parseMeta(lead.notes)
                      return (
                        <tr key={String(lead.id)} className="border-t border-slate-100 transition hover:bg-blue-50/30">
                          ...
                          <td><span className="block max-w-[180px] truncate text-xs font-black text-slate-700">{text(leadMeta.qualification?.need, lead.lead_type, "À qualifier")}</span>{duplicate ? <span className="mt-1 inline-flex rounded-full bg-rose-50 px-2 py-0.5 text-[10px] font-black text-rose-600">Doublon</span> : null}</td>
            ...
            <SidePanel title="File d’attente — à traiter aujourd’hui" count={todayQueue.length} action="Voir tous" onAction={() => { resetFilters(); setOnlyDue(true) }}>
              {todayQueue.length ? todayQueue.map((lead) => (
                <button type="button" key={String(lead.id)} onClick={() => setSelectedLead(lead)} className="flex w-full items-center justify-between gap-3 rounded-2xl px-3 py-2.5 text-left hover:bg-slate-50">
                  ...
                  <span className={`rounded-full px-2 py-1 text-[10px] font-black ${new Date(text(lead.next_followup_at)).getTime() < Date.now() ? "bg-rose-50 text-rose-700" : "bg-amber-50 text-amber-700"}`}>{new Date(text(lead.next_followup_at)).getTime() < Date.now() ? "En retard" : "Due"}</span>
...
function SignalPill({ tone, children }: { tone: "emerald" | "blue"; children: ReactNode }) {
  const classes = tone === "emerald" ? "border-emerald-100 bg-emerald-50 text-emerald-700" : "border-blue-100 bg-blue-50 text-blue-700"
...
function RevenueCommandDatum({ label, value, danger = false, warning = false }: { label: string; value: number; danger?: boolean; warning?: boolean }) {
  return <div><span className={`block text-3xl font-black tabular-nums ${danger ? "text-rose-300" : warning ? "text-amber-300" : "text-white"}`}>{value}</span><span className="mt-1 block text-[9px] font-black uppercase tracking-[0.1em] text-slate-400">{label}</span></div>
}

function PipelineDatum({ label, value, success = false }: { label: string; value: number; success?: boolean }) {
  return <div className="text-left"><span className={`block text-2xl font-black tabular-nums ${success ? "text-emerald-700" : "text-slate-950"}`}>{value}</span><span className="mt-1 block text-[10px] font-black uppercase tracking-[0.1em] text-slate-500">{label}</span></div>
}
...
function FunnelPanel({ funnel }: { funnel: { received: number; qualified: number; negotiating: number; ready: number } }) {
  ...
  const steps = [
    ...
    { label: "Leads qualifiés", value: funnel.qualified, tone: "bg-blue-600" },
  ...
  return <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h3 className="text-base font-black text-slate-950">Pipeline de qualification et conversion</h3><p className="mt-1 text-xs font-semibold text-slate-500">Funnel calculé uniquement sur les statuts réels.</p></div><Target size={19} className="text-blue-700" /></div><div className="mt-5 space-y-3">{steps.map((step) => <div key={step.label} className="grid grid-cols-[150px_minmax(0,1fr)_55px] items-center gap-3"><span className="text-xs font-black text-slate-700">{step.label}</span><div className="h-8 overflow-hidden rounded-lg bg-slate-100"><div className={`flex h-full items-center justify-end px-3 text-xs font-black text-white ${step.tone}`} style={{ width: `${Math.max(step.value ? 12 : 0, Math.round((step.value / max) * 100))}%` }}>{step.value || ""}</div></div><span className="text-right text-xs font-black text-slate-600">{max ? Math.round((step.value / max) * 100) : 0}%</span></div>)}</div></section>
}
...
function ImportLeadsModal({ draft, setDraft, fileInputRef, ambassadors, territories, saving, error, onClose, onSubmit }: { draft: ImportDraft; setDraft: (value: ImportDraft) => void; fileInputRef: { current: HTMLInputElement | null }; ambassadors: AnyRow[]; territories: AnyRow[]; saving: boolean; error?: string | null; onClose: () => void; onSubmit: () => void }) {
  return <EnterpriseModal title="Importer des leads" subtitle="Import CSV contrôlé, prévention des doublons, fallback d’attribution et création réelle ligne par ligne avec traçabilité de lot." icon={FileSpreadsheet} onClose={onClose} footer={<div className="flex flex-wrap items-center justify-between gap-3"><div className="text-xs font-bold text-slate-500">{draft.rows.length} ligne(s) chargée(s) · {draft.skipDuplicates ? "Doublons ignorés" : "Doublons autorisés"}</div><div className="flex gap-2"><ModalButton onClick={onClose}>Annuler</ModalButton><ModalButton primary icon={saving ? Loader2 : FileSpreadsheet} disabled={saving || !draft.rows.length} onClick={onSubmit}>{saving ? "Import en cours…" : "Importer dans la source réelle"}</ModalButton></div></div>}>
    ...
    <div className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]">
      ...
      <section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h3 className="text-lg font-black text-slate-950">Prévisualisation contrôlée</h3><p className="mt-1 text-xs font-semibold text-slate-500">Les 12 premières lignes sont affichées avant création.</p></div><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-black text-blue-700">{draft.rows.length} ligne(s)</span></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-xs"><thead className="bg-slate-50 text-[9px] font-black uppercase tracking-[0.12em] text-slate-500"><tr><th className="px-3 py-3">Nom</th><th>Téléphone</th><th>Email</th><th>Ville</th><th>Source</th><th>Besoin</th></tr></thead><tbody>{draft.rows.slice(0, 12).map((row, index) => <tr key={index} className="border-t border-slate-100"><td className="px-3 py-3 font-black">{text(row.lead_name, row.contact_name, row.parent_name, row.nom, row.name, "—")}</td><td>{text(row.phone, row.telephone, "—")}</td><td>{text(row.email, "—")}</td><td>{text(row.city, row.ville, draft.cityFallback)}</td><td>{text(row.source, draft.sourceFallback)}</td><td>{text(row.identified_need, row.need, row.besoin, "À qualifier")}</td></tr>)}</tbody></table></div>{!draft.rows.length ? <EmptyState icon={FileSpreadsheet} title="Aucun fichier chargé" detail="Chargez un CSV pour contrôler les lignes avant synchronisation." action="Choisir un fichier" onAction={() => fileInputRef.current?.click()} /> : null}</section>
    </div>
  </EnterpriseModal>
}
...
function AssignLeadsModal({ draft, setDraft, leads, ambassadors, territories, saving, error, onClose, onSubmit }: { draft: AssignDraft; setDraft: (value: AssignDraft) => void; leads: AnyRow[]; ambassadors: AnyRow[]; territories: AnyRow[]; saving: boolean; error?: string | null; onClose: () => void; onSubmit: () => void }) {
  ...
  return <EnterpriseModal title="Affecter des leads" subtitle="Composez une attribution contrôlée: sélection réelle, ambassadeur responsable, renforts, territoire, priorité, relance et trace managériale." icon={UserRoundCheck} onClose={onClose} footer={<div className="flex flex-wrap items-center justify-between gap-3"><div className="text-xs font-bold text-slate-500">Contrôles {[draft.leadIds.length > 0, Boolean(draft.ambassadorId), Boolean(draft.manager), Boolean(draft.nextFollowupAt), Boolean(draft.reason)].filter(Boolean).length}/5 · {draft.leadIds.length} lead(s) · charge ambassadeur {activeMissions} mission(s) active(s)</div><div className="flex gap-2"><ModalButton onClick={onClose}>Fermer</ModalButton><ModalButton primary icon={saving ? Loader2 : UserCheck} disabled={saving || !draft.leadIds.length || !draft.ambassadorId} onClick={onSubmit}>{saving ? "Affectation…" : "Affecter et synchroniser"}</ModalButton></div></div>}>
    <ModalError error={error} />
    <div className="grid gap-5 xl:grid-cols-[0.85fr_1.15fr_0.8fr]">
      ...
      <aside className="space-y-4"><section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-blue-700">Contrôles d’attribution</p><p className="mt-2 text-4xl font-black tabular-nums text-slate-950">{[draft.leadIds.length > 0, Boolean(draft.ambassadorId), Boolean(draft.nextFollowupAt), Boolean(draft.manager), activeMissions <= 3].filter(Boolean).length}/5</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-blue-600" style={{ width: `${readiness}%` }} /></div></section><section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-base font-black text-slate-950">Contrôles opérationnels</h3><div className="mt-3 space-y-2"><CheckRow done={draft.leadIds.length > 0} label="Lead(s) sélectionné(s)" /><CheckRow done={Boolean(draft.ambassadorId)} label="Responsable défini" /><CheckRow done={Boolean(draft.nextFollowupAt)} label="Relance datée" /><CheckRow done={Boolean(draft.manager)} label="Owner identifié" /><CheckRow done={activeMissions <= 3} label={`Charge acceptable (${activeMissions} mission(s))`} /></div></section></aside>
    </div>
  </EnterpriseModal>
}
...
function SourceValidationModal({ draft, setDraft, leads, ambassadors, saving, error, onClose, onSubmit }: { draft: SourceDraft; setDraft: (value: SourceDraft) => void; leads: AnyRow[]; ambassadors: AnyRow[]; saving: boolean; error?: string | null; onClose: () => void; onSubmit: () => void }) {
  ...
  return <EnterpriseModal title="Valider la source et l’attribution" subtitle="Contrôle referral, code promo, campagne, ambassadeur source et preuve avant verrouillage de l’attribution commerciale." icon={ShieldCheck} onClose={onClose} footer={<div className="flex flex-wrap items-center justify-between gap-3"><div className="text-xs font-bold text-slate-500">Contrôles {[draft.leadIds.length > 0, Boolean(draft.source), Boolean(draft.validator), Boolean(draft.ambassadorId || draft.referralCode || draft.promoCode), Boolean(draft.proofReference || draft.note)].filter(Boolean).length}/5 · décision {draft.decision}</div><div className="flex gap-2"><ModalButton onClick={onClose}>Fermer</ModalButton><ModalButton primary icon={saving ? Loader2 : ShieldCheck} disabled={saving || !draft.leadIds.length || !draft.validator} onClick={onSubmit}>{saving ? "Validation…" : "Valider et journaliser"}</ModalButton></div></div>}>
    <ModalError error={error} />
    <div className="grid gap-5 xl:grid-cols-[0.85fr_1.15fr_0.75fr]">
      ...
      <aside className="space-y-4"><section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm"><p className="text-[10px] font-black uppercase tracking-[0.14em] text-emerald-700">Contrôles de validation</p><p className="mt-2 text-4xl font-black tabular-nums text-slate-950">{[draft.leadIds.length > 0, Boolean(draft.source), Boolean(draft.validator), Boolean(draft.ambassadorId || draft.referralCode || draft.promoCode), Boolean(draft.proofReference || draft.note)].filter(Boolean).length}/5</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-emerald-500" style={{ width: `${readiness}%` }} /></div></section><section className="rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-base font-black text-slate-950">Garde-fous attribution</h3><div className="mt-3 space-y-2"><CheckRow done={draft.leadIds.length > 0} label="Périmètre défini" /><CheckRow done={Boolean(draft.source)} label="Source normalisée" /><CheckRow done={Boolean(draft.validator)} label="Validateur nommé" /><CheckRow done={Boolean(draft.ambassadorId || draft.referralCode || draft.promoCode)} label="Attribution traçable" /><CheckRow done={Boolean(draft.proofReference || draft.note)} label="Preuve ou justification" /></div></section></aside>
    </div>
  </EnterpriseModal>
}
