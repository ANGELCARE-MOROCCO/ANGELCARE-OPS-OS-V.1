...
import { CertificationPulse } from "./CertificationPulse";
...
const workspaceProfiles: Record<string, { label: string; institution: string; accent: string; accentSoft: string }> = {
  ...
  artifacts: { label: "Institutional Documents", institution: "Capital Document & Artifact Factory", accent: "#9f1239", accentSoft: "#ffe4e6" },
  ...
  "ai-operations": { label: "AI Operations", institution: "External Intelligence Operations Control", accent: "#4f46e5", accentSoft: "#e0e7ff" },
  ...
  registry: { label: "Institutional Control", institution: "Canonical Record Registry", accent: "#28527a", accentSoft: "#e8f0f8" },
  certification: { label: "Release Authority", institution: "Institutional Certification Center", accent: "#9f1239", accentSoft: "#ffe4e6" },
};

function profileFor(workspaceKey?: string) {
  const safeWorkspaceKey =
    typeof workspaceKey === "string" && workspaceKey.trim()
      ? workspaceKey.trim()
      : "orchestrator";

  return workspaceProfiles[safeWorkspaceKey] || {
    label: "Capital Institution",
    institution: safeWorkspaceKey.replaceAll("-", " "),
    accent: "#1d4ed8",
    accentSoft: "#dbeafe",
  };
}
...
export function AcCapitalShell({ actor, workspaceKey, title, subtitle, envelope, insights, primaryAction, onPrimaryAction, children }: {
  ...
  return <div className={styles.shell} style={shellStyle} data-workspace={workspaceKey}>
    <aside className={`${styles.sidebar} ${navOpen ? styles.sidebarOpen : ""}`}>
      ...
      <div className={styles.roleCard}><ShieldCheck size={18} /><div><strong>{safeActor.role}</strong><span>{safeActor.name}</span></div></div>
    ...
    <div className={styles.main}>
      ...
      <section className={styles.institutionHeader}>
        ...
        <div className={styles.institutionAuthority}><span>Operating authority</span><strong>{safeActor.role}</strong><small>{safeActor.name}</small></div>
      </section>
      ...
      {workspaceKey !== "certification" ? <CertificationPulse workspaceKey={workspaceKey} /> : null}
