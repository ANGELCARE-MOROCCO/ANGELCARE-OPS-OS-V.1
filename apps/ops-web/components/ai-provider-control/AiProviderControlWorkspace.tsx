...
type TabKey = 'command' | 'dossiers' | 'credentials' | 'assignments' | 'routing' | 'capacity' | 'models' | 'grounding' | 'usage' | 'costs' | 'incidents' | 'versions' | 'audit' | 'simulation'
...
function DossierForm({ busy, onSubmit }: { busy: boolean; onSubmit: (payload: JsonRecord) => void }) {
  return <form className={styles.form} onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); onSubmit(Object.fromEntries(data.entries())) }}><div className={styles.formGrid}><Field label="Nom du dossier"><input name="name" required placeholder="Gemini Marketing Production"/></Field><Field label="Code"><input name="code" placeholder="GEMINI_MARKETING_PROD"/></Field><Field label="Libellé du compte"><input name="accountLabel" placeholder="Compte Google Marketing"/></Field><Field label="ID projet Google"><input name="externalProjectId" placeholder="angelcare-market-ai"/></Field><Field label="Nom du capacity pool"><input name="projectName" placeholder="Marketing AI Project"/></Field><Field label="Tier de facturation"><select name="billingTier" defaultValue="free"><option value="free">Free</option><option value="paid">Paid</option><option value="enterprise">Enterprise</option></select></Field><Field label="RPM fournisseur"><input type="number" name="providerRpm" min="0" placeholder="5"/></Field><Field label="TPM fournisseur"><input type="number" name="providerTpm" min="0" placeholder="250000"/></Field><Field label="RPD fournisseur"><input type="number" name="providerRpd" min="0" placeholder="20"/></Field><Field label="Grounded RPD fournisseur"><input type="number" name="providerGroundedRpd" min="0" placeholder="1500"/></Field></div><div className={styles.formFooter}><p>Le plafond Google est informatif. SANILA impose ses propres limites séparément.</p><Submit busy={busy} label="Créer le dossier"/></div></form>
}
...
function CredentialForm({ snapshot, busy, onSubmit }: { snapshot: AiProviderSnapshot | null; busy: boolean; onSubmit: (payload: JsonRecord) => void }) {
  return <form className={styles.form} onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const data = new FormData(event.currentTarget); onSubmit(Object.fromEntries(data.entries())) }}><div className={styles.securityNotice}><ShieldCheck size={20}/><div><strong>Secret write-only</strong><span>Après validation, seule l’empreinte restera visible.</span></div></div><div className={styles.formGrid}><Field label="Dossier"><select name="dossierId" required>{snapshot?.dossiers.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></Field><Field label="Capacity pool"><select name="capacityPoolId"><option value="">Automatique</option>{snapshot?.pools.map((item) => <option value={item.id} key={item.id}>{item.project_name}</option>)}</select></Field><Field label="Type de clé"><select name="keyType" defaultValue="auth_key"><option value="auth_key">Gemini Auth Key</option><option value="standard_key">Standard API Key</option></select></Field><Field label="Secret Gemini" hint="Le secret est envoyé uniquement au serveur puis chiffré dans Vault."><input type="password" name="secret" required autoComplete="new-password" placeholder="Coller la clé ici"/></Field></div><div className={styles.formFooter}><p>La credential sera créée en état « testing » avant activation.</p><Submit busy={busy} label="Chiffrer et versionner"/></div></form>
}
