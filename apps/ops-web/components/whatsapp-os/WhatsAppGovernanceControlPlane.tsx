...
function DeviceDrawer({ device, overview, onClose, reload }: { device: WhatsAppControlPlaneDevice; overview: WhatsAppControlPlaneOverview; onClose: () => void; reload: () => Promise<void> }) {
  ...
  const locked = form.desired_mode === "locked"
...
export default function WhatsAppGovernanceControlPlane({ initialView = "fleet", embedded = false }: { initialView?: InternalView; embedded?: boolean } = {}) {
  ...
  return <div className="space-y-6">
    {!embedded ? <>
    <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-[linear-gradient(135deg,#07172f,#102e67_58%,#0d6b55)] p-6 text-white shadow-xl"><div className="flex flex-wrap items-start justify-between gap-5"><div><div className="flex flex-wrap gap-2"><Pill tone="green">MZ14 Governance Control Plane</Pill><Pill tone="blue">Route unique /whatsapp-os/admin</Pill><Pill tone="slate">Desktop 1.7.2 gelé</Pill></div><h2 className="mt-5 text-3xl font-black tracking-[-.055em] lg:text-4xl">Piloter la flotte depuis un seul centre de vérité.</h2><p className="mt-3 max-w-4xl text-sm font-semibold leading-7 text-blue-100">Consigne désirée, état réellement signalé, dérive, commandes, accusés, alertes et preuve d’audit sont réunis sans modifier les applications Windows ou macOS.</p></div><button onClick={() => void load(true)} className="inline-flex h-11 items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-4 text-xs font-black text-white backdrop-blur"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />Recalculer et synchroniser les alertes</button></div></section>
