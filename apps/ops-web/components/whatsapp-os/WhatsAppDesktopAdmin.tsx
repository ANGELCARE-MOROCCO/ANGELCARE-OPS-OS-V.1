...
import { summarizeMetadata } from "@/lib/whatsapp-desktop/control-plane"
import type { LucideIcon } from "lucide-react"
import CorporateStationAdmin from "@/components/whatsapp-os/CorporateStationAdmin"
import WhatsAppDeviceFleet from "@/components/whatsapp-os/WhatsAppDeviceFleet"
import WhatsAppAssignmentAdmin from "@/components/whatsapp-os/WhatsAppAssignmentAdmin"
import WhatsAppGovernanceControlPlane from "@/components/whatsapp-os/WhatsAppGovernanceControlPlane"
      ...
      <section className="mx-auto max-w-[1800px] overflow-hidden rounded-[34px] border border-white bg-white shadow-[0_30px_100px_rgba(15,23,42,.11)] ring-1 ring-slate-200/80">
        <header className="relative overflow-hidden border-b border-slate-200 bg-[linear-gradient(135deg,#ffffff,#eff6ff_52%,#ecfdf5)] px-5 py-6 lg:px-8 lg:py-7">
          ...
          <div className="relative flex flex-wrap items-start justify-between gap-5">
            <div><div className="flex flex-wrap items-center gap-2"><Badge tone="red">Protection anti-verrouillage MZ16</Badge><Badge tone="green">Governance Control Plane MZ14</Badge><Badge tone="blue">Administration Corporate Unifiée</Badge><Badge tone="slate">Desktop 1.7.3 gelé</Badge></div><h1 className="mt-4 text-3xl font-black tracking-[-0.055em] text-slate-950 lg:text-5xl">Administration WhatsApp Desktop</h1><p className="mt-3 max-w-4xl text-sm font-semibold leading-7 text-slate-600">Centre de commandement unique pour la flotte Windows et macOS : état désiré, état signalé, synchronisation, sécurité anti-verrouillage, politiques, commandes, alertes et audit — sans centraliser les cookies ni messages WhatsApp.</p></div>
        ...
        <div className="p-5 lg:p-7">
          {tab === "overview" && data ? <WhatsAppGovernanceControlPlane initialView="fleet" /> : null}
          ...
          {tab === "assignments" && data ? <WhatsAppAssignmentAdmin data={data} form={assignmentForm} setForm={setAssignmentForm} busy={busy} run={run} reload={load} setNotice={setNotice} setError={setError} /> : null}

          {tab === "devices" && data ? <WhatsAppDeviceFleet data={data} reload={load} setNotice={setNotice} setError={setError} /> : null}
          ...
          {tab === "commands" && data ? <WhatsAppGovernanceControlPlane initialView="commands" embedded /> : null}
          {tab === "security" && data ? <WhatsAppGovernanceControlPlane initialView="interventions" embedded /> : null}
