...
export default function WhatsAppDesktopAdmin() {
  ...
  const summaryCards: Array<[LucideIcon, string, number, string]> = [
    [MessageCircleMore, "Espaces actifs", data?.counts.active_workspaces ?? 0, "blue"],
    [Users, "Affectations", data?.counts.active_assignments ?? 0, "violet"],
    [Laptop2, "Appareils", data?.counts.devices ?? 0, "slate"],
    [Wifi, "En ligne", data?.counts.online_devices ?? 0, "green"],
    [Clock3, "En attente", (data?.counts.pending_devices ?? 0) + (data?.counts.pending_requests ?? 0), "amber"],
    [ShieldAlert, "Alertes ouvertes", data?.counts.open_security_events ?? 0, data?.counts.open_security_events ? "red" : "green"],
  ]
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#f8fbff,#f8fafc)] p-4 lg:p-7">
      <section className="mx-auto max-w-[1800px] overflow-hidden rounded-[34px] border border-white bg-white shadow-[0_30px_100px_rgba(15,23,42,.11)] ring-1 ring-slate-200/80">
        <header className="relative overflow-hidden border-b border-slate-200 bg-[linear-gradient(135deg,#ffffff,#eff6ff_52%,#ecfdf5)] px-5 py-6 lg:px-8 lg:py-7">
          ...
          {data ? <div className="relative mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">{summaryCards.map(([Icon, label, value, color]) => <div key={String(label)} className="rounded-[20px] border border-white/90 bg-white/85 p-4 shadow-sm backdrop-blur"><Icon className={`h-4 w-4 ${color === "green" ? "text-emerald-600" : color === "red" ? "text-red-600" : color === "amber" ? "text-amber-600" : color === "violet" ? "text-violet-600" : color === "blue" ? "text-blue-600" : "text-slate-600"}`} /><p className="mt-3 text-2xl font-black tracking-[-0.04em] text-slate-950">{String(value)}</p><p className="mt-1 text-[9px] font-black uppercase tracking-[0.14em] text-slate-400">{String(label)}</p></div>)}</div> : null}
        </header>
...
function PolicyWorkspace({ data, workspaceId, setWorkspaceId, policy, setPolicy, busy, run }: any) { if (!policy) return <LoaderCircle className="h-8 w-8 animate-spin text-emerald-600" />; const boolFields = [["require_new_device_approval","Approbation obligatoire"],["clear_session_on_revocation","Effacer session après révocation"],["allow_downloads","Téléchargements"],["allow_uploads","Téléversements"],["allow_microphone","Microphone"],["allow_camera","Caméra"],["allow_notifications","Notifications"],["allow_external_open","Ouverture externe"],["allow_local_cache_clear","Effacement cache local"],["allow_local_session_clear","Effacement session local"]]; return <div className="grid gap-6 xl:grid-cols-[320px_1fr]"><div className="rounded-[24px] border border-slate-200 p-4"><p className="text-sm font-black text-slate-950">Espace configuré</p><select value={workspaceId} onChange={(event) => setWorkspaceId(event.target.value)} className="mt-3 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-black">{data.workspaces.map((row: Row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select><p className="mt-4 text-xs font-semibold leading-5 text-slate-500">Les politiques sont appliquées par le desktop au prochain renouvellement de bail.</p></div><form onSubmit={(event) => { event.preventDefault(); void run("save-policy", () => api("/api/whatsapp-desktop/policies", { method: "PATCH", body: JSON.stringify({ ...policy, workspace_id: workspaceId, reason: "Mise à jour politique depuis Mega ZIP 3" }) }), "Politique enregistrée.") }} className="rounded-[24px] border border-slate-200 p-5"><div className="grid gap-4 md:grid-cols-3">{[["lease_duration_minutes","Durée du bail (min)"],["offline_grace_minutes","Grâce hors ligne (min)"],["maximum_devices_per_user","Appareils par utilisateur"],["maximum_users","Utilisateurs maximum"],["heartbeat_active_seconds","Heartbeat actif (sec)"],["heartbeat_background_seconds","Heartbeat arrière-plan (sec)"]].map(([key,label]) => <label key={key} className="text-xs font-black text-slate-600">{label}<input type="number" value={policy[key] ?? 0} onChange={(event) => setPolicy({ ...policy, [key]: Number(event.target.value) })} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-black" /></label>)}</div><div className="mt-5 grid gap-3 md:grid-cols-2">{boolFields.map(([key,label]) => <label key={key} className="flex items-center justify-between rounded-xl border border-slate-200 p-3 text-xs font-black text-slate-700"><span>{label}</span><input type="checkbox" checked={Boolean(policy[key])} onChange={(event) => setPolicy({ ...policy, [key]: event.target.checked })} className="h-4 w-4" /></label>)}</div><label className="mt-5 block text-xs font-black text-slate-600">Version minimale<input value={policy.minimum_desktop_version || "1.2.0"} onChange={(event) => setPolicy({ ...policy, minimum_desktop_version: event.target.value })} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm font-black" /></label><button disabled={busy !== null} className="mt-5 inline-flex h-11 items-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-black text-white"><Save className="h-4 w-4" />Enregistrer la politique</button></form></div> }
