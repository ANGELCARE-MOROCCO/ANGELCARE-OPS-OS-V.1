...
export default function EmailOSEnterpriseProductionWorkspace() {
  ...
  async function load() {
    setLoading(true)
    const result = await api("/api/email-os/enterprise-workspace")
    if (result.ok) {
  ...
  async function runMessageAction(action: string, payload: any = {}) {
    ...
    const result = await api("/api/email-os/message-action", { method: "POST", body: JSON.stringify({ messageId: selected.id, action, source, payload }) })
