...
import {
  ...
  UsersRound, PauseCircle, PlayCircle, Workflow, SendHorizontal, Megaphone,
} from "lucide-react"
import {
  cx, EmptyState, LoadingPanel, Metric, NoticeBanner, ProgressBar, ReasonConfirmDialog, SectionTitle, StatusPill,
...
export default function ExecutiveControlWorkspace() {
  ...
  const [runtimeControl, setRuntimeControl] = useState<any>(null)
  const [controlBusy, setControlBusy] = useState<string | null>(null)
  const [pendingControl, setPendingControl] = useState<{ action: string; label: string } | null>(null)
  ...
  async function loadRuntimeControl() {
    try { setRuntimeControl(await acApi<any>("/api/ac-whatsapp/executive/control")) } catch { setRuntimeControl(null) }
  }
  useEffect(() => { void loadAnalytics(); void loadRuntimeControl() }, [])

  async function runtimeAction(action: string, label: string, reason?: string) {
    if (!reason) { setPendingControl({ action, label }); return }
    setControlBusy(action)
    try {
      setRuntimeControl(await acApi<any>("/api/ac-whatsapp/executive/control", { method: "POST", body: JSON.stringify({ action, reason }) }))
      setNotice({ tone: "success", title: label, description: "La commande globale a été appliquée côté serveur et enregistrée dans l’audit." })
      await Promise.all([refresh().catch(() => undefined), loadAnalytics()])
  ...
  const a = analytics
  const canRuntimeControl = Boolean(data?.actor.permissions?.some((permission: string) => ["ac-whatsapp.security.manage","ac-whatsapp.*","*"].includes(permission)))
  ...
  async function synchronize() { await Promise.all([refresh().catch(() => undefined), loadAnalytics(), loadRuntimeControl()]); setNotice({ tone: "success", title: "Executive Control synchronisé", description: "Les indicateurs et les signaux de risque ont été actualisés." }) }
  ...
  return <div className="space-y-4">
    <SectionTitle eyebrow="Master Workspace 06 · Executive Control" title="Voir l’impact, le risque et la responsabilité en quelques secondes." description="Une lecture exécutive honnête de la disponibilité, des conversations, des campagnes et des exceptions nécessitant une intervention humaine." action={<div className="flex flex-wrap gap-2"><button type="button" onClick={exportSnapshot} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-[9px] font-black text-slate-700"><Download className="h-4 w-4" />Exporter la preuve</button><button type="button" onClick={() => void synchronize()} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-3.5 py-2.5 text-[9px] font-black text-white"><RefreshCw className={cx("h-4 w-4", busy && "animate-spin")} />Synchroniser</button></div>} />
    ...
    <Surface>
      <SurfaceHeader eyebrow="Emergency governance" title="Commandes globales de sécurité" icon={ShieldAlert} />
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <RuntimeControlCard icon={SendHorizontal} title="Sortant global" active={Boolean(runtimeControl?.outbound_paused)} activeLabel="PAUSE ACTIVE" description="Bloque immédiatement les nouveaux envois manuels, automatiques et le worker durable sans déconnecter OpenWA." busy={controlBusy !== null || !canRuntimeControl} onPause={() => void runtimeAction("pause_outbound","Sortant global mis en pause")} onResume={() => void runtimeAction("resume_outbound","Sortant global réactivé")} />
        <RuntimeControlCard icon={Workflow} title="Automatisations" active={Boolean(runtimeControl?.automation_paused)} activeLabel="AUTO PAUSÉ" description="Suspend l’évaluation des règles Auto Reply. Les réponses humaines restent gouvernées par l’état du sortant global." busy={controlBusy !== null || !canRuntimeControl} onPause={() => void runtimeAction("pause_automation","Automatisations mises en pause")} onResume={() => void runtimeAction("resume_automation","Automatisations réactivées")} />
        <RuntimeControlCard icon={Megaphone} title="Campagnes" active={Boolean(runtimeControl?.campaigns_paused)} activeLabel="CAMPAGNES PAUSÉES" description="Empêche les lancements et met en attente les éléments de campagne déjà réclamés par le worker." busy={controlBusy !== null || !canRuntimeControl} onPause={() => void runtimeAction("pause_campaigns","Campagnes mises en pause")} onResume={() => void runtimeAction("resume_campaigns","Campagnes réactivées")} />
      </div>
      {runtimeControl?.reason ? <p className="mt-3 rounded-xl bg-slate-50 px-3 py-2 text-[10px] font-semibold text-slate-500">Dernier motif · {runtimeControl.reason}</p> : null}
      {!canRuntimeControl ? <p className="mt-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[10px] font-semibold text-slate-500">Lecture seule · la permission Sécurité & incidents est requise pour modifier ces commandes globales.</p> : null}
    </Surface>
    ...
    {tab === "risk" ? <Surface><SurfaceHeader eyebrow="Executive intervention desk" title="Risques, impact et propriétaire attendu" icon={ShieldAlert} /><div className="mt-5 space-y-3">{criticalRisks.length ? criticalRisks.map((risk, index) => <div key={`${risk.title}-${index}`} className="grid gap-4 rounded-[24px] border border-slate-200 p-4 md:grid-cols-[50px_1fr_180px_130px] md:items-center"><div className={cx("grid h-11 w-11 place-items-center rounded-2xl text-white", risk.severity === "critical" ? "bg-rose-600" : risk.severity === "high" ? "bg-amber-500" : "bg-slate-500")}><AlertTriangle className="h-4 w-4" /></div><div><p className="text-[10px] font-black text-slate-900">{risk.title}</p><p className="mt-1 text-[9px] font-semibold leading-5 text-slate-500">{risk.description}</p></div><div><p className="text-[10px] font-black uppercase tracking-[.12em] text-slate-400">Propriétaire attendu</p><p className="mt-1 text-[9px] font-black text-slate-700">{risk.owner}</p></div><StatusPill status={risk.severity} compact /></div>) : <EmptyState title="Aucune intervention critique" description="Les signaux disponibles ne font apparaître aucun risque nécessitant une décision exécutive immédiate." icon={CheckCircle2} />}</div></Surface> : null}

    {tab === "audit" ? <Surface><SurfaceHeader eyebrow="Governance ledger" title="Actions et preuves récentes" icon={ShieldCheck} /><div className="mt-5 overflow-hidden rounded-2xl border border-slate-200"><div className="grid grid-cols-[160px_1fr_160px_120px] gap-3 bg-slate-50 px-4 py-3 text-[10px] font-black uppercase tracking-[.13em] text-slate-400"><span>Horodatage</span><span>Action</span><span>Entité</span><span>Acteur</span></div>{(data?.auditEvents || []).slice(0, 80).map((event: any, index) => <div key={event.id || index} className="grid grid-cols-[160px_1fr_160px_120px] gap-3 border-t border-slate-100 px-4 py-3"><span className="text-[9px] font-bold text-slate-500">{formatDateTime(event.created_at)}</span><span className="truncate text-[9px] font-black text-slate-700">{event.action || event.event_type || "Action"}</span><span className="truncate text-[9px] font-semibold text-slate-500">{event.entity_type || "—"}</span><span className="truncate text-[9px] font-semibold text-slate-500">{event.actor_name || event.actor_user_id || "Système"}</span></div>)}{!data?.auditEvents.length ? <div className="p-5"><EmptyState compact title="Aucun événement d’audit" description="Les actions auditées apparaîtront dans ce registre de gouvernance." icon={ShieldCheck} /></div> : null}</div></Surface> : null}
  {pendingControl ? <ReasonConfirmDialog danger title={pendingControl.label} description="Cette commande modifie le comportement global d’AC WhatsApp. Le motif sera conservé dans l’audit." confirmLabel="Confirmer la commande" onClose={() => setPendingControl(null)} onConfirm={async (reason) => { const current = pendingControl; setPendingControl(null); await runtimeAction(current.action, current.label, reason) }} /> : null}
  </div>
}

function RuntimeControlCard({ icon: Icon, title, active, activeLabel, description, busy, onPause, onResume }: { icon: any; title: string; active: boolean; activeLabel: string; description: string; busy: boolean; onPause: () => void; onResume: () => void }) {
  return <div className={cx("rounded-[22px] border p-4", active ? "border-rose-200 bg-rose-50" : "border-slate-200 bg-white")}>
    <div className="flex items-start justify-between gap-3"><div className={cx("grid h-10 w-10 place-items-center rounded-2xl", active ? "bg-rose-700 text-white" : "bg-slate-950 text-white")}><Icon className="h-4 w-4" /></div><StatusPill status={active ? "paused" : "active"} label={active ? activeLabel : "ACTIF"} compact /></div>
    <p className="mt-3 text-[11px] font-black text-slate-950">{title}</p><p className="mt-1 text-[10px] font-semibold leading-5 text-slate-500">{description}</p>
    <button type="button" disabled={busy} onClick={active ? onResume : onPause} className={cx("mt-3 inline-flex items-center gap-2 rounded-xl px-3 py-2 text-[10px] font-black disabled:opacity-40", active ? "bg-emerald-600 text-white" : "bg-slate-950 text-white")}>{active ? <PlayCircle className="h-3.5 w-3.5" /> : <PauseCircle className="h-3.5 w-3.5" />}{active ? "Réactiver" : "Mettre en pause"}</button>
  </div>
}

function ExecutiveGauge({ label, value, tone, detail }: { label: string; value: number; tone: "emerald" | "blue" | "violet" | "amber"; detail: string }) { return <div className="rounded-[24px] border border-slate-200 p-4"><div className="flex items-end justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-slate-400">{label}</p><p className="mt-2 text-3xl font-black tracking-[-.05em] text-slate-950">{value}%</p></div><StatusPill status={value >= 80 ? "active" : value >= 50 ? "warning" : "error"} compact /></div><div className="mt-4"><ProgressBar value={value} tone={tone} /></div><p className="mt-3 text-[10px] font-semibold text-slate-400">{detail}</p></div> }
...
function Total({ label, value }: { label: string; value: number }) { return <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4"><p className="text-3xl font-black tracking-[-.05em] text-slate-950">{value}</p><p className="mt-1 text-[10px] font-black uppercase tracking-[.13em] text-slate-400">{label}</p></div> }
