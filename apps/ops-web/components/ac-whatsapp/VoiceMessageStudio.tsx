...
export function VoiceMessageStudio({
  ...
  return <div className="relative">
    <button
      ...
      className={cx(
        "inline-flex items-center gap-1.5 rounded-xl px-2.5 py-2 text-[10px] font-black transition",
    ...
    {phase !== "idle" ? <div className="absolute bottom-12 left-0 z-50 w-[min(430px,82vw)] acw-apex-floating-surface rounded-[16px] border border-slate-200 bg-white p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[.16em] text-slate-600">Message vocal · contrôle avant envoi</p>
        ...
        <span className={cx("rounded-full px-3 py-1 text-[10px] font-black", phase === "recording" ? "bg-rose-100 text-rose-950 ring-1 ring-rose-300" : "bg-emerald-100 text-emerald-950 ring-1 ring-emerald-300")}>{formatSeconds(elapsed)}</span>
      </div>

      {phase === "recording" ? <div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1"><p className="text-[10px] font-black text-slate-950">Le microphone est actif</p><p className="mt-1 text-[10px] font-semibold text-slate-600">Parlez clairement. Durée maximale : 10 minutes.</p></div>
        </div>
        ...
        <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={cancelRecording} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 text-[10px] font-black text-slate-900"><Trash2 className="h-3.5 w-3.5" />Annuler</button><button type="button" onClick={stopRecording} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2 text-[10px] font-black text-white"><Square className="h-3.5 w-3.5 fill-current" />Terminer</button></div>
      </div> : null}

      {(phase === "preview" || phase === "sending") && recordedUrl ? <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
        <audio src={recordedUrl} controls preload="metadata" className="w-full" />
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2"><p className="text-[10px] font-semibold text-slate-600">Écoutez avant l’envoi. Le contact recevra une véritable note vocale WhatsApp.</p><div className="flex gap-2"><button type="button" disabled={phase === "sending"} onClick={resetRecording} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 text-[10px] font-black text-slate-900 disabled:opacity-40"><RotateCcw className="h-3.5 w-3.5" />Recommencer</button><button type="button" disabled={phase === "sending"} onClick={() => void sendVoiceMessage()} className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-2 text-[10px] font-black text-white disabled:opacity-60">{phase === "sending" ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />}{phase === "sending" ? "Envoi…" : "Envoyer le vocal"}</button></div></div>
      </div> : null}
    </div> : null}
  </div>
}
  ...
  return <div className={cx("rounded-2xl border p-3", inverted ? "border-white/15 bg-white/10" : "border-emerald-200 bg-emerald-50")}>
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className={cx("mt-1 flex items-center justify-between text-[10px] font-black", inverted ? "text-slate-300" : "text-slate-600")}><span>{formatSeconds(currentTime)} / {formatSeconds(duration)}</span><span className="inline-flex items-center gap-1"><Volume2 className="h-3 w-3" />Message vocal</span></div>
      </div>
      <button type="button" onClick={changeSpeed} className={cx("inline-flex h-8 min-w-12 items-center justify-center gap-1 rounded-xl px-2 text-[10px] font-black", inverted ? "border border-white/20 bg-white/10 text-white" : "border border-emerald-300 bg-white text-emerald-950")} title="Vitesse de lecture"><Gauge className="h-3 w-3" />{PLAYBACK_SPEEDS[speedIndex]}×</button>
    </div>
    {error ? <p className={cx("mt-2 rounded-lg px-2 py-1 text-[10px] font-bold", inverted ? "bg-rose-100 text-rose-950" : "border border-rose-200 bg-rose-50 text-rose-950")}>{error}</p> : null}
  </div>
}
