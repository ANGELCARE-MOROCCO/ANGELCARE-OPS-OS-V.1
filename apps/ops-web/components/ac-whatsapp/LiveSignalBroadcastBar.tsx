...
function severityFor(signal: SignalBlueprint, value: string) {
  const score = signalScore(signal, value)
  if (signal.tone === "rose" && score >= 85) return { label: "CRITIQUE", level: "critical" }
  if (signal.tone === "amber" && score >= 55) return { label: "ATTENTION", level: "attention" }
  if (signal.tone === "violet") return { label: "AUTOMATION", level: "automation" }
  if (signal.tone === "emerald") return { label: "STABLE", level: "healthy" }
  if (signal.tone === "blue") return { label: "LIVE", level: "live" }
  return { label: "INFO", level: "info" }
}
...
export default function LiveSignalBroadcastBar({ data, activeWorkspace }: { data?: AcWhatsAppBootstrap | null; activeWorkspace: string }) {
  ...
  const renderRun = (copy: "primary" | "duplicate") => <div className={cx("acw-broadcast-run", copy === "duplicate" && "acw-broadcast-run-duplicate")} aria-hidden={copy === "duplicate" ? true : undefined}>
    {rankedSignals.map(({ signal, value }, index) => {
      ...
      const severity = severityFor(signal, value)
      return <div key={`${copy}:${signal.id}`} className="acw-broadcast-signal" data-severity={severity.level} title={`${signal.title} — ${signal.caption}`}>
        <span className="acw-broadcast-signal-icon"><Icon className={cx("h-3.5 w-3.5 shrink-0", tone.icon)} aria-hidden="true" /></span>
        <span className="acw-broadcast-severity">{severity.label}</span>
        <span className="acw-broadcast-copy"><span className="acw-broadcast-title">{signal.title}</span><span className="acw-broadcast-caption">{signal.caption}</span></span>
  ...
  return <section data-acw-live-feed aria-label={`Flux live AngelCare · ${selectedMode.label}`} className="acw-broadcast-shell">
    ...
    <div className="acw-broadcast-context" title="Survolez le flux pour le mettre en pause">
      <span className="hidden 2xl:inline">{activeWorkspace}</span><span className="acw-broadcast-telemetry">{count(data?.counts.unread)} NON LUS</span><span className="acw-broadcast-telemetry is-risk">{count(data?.counts.securityOpen)} RISQUES</span><span className="acw-broadcast-pause-hint">SURVOL = PAUSE</span>
    </div>
