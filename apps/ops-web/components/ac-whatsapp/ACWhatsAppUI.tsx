...
export function StatusPill({ status, label, compact = false }: { status: string; label?: string; compact?: boolean }) {
  ...
  return (
    <span className={cx("inline-flex shrink-0 items-center rounded-full border font-extrabold tracking-[-.01em]", compact ? "gap-1 px-2 py-1 text-[9px]" : "gap-1.5 px-2.5 py-1.5 text-[10px]", classes)}>
  ...
  return (
    <div className="flex flex-col gap-4 border-b border-slate-200 pb-4 xl:flex-row xl:items-end xl:justify-between">
      <div className="min-w-0">
        ...
        <h1 className="mt-1.5 max-w-5xl text-[clamp(1.45rem,2.1vw,2.15rem)] font-black leading-[1.05] tracking-[-.045em] text-slate-950">{title}</h1>
...
export function Surface({ children, className, padded = true }: { children: ReactNode; className?: string; padded?: boolean }) {
  return <section className={cx("overflow-hidden rounded-[18px] border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,.02),0_8px_28px_rgba(15,23,42,.035)]", padded && "p-4", className)}>{children}</section>
}
