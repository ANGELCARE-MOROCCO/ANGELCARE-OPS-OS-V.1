...
function MessageBubble({ message }: { message: AcWhatsAppMessage }) { const outbound = message.direction === "outbound"; const internal = message.direction === "internal"; const identity = message.sender_identity || { display_name: message.sender_display_name_snapshot || (outbound ? "Opérateur non résolu" : internal ? "Membre AngelCare" : "Contact non identifié"), role: message.sender_role_snapshot || (outbound ? "Membre AngelCare" : internal ? "Note interne" : "Contact"), type: message.sender_type || message.direction, origin: message.message_origin }; return <div className={cx("flex", outbound ? "justify-end" : "justify-start")}><div className={cx("max-w-[84%] rounded-[22px] border px-4 py-3 shadow-sm", outbound ? "rounded-br-md border-slate-900 bg-slate-950 text-white" : internal ? "border-amber-400 bg-amber-100 text-slate-950" : "rounded-bl-md border-slate-300 bg-white text-slate-950")}><div className={cx("mb-2 flex flex-wrap items-center gap-2 border-b pb-2", outbound ? "border-white/15" : internal ? "border-amber-300" : "border-slate-200")}><span className={cx("text-[9px] font-black", outbound ? "text-sky-200" : "text-slate-950")}>{identity.display_name}</span><span className={cx("text-[8px] font-bold", outbound ? "text-slate-300" : "text-slate-600")}>{identity.role}</span>{message.campaign_name_snapshot ? <span className="rounded-md bg-violet-100 px-2 py-1 text-[7px] font-black text-violet-950">{message.campaign_name_snapshot}</span> : null}{message.responsible_identity?.display_name ? <span className={cx("text-[7px] font-bold", outbound ? "text-slate-300" : "text-slate-600")}>Responsable : {message.responsible_identity.display_name}</span> : null}</div><div className="whitespace-pre-wrap text-[12px] font-semibold leading-6">{message.body || message.caption || `[${message.message_type}]`}</div><div className={cx("mt-2 flex items-center justify-end gap-2 text-[8px] font-bold", outbound ? "text-slate-300" : "text-slate-600")}><span>{formatDateTime(message.sent_at || message.received_at || message.created_at)}</span>{outbound ? <><StatusPill status={message.status} compact />{["delivered", "read"].includes(message.status) ? <CheckCheck className="h-3.5 w-3.5" /> : null}</> : null}</div>{message.error_message ? <p className="mt-2 rounded-lg border border-rose-300 bg-rose-100 px-2 py-1 text-[8px] font-bold text-rose-950">{message.error_message}</p> : null}</div></div> }
