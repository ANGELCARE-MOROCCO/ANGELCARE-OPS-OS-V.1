...
export default function LiveCommandWorkspace() {
  ...
  return <div className="space-y-2">
    ...
    <section className={cx("grid overflow-hidden rounded-[18px] border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,.02),0_18px_50px_rgba(15,23,42,.05)] xl:h-[calc(100dvh-128px)] xl:min-h-[650px] xl:max-h-[1040px]", focusMode ? "xl:grid-cols-1" : queueCollapsed && dockCollapsed ? "xl:grid-cols-[76px_minmax(0,1fr)_76px]" : queueCollapsed ? "xl:grid-cols-[76px_minmax(0,1fr)_360px]" : dockCollapsed ? "xl:grid-cols-[336px_minmax(0,1fr)_76px]" : "xl:grid-cols-[336px_minmax(640px,1fr)_360px]")} data-mz7-theatre="true" style={mz7Layout.gridStyle}>
      {!focusMode ? <aside className="flex min-h-0 flex-col overflow-hidden border-b border-slate-200 bg-[#f8fafc] xl:border-b-0 xl:border-r">
        <div className="flex shrink-0 items-center justify-between border-b border-slate-200 px-3 py-3">
          {!queueCollapsed ? <div><p className="text-[10px] font-black uppercase tracking-[.17em] text-slate-500">Smart Relationship Queue</p><p className="mt-1 text-[12px] font-black text-slate-950">Conversations prioritaires</p></div> : null}
        ...
        {!queueCollapsed ? <>
          <div className="shrink-0 border-b border-slate-200 p-3">
            <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nom, organisation, numéro…" className="w-full rounded-xl border border-slate-300 bg-white py-3 pl-10 pr-3 text-[11px] font-bold text-slate-950 outline-none placeholder:text-slate-500 focus:border-slate-700" /></div>
          ...
          <div className="flex shrink-0 items-center justify-between px-3 py-2"><p className="text-[9px] font-black uppercase tracking-[.15em] text-slate-600">File intelligente</p><span className="rounded-full border border-slate-300 bg-white px-2.5 py-1 text-[9px] font-black text-slate-900">{filtered.length}</span></div>
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-2 pb-2 [scrollbar-gutter:stable]">{filtered.length ? filtered.map((row) => <TheatreConversationRow key={row.id} row={row} active={row.id === selectedId} onClick={() => setSelectedId(row.id)} />) : <EmptyState compact title="Aucune conversation" description="Aucun résultat pour ce filtre." icon={Filter} />}</div>
...
<main className="flex min-h-0 min-w-0 flex-col overflow-hidden bg-white">
        {selected ? <>
          ...
          {theatre.presence.length > 1 ? <div className="shrink-0 border-t border-amber-200 bg-amber-50 px-4 py-2"><div className="mx-auto flex max-w-[920px] items-center gap-2 text-[10px] font-black text-amber-950"><Radio className="h-3.5 w-3.5" />{theatre.presence.length} opérateurs consultent cette conversation. Vérifiez la présence avant d’envoyer pour éviter une réponse simultanée.</div></div> : null}
...
<aside className="flex min-h-0 flex-col overflow-hidden border-t border-slate-200 bg-[#f7f9fc] xl:border-l xl:border-t-0">
        <div className="flex shrink-0 items-center justify-between border-b border-slate-200 px-3 py-3">
          {!dockCollapsed ? <div><p className="text-[10px] font-black uppercase tracking-[.17em] text-slate-500">Relationship Intelligence</p><p className="mt-1 text-[12px] font-black text-slate-950">Contexte & décision</p></div> : null}
        ...
        {!dockCollapsed && selected ? <>
          <div className="grid shrink-0 grid-cols-4 border-b border-slate-200 bg-white p-1.5">{([ ["profile","Profil",CircleUserRound], ["intelligence","Intelligence",Sparkles], ["actions","Actions",Target], ["history","Historique",History] ] as const).map(([id,label,Icon]) => <button key={id} type="button" onClick={() => setDockTab(id)} className={cx("flex flex-col items-center gap-1 rounded-xl px-1 py-2 text-[10px] font-black", dockTab === id ? "bg-slate-950 text-white" : "text-slate-600 hover:bg-slate-100")}><Icon className="h-3.5 w-3.5" />{label}</button>)}</div>
...
function TheatreConversationRow({ row, active, onClick }: { row: AcWhatsAppConversation; active: boolean; onClick: () => void }) {
  ...
  const waiting = row.status === "waiting_customer" ? "Attente client" : row.status === "escalated" ? "Escalade" : row.assigned_user_id ? "Attribuée" : "À attribuer"
  return <button type="button" onClick={onClick} className={cx("group relative mb-1 w-full overflow-hidden rounded-xl border px-3 py-2.5 text-left transition-colors", active ? "border-slate-950 bg-slate-950 text-white" : "border-transparent bg-transparent text-slate-950 hover:border-slate-200 hover:bg-white")}>
    {active ? <span className="absolute inset-y-2 left-0 w-0.5 rounded-r-full bg-rose-500" /> : null}
    <div className="flex gap-2.5">
      <div className={cx("grid h-9 w-9 shrink-0 place-items-center rounded-xl text-[9px] font-black", active ? "bg-white/10 text-white" : "border border-slate-200 bg-white text-slate-700")}>{initials(row.contact?.display_name)}</div>
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2"><p className="truncate text-[10.5px] font-black">{row.contact?.display_name || row.contact?.phone_number_e164 || "Contact à identifier"}</p><span className={cx("shrink-0 text-[10px] font-bold tabular-nums", active ? "text-slate-300" : "text-slate-400")}>{formatRelative(row.last_message_at)}</span></div>
        <p className={cx("mt-0.5 truncate text-[10px] font-bold", active ? "text-slate-300" : "text-slate-500")}>{row.contact?.organization_name || row.queue?.name || row.account?.name || "Relation à qualifier"}</p>
        <p className={cx("mt-1.5 truncate text-[9.5px] font-semibold", active ? "text-slate-100" : "text-slate-700")}>{row.last_message_preview || "Nouvelle conversation"}</p>
        <div className="mt-1.5 flex min-w-0 items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-1.5">{metadata.pinned ? <Bookmark className={cx("h-3 w-3 shrink-0", active ? "text-amber-300" : "text-amber-600")} /> : null}<span className={cx("truncate text-[10px] font-black", active ? "text-slate-400" : row.status === "escalated" ? "text-rose-700" : !row.assigned_user_id ? "text-amber-700" : "text-slate-400")}>{waiting}</span></div>
          {row.unread_count > 0 ? <span className={cx("grid h-5 min-w-5 place-items-center rounded-full px-1 text-[10px] font-black", active ? "bg-white text-slate-950" : "bg-rose-600 text-white")}>{row.unread_count}</span> : <span className={cx("text-[10px] font-bold", active ? "text-slate-400" : "text-slate-400")}>{row.priority === "high" || row.priority === "urgent" ? humanValue(row.priority, "") : ""}</span>}
        </div>
      </div>
    </div>
  </button>
...
function ConversationCommandBar({ selected, actorName, focusMode, messageSearch, messageSearchRef, onMessageSearch, onJumpSearch, onFocus, onResolve, onAssign, onFollowup, onOpenTools, menuOpen, onMenu, menu }: { selected: AcWhatsAppConversation; actorName: string; focusMode: boolean; messageSearch: string; messageSearchRef: React.RefObject<HTMLInputElement | null>; onMessageSearch: (value: string) => void; onJumpSearch: () => void; onFocus: () => void; onResolve: () => void; onAssign: () => void; onFollowup: () => void; onOpenTools: () => void; menuOpen: boolean; onMenu: () => void; menu: React.ReactNode }) {
  ...
  return <div className="relative z-20 shrink-0 border-b border-slate-200 bg-white px-3.5 py-2.5">
    <div className="flex flex-wrap items-center justify-between gap-2.5">
      <div className="flex min-w-0 items-center gap-2.5">
        <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-950 text-[10px] font-black text-white">{initials(selected.contact?.display_name)}</div>
...
function TheatreMessageBubble({ message, onQuote, onArtifact, onTranslate }: { message: AcWhatsAppMessage; onQuote: (message: AcWhatsAppMessage) => void; onArtifact: (type: string, message: AcWhatsAppMessage) => void; onTranslate: (message: AcWhatsAppMessage) => void }) {
  ...
  return <div tabIndex={-1} data-message-id={message.id} data-message-direction={message.direction} data-message-search={searchable} className={cx("group flex scroll-mt-20 outline-none focus-visible:ring-2 focus-visible:ring-sky-500/40", outbound ? "justify-end" : "justify-start")} >
    <div className={cx("relative max-w-[74%] rounded-[16px] border px-3.5 py-2.5 shadow-[0_1px_2px_rgba(15,23,42,.04),0_6px_18px_rgba(15,23,42,.035)]", outbound ? "rounded-br-[5px] border-slate-950 bg-slate-950 text-white" : internal ? "border-violet-300 bg-violet-50 text-slate-950" : "rounded-bl-[5px] border-slate-200 bg-white text-slate-950")}>
...
function MessageAction({ label, icon: Icon, onClick }: { label: string; icon: typeof Copy; onClick: () => void }) { return <button role="menuitem" type="button" onClick={onClick} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-[9px] font-black text-slate-950 hover:bg-slate-100"><Icon className="h-3.5 w-3.5 text-slate-700" /><span>{label}</span></button> }
...
function ResponseCommandDeck({ selected, actorName, composerRef, composer, setComposer, noteMode, setNoteMode, sending, draftStatus, onSend, onAi, onFollowup, onOpenResponses, onToggleAutomation, onOpenTools, onSent, onNotice }: { selected: AcWhatsAppConversation; actorName: string; composerRef: React.RefObject<HTMLTextAreaElement | null>; composer: string; setComposer: (value: string) => void; noteMode: boolean; setNoteMode: (value: boolean) => void; sending: boolean; draftStatus: string; onSend: () => void; onAi: (action: string) => void; onFollowup: () => void; onOpenResponses: () => void; onToggleAutomation: () => void; onOpenTools: () => void; onSent: () => Promise<void>; onNotice: (tone: "success" | "danger" | "warning" | "info", title: string, description: string) => void }) {
  ...
  return <div className="shrink-0 border-t border-slate-200 bg-[#fbfcfe] px-3.5 py-2.5"><div className="mx-auto max-w-[980px]">
    ...
    <div className={cx("overflow-hidden rounded-[16px] border bg-white shadow-[0_1px_2px_rgba(15,23,42,.04),0_8px_25px_rgba(15,23,42,.035)]", noteMode ? "border-violet-300" : "border-slate-300 focus-within:border-slate-500")}>
      <textarea ref={composerRef} value={composer} onChange={(event) => { const next = event.target.value; setComposer(next); if (!noteMode && next.trimStart().startsWith("/")) onOpenResponses() }} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") onSend() }} rows={3} placeholder={noteMode ? "Note interne — invisible au contact" : "Écrire une réponse…  Tapez / pour rechercher une réponse enregistrée"} className={cx("max-h-48 min-h-[78px] w-full resize-y bg-transparent px-3.5 py-3 text-[12.5px] font-semibold leading-5 text-slate-950 outline-none placeholder:text-slate-400", noteMode && "bg-violet-50/40")} />
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 bg-white px-2 py-1.5">
        ...
        <div className="flex items-center gap-2"><span className="hidden text-[10px] font-bold text-slate-400 sm:inline">⌘↵ envoyer</span><button type="button" onClick={onSend} disabled={!composer.trim() || sending} className={cx("inline-flex h-9 items-center gap-2 rounded-lg px-4 text-[10px] font-black disabled:cursor-not-allowed disabled:opacity-40", noteMode ? "bg-violet-600 text-white" : "bg-rose-600 text-white")}><Send className="h-3.5 w-3.5" />{sending ? "Traitement…" : noteMode ? "Conserver" : "Envoyer"}</button></div>
      </div>
    </div>
  </div></div>
}
...
function DockCard({ eyebrow, title, icon: Icon, children }: { eyebrow: string; title: string; icon: typeof Sparkles; children: React.ReactNode }) { return <div className="rounded-[16px] border border-slate-200 bg-white p-3.5"><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#071022] text-white"><Icon className="h-4 w-4" /></div><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-slate-500">{eyebrow}</p><h3 className="mt-1 text-[12px] font-black text-slate-950">{title}</h3></div></div><div className="mt-4">{children}</div></div> }
...
function Signal({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-[10px] font-black uppercase tracking-[.12em] text-slate-500">{label}</p><p className="mt-1 text-[10px] font-black text-slate-950">{value}</p></div> }
