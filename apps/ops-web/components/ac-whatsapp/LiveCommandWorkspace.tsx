...
export default function LiveCommandWorkspace() {
  ...
  return <div className="space-y-5">
    ...
    <section className="grid min-h-[720px] overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_22px_70px_rgba(15,23,42,.07)] xl:h-[calc(100dvh-190px)] xl:min-h-[640px] xl:max-h-[920px] xl:grid-cols-[350px_minmax(500px,1fr)_340px]">
      <aside className="flex min-h-0 flex-col overflow-hidden border-b border-slate-200 bg-slate-50/70 xl:border-b-0 xl:border-r">
        ...
        <div className="flex shrink-0 items-center justify-between px-4 py-3"><p className="text-[9px] font-black uppercase tracking-[.15em] text-slate-600">File intelligente</p><span className="rounded-full bg-white px-2.5 py-1 text-[9px] font-black text-slate-900 ring-1 ring-slate-300">{filtered.length}</span></div>
        <div className="max-h-[610px] overflow-y-auto overscroll-contain px-2 pb-3 [scrollbar-gutter:stable] xl:min-h-0 xl:max-h-none xl:flex-1">{filtered.length ? filtered.map((row) => <ConversationRow key={row.id} row={row} active={row.id === selectedId} onClick={() => setSelectedId(row.id)} />) : <EmptyState compact title="Aucune conversation" description="Cette file se remplira automatiquement selon les états live et les filtres sélectionnés." icon={Filter} />}</div>
      </aside>

      <main className="flex min-h-0 min-w-0 flex-col overflow-hidden border-b border-slate-200 xl:border-b-0 xl:border-r">
        {selected ? <>
          <div className="relative flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-4">
...
function MessageBubble({ message }: { message: AcWhatsAppMessage }) {
  ...
  return <div className={cx("flex", outbound ? "justify-end" : "justify-start")}>
    <div className={cx("max-w-[84%] rounded-[22px] border px-4 py-3 shadow-sm", outbound ? "rounded-br-md border-slate-900 bg-slate-950 text-white" : internal ? "border-amber-400 bg-amber-100 text-slate-950" : "rounded-bl-md border-slate-300 bg-white text-slate-950")}>
      ...
      {voice ? <VoiceMessagePlayer message={message} inverted={outbound} /> : visualMedia ? <MessageAttachmentPreview message={message} inverted={outbound} /> : <div className="whitespace-pre-wrap text-[12px] font-semibold leading-6">{message.body || message.caption || `[${message.message_type}]`}</div>}
      {(voice || visualMedia) && (message.caption || message.body) ? <p className={cx("mt-2 text-[9px] font-semibold", outbound ? "text-slate-300" : "text-slate-700")}>{message.caption || message.body}</p> : null}
