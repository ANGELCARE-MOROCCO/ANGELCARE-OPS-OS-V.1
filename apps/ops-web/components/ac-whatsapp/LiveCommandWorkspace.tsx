...
export default function LiveCommandWorkspace() {
  ...
  return <div className="space-y-2">
    ...
    <section className={cx("acw-live-theatre grid overflow-hidden rounded-[22px] border border-slate-200/90 bg-white shadow-[0_1px_2px_rgba(7,20,38,.03),0_22px_70px_rgba(7,20,38,.08)]", focusMode ? "xl:grid-cols-1" : queueCollapsed && dockCollapsed ? "xl:grid-cols-[76px_minmax(0,1fr)_76px]" : queueCollapsed ? "xl:grid-cols-[76px_minmax(0,1fr)_350px]" : dockCollapsed ? "xl:grid-cols-[370px_minmax(0,1fr)_76px]" : "xl:grid-cols-[370px_minmax(620px,1fr)_350px]")} data-mz7-theatre="true" style={mz7Layout.gridStyle}>
      {!focusMode ? <aside className="acw-queue-panel flex min-h-0 flex-col overflow-hidden border-b border-slate-200 bg-[#f6f8fb] xl:border-b-0 xl:border-r">
        ...
        {!queueCollapsed ? <>
          <div className="shrink-0 border-b border-slate-200/90 bg-white/55 p-3">
            ...
            <div className="mt-2 flex gap-1 overflow-x-auto pb-1">{smartViews.map((item) => { const Icon = item.icon; return <button key={item.id} type="button" onClick={() => setView(item.id)} className={cx("flex shrink-0 items-center gap-1.5 rounded-lg border px-2.5 py-1.5 text-[9px] font-black", view === item.id ? "border-slate-950 bg-slate-950 text-white" : "border-slate-300 bg-white text-slate-800 hover:border-slate-500")}><Icon className="h-3.5 w-3.5" />{item.label}</button> })}</div>
          </div>
...
function TheatreConversationRow({ row, active, onClick }: { row: AcWhatsAppConversation; active: boolean; onClick: () => void }) {
  ...
  const owner = String((row.assigned_user as any)?.display_name || (row.assigned_user as any)?.full_name || (row.assigned_user as any)?.name || (row.assigned_user as any)?.email || "").trim()
  const statusLabel = row.status === "waiting_customer" ? "Attente client" : row.status === "escalated" ? "Escalade" : row.status === "new" ? "Nouvelle" : row.status === "in_progress" ? "En traitement" : row.assigned_user_id ? "Attribuée" : "À attribuer"
  const labels = (row.labels || []).map((entry) => (entry?.label || {}) as Record<string, any>).filter((label) => label && (label.name || label.code)).slice(0, 2)
  const accent = row.status === "escalated" ? "bg-rose-500" : row.status === "waiting_customer" ? "bg-amber-400" : row.unread_count > 0 ? "bg-sky-500" : !row.assigned_user_id ? "bg-orange-400" : "bg-emerald-500"
  const stateTone = row.status === "escalated" ? "border-rose-200 bg-rose-50 text-rose-800" : row.status === "waiting_customer" ? "border-amber-200 bg-amber-100 text-slate-950" : row.status === "new" ? "border-violet-200 bg-violet-50 text-violet-900" : !row.assigned_user_id ? "border-orange-200 bg-orange-50 text-orange-900" : "border-sky-200 bg-sky-50 text-sky-900"
  const direction = String(row.last_message_direction || "").toLowerCase()
  return <button type="button" onClick={onClick} aria-current={active ? "true" : undefined} className={cx("group relative mb-2 w-full overflow-hidden rounded-[16px] border px-3 py-3 text-left transition-[background,border-color,box-shadow,transform] duration-150", active ? "border-slate-950 bg-[linear-gradient(145deg,#071426_0%,#0a1830_72%,#102442_100%)] text-white shadow-[0_12px_30px_rgba(7,20,38,.18)]" : "border-slate-200/90 bg-white text-slate-950 shadow-[0_1px_2px_rgba(7,20,38,.025)] hover:-translate-y-px hover:border-slate-300 hover:shadow-[0_10px_24px_rgba(7,20,38,.08)]") }>
    <span className={cx("absolute inset-y-0 left-0 w-1", accent)} />
    <div className="flex gap-3 pl-0.5">
      <div className={cx("relative grid h-11 w-11 shrink-0 place-items-center rounded-[14px] text-[10px] font-black", active ? "border border-white/10 bg-white/10 text-white shadow-inner" : "border border-slate-200 bg-[linear-gradient(145deg,#f8fafc,#eef2f7)] text-slate-800")}>
        {initials(row.contact?.display_name)}
        {row.unread_count > 0 ? <span className={cx("absolute -right-1.5 -top-1.5 grid h-5 min-w-5 place-items-center rounded-full border-2 px-1 text-[9px] font-black", active ? "border-[#071426] bg-rose-500 text-white" : "border-white bg-rose-600 text-white")}>{row.unread_count}</span> : null}
      </div>
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-[11px] font-black tracking-[-.01em]">{row.contact?.display_name || row.contact?.phone_number_e164 || "Contact à identifier"}</p><p className={cx("mt-0.5 truncate text-[9.5px] font-bold", active ? "text-slate-300" : "text-slate-500")}>{row.contact?.organization_name || row.queue?.name || row.account?.name || "Relation à qualifier"}</p></div><span className={cx("shrink-0 pt-0.5 text-[9px] font-black tabular-nums", active ? "text-slate-300" : "text-slate-400")}>{formatRelative(row.last_message_at)}</span></div>
        <p className={cx("mt-2 line-clamp-2 min-h-[30px] text-[9.5px] font-semibold leading-[15px]", active ? "text-slate-100" : "text-slate-700")}>{row.last_message_preview || "Nouvelle conversation"}</p>
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          <span className={cx("inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[9px] font-black", active ? row.status === "waiting_customer" ? "border-amber-300 bg-amber-300 text-slate-950" : "border-white/15 bg-white/10 text-white" : stateTone)}><span className={cx("h-1.5 w-1.5 rounded-full", active && row.status !== "waiting_customer" ? "bg-white/80" : accent)} />{statusLabel}</span>
          {row.priority && ["high","urgent","vip","critical"].includes(String(row.priority).toLowerCase()) ? <span className={cx("rounded-full border px-2 py-1 text-[9px] font-black", active ? "border-rose-300/40 bg-rose-400/15 text-rose-100" : "border-rose-200 bg-rose-50 text-rose-800")}>{humanValue(row.priority, "Prioritaire")}</span> : null}
          {labels.map((label, index) => <span key={String(label.id || label.code || label.name || index)} className={cx("inline-flex max-w-[120px] items-center gap-1 rounded-full border px-2 py-1 text-[9px] font-black", active ? "border-white/15 bg-white/[.07] text-slate-100" : "border-slate-200 bg-slate-50 text-slate-700")}><span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: String(label.color || "#64748b") }} /><span className="truncate">{String(label.name || label.code || "Tag")}</span></span>)}
        </div>
        <div className={cx("mt-2 flex items-center justify-between gap-2 border-t pt-2", active ? "border-white/10" : "border-slate-100")}>
          <div className={cx("flex min-w-0 items-center gap-1.5 text-[9px] font-black", active ? "text-sky-200" : owner ? "text-slate-700" : "text-orange-700")}><UserRoundCheck className="h-3 w-3 shrink-0" /><span className="truncate">{owner || "Non attribuée"}</span></div>
          <div className={cx("flex shrink-0 items-center gap-1.5 text-[9px] font-black uppercase tracking-[.08em]", active ? "text-slate-400" : "text-slate-400")}>{metadata.pinned ? <Bookmark className={cx("h-3 w-3", active ? "text-amber-300" : "text-amber-600")} /> : null}<span>{direction === "inbound" ? "Entrant" : direction === "outbound" ? "Sortant" : row.account?.name || "WhatsApp"}</span></div>
        </div>
      </div>
    </div>
  </button>
...
function ConversationCommandBar({ selected, actorName, focusMode, messageSearch, messageSearchRef, onMessageSearch, onJumpSearch, onFocus, onResolve, onAssign, onFollowup, onOpenTools, menuOpen, onMenu, menu }: { selected: AcWhatsAppConversation; actorName: string; focusMode: boolean; messageSearch: string; messageSearchRef: React.RefObject<HTMLInputElement | null>; onMessageSearch: (value: string) => void; onJumpSearch: () => void; onFocus: () => void; onResolve: () => void; onAssign: () => void; onFollowup: () => void; onOpenTools: () => void; menuOpen: boolean; onMenu: () => void; menu: React.ReactNode }) {
  const owner = String((selected.assigned_user as any)?.display_name || (selected.assigned_user as any)?.full_name || actorName)
  return <div className="relative z-20 shrink-0 border-b border-slate-200 bg-[linear-gradient(180deg,#ffffff_0%,#fbfdff_100%)] px-3.5 py-3 shadow-[0_8px_22px_rgba(7,20,38,.035)]">
    <div className="flex flex-wrap items-center justify-between gap-2.5">
      <div className="flex min-w-0 items-center gap-2.5">
        <div className="grid h-11 w-11 shrink-0 place-items-center rounded-[14px] bg-[linear-gradient(145deg,#071426,#102442)] text-[10px] font-black text-white shadow-[0_8px_18px_rgba(7,20,38,.16)]">{initials(selected.contact?.display_name)}</div>
        <div className="min-w-0"><div className="flex flex-wrap items-center gap-1.5"><h2 className="truncate text-[13px] font-black tracking-[-.02em] text-slate-950">{selected.contact?.display_name || selected.contact?.phone_number_e164 || "Contact à identifier"}</h2><StatusPill status={selected.status} compact />{["high","urgent","vip"].includes(String(selected.priority || "")) ? <StatusPill status={selected.priority || "normal"} compact /> : null}</div><p className="mt-0.5 truncate text-[10px] font-semibold text-slate-500">{[selected.contact?.organization_name, selected.contact?.phone_number_e164, selected.account?.name, `Responsable · ${owner}`].filter(Boolean).join("  ·  ")}</p></div>
      </div>
      <div className="flex flex-wrap items-center gap-1.5">
        <div className="relative hidden xl:block"><Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400" /><input ref={messageSearchRef} value={messageSearch} onChange={(event) => onMessageSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") onJumpSearch() }} placeholder="Rechercher dans le fil" className="h-8 w-48 rounded-lg border border-slate-200 bg-slate-50 pl-8 pr-2.5 text-[9px] font-bold text-slate-950 outline-none focus:border-slate-500 focus:bg-white" /></div>
        <button type="button" onClick={onAssign} className="hidden h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 text-[10px] font-black text-slate-700 hover:bg-slate-50 lg:inline-flex"><UserRoundCog className="h-3.5 w-3.5" />Attribuer</button>
        <button type="button" onClick={onFollowup} className="hidden h-8 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 text-[10px] font-black text-slate-700 hover:bg-slate-50 lg:inline-flex"><AlarmClock className="h-3.5 w-3.5" />Relance</button>
        <button type="button" onClick={onOpenTools} className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-50" title="Commandes professionnelles"><Command className="h-3.5 w-3.5" /></button>
        <button type="button" onClick={onFocus} className={cx("grid h-8 w-8 place-items-center rounded-lg border", focusMode ? "border-slate-950 bg-slate-950 text-white" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50")} title="Focus conversation"><LayoutDashboard className="h-3.5 w-3.5" /></button>
        <button type="button" onClick={onResolve} className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 text-[10px] font-black text-emerald-900 hover:bg-emerald-100"><CheckCheck className="h-3.5 w-3.5" />{selected.status === "resolved" ? "Rouvrir" : "Résoudre"}</button>
        <div className="relative"><button type="button" aria-expanded={menuOpen} onClick={onMenu} className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 bg-white text-slate-700 hover:bg-slate-50"><MoreHorizontal className="h-3.5 w-3.5" /></button>{menu}</div>
      </div>
    </div>
  </div>
}
...
function TheatreMessageBubble({ message, onQuote, onArtifact, onTranslate }: { message: AcWhatsAppMessage; onQuote: (message: AcWhatsAppMessage) => void; onArtifact: (type: string, message: AcWhatsAppMessage) => void; onTranslate: (message: AcWhatsAppMessage) => void }) {
  ...
  return <div tabIndex={-1} data-message-id={message.id} data-message-direction={message.direction} data-message-search={searchable} className={cx("group flex scroll-mt-20 outline-none focus-visible:ring-2 focus-visible:ring-sky-500/40", outbound ? "justify-end" : "justify-start")} >
    <div className={cx("acw-message-bubble relative max-w-[82%] rounded-[20px] border px-4 py-3 shadow-[0_1px_2px_rgba(7,20,38,.04),0_12px_30px_rgba(7,20,38,.055)]", outbound ? "acw-message-outbound rounded-br-[6px] border-[#071426] bg-[linear-gradient(145deg,#071426_0%,#0b1830_74%,#102442_100%)] text-white" : internal ? "rounded-bl-[6px] border-violet-300 bg-[linear-gradient(145deg,#faf5ff,#f5f3ff)] text-slate-950" : "acw-message-inbound rounded-bl-[6px] border-slate-200 bg-white text-slate-950")}>
      ...
      {open ? <div role="menu" className="absolute right-2 top-10 z-[140] w-64 rounded-[16px] border border-slate-200 bg-white p-1.5 text-slate-950 shadow-[0_24px_70px_rgba(7,20,38,.22)] acw-floating-surface acw-message-menu"><MessageAction label="Répondre / citer" icon={MessageSquareQuote} onClick={() => { onQuote(message); setOpen(false) }} /><MessageAction label="Traduire" icon={Languages} onClick={() => { onTranslate(message); setOpen(false) }} /><MessageAction label="Épingler comme jalon" icon={Milestone} onClick={() => { onArtifact("milestone", message); setOpen(false) }} /><MessageAction label="Extraire un engagement" icon={ClipboardCheck} onClick={() => { onArtifact("commitment", message); setOpen(false) }} /><MessageAction label="Marquer comme preuve" icon={Eye} onClick={() => { onArtifact("evidence", message); setOpen(false) }} /><MessageAction label="Créer une mission" icon={ListChecks} onClick={() => { onArtifact("task", message); setOpen(false) }} /><MessageAction label="Copier" icon={Copy} onClick={() => { void navigator.clipboard.writeText(String(message.body || message.caption || "")); setOpen(false) }} /></div> : null}
      <div className={cx("mb-2.5 flex flex-wrap items-center gap-1.5 border-b pb-2 pr-8", outbound ? "border-white/15" : internal ? "border-violet-200" : "border-slate-200")}><span className={cx("text-[10px] font-black", outbound ? "text-sky-200" : "text-slate-950")}>{identity.display_name}</span><span className={cx("text-[10px] font-bold", outbound ? "text-slate-300" : "text-slate-600")}>{identity.role}</span>{internal ? <span className="rounded-md bg-violet-200 px-2 py-1 text-[10px] font-black text-violet-950">INTERNE · INVISIBLE AU CONTACT</span> : null}</div>
      {voice ? <VoiceMessagePlayer message={message} inverted={outbound} /> : visualMedia ? <MessageAttachmentPreview message={message} inverted={outbound} /> : readableBody ? <div className="whitespace-pre-wrap text-[12.5px] font-semibold leading-[1.62]">{readableBody}</div> : null}
...
function ResponseCommandDeck({ selected, actorName, composerRef, composer, setComposer, noteMode, setNoteMode, sending, draftStatus, onSend, onAi, onFollowup, onOpenResponses, onToggleAutomation, onOpenTools, onSent, onNotice }: { selected: AcWhatsAppConversation; actorName: string; composerRef: React.RefObject<HTMLTextAreaElement | null>; composer: string; setComposer: (value: string) => void; noteMode: boolean; setNoteMode: (value: boolean) => void; sending: boolean; draftStatus: string; onSend: () => void; onAi: (action: string) => void; onFollowup: () => void; onOpenResponses: () => void; onToggleAutomation: () => void; onOpenTools: () => void; onSent: () => Promise<void>; onNotice: (tone: "success" | "danger" | "warning" | "info", title: string, description: string) => void }) {
  const draftLabel = draftStatus === "saved" ? "Brouillon sauvegardé" : draftStatus === "saving" ? "Sauvegarde…" : "Brouillon prêt"
  return <div className="acw-composer-dock shrink-0 border-t border-slate-200 bg-white/94 px-3.5 py-3 backdrop-blur-sm"><div className="mx-auto max-w-[1120px]">
    <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2 px-0.5">
      <div className="flex items-center gap-1"><button type="button" onClick={() => setNoteMode(false)} className={cx("rounded-lg px-2.5 py-1.5 text-[10px] font-black", !noteMode ? "bg-slate-950 text-white" : "text-slate-500 hover:bg-white")}>Réponse client</button><button type="button" onClick={() => setNoteMode(true)} className={cx("rounded-lg px-2.5 py-1.5 text-[10px] font-black", noteMode ? "bg-violet-600 text-white" : "text-slate-500 hover:bg-white")}>Note interne</button>{!selected.automation_paused ? <span className="ml-1 hidden items-center gap-1 rounded-md bg-emerald-50 px-2 py-1 text-[10px] font-black text-emerald-800 sm:inline-flex"><Bot className="h-3 w-3" />Automatisation active</span> : <span className="ml-1 hidden items-center gap-1 rounded-md bg-amber-50 px-2 py-1 text-[10px] font-black text-amber-900 sm:inline-flex"><Bot className="h-3 w-3" />Automatisation suspendue</span>}</div>
      <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400"><span>{actorName}</span><span>·</span><span>{selected.account?.name || "Compte WhatsApp"}</span><span>·</span><span className={cx(draftStatus === "saved" ? "text-emerald-700" : draftStatus === "saving" ? "text-amber-700" : "text-slate-400")}>{draftLabel}</span></div>
    </div>
    <div className={cx("overflow-hidden rounded-[18px] border bg-white shadow-[0_1px_2px_rgba(7,20,38,.04),0_14px_34px_rgba(7,20,38,.07)] transition-shadow focus-within:shadow-[0_1px_2px_rgba(7,20,38,.04),0_18px_46px_rgba(14,116,196,.10)]", noteMode ? "border-violet-300" : "border-slate-300 focus-within:border-sky-400")}>
      <textarea ref={composerRef} value={composer} onChange={(event) => { const next = event.target.value; setComposer(next); if (!noteMode && next.trimStart().startsWith("/")) onOpenResponses() }} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") onSend() }} rows={3} placeholder={noteMode ? "Note interne — invisible au contact" : "Écrire une réponse…  Tapez / pour rechercher une réponse enregistrée"} className={cx("max-h-52 min-h-[86px] w-full resize-y bg-transparent px-4 py-3.5 text-[13px] font-semibold leading-6 text-slate-950 outline-none placeholder:text-slate-400", noteMode && "bg-violet-50/40")} />
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 bg-white px-2 py-1.5">
        <div className="flex flex-wrap items-center gap-0.5"><AttachmentMessageStudio conversationId={selected.id} disabled={noteMode || sending} onSent={onSent} onSuccess={(title, description) => onNotice("success", title, description)} onQueued={(title, description) => onNotice("warning", title, description)} onError={(title, description) => onNotice("danger", title, description)} /><ComposerButton icon={FileText} label="Réponses" onClick={onOpenResponses} /><VoiceMessageStudio conversationId={selected.id} disabled={noteMode || sending} onSent={onSent} onSuccess={(title, description) => onNotice("success", title, description)} onQueued={(title, description) => onNotice("warning", title, description)} onError={(title, description) => onNotice("danger", title, description)} /><ComposerButton icon={WandSparkles} label="Studio IA" onClick={() => onAi("reply_matrix")} /><ComposerButton icon={AlarmClock} label="Relance" onClick={onFollowup} /><button type="button" onClick={onToggleAutomation} className={cx("inline-flex items-center gap-1.5 rounded-lg px-2.5 py-2 text-[10px] font-black hover:bg-slate-50", selected.automation_paused ? "text-amber-800" : "text-emerald-800")}><Bot className="h-3.5 w-3.5" />{selected.automation_paused ? "Reprendre auto" : "Prendre la main"}</button><ComposerButton icon={SlidersHorizontal} label="Plus" onClick={onOpenTools} /></div>
        <div className="flex items-center gap-2"><span className="hidden text-[10px] font-bold text-slate-400 sm:inline">⌘↵ envoyer</span><button type="button" onClick={onSend} disabled={!composer.trim() || sending} className={cx("inline-flex h-10 items-center gap-2 rounded-xl px-5 text-[10px] font-black shadow-[0_8px_18px_rgba(238,54,92,.16)] transition hover:-translate-y-px disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0", noteMode ? "bg-violet-600 text-white" : "bg-rose-600 text-white")}><Send className="h-3.5 w-3.5" />{sending ? "Traitement…" : noteMode ? "Conserver" : "Envoyer"}</button></div>
      </div>
    </div>
  </div></div>
}
...
function IntelligenceDock({ tab, selected, detail, theatre, latestInbound, relationshipScore, onAi, onArtifact, onAssign, onFollowup, onUpdate }: { tab: DockTab; selected: AcWhatsAppConversation; detail: ConversationDetail | null; theatre: TheatreData; latestInbound?: AcWhatsAppMessage; relationshipScore: number; onAi: (action: string) => void; onArtifact: (type: string, title: string) => void; onAssign: () => void; onFollowup: () => void; onUpdate: (patch: Record<string, unknown>, success: string) => void }) {
  ...
  return <div className="space-y-3"><DockCard eyebrow="Présence" title="Équipe active" icon={Radio}>{theatre.presence.length ? <div className="space-y-2">{theatre.presence.map((presence) => <div key={presence.user_id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-3"><div><p className="text-[9px] font-black text-slate-950">{presence.display_name_snapshot || "Utilisateur AngelCare"}</p><p className="mt-1 text-[10px] font-semibold text-slate-500">{presence.role_snapshot || presence.activity || "Actif"}</p></div><span className="h-2.5 w-2.5 rounded-full bg-emerald-500" /></div>)}</div> : <p className="text-[9px] font-semibold text-slate-600">Vous êtes le seul utilisateur actif sur cette conversation.</p>}</DockCard><DockCard eyebrow="Artifacts & audit" title="Chronologie structurée" icon={Workflow}><div className="space-y-2">{theatre.artifacts.slice(0, 10).map((artifact) => <div key={artifact.id} className="rounded-xl border border-slate-200 bg-white p-3"><div className="flex items-start justify-between gap-2"><p className="text-[9px] font-black text-slate-950">{artifact.title}</p><StatusPill status={artifact.status || "open"} compact /></div><p className="mt-1 text-[10px] font-semibold text-slate-500">{artifact.artifact_type.replaceAll("_", " ")} · {formatRelative(artifact.created_at)}</p></div>)}{!theatre.artifacts.length ? <p className="text-[9px] font-semibold text-slate-600">Aucun jalon structuré. Utilisez les commandes avancées.</p> : null}</div></DockCard></div>
}

function DockCard({ eyebrow, title, icon: Icon, children }: { eyebrow: string; title: string; icon: typeof Sparkles; children: React.ReactNode }) { return <div className="acw-dock-card relative overflow-hidden rounded-[18px] border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(7,20,38,.025),0_10px_28px_rgba(7,20,38,.045)]"><div className="pointer-events-none absolute -right-10 -top-12 h-28 w-28 rounded-full bg-sky-100/45 blur-2xl" /><div className="relative flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-[13px] bg-[linear-gradient(145deg,#071426,#102442)] text-white shadow-[0_8px_18px_rgba(7,20,38,.14)]"><Icon className="h-4 w-4" /></div><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-slate-500">{eyebrow}</p><h3 className="mt-1 text-[12.5px] font-black tracking-[-.01em] text-slate-950">{title}</h3></div></div><div className="relative mt-4">{children}</div></div> }
function DockAction({ label, icon: Icon, onClick }: { label: string; icon: typeof Sparkles; onClick: () => void }) { return <button type="button" onClick={onClick} className="group flex w-full items-center justify-between rounded-xl border border-slate-200 bg-[linear-gradient(180deg,#ffffff,#f8fafc)] px-3 py-2.5 text-left text-[9.5px] font-black text-slate-800 shadow-[0_1px_1px_rgba(7,20,38,.02)] transition hover:-translate-y-px hover:border-sky-200 hover:bg-white hover:text-slate-950 hover:shadow-[0_8px_18px_rgba(7,20,38,.06)]"><span className="flex items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-lg bg-slate-100 text-slate-700 group-hover:bg-sky-50 group-hover:text-sky-700"><Icon className="h-3.5 w-3.5" /></span>{label}</span><ChevronRight className="h-3.5 w-3.5 text-slate-300 group-hover:text-sky-600" /></button> }
function Signal({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-slate-200 bg-[linear-gradient(145deg,#ffffff,#f7f9fc)] p-3 shadow-[0_1px_1px_rgba(7,20,38,.02)]"><p className="text-[9px] font-black uppercase tracking-[.12em] text-slate-500">{label}</p><p className="mt-1.5 text-[11px] font-black text-slate-950">{value}</p></div> }
...
function FeatureCommandCenter({ onClose, onExecute }: { onClose: () => void; onExecute: (feature: FeatureDefinition) => void }) {
  ...
  return <div className="fixed inset-0 z-[90] flex justify-center bg-slate-950/42 p-4 pt-[5vh] backdrop-blur-[3px]"><button type="button" className="absolute inset-0" onClick={onClose} aria-label="Fermer" /><div className="acw-apex-floating-surface relative flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-[20px] border bg-white">
