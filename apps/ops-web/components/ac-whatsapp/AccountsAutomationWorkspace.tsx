...
import { Activity, AlertTriangle, Bot, CheckCircle2, CirclePause, CirclePlay, CloudCog, Copy, KeyRound, Link2, MessageSquareText, PlugZap, Plus, QrCode, RadioTower, RefreshCw, ServerCog, ShieldCheck, Smartphone, Unplug, Webhook, Wifi, Workflow, Pencil, Trash2, RotateCcw, } from "lucide-react";
import type { AcWhatsAppAccount } from "@/lib/ac-whatsapp/types";
import { cx, DetailRow, EmptyState, HealthBadge, LoadingPanel, Metric, ModalFrame, NoticeBanner, ProgressBar, ReasonConfirmDialog, SectionTitle, StatusPill, Surface, SurfaceHeader, WorkspaceTabs, } from "./ACWhatsAppUI";
import { acApi, formatRelative, friendlyAcError, percentage, useAcWhatsApp } from "./useAcWhatsApp";
import ResponseLibraryManager from "./ResponseLibraryManager";
import AutomationRuleStudioPro from "./AutomationRuleStudioPro";
import MediaVaultRetentionWorkspace from "./MediaVaultRetentionWorkspace";
...
export default function AccountsAutomationWorkspace() {
    ...
    useEffect(() => { if (!selectedId && accounts[0])
        setSelectedId(accounts[0].id); }, [accounts, selectedId]);
    useEffect(() => {
        const viewToTab: Record<string, string> = {
            accounts: "accounts", runtime: "runtime", responses: "templates", templates: "templates",
            automation: "automation", "media-vault": "storage", storage: "storage",
        };
        if (requestedView && viewToTab[requestedView])
            setTab(viewToTab[requestedView]);
    }, [requestedView]);
    async function action(account: AcWhatsAppAccount, actionName: "start" | "stop" | "logout" | "qr" | "pairing" | "sync" | "pause" | "resume") {
        ...
        try {
            ...
            if (actionName === "qr" || actionName === "pairing") {
                setPairPayload({ action: actionName, account, result });
                setPairOpen(true);
            }
            await refresh();
            if (!["qr", "pairing"].includes(actionName))
                setNotice({ tone: "success", title: "Commande runtime exécutée", description: `La commande « ${actionName} » a été envoyée à OpenWA et le statut AngelCare a été synchronisé.` });
        }
        catch (cause) {
    ...
    if (loading && !data)
        return <LoadingPanel label="Ouverture de Accounts & Automation"/>;
    ...
    <SectionTitle eyebrow="Master Workspace 05 · Accounts & Automation" title="Piloter les comptes, les sessions et l’automatisation avec confiance." description="Provisionnement guidé, QR, santé OpenWA, capacités commerciales, modèles et webhooks sous une administration AngelCare lisible et traçable." action={<button type="button" onClick={() => setCreateOpen(true)} className="inline-flex h-10 items-center gap-2 rounded-xl bg-rose-600 px-3.5 text-[9px] font-black text-white"><Plus className="h-4 w-4"/>Connecter un compte</button>}/>
    ...
    <WorkspaceTabs active={tab} onChange={setTab} tabs={[
            ...
            { id: "templates", label: "Réponses enregistrées", icon: MessageSquareText, count: data?.templates.length || 0 },
            { id: "automation", label: "Auto Reply Studio", icon: Workflow },
            { id: "storage", label: "Media Vault & rétention", icon: CloudCog },
    ...
    {tab === "accounts" ? <div className="grid gap-5 2xl:grid-cols-[1fr_380px]">
      ...
      <AccountInspector account={selected} health={Boolean(data?.health.openwaReachable)} onAction={(account, name) => void action(account, name)} busy={busy} onEdit={() => setAccountEditOpen(true)} onChanged={async () => { await refresh(); setNotice({ tone: "success", title: "Compte mis à jour", description: "Le cycle de vie du compte a été modifié et audité." }) }}/>
    </div> : null}

    {tab === "templates" ? <ResponseLibraryManager /> : null}
    {tab === "automation" ? <AutomationRuleStudioPro accounts={accounts} /> : null}
    {tab === "storage" ? <MediaVaultRetentionWorkspace accounts={accounts} /> : null}
    {tab === "runtime" ? <RuntimeCockpit data={data} onRefresh={() => void refresh()}/> : null}

    {accountEditOpen && selected && data ? <AccountSettingsModal account={selected} queues={data.queues} onClose={() => setAccountEditOpen(false)} onSaved={async () => { setAccountEditOpen(false); await refresh(); setNotice({ tone: "success", title: "Compte enregistré", description: "La configuration persistée a été mise à jour." }) }} /> : null}
...
function AccountSettingsModal({ account, queues, onClose, onSaved }: { account: AcWhatsAppAccount; queues: Array<{ id: string; name: string }>; onClose: () => void; onSaved: () => void }) {
    const [form, setForm] = useState({ id: account.id, name: account.name, phone_number_e164: account.phone_number_e164 || "", department: account.department || "", purpose: account.purpose || "", default_queue_id: account.default_queue_id || "", outbound_enabled: account.outbound_enabled, campaigns_enabled: account.campaigns_enabled, cold_prospecting_enabled: account.cold_prospecting_enabled, bulk_messaging_enabled: account.bulk_messaging_enabled });
    const [busy, setBusy] = useState(false);
    const [notice, setNotice] = useState<Notice | null>(null);
    async function save() { setBusy(true); try { await acApi("/api/ac-whatsapp/accounts", { method: "PATCH", body: JSON.stringify(form) }); onSaved(); } catch (cause) { setNotice({ ...friendlyAcError(cause), tone: "danger" }); } finally { setBusy(false); } }
    return <ModalFrame wide title="Configurer le compte" eyebrow="Account lifecycle" description="Les paramètres sont persistés. Le retrait du compte est séparé et protégé par analyse d’impact." onClose={onClose} footer={<button type="button" disabled={busy || !form.name.trim()} onClick={() => void save()} className="rounded-xl bg-slate-950 px-4 py-2.5 text-[9px] font-black text-white disabled:opacity-40">Enregistrer</button>}>{notice ? <NoticeBanner tone={notice.tone || "danger"} title={notice.title} description={notice.description}/> : null}<div className="account-editor grid gap-4 md:grid-cols-2"><EditField label="Nom"><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}/></EditField><EditField label="Numéro E.164"><input value={form.phone_number_e164} onChange={(e) => setForm({ ...form, phone_number_e164: e.target.value })}/></EditField><EditField label="Département"><input value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })}/></EditField><EditField label="File par défaut"><select value={form.default_queue_id} onChange={(e) => setForm({ ...form, default_queue_id: e.target.value })}><option value="">Aucune</option>{queues.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></EditField><div className="md:col-span-2"><EditField label="Usage principal"><input value={form.purpose} onChange={(e) => setForm({ ...form, purpose: e.target.value })}/></EditField></div>{([['outbound_enabled','Outbound'],['campaigns_enabled','Campagnes'],['cold_prospecting_enabled','Prospection'],['bulk_messaging_enabled','Bulk']] as const).map(([key,label]) => <label key={key} className="flex items-center justify-between rounded-2xl border border-slate-200 p-3"><span className="text-[9px] font-black text-slate-800">{label}</span><input type="checkbox" checked={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.checked })}/></label>)}</div><style jsx global>{`.account-editor input,.account-editor select{width:100%;border:1px solid #e2e8f0;border-radius:14px;background:#fff;padding:11px 12px;font-size:11px;font-weight:700;color:#0f172a;outline:none}`}</style></ModalFrame>;
}

function accountStateMeta(status: string) {
    const normalized = String(status || "draft").toLowerCase();
    if (normalized === "connected")
        return { label: "Connecté", detail: "Transport actif", tone: "emerald" as const, icon: Wifi };
    if (["qr_required", "pairing_required"].includes(normalized))
        return { label: "QR requis", detail: "Authentification attendue", tone: "amber" as const, icon: QrCode };
    if (["starting", "authenticating"].includes(normalized))
        return { label: "Connexion en cours", detail: "OpenWA initialise la session", tone: "blue" as const, icon: RefreshCw };
    if (normalized === "paused")
        return { label: "En pause", detail: "Transport suspendu", tone: "violet" as const, icon: CirclePause };
    if (["error", "degraded", "authentication_lost"].includes(normalized))
        return { label: "À rétablir", detail: "Intervention requise", tone: "rose" as const, icon: AlertTriangle };
    if (normalized === "disconnected")
        return { label: "Déconnecté", detail: "Session inactive", tone: "slate" as const, icon: Unplug };
    return { label: "À configurer", detail: "Session non activée", tone: "slate" as const, icon: Smartphone };
}
function AccountCard({ account, selected, busy, onSelect, onAction }: {
    account: AcWhatsAppAccount;
    selected: boolean;
    busy: boolean;
    onSelect: () => void;
    onAction: (action: "start" | "stop" | "qr" | "pairing" | "sync" | "pause" | "resume" | "logout") => void;
}) {
    ...
    <button type="button" onClick={onSelect} className="w-full rounded-2xl text-left outline-none focus-visible:ring-2 focus-visible:ring-sky-400 focus-visible:ring-offset-4 focus-visible:ring-offset-[#07111f]">
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-3.5">
          <div className={cx("grid h-12 w-12 shrink-0 place-items-center rounded-[18px] border", selected ? "border-white/10 bg-white/[.07] text-sky-200" : "border-slate-200 bg-slate-50 text-slate-700")}><Smartphone className="h-5 w-5"/></div>
      ...
      <div className={cx("mt-5 rounded-xl border p-3.5", selected ? "border-white/10 bg-white/[.045]" : "border-slate-200 bg-slate-50/80")}>
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className={cx("text-[10px] font-black uppercase tracking-[.16em]", selected ? "text-slate-400" : "text-slate-500")}>Configuration du compte</p>
    ...
    <div className={cx("mt-5 flex flex-wrap items-center gap-2 border-t pt-4", selected ? "border-white/10" : "border-slate-200")}>
      ...
      {busy ? <span className={cx("ml-auto inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[.12em]", selected ? "text-sky-200" : "text-sky-700")}><RefreshCw className="h-3 w-3 animate-spin"/>Commande en cours</span> : null}
    </div>
  </article>;
}
function AccountStateBadge({ status }: {
    status: string;
}) {
    ...
    return <span className={cx("inline-flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-2 text-[10px] font-black uppercase tracking-[.12em] shadow-sm", tone)}><Icon className="h-3.5 w-3.5"/>{state.label}</span>;
}
function AccountMetric({ label, value, selected }: {
    label: string;
    ...
    <p className={cx("mt-1.5 truncate text-[10px] font-black uppercase tracking-[.13em]", selected ? "text-slate-400" : "text-slate-500")}>{label}</p>
  </div>;
}
function AccountAction({ label, icon: Icon, selected, disabled, variant, onClick }: {
    label: string;
    icon: typeof QrCode;
    selected: boolean;
    disabled: boolean;
    ...
    const classes = variant === "primary"
        ...
        : variant === "secondary"
            ...
            : selected ? "border border-white/20 bg-white/[.08] text-white hover:bg-white/[.14]" : "border border-slate-300 bg-white text-slate-900 hover:border-slate-500 hover:bg-slate-100";
...
function AccountInspector({ account, health, onAction, busy, onEdit, onChanged }: {
    account: AcWhatsAppAccount | null;
    health: boolean;
    onAction: (account: AcWhatsAppAccount, action: "start" | "stop" | "qr" | "pairing" | "sync" | "pause" | "resume" | "logout") => void;
    busy: string | null;
    onEdit: () => void;
    ...
    if (!account)
        return <Surface><EmptyState compact title="Sélectionnez un compte" description="Le statut, la session et les commandes de provisioning apparaîtront ici." icon={RadioTower}/></Surface>;
    const activeAccount = account;
    const retired = Boolean((activeAccount.settings as any)?.retired);
    async function retireOrRestore() {
        if (retired) {
            await acApi("/api/ac-whatsapp/accounts", { method: "PATCH", body: JSON.stringify({ id: activeAccount.id, action: "restore", reason: "Restauration administrateur" }) });
            await onChanged();
            return;
        }
        setRetireOpen(true);
    }
    return <><div className="space-y-4"><Surface><SurfaceHeader eyebrow="Account command" title={account.name} icon={Smartphone} action={<HealthBadge good={health && account.status !== "error" && !retired} goodLabel={retired ? "Compte retiré" : "Gateway protégé"} badLabel="À vérifier"/>}/><div className="mt-4"><DetailRow label="Statut" value={<StatusPill status={retired ? "retired" : account.status} compact/>}/><DetailRow label="Numéro" value={account.phone_number_e164}/><DetailRow label="Session" value={account.openwa_session_name} mono/><DetailRow label="Engine" value={account.engine_type}/><DetailRow label="Dernière activité" value={formatRelative(account.last_activity_at)}/><DetailRow label="Dernière erreur" value={account.last_error || "Aucune"}/></div><div className="mt-4 flex gap-2"><button type="button" onClick={onEdit} className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 text-[10px] font-black text-slate-800"><Pencil className="h-3.5 w-3.5"/>Modifier</button><button type="button" onClick={() => void retireOrRestore()} className={cx("inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-[10px] font-black", retired ? "border-blue-300 bg-blue-50 text-blue-700" : "border-rose-300 bg-rose-50 text-rose-700")}>{retired ? <RotateCcw className="h-3.5 w-3.5"/> : <Trash2 className="h-3.5 w-3.5"/>}{retired ? "Restaurer" : "Retirer"}</button></div></Surface><Surface><SurfaceHeader eyebrow="Capabilities" title="Doctrine de communication" icon={ShieldCheck}/><div className="mt-4 grid grid-cols-2 gap-2"><Capability label="Outbound" active={account.outbound_enabled}/><Capability label="Campagnes" active={account.campaigns_enabled}/><Capability label="Prospection" active={account.cold_prospecting_enabled}/><Capability label="Bulk" active={account.bulk_messaging_enabled}/></div></Surface><Surface><SurfaceHeader eyebrow="Provisioning" title="Actions de session" icon={CloudCog}/><div className="mt-4 grid grid-cols-2 gap-2"><Control label="Afficher le QR" icon={QrCode} onClick={() => onAction(account, "qr")}/><Control label="Code d’appairage" icon={KeyRound} onClick={() => onAction(account, "pairing")}/><Control label="Synchroniser" icon={RefreshCw} onClick={() => onAction(account, "sync")}/><Control label={account.status === "connected" ? "Arrêter" : "Démarrer"} icon={account.status === "connected" ? Unplug : CirclePlay} onClick={() => onAction(account, account.status === "connected" ? "stop" : "start")}/></div>{busy?.startsWith(account.id) ? <p className="mt-3 text-[9px] font-black text-slate-600">Commande OpenWA en cours…</p> : null}</Surface></div>{retireOpen ? <ReasonConfirmDialog danger title="Retirer ce compte d’AC WhatsApp" description="La session sera retirée de l’exploitation active sans effacer l’historique de communication." confirmLabel="Retirer le compte" confirmationText="RETIRER COMPTE" onClose={() => setRetireOpen(false)} onConfirm={async (reason) => { setRetireOpen(false); await acApi("/api/ac-whatsapp/accounts", { method: "DELETE", body: JSON.stringify({ id: activeAccount.id, reason }) }); await onChanged(); }} /> : null}</>;
}
function Capability({ label, active }: {
    label: string;
    active: boolean;
}) { return <div className={cx("rounded-xl border p-3", active ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-slate-50")}><div className="flex items-center justify-between"><span className="text-[9px] font-black text-slate-700">{label}</span>{active ? <CheckCircle2 className="h-4 w-4 text-emerald-600"/> : <CirclePause className="h-4 w-4 text-slate-400"/>}</div></div>; }
function Control({ label, icon: Icon, onClick }: {
    label: string;
    icon: typeof QrCode;
    onClick: () => void;
}) { return <button type="button" onClick={onClick} className="rounded-2xl border border-slate-300 bg-white p-3 text-left text-slate-950 hover:border-slate-500 hover:bg-slate-50"><Icon className="h-4 w-4 text-slate-900"/><p className="mt-2 text-[9px] font-black text-slate-950">{label}</p></button>; }
function TemplateStudio({ templates }: {
    templates: Array<Record<string, unknown>>;
}) { return <Surface><SurfaceHeader eyebrow="Template studio" title="Bibliothèque de modèles gouvernés" icon={MessageSquareText} action={<button type="button" className="rounded-xl border border-slate-200 px-3 py-2 text-[9px] font-black text-slate-600">Nouveau modèle</button>}/><div className="mt-5 grid gap-3 lg:grid-cols-2 xl:grid-cols-3">{templates.length ? templates.map((template: any, index) => <div key={template.id || index} className="rounded-[24px] border border-slate-200 p-4"><div className="flex items-start justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-rose-600">{template.category || template.template_type || "Modèle"}</p><p className="mt-2 text-sm font-black text-slate-950">{template.name || template.title || `Modèle ${index + 1}`}</p></div><StatusPill status={template.status || "draft"} compact/></div><p className="mt-4 line-clamp-5 whitespace-pre-wrap rounded-2xl bg-slate-50 p-3 text-[10px] font-semibold leading-5 text-slate-600">{template.body || template.content || template.message_body || "Contenu non exposé dans ce résumé."}</p></div>) : <div className="lg:col-span-2 xl:col-span-3"><EmptyState title="Bibliothèque vide" description="Les modèles approuvés apparaîtront ici avec leur catégorie, langue, variables et statut." icon={MessageSquareText}/></div>}</div></Surface>; }
function AutomationStudio({ accounts }: {
    accounts: AcWhatsAppAccount[];
}) { const doctrines = [{ title: "Réponse entrante", detail: "Router vers la bonne file et attribuer selon capacité", icon: Workflow }, { title: "Relance sans réponse", detail: "Créer une tâche humaine après le délai défini", icon: Activity }, { title: "Signal commercial", detail: "Qualifier le contact et ouvrir une opportunité", icon: Bot }, { title: "Escalade risque", detail: "Alerter le superviseur et protéger l’historique", icon: ShieldCheck }]; return <Surface><SurfaceHeader eyebrow="Automation control" title="Doctrines d’automatisation explicables" icon={Workflow}/><div className="mt-5 grid gap-4 lg:grid-cols-2">{doctrines.map((item) => { const Icon = item.icon; return <div key={item.title} className="rounded-[24px] border border-slate-200 bg-slate-50 p-4"><div className="flex items-start justify-between"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-slate-950 text-white"><Icon className="h-4 w-4"/></div><StatusPill status="draft" label="À configurer" compact/></div><p className="mt-4 text-sm font-black text-slate-900">{item.title}</p><p className="mt-2 text-[9px] font-semibold leading-5 text-slate-500">{item.detail}</p></div>; })}</div><div className="mt-5"><NoticeBanner tone="info" title="État honnête" description={`${accounts.length} compte(s) sont disponibles. Les cartes ci-dessus décrivent les doctrines prévues ; elles ne sont pas présentées comme actives sans configuration persistée.`}/></div></Surface>; }
function RuntimeCockpit({ data, onRefresh }: {
    data: NonNullable<ReturnType<typeof useAcWhatsApp>["data"]> | null;
    onRefresh: () => void;
}) {
    if (!data)
        return null;
...
function RuntimeLayer({ icon: Icon, title, good, goodText, badText, meaning }: {
    ...
    badText: string;
    meaning: string;
}) { return <div className="rounded-[24px] border border-slate-200 p-4"><div className="flex items-start justify-between"><div className={cx("grid h-10 w-10 place-items-center rounded-2xl", good ? "bg-emerald-600 text-white" : "bg-rose-600 text-white")}><Icon className="h-4 w-4"/></div><StatusPill status={good ? "active" : "error"} label={good ? goodText : badText} compact/></div><p className="mt-4 text-sm font-black text-slate-900">{title}</p><p className="mt-2 text-[9px] font-semibold leading-5 text-slate-500">{meaning}</p></div>; }
function Meaning({ label, text }: {
    label: string;
    text: string;
}) { return <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3"><p className="text-[9px] font-black text-slate-800">{label}</p><p className="mt-1 text-[10px] font-semibold leading-4 text-slate-500">{text}</p></div>; }
function AccountProvisioning({ data, onClose, onCreated }: {
    data: NonNullable<ReturnType<typeof useAcWhatsApp>["data"]>;
    onClose: () => void;
    onCreated: (account: AcWhatsAppAccount) => void;
}) {
    ...
    async function submit() { setBusy(true); try {
        const account = await acApi<AcWhatsAppAccount>("/api/ac-whatsapp/accounts", { method: "POST", body: JSON.stringify(form) });
        onCreated(account);
    }
    catch (cause) {
        setNotice({ ...friendlyAcError(cause), tone: "danger" });
    ...
    <div className="mb-5 grid grid-cols-5 gap-1">{labels.map((label, index) => <div key={label}><div className={cx("h-1.5 rounded-full", index + 1 <= step ? "bg-rose-600" : "bg-slate-100")}/><p className="mt-1 hidden text-center text-[10px] font-black text-slate-400 sm:block">{label}</p></div>)}</div>{notice ? <NoticeBanner tone="danger" title={notice.title} description={notice.description}/> : null}
    ...
    {step === 3 ? <div className="grid gap-3 sm:grid-cols-2">{([["outbound_enabled", "Envoi direct", "Autorise les réponses opérateur"], ["campaigns_enabled", "Campagnes", "Autorise les missions commerciales"], ["cold_prospecting_enabled", "Prospection", "Autorise la prospection déclarée"], ["bulk_messaging_enabled", "Bulk", "Autorise la file de destinataires"]] as const).map(([key, title, detail]) => <label key={key} className={cx("rounded-[22px] border p-4", form[key] ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-slate-50")}><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black text-slate-800">{title}</p><p className="mt-1 text-[10px] font-semibold text-slate-500">{detail}</p></div><input type="checkbox" checked={form[key]} onChange={(e) => setForm({ ...form, [key]: e.target.checked })}/></div></label>)}</div> : null}
...
function ProvisionStep({ icon: Icon, title, text }: {
    icon: typeof ServerCog;
    title: string;
    text: string;
...
function Summary({ label, value }: {
    label: string;
    value: string;
}) { return <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="text-[10px] font-black uppercase tracking-[.13em] text-slate-400">{label}</p><p className="mt-2 text-sm font-black text-slate-800">{value}</p></div>; }
function PairingModal({ payload, onClose, onRefresh, onConnected }: {
    payload: any;
    onClose: () => void;
    onRefresh: () => void;
        ...
        const poll = async () => {
            try {
                ...
                if (["ready", "connected"].includes(status)) {
                    setConfirmed(true);
                    window.setTimeout(() => { if (active)
                        void onConnected(); }, 700);
                    return;
                }
            }
            catch (cause) {
