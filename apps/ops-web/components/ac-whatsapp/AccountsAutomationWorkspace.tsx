...
import { Activity, AlertTriangle, Bot, CheckCircle2, CirclePause, CirclePlay, CloudCog, Copy, KeyRound, Link2, MessageSquareText, PlugZap, Plus, QrCode, RadioTower, RefreshCw, ServerCog, ShieldCheck, Smartphone, Unplug, Webhook, Wifi, Workflow, } from "lucide-react";
...
function AccountInspector({ account, health, onAction, busy }: {
...
function Capability({ label, active }: {
    label: string;
    active: boolean;
}) { return <div className={cx("rounded-2xl border p-3", active ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-slate-50")}><div className="flex items-center justify-between"><span className="text-[9px] font-black text-slate-700">{label}</span>{active ? <CheckCircle2 className="h-4 w-4 text-emerald-600"/> : <CirclePause className="h-4 w-4 text-slate-400"/>}</div></div>; }
...
function Meaning({ label, text }: {
    label: string;
    text: string;
}) { return <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3"><p className="text-[9px] font-black text-slate-800">{label}</p><p className="mt-1 text-[8px] font-semibold leading-4 text-slate-500">{text}</p></div>; }
...
function Summary({ label, value }: {
    label: string;
    value: string;
}) { return <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="text-[8px] font-black uppercase tracking-[.13em] text-slate-400">{label}</p><p className="mt-2 text-sm font-black text-slate-800">{value}</p></div>; }
