...
export type GlobalTone = 'blue' | 'green' | 'amber' | 'red' | 'purple' | 'cyan' | 'slate'

const tones: Record<GlobalTone, { bg: string; text: string; border: string; glow: string; dark: string }> = {
...
export function GlobalHero({ eyebrow, title, subtitle, actions, stats, tone = 'blue' }: { eyebrow: string; title: string; subtitle: string; actions?: ReactNode; stats?: { label: string; value: ReactNode; detail?: string }[]; tone?: GlobalTone }) {
  const t = tones[tone]
  return (
...
export function GlobalButton({ href, children, tone = 'blue' }: { href: string; children: ReactNode; tone?: GlobalTone }) {
  const t = tones[tone]
...
export function GlobalMetric({ label, value, detail, tone = 'blue' }: { label: string; value: ReactNode; detail: string; tone?: GlobalTone }) {
  const t = tones[tone]
  return (
...
export function GlobalPanel({ title, subtitle, children, tone = 'blue' }: { title: string; subtitle?: string; children: ReactNode; tone?: GlobalTone }) {
  const t = tones[tone]
  return (
...
export function GlobalRow({ title, meta, status, href, tone = 'slate' }: { title: ReactNode; meta: ReactNode; status: ReactNode; href?: string; tone?: GlobalTone }) {
  const t = tones[tone]
  const body = (
...
export function GlobalNavStrip({ items }: { items: { label: string; href: string; tone?: GlobalTone }[] }) {
  return (
