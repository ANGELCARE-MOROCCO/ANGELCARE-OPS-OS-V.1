...
$BackupDirectory = Join-Path $TargetDirectory "backups\sender-identity-$Stamp"
New-Item -ItemType Directory -Path $BackupDirectory -Force | Out-Null

if (Test-Path $TargetFile) {
  Copy-Item $TargetFile (Join-Path $BackupDirectory "server.js.before-sender-identity") -Force
}
...
Write-Host "Sender identity bridge update deployed successfully." -ForegroundColor Green
