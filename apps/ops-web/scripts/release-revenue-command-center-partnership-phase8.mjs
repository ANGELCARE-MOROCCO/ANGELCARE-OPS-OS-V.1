...
const proofFile=path.join(proofDir,"revenue-command-center-phase8-build-proof.json")
const results=[]
...
if(!versionAtLeast(process.version,"22.17.0")){
  fail(`Node ${process.version} is below the production-safe minimum v22.17.0. Use Node 22.17+ or Node 24.`)
}
if(!fs.existsSync(path.join(root,"node_modules",".bin","next"))){
  fail("Dependencies are not fully installed. Run npm ci successfully before this release gate.")
}
...
command("Focused Phase 8 TypeScript","npx",["tsc","-p","tsconfig.revenue-command-center-partnership-phase8.json","--pretty","false"])
...
if(impure.length){
  impure.slice(0,80).forEach(item=>console.error(`IMPURE ${item}`))
  fail(`CSS Module selector purity found ${impure.length} suspicious branch(es).`)
}
...
const proof={
  status:"PASSED",
  phase:"Revenue Command Center — Mega ZIP 8",
  startedAt,
