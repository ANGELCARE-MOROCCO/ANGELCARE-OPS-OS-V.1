...
const { getDataDir, getPublicAppUrl, getShopifyBasePath } = require('../config/env');

function ensureTrafficDir(env) {
  const dataDir = getDataDir(env);
  fs.mkdirSync(dataDir, { recursive: true, mode: 0o700 });
  return dataDir;
}

function getTrafficLogPath(env) {
  return path.join(ensureTrafficDir(env), 'traffic.jsonl');
}

function hashValue(env, value) {
...
function normalizeTrafficEvent(env, req, payload = {}) {
...
function appendTrafficEvent(env, event) {
  fs.appendFileSync(getTrafficLogPath(env), `${JSON.stringify(event)}\n`, { mode: 0o600 });
}

function readTrafficEvents(env, options = {}) {
...
function incrementCount(map, key, amount = 1) {
  const normalizedKey = key || 'Direct / unknown';
...
function summarizeTraffic(env, options = {}) {
  const events = readTrafficEvents(env, options);
  ...
  return {
    ...
    trackingScriptUrl: `${getPublicAppUrl(env)}${getShopifyBasePath(env)}/api/traffic/script.js`,
    note: 'Traffic appears after the storefront loads the tracking script and can reach this API URL.',
  };
}

function renderTrafficScript() {
  return `(() => {
