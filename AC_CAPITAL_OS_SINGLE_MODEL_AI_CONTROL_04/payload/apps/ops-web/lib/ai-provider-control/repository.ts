...
export async function executeAiProviderAction(action: string, payload: JsonRecord, actor: { id: string; name: string }) {
  ...
  if (action === 'apply_ac_capital_single_model_profile') {
    const modelCode = clean(payload.modelCode || 'gemini-2.5-flash')
    if (modelCode !== 'gemini-2.5-flash') throw new Error('AC_CAPITAL_SINGLE_MODEL_MUST_BE_GEMINI_2_5_FLASH')
    ...
    const modelRows = [
      {
        ...
        display_name: 'Gemini 2.5 Flash — AC Capital Grounded Research',
      ...
      },
      {
        dossier_id: assignment.dossier_id,
        model_code: modelCode,
        display_name: 'Gemini 2.5 Flash — AC Capital Intelligence',
    ...
    const quotaRow = {
      ...
      metadata: {
        ...
        providerDeclaredGroundedRpd: 500,
        selectedModel: modelCode,
        providerTier: 'free',
        source: 'AC_CAPITAL_SINGLE_MODEL_AI_CONTROL_04',
      },
