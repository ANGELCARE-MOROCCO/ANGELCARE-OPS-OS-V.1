...
-- AC_CAPITAL_FREE_PROVIDER_LEGACY_TABLE_COMPATIBILITY_05
-- Additive compatibility with the existing MZ11 AI Command Center tables.
-- Existing records and legacy columns are preserved.

alter table public.ac_capital_ai_agents
  add column if not exists agent_key text,
  add column if not exists name text,
  add column if not exists description text,
  add column if not exists category text not null default 'external-research',
  add column if not exists search_provider_key text not null default 'tavily',
  add column if not exists analysis_provider_key text not null default 'openrouter',
  add column if not exists trigger_mode text not null default 'manual',
  add column if not exists frequency_key text not null default 'daily',
  ...
  add column if not exists prompt_doctrine text not null default '',
  add column if not exists failure_policy jsonb not null default '{}'::jsonb,
...
set
  agent_key = coalesce(
    nullif(agent_key, ''),
    'legacy-agent-' || id::text
  ),
  name = coalesce(
    nullif(name, ''),
    nullif(agent_name, ''),
    nullif(agent_key, ''),
    'Legacy AC Capital Agent'
  ),
  agent_name = coalesce(
    nullif(name, ''),
    nullif(agent_name, ''),
  ...
  description = coalesce(description, purpose),
  purpose = coalesce(purpose, description),
  status = case lower(coalesce(status, 'paused'))
...
alter table public.ac_capital_ai_agents
  alter column agent_key set default 'legacy-agent',
  alter column agent_key set not null,
  alter column name set default 'Unnamed AC Capital Agent',
  alter column name set not null;

create unique index if not exists
  ac_capital_ai_agents_agent_key_compat_uidx
on public.ac_capital_ai_agents(agent_key);

create or replace function
  public.ac_capital_ai_agents_legacy_sync_05()
returns trigger
language plpgsql
as $function$
begin
  new.agent_key := coalesce(
    nullif(new.agent_key, ''),
    'legacy-agent-' || new.id::text
  );
  ...
  new.agent_name := new.name;
  new.description := coalesce(new.description, new.purpose);
  new.purpose := coalesce(new.purpose, new.description);

  new.status := case lower(coalesce(new.status, 'paused'))
    when 'active' then 'active'
    when 'disabled' then 'disabled'
    else 'paused'
  end;
...
drop trigger if exists
  ac_capital_ai_agents_legacy_sync_05
on public.ac_capital_ai_agents;

create trigger ac_capital_ai_agents_legacy_sync_05
before insert or update
on public.ac_capital_ai_agents
for each row
execute function public.ac_capital_ai_agents_legacy_sync_05();


alter table public.ac_capital_ai_agent_runs
  add column if not exists agent_key text,
  add column if not exists trigger_type text not null default 'manual',
  add column if not exists phase text not null default 'queued',
  add column if not exists research_query text,
  add column if not exists search_provider_key text,
  add column if not exists analysis_provider_key text,
  add column if not exists selected_analysis_model text,
  ...
  add column if not exists analysis_request_id text,
  add column if not exists search_http_status integer,
  ...
  add column if not exists tavily_credits numeric not null default 0,
  add column if not exists input_tokens integer not null default 0,
  add column if not exists output_tokens integer not null default 0,
  add column if not exists sources_returned integer not null default 0,
  add column if not exists sources_persisted integer not null default 0,
  add column if not exists opportunities_created integer not null default 0,
  add column if not exists opportunities_rejected integer not null default 0,
  add column if not exists duplicates_detected integer not null default 0,
  add column if not exists internal_actions jsonb not null default '{}'::jsonb,
...
update public.ac_capital_ai_agent_runs r
set agent_key = a.agent_key
from public.ac_capital_ai_agents a
where r.agent_id = a.id
  and nullif(r.agent_key, '') is null;
...
alter table public.ac_capital_ai_agent_runs
  alter column agent_key set default 'legacy-agent-run',
  alter column agent_key set not null;

create or replace function
  public.ac_capital_ai_agent_runs_legacy_sync_05()
returns trigger
language plpgsql
as $function$
begin
  if nullif(new.agent_key, '') is null
  ...
  new.trigger_type := coalesce(
    nullif(new.trigger_type, ''),
    case
      when coalesce(new.input_source, '') ilike '%schedul%'
        then 'scheduled'
      else 'manual'
    end
  );
...
drop trigger if exists
  ac_capital_ai_agent_runs_legacy_sync_05
on public.ac_capital_ai_agent_runs;

create trigger ac_capital_ai_agent_runs_legacy_sync_05
before insert or update
on public.ac_capital_ai_agent_runs
for each row
execute function public.ac_capital_ai_agent_runs_legacy_sync_05();
