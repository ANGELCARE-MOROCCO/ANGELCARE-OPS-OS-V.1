-- ANGELCARE RCC DAILY TASKS REAL PRODUCTION COMMAND CENTER V1
-- Zero demo/static/local storage. Real source: revenue_tasks + revenue_prospects + revenue_appointments.
...
select
  ...
  count(*) filter (where status = 'open' and coalesce(start_at::date, due_date, created_at::date) <= current_date)::integer as in_progress_tasks,
  count(*) filter (where status = 'open' and coalesce(start_at::date, due_date, created_at::date) > current_date)::integer as pending_tasks,
  ...
  round(
    ...
    else (count(*) filter (where status = 'done')::numeric / count(*)::numeric) * 100 end
...
begin
  ...
  perform public.revenue_log_event('task', p_task_id::text, 'task.' || p_action, 'Task ' || p_action, v_task.title, p_actor, 'info', to_jsonb(v_task));
