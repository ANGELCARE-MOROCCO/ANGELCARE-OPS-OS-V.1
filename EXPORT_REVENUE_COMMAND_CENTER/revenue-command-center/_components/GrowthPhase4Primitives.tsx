...
export function scoreTone(score: number) {
  if (score >= 75) return { color: '#16a34a', label: 'HIGH POTENTIAL', icon: '🟢' }
  if (score >= 45) return { color: '#d97706', label: 'MEDIUM', icon: '🟡' }
  return { color: '#dc2626', label: 'LOW / HARD', icon: '🔴' }
}
...
export function BoardCard({ title, subtitle, href, badge, value }: { title: string; subtitle?: string; href: string; badge?: React.ReactNode; value?: string }) {
  return (
    <Link href={href} style={{ textDecoration: 'none' }}>
      <article style={cardStyle}>
        <div style={cardTopStyle}>{badge || <Badge>ACTIVE</Badge>}{value ? <strong>{value}</strong> : null}</div>
        <h3 style={cardTitleStyle}>{title}</h3>
        <p style={cardTextStyle}>{subtitle || 'No details provided.'}</p>
      </article>
    </Link>
  )
}
...
const cardStyle: React.CSSProperties = { border: '1px solid #e2e8f0', borderRadius: 22, padding: 16, background: 'linear-gradient(180deg,#fff,#f8fafc)', color: '#0f172a', display: 'grid', gap: 10, minHeight: 160 }
