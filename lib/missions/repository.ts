...
export function missionToControlRecord(row: MissionRow, children: MissionRow[] = []): MissionControlRecord {
  ...
  return {
    id: Number(row.id),
    code: (row as any).dossier_reference || (row as any).mission_reference || `MISSION-${row.id}`,
...
export async function listMissionControlRecords(): Promise<MissionControlRecord[]> {
  const rows = await listMissionRows()
...
export async function getMissionRow(id: number): Promise<MissionRow | null> {
  const supabase = await createClient()
  const { data, error } = await supabase.from('missions').select('*, caregivers(*), contracts(*)').eq('id', id).maybeSingle()
  if (error) throw new Error(error.message)
...
export async function getMissionDossier(id: number): Promise<MissionDossier | null> {
  ...
  if (!row) return null
  const subRows = await listMissionRows({ parentMissionId: id })
  const [{ data: events }, { data: routes }, { data: params }, { data: days }, { data: transport }, { data: allowances }, { data: program }] = await Promise.all([
    supabase.from('mission_events').select('*').eq('mission_id', id).order('created_at', { ascending: false }).limit(200),
    supabase.from('mission_routes').select('*').eq('mission_id', id).order('sort_order', { ascending: true }),
    supabase.from('mission_parameters').select('*').eq('mission_id', id).maybeSingle(),
    supabase.from('mission_parameter_days').select('*').eq('mission_id', id).order('sort_order', { ascending: true }),
    supabase.from('mission_transport').select('*').eq('mission_id', id).maybeSingle(),
    supabase.from('mission_allowances').select('*').eq('mission_id', id).maybeSingle(),
    supabase.from('mission_program_lines').select('*').eq('mission_id', id).order('sort_order', { ascending: true }),
  ])
  return {
    mission: missionToControlRecord(row, subRows),
    raw: row,
    subMissions: subRows.map((child) => missionToControlRecord(child)),
...
export async function patchMission(id: number, patch: Record<string, unknown>): Promise<MissionRow> {
  const supabase = await createClient()
  const { data, error } = await supabase.from('missions').update(patch).eq('id', id).select('*, caregivers(*), contracts(*)').single()
  if (error) throw new Error(error.message)
  return data as MissionRow
}
