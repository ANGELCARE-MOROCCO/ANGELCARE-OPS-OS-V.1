...
async function saveAppUserWithFallback(supabase: SupabaseClient, existing: AnyRecord | null, fullRow: AnyRecord) {
  const save = async (row: AnyRecord) => {
    if (existing?.id) {
      return safeMutation<AnyRecord>(supabase.from('app_users').update(row).eq('id', existing.id).select('*').single())
    }
    return safeMutation<AnyRecord>(supabase.from('app_users').insert(row).select('*').single())
  }
  ...
  throw new CareLinkMobileLoginError('Unable to sync CareLink Mobile app user.', 'carelink_mobile_app_user_sync_failed', 500)
}
