...
export default function DashboardLiveClient({
  ...
  const feedText = useMemo(() => {
    const base =
      feedItems.length > 0
        ? feedItems
        : ['ANGELCARE OPS FEED • Aucun signal critique actif • supervision stable']

    return `${base.join('   ✦   ')}   ✦   ${base.join('   ✦   ')}`
  }, [feedItems])

  if (mode === 'clock') {
    const clock = mounted && now ? formatClock(now) : null
  ...
  return (
    <div
      style={{
        ...
        zIndex: 50,
        background:
          'linear-gradient(90deg, #7f1d1d 0%, #991b1b 35%, #b91c1c 65%, #7f1d1d 100%)',
