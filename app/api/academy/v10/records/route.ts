...
function normalize(row: any) {
  return {
    module_key: row.module_key || row.module || "revenue_hq",
    page_key: row.page_key || row.page || null,
    record_type: row.record_type || row.module_key || "command",
    title: row.title || row.name || "Revenue command record",
    description: row.description || row.note || null,
    owner_name: row.owner_name || row.owner || null,
    department: row.department || "Revenue Command",
...
export async function GET(req: NextRequest) {
  ...
  let query = supabase.from("revenue_command_records").select("*").order("updated_at", { ascending: false }).limit(250)
...
export async function PATCH(req: NextRequest) {
  ...
  delete (updates as any).metadata.raw_payload.id
...
export async function DELETE(req: NextRequest) {
  ...
  await audit(supabase, { id, module_key: payload.module_key, page_key: payload.page_key, action_key: "archive_record", selected_count: 1, status: error ? "failed" : "logged", error: error?.message || null })
