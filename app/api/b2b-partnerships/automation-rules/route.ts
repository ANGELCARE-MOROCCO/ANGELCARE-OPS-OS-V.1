...
export async function GET() {
  try {
    ...
    const { data, error } = await g.db.from('b2b_automation_rules').select('*').order('created_at', { ascending: false })
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = {
      name: body.name || 'Automation rule',
      trigger_key: body.trigger_key || body.trigger || 'manual',
    ...
    const { data, error } = await g.db.from('b2b_automation_rules').insert(payload).select('*').single()

    if (error) {
      console.error('[B2B_AUTOMATION_RULES_POST_FAILED]', error)
