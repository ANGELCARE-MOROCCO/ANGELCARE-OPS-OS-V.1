...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await g.db.from('b2b_calls').select('*').order('created_at', { ascending: false }).limit(limit)
    ...
    console.error('[B2B_CALLS_GET_CRASHED]', error)
    return NextResponse.json({ ok: true, data: [] })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = {
      ...
      caller_id: g.actor.id,
      call_type: nullableText(body.call_type) || 'partnership_development',
      call_result: nullableText(body.call_result),
      duration_minutes: Number(body.duration_minutes || 0),
      summary: nullableText(body.summary),
      objections: nullableText(body.objections),
    ...
    const { data, error } = await g.db.from('b2b_calls').insert(payload).select('*').single()

    if (error) {
      console.error('[B2B_CALLS_POST_FAILED]', error)
      return NextResponse.json({ ok: false, error: 'Unable to log call.' }, { status: 500 })
    }

    return NextResponse.json({ ok: true, data }, { status: 201 })
  } catch (error) {
    console.error('[B2B_CALLS_POST_CRASHED]', error)
    return NextResponse.json({ ok: false, error: 'Unable to log call.' }, { status: 500 })
  }
}
