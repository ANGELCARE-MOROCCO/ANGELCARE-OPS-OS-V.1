...
export async function GET() {
  try {
    ...
    const { data, error } = await g.db.from('b2b_import_batches').select('*').order('created_at', { ascending: false })
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await g.db.from('b2b_import_batches').insert({
      name: body.name || 'B2B Import Batch',
    ...
    if (error) {
      console.error('[B2B_IMPORTS_POST_FAILED]', error)
      return NextResponse.json({ ok: false, error: 'Unable to create import.' }, { status: 500 })
    }

    if (rows.length) {
      const importRows = rows.map((row: any, index: number) => ({
        batch_id: data.id,
        row_index: index + 1,
        raw_data: row,
        validation_status: row.name ? 'valid' : 'invalid',
        validation_errors: row.name ? [] : ['Missing name'],
      }))
      await g.db.from('b2b_import_rows').insert(importRows)
    }
