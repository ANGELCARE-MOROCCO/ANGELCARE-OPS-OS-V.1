...
async function resolveStaff(supabase: Awaited<ReturnType<typeof createClient>>, userId: string) {
  let direct: AnyRecord | null = null
  ...
  try {
    const { data } = await supabase
      ...
      .eq("user_id", userId)
      .maybeSingle()
    direct = data as AnyRecord | null
  } catch {}

  if (direct) return direct as AnyRecord

  let appUser: AnyRecord | null = null
  ...
  try {
    const { data } = await supabase
      ...
      .eq("id", userId)
      .maybeSingle()
    appUser = data as AnyRecord | null
  } catch {}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Keep overhead buttons operational even when the new enterprise engine sees stale/mixed status.
    // The compatibility writer below still records the punch and returns a fresh canPunch map.
    if (canUseNewEngineError(message) && !String(source).includes("overhead_panel")) {
