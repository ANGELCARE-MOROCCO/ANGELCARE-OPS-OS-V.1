...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    await recordMissionEvent({ missionId: Number(id), eventType: 'mobile_arrival_confirmed', content: body.note || 'Arrivée confirmée depuis CareLink mobile', source: 'carelink_mobile' })
    await createNotification({
      type: 'mission_update',
      title: 'Arrivée confirmée',
      body: body.note || `L’arrivée a été confirmée pour la mission ${String(mission.id)}.`,
      priority: 'normal',
      missionId: Number(id),
