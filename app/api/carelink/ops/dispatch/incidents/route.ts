...
export async function GET() {
  ...
  return NextResponse.json({ ok: true, source: payload.source, generatedAt: payload.generatedAt, data: (payload as any)['incidents'] || [] }, { headers: { 'Cache-Control': 'no-store, no-cache, must-revalidate' } })
}

export async function POST(request: Request) {
  const body = await request.json().catch(() => ({}))
  const result = await runCareLinkDispatchWorkspaceAction({ action: 'create_incident' as any, payload: body })
