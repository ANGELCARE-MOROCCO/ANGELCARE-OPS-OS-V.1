...
import { acknowledgeAlert } from '@/lib/carelink/mobile-persistence'
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    await requireCareLinkMobileLinkedRowAccess('carelink_alerts', id, 'can_view_missions')
    if (body.missionId) await requireCareLinkMobileMissionAccess(Number(body.missionId), 'can_view_missions')
    const data = await acknowledgeAlert(id, body.missionId ? Number(body.missionId) : null, body.note || null)
    return NextResponse.json({ ok: true, acknowledged: true, data })
  } catch (error) {
    return carelinkMobileErrorResponse(error, 'Alert acknowledgement failed')
  }
}
