...
export default function HrOsShell({
  ...
  return (
    <main style={page}>
      <section style={hero}>
        <div style={heroLeft}>
          <div style={badge}>ANGELCARE HR-OS · ENTERPRISE PEOPLE COMMAND</div>
          <h1 style={titleStyle}>{title}</h1>
          <p style={subtitleStyle}>{subtitle}</p>
        </div>
        <div style={heroRight}>
          <div style={liveRow}><span style={liveDot} /> HR CONTROL READY</div>
          <strong style={{ fontSize: 28 }}>Operating Stack</strong>
          <small style={{ color: '#cbd5e1', fontWeight: 800, lineHeight: 1.55 }}>
            Execution, intelligence, real-time alerts, governance, scenario planning and onboarding control.
          </small>
...
const hero: React.CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 380px', gap: 24, padding: 32, borderRadius: 34, background: 'radial-gradient(circle at top left,#2563eb,#020617 62%)', color: '#fff', boxShadow: '0 32px 90px rgba(2,6,23,.38)', overflow: 'hidden' }
