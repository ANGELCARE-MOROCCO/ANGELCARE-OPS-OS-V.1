...
const stats = [
    ["Stored files", "238", "indexed"],
    ["Expiring", "16", "needs action"],
    ["Missing", "22", "staff gaps"],
    ["Validated", "91%", "compliance rate"],
]

const heroActions = ["Upload Document", "Run Expiry Scan", "Export Vault"]

const panels = [
    { icon: "📁", title: "Document registry", text: "Centralize staff documents, contract files, IDs and evidence.", actions: ['Upload', 'Validate', 'Tag'] },
    { icon: "⏳", title: "Expiry control", text: "Detect expiring CIN, contracts, insurance, medical and certification files.", actions: ['Renew', 'Notify', 'Escalate'] },
    { icon: "🔎", title: "Audit readiness", text: "Prepare compliance packs and missing-document reports.", actions: ['Audit', 'Export', 'Lock file'] },
]

const tableColumns = ["Document", "Staff", "Expiry", "Status", "Action"]

const tableRows = [
    ["Contract", "Fatima R.", "2026-08-01", "Valid", "Open"],
    ["CIN", "Mehdi T.", "2026-05-20", "Expiring", "Renew"],
    ["Certificate", "Caregiver Pool", "Missing", "Gap", "Request"],
]

export default function Page() {
  return (
    <AppShell
      title="HR Document Vault"
      subtitle="Staff files, contracts, IDs, certifications, expiry alerts, validation workflow and document compliance."
      breadcrumbs={[{ label: 'HR' }, { label: "HR Document Vault" }]}
      actions={
        <>
    ...
    >
      <main style={styles.page}>
        ...
        >
          <div>
            <div style={styles.badge}>documents / vault</div>
            <h1 style={styles.title}>HR Document Vault</h1>
            <p style={styles.subtitle}>Staff files, contracts, IDs, certifications, expiry alerts, validation workflow and document compliance.</p>
          </div>
