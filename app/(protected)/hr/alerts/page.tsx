...
async function readEndpoint() {
  try {
    ...
    const url = base ? `${base.startsWith('http') ? base : `https://${base}`}/api/hr/sync/diagnose` : '/api/hr/sync/diagnose'
...
export default async function Page() {
  ...
  const cards = [
    ['Live endpoint', '/api/hr/sync/diagnose'],
      ...
      <section className="rounded-[32px] border border-white/10 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 p-8 shadow-2xl">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-4xl font-black tracking-tight">HR Alerts</h1>
            <p className="mt-3 max-w-3xl text-sm font-semibold leading-7 text-slate-300">Live HR risks, attendance exceptions, compliance warnings and operational escalations.</p>
          </div>
