    ...
    <div className="flex min-h-screen">
      ...
      <section className="min-w-0 flex-1 p-4 lg:p-6">
        ...
        <section id="attendance" className="mt-6 grid gap-5 xl:grid-cols-4">
          {[['Attendance', command.totals.attendance, '/hr/attendance', CalendarCheck], ['Leave approvals', command.totals.leave, '/hr/approvals', Clock3], ['Payroll inputs', command.totals.payroll, '/hr/payroll', WalletCards], ['Contracts', command.totals.contracts, '/hr/contracts', FileBadge2]].map(([label, val, href, Icon]: any) => <Link key={label} href={href} className="group rounded-[30px] border border-white/80 bg-white/90 p-5 shadow-xl shadow-slate-200/60 transition hover:-translate-y-1"><div className="flex items-center justify-between"><div className="rounded-2xl bg-violet-50 p-3 text-violet-700"><Icon className="h-5 w-5" /></div><ArrowUpRight className="h-5 w-5 text-slate-300 group-hover:text-violet-600" /></div><div className="mt-5 text-3xl font-black text-slate-950">{val}</div><div className="mt-1 text-sm font-black text-slate-600">{label}</div><p className="mt-2 text-xs font-bold leading-5 text-slate-500">Open the linked execution space for real operations and validations.</p></Link>)}
        </section>

        <section id="actions" className="mt-6 rounded-[34px] border border-white/80 bg-white/90 p-5 shadow-2xl shadow-slate-200/70"><SectionTitle eyebrow="Execution spaces" title="Upper navigation operational layers" subtitle="Fast access to every real HR workspace connected around employees." /><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{[['Create employee','#open-add-employee',UserCog],['Import / sync','/hr/sync-center',DatabaseZap],['Identity mapping','/hr/attendance/identity-map',Fingerprint],['Roster planner','/hr/rosters/planner',Workflow],['Onboarding','/hr/onboarding',ClipboardCheck],['Documents','/hr/documents',FileText],['Training','/hr/training',GraduationCap],['Performance','/hr/performance-matrix',UserRoundCheck],['Compliance','/hr/compliance',ShieldCheck],['Audit trail','/hr/audit',Activity],['Bulk actions','/hr/bulk-actions',Layers3],['Reports','/hr/reports/export',BarChart3]].map(([label, href, Icon]: any) => href === '#open-add-employee' ? <button key={label} onClick={() => setAddOpen(true)} className="text-left rounded-2xl border border-slate-200 bg-white p-4 font-black text-slate-800 transition hover:border-violet-200 hover:bg-violet-50"><Icon className="mb-3 h-5 w-5 text-violet-600" />{label}</button> : <Link key={label} href={href} className="rounded-2xl border border-slate-200 bg-white p-4 font-black text-slate-800 transition hover:border-violet-200 hover:bg-violet-50"><Icon className="mb-3 h-5 w-5 text-violet-600" />{label}</Link>)}</div></section>
      </section>
    </div>
  </main>
</>
