...
async function readEndpoint() {
  try {
    ...
    const url = base ? `${base.startsWith('http') ? base : `https://${base}`}/api/hr/attendance/status` : '/api/hr/attendance/status'
...
export default async function Page() {
  ...
  const cards = [
    ['Live endpoint', '/api/hr/attendance/status'],
      ...
      <section className="rounded-[32px] border border-white/10 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 p-8 shadow-2xl">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-4xl font-black tracking-tight">Time Tracking</h1>
            <p className="mt-3 max-w-3xl text-sm font-semibold leading-7 text-slate-300">Unified time tracking, punch control, late arrivals, overtime and missing-punch risks.</p>
          </div>
