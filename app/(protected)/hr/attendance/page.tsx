...
const stats = [
    ["Present", "42", "in service"],
    ["Late", "5", "needs reason"],
    ["Absent", "8", "coverage risk"],
    ["On break", "6", "active pauses"],
]

const heroActions = ["Refresh Attendance", "Create Absence Case", "Open Pointage"]

const panels = [
    { icon: "🕒", title: "Live pointage board", text: "Track shift-in, lunch, return and shift-out events with anomaly detection.", actions: ['Refresh', 'Flag late', 'Export'] },
    { icon: "🚨", title: "Absence intervention", text: "Identify no-shows, missing punch, abandoned shifts and uncovered posts.", actions: ['Call staff', 'Create incident', 'Assign replacement'] },
    { icon: "📊", title: "Coverage analytics", text: "Compare roster requirement versus actual attendance by department and duty.", actions: ['Daily view', 'Weekly view', 'Department view'] },
]

const tableColumns = ["Staff", "Department", "Signal", "Risk", "Action"]

const tableRows = [
    ["Amina B.", "Caregiving", "No shift-in", "High", "Call"],
    ["Youssef M.", "Office", "Late 18m", "Medium", "Request reason"],
    ["Sara K.", "Sales", "Present", "Low", "Open profile"],
]

export default function Page() {
  return (
    <AppShell
      title="Attendance Control Tower"
      subtitle="Daily pointage, presence, break state, late arrival, absence, shift-out and workforce coverage command."
      breadcrumbs={[{ label: 'HR' }, { label: "Attendance Control Tower" }]}
      actions={
        <>
    ...
    >
      <main style={styles.page}>
        ...
        >
          <div>
            <div style={styles.badge}>attendance / pointage</div>
            <h1 style={styles.title}>Attendance Control Tower</h1>
            <p style={styles.subtitle}>Daily pointage, presence, break state, late arrival, absence, shift-out and workforce coverage command.</p>
          </div>
