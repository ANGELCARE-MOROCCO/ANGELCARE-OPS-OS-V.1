...
const stats = [
    ["Open risks", "8", "needs review"],
    ["Discipline", "4", "active cases"],
    ["Policy gaps", "3", "missing acknowledgement"],
    ["Resolved", "19", "this month"],
]

const heroActions = ["Create Case", "Run Audit", "Export Risk"]

const panels = [
    { icon: "⚖️", title: "Disciplinary workflow", text: "Create warnings, corrective actions, decisions and closure evidence.", actions: ['New case', 'Assign owner', 'Close'] },
    { icon: "📜", title: "Policy compliance", text: "Track policy acknowledgement, missing signatures and audit evidence.", actions: ['Request signature', 'Audit', 'Notify'] },
    { icon: "🚨", title: "Incident linkage", text: "Connect incidents, attendance, documents and performance into one HR risk file.", actions: ['Link incident', 'Escalate', 'Investigate'] },
]

const tableColumns = ["Case", "Severity", "Owner", "Status", "Action"]

const tableRows = [
    ["Repeated lateness", "Medium", "HR", "Open", "Review"],
    ["Missing document", "High", "Admin", "Pending", "Request"],
    ["Policy breach", "High", "Manager", "Escalated", "Investigate"],
]

export default function Page() {
  return (
    <AppShell
      title="HR Compliance & Risk Center"
      subtitle="Disciplinary actions, HR risk, policy control, incident linkage, audit trail and corrective measures."
      breadcrumbs={[{ label: 'HR' }, { label: "HR Compliance & Risk Center" }]}
      actions={
        <>
    ...
    >
      <main style={styles.page}>
        ...
        >
          <div>
            <div style={styles.badge}>compliance / risk</div>
            <h1 style={styles.title}>HR Compliance & Risk Center</h1>
            <p style={styles.subtitle}>Disciplinary actions, HR risk, policy control, incident linkage, audit trail and corrective measures.</p>
          </div>
