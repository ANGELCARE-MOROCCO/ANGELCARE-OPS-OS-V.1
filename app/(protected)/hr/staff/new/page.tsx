...
const stats = [
    ["Departments", "9", "active areas"],
    ["Understaffed", "2", "capacity pressure"],
    ["Managers", "6", "assigned owners"],
    ["Open roles", "14", "hiring needs"],
]

const heroActions = ["Create Department", "Assign Owner", "Open Capacity"]

const panels = [
    { icon: "🏢", title: "Department map", text: "Control department owners, missions, permissions and workforce load.", actions: ['Edit owner', 'Open capacity', 'Review roles'] },
    { icon: "🧩", title: "Role architecture", text: "Map positions to departments and make staffing gaps visible.", actions: ['Add role', 'Move staff', 'View gaps'] },
    { icon: "📈", title: "Capacity balance", text: "Track overload, vacancy, replacement need and performance by department.", actions: ['Capacity board', 'Hiring plan', 'Shift load'] },
]

const tableColumns = ["Department", "Owner", "Capacity", "Risk", "Control"]

const tableRows = [
    ["Caregiving", "Ops Manager", "82%", "Medium", "Open"],
    ["HR", "HR Lead", "74%", "Low", "Configure"],
    ["Sales", "Revenue Lead", "62%", "High", "Staffing plan"],
]

export default function Page() {
  return (
    <AppShell
      title="New Staff Onboarding Command"
      subtitle="Create new staff profile, assign position, department, documents, permissions, onboarding and readiness controls."
      breadcrumbs={[{ label: 'HR' }, { label: "New Staff Onboarding Command" }]}
      actions={
        <>
    ...
    >
      <main style={styles.page}>
        ...
        >
          <div>
            <div style={styles.badge}>departments / structure</div>
            <h1 style={styles.title}>New Staff Onboarding Command</h1>
            <p style={styles.subtitle}>Create new staff profile, assign position, department, documents, permissions, onboarding and readiness controls.</p>
          </div>
