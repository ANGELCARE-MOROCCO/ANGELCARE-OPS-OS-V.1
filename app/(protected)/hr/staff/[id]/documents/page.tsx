...
export default async function StaffSubPage({ params }: any) {
  ...
  return <AppShell title="Staff Documents" subtitle="Staff Documents records." breadcrumbs={[{label:'HR',href:'/hr'},{label:'Staff',href:'/hr/staff'},{label:staff.full_name || 'Staff',href:`/hr/staff/${staff.id}`},{label:'Staff Documents'}]} actions={<PageAction href={`/hr/staff/${staff.id}`} variant="light">Staff 360</PageAction>}>
    <HRHero title="Staff Documents" subtitle={`${staff.full_name || staff.name || 'Staff'} • restored subpage`} />
