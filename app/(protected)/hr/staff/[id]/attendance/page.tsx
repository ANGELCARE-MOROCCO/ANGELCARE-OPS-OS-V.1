...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = (data['attendance'] || data.docs || data.reviews || []).slice(0,100)
  const fields = fieldMap['attendance'] || ['title','status','created_at']
  return <AppShell><StaffShell>
    <StaffHero eyebrow="Staff 360" title="Staff Attendance" subtitle={staff?.full_name || staff?.name || 'Staff record'} actions={<><StaffButton href={`/hr/staff/${id}`}>Back to 360</StaffButton><StaffButton href={`/hr/staff/${id}/edit`} variant="light">Edit profile</StaffButton></>} />
    <StaffPanel title="Staff Attendance" subtitle="Synchronized staff records.">
