...
export default async function StaffSubPage({ params }: any) {
  ...
  return <AppShell title="Staff Attendance" subtitle="Staff Attendance records." breadcrumbs={[{label:'HR',href:'/hr'},{label:'Staff',href:'/hr/staff'},{label:staff.full_name || 'Staff',href:`/hr/staff/${staff.id}`},{label:'Staff Attendance'}]} actions={<PageAction href={`/hr/staff/${staff.id}`} variant="light">Staff 360</PageAction>}>
    <HRHero title="Staff Attendance" subtitle={`${staff.full_name || staff.name || 'Staff'} • restored subpage`} />
