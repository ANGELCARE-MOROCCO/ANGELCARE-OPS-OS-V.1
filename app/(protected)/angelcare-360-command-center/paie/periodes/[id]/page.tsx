...
export default async function Angelcare360PayrollPeriodDetailPage({ params }: { params: { id: string } }) {
  const context = await getAngelcare360PayrollContext()
  const period = await getAngelcare360PayrollPeriodById(params.id)
  if (!period) notFound()
