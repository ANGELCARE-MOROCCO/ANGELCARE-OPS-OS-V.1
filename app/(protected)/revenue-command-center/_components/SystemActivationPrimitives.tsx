...
export function EventCard({ event }: { event: any }) {
  const tone = event.severity === 'critical' ? '#dc2626' : event.severity === 'warning' ? '#d97706' : '#2563eb'
  return (
    <article style={eventCardStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
        <Badge tone={tone}>{String(event.severity || 'info').toUpperCase()}</Badge>
        <small style={{ color: '#64748b', fontWeight: 800 }}>{formatDate(event.created_at)}</small>
      </div>
      <h3 style={cardTitleStyle}>{event.title || event.event_type}</h3>
      <p style={cardTextStyle}>{event.message}</p>
      <strong style={{ color: '#0f172a' }}>Action: {event.action_taken || 'logged'}</strong>
    </article>
  )
}
...
const eventCardStyle: React.CSSProperties = { padding: 16, borderRadius: 18, background: 'linear-gradient(180deg,#fff,#f8fafc)', border: '1px solid #e2e8f0', display: 'grid', gap: 10, color: '#0f172a' }
