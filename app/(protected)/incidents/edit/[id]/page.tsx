...
export default async function EditIncidentPage({
  ...
  const [incidentRes, missionsRes, caregiversRes, familiesRes, leadsRes] =
    await Promise.all([
      ...
      supabase
        .from('families')
        .select('id, family_name, parent_name')
        .order('id', { ascending: false })
        .limit(100),
      supabase
        .from('leads')
  ...
  if (incidentRes.error) {
    return <main style={{ padding: 32 }}>Erreur : {incidentRes.error.message}</main>
  }
    ...
    <main style={pageStyle}>
      <div style={headerStyle}>
        <div>
          ...
          <h1 style={titleStyle}>Modifier incident #{incident.id}</h1>
          <p style={subtitleStyle}>
            Mise à jour du statut, de la sévérité, des liens opérationnels et des notes de résolution.
          </p>
        </div>

        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <Link href={`/incidents/${incident.id}`} style={secondaryButtonStyle}>
            ← Retour incident
          </Link>
          <Link href="/incidents" style={secondaryButtonStyle}>
            Directory incidents
      ...
      <form action={updateIncident} style={formGridStyle}>
        <section style={panelStyle}>
          ...
          <div style={twoColGridStyle}>
            <Field
              ...
              name="incident_title"
              defaultValue={incident.incident_title || ''}
            />
            <Field
              label="Type incident"
              name="incident_type"
            ...
            <Field
              label="Module source"
              name="source_module"
              defaultValue={incident.source_module || ''}
            />
            <Field
              label="Assigné à"
              name="assigned_to"
            ...
            <Field
              label="Créé par"
              name="created_by"
              defaultValue={incident.created_by || 'AngelCare OpsOS'}
            />
          </div>
        </section>
        ...
        <section style={panelStyle}>
          ...
          <div style={twoColGridStyle}>
            <SelectLinkedField
              label="Mission liée"
              name="mission_id"
              defaultValue={incident.mission_id ? String(incident.mission_id) : ''}
            ...
            <SelectLinkedField
              label="Caregiver lié"
              name="caregiver_id"
              defaultValue={incident.caregiver_id ? String(incident.caregiver_id) : ''}
            ...
            <SelectLinkedField
              label="Famille liée"
              name="family_id"
              defaultValue={incident.family_id ? String(incident.family_id) : ''}
            ...
            <SelectLinkedField
              label="Lead lié"
              name="lead_id"
              defaultValue={incident.lead_id ? String(incident.lead_id) : ''}
