...
export default async function AcademyDetailPage({ params }: { params: { id: string } }) {
 ...
 const path = '/academy/courses/' + params.id
 return <AppShell title="Course Command Folder" subtitle="Deep Academy execution workspace" breadcrumbs={[{label:'Academy',href:'/academy'},{label:'Courses',href:'/academy/courses'},{label:String(title)}]} actions={<PageAction href="/academy/courses">Back</PageAction>}>
 <div style={page}>
  ...
  <Card><SectionTitle title="Quick Production Update" subtitle="Update the operational data without leaving the folder."/><form action={updateAcademyRecord} style={formGrid}><input type="hidden" name="table" value="academy_courses"/><input type="hidden" name="id" value={record.id}/><input type="hidden" name="path" value={path}/><Field label="Title"><Input name="title" defaultValue={record?.title || ''} /></Field><Field label="Category"><Input name="category" defaultValue={record?.category || ''} /></Field><Field label="Level"><Input name="level" defaultValue={record?.level || ''} /></Field><Field label="Duration Hours"><Input name="duration_hours" defaultValue={record?.duration_hours || ''} /></Field><Field label="Price"><Input name="price" defaultValue={record?.price || ''} /></Field><Field label="Status"><Input name="status" defaultValue={record?.status || ''} /></Field><Field label="Description"><Input name="description" defaultValue={record?.description || ''} /></Field><Submit>Save production update</Submit></form></Card>
  <div style={grid2}>
