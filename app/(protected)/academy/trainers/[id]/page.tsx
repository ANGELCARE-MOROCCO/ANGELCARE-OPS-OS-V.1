...
export default async function AcademyDetailPage({ params }: { params: { id: string } }) {
 ...
 const path = '/academy/trainers/' + params.id
 return <AppShell title="Trainer Command Folder" subtitle="Deep Academy execution workspace" breadcrumbs={[{label:'Academy',href:'/academy'},{label:'Trainers',href:'/academy/trainers'},{label:String(title)}]} actions={<PageAction href="/academy/trainers">Back</PageAction>}>
 <div style={page}>
  ...
  <Card><SectionTitle title="Quick Production Update" subtitle="Update the operational data without leaving the folder."/><form action={updateAcademyRecord} style={formGrid}><input type="hidden" name="table" value="academy_trainers"/><input type="hidden" name="id" value={record.id}/><input type="hidden" name="path" value={path}/><Field label="Full Name"><Input name="full_name" defaultValue={record?.full_name || ''} /></Field><Field label="Phone"><Input name="phone" defaultValue={record?.phone || ''} /></Field><Field label="Email"><Input name="email" defaultValue={record?.email || ''} /></Field><Field label="Specialty"><Input name="specialty" defaultValue={record?.specialty || ''} /></Field><Field label="Status"><Input name="status" defaultValue={record?.status || ''} /></Field><Field label="Notes"><Input name="notes" defaultValue={record?.notes || ''} /></Field><Submit>Save production update</Submit></form></Card>
  <div style={grid2}>
