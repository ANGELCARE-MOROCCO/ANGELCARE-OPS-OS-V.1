...
export default async function AcademyDetailPage({ params }: { params: { id: string } }) {
 ...
 const path = '/academy/alerts-sales/' + params.id
 return <AppShell title="Alert / Sales Signal Command Folder" subtitle="Deep Academy execution workspace" breadcrumbs={[{label:'Academy',href:'/academy'},{label:'Alert / Sales Signals',href:'/academy/alerts-sales'},{label:String(title)}]} actions={<PageAction href="/academy/alerts-sales">Back</PageAction>}>
 <div style={page}>
  ...
  <Card><SectionTitle title="Quick Production Update" subtitle="Update the operational data without leaving the folder."/><form action={updateAcademyRecord} style={formGrid}><input type="hidden" name="table" value="academy_alerts"/><input type="hidden" name="id" value={record.id}/><input type="hidden" name="path" value={path}/><Field label="Title"><Input name="title" defaultValue={record?.title || ''} /></Field><Field label="Message"><Input name="message" defaultValue={record?.message || ''} /></Field><Field label="Severity"><Input name="severity" defaultValue={record?.severity || ''} /></Field><Field label="Status"><Input name="status" defaultValue={record?.status || ''} /></Field><Field label="Due At"><Input name="due_at" defaultValue={record?.due_at || ''} /></Field><Field label="Related Trainee Id"><Input name="related_trainee_id" defaultValue={record?.related_trainee_id || ''} /></Field><Submit>Save production update</Submit></form></Card>
  <div style={grid2}>
