...
export default function MarketOSIndexCommandCenter() {
  ...
  return (
    <main data-market-os-root className="min-h-screen bg-slate-50 p-4 text-slate-950 md:p-6">
      <section className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <a href="/api/market-os/health" target="_blank" className="rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-950 hover:bg-white">Health</a>
            </div>
          </div>
        </header>
        ...
        <section className="grid gap-4 md:grid-cols-4">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase text-slate-9500">Records</p><p className="mt-2 text-3xl font-black">{data.records.length}</p></div>
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase text-slate-9500">Open Work</p><p className="mt-2 text-3xl font-black">{openRecords}</p></div>
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase text-slate-9500">Risk / Critical</p><p className="mt-2 text-3xl font-black">{riskRecords}</p></div>
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase text-slate-9500">Closed Output</p><p className="mt-2 text-3xl font-black">{closedRecords}</p></div>
        </section>

        <section className="grid gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
          <aside className="space-y-6">
            <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
              <p className="text-xs font-black uppercase tracking-[0.25em] text-slate-9500">Quick Execution</p>
              <div className="mt-4 space-y-3">
                {quickActions.map((item) => (
                  <button key={item.action} type="button" onClick={() => runQuickAction(item)} disabled={Boolean(saving)} className="w-full rounded-2xl bg-white px-4 py-3 text-left text-sm font-black text-slate-950 shadow-sm hover:bg-white disabled:cursor-not-allowed disabled:opacity-50">
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
              <p className="text-xs font-black uppercase tracking-[0.25em] text-slate-9500">Agents</p>
              <div className="mt-4 space-y-3">
                {data.agents.length === 0 && <p className="text-sm font-bold text-slate-9500">No agents loaded.</p>}
                {data.agents.map((agent) => (
                  <div key={agent.id || agent.agent_key} className="rounded-2xl border border-slate-200 p-3">
                    ...
                    <p className="text-xs font-bold text-slate-9500">{agent.role}</p>
          ...
          <section className="space-y-6">
            <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <div><p className="text-xs font-black uppercase tracking-[0.25em] text-slate-9500">Operational navigation</p><h2 className="text-xl font-black">Execution pages</h2></div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {navItems.map((item) => (
                  <Link key={item.href} href={item.href} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:border-slate-950 hover:bg-white">
                    ...
                    <p className="mt-1 text-xs font-bold uppercase tracking-[0.18em] text-slate-9500">{item.engine}</p>
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="grid gap-6 xl:grid-cols-2">
              <Panel title="Recent records">
                {loading && <p className="text-sm font-bold text-slate-9500">Loading...</p>}
                {!loading && data.records.length === 0 && <p className="text-sm font-bold text-slate-9500">No records yet.</p>}
                {data.records.slice(0, 12).map((record) => (
                  <div key={record.id} className="rounded-2xl border border-slate-200 p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div><p className="font-black">{record.title || 'Untitled'}</p><p className="text-xs font-bold text-slate-9500">{record.engine || 'system'} · {record.record_type || record.kind || 'task'} · {record.owner_agent || record.owner || 'unassigned'}</p></div>
                    ...
                    <p className="mt-3 text-xs font-bold text-slate-500">Updated: {safeDate(record.updated_at || record.created_at)}</p>
                  </div>
                ))}
              </Panel>

              <Panel title="KPI control">
                {data.kpis.length === 0 && <p className="text-sm font-bold text-slate-9500">No KPIs loaded.</p>}
                {data.kpis.map((kpi) => (
                  <div key={kpi.id || kpi.kpi_key} className="rounded-2xl border border-slate-200 p-4">
                    <div className="flex items-center justify-between gap-3">
                      <div><p className="text-xs font-black uppercase text-slate-9500">{kpi.label || kpi.kpi_key}</p><p className="mt-1 text-2xl font-black">{Number(kpi.current_value || 0).toLocaleString()} <span className="text-sm text-slate-9500">{kpi.unit}</span></p></div>
                      <span className={`rounded-full border px-2 py-1 text-xs font-black ${badgeClass(kpi.status)}`}>{kpi.status || 'empty'}</span>
                    </div>
                    <p className="mt-2 text-xs font-bold text-slate-9500">Target: {Number(kpi.target_value || 0).toLocaleString()}</p>
                  </div>
                ))}
              </Panel>
            </div>
            ...
            <div className="grid gap-6 xl:grid-cols-2">
              <Panel title="Recent actions">
                {data.actions.length === 0 && <p className="text-sm font-bold text-slate-9500">No actions yet.</p>}
                {data.actions.slice(0, 12).map((action) => (
                  <div key={action.id} className="rounded-2xl bg-slate-50 p-3"><p className="font-black">{action.action_label || action.action_key}</p><p className="text-xs font-bold text-slate-9500">{action.engine || 'system'} · {action.actor_name || 'operator'} · {safeDate(action.created_at)}</p></div>
                ))}
              </Panel>

              <Panel title="Audit log">
                {data.audit.length === 0 && <p className="text-sm font-bold text-slate-9500">No audit events yet.</p>}
                {data.audit.slice(0, 12).map((event) => (
                  <div key={event.id} className="rounded-2xl bg-slate-50 p-3"><p className="font-black">{event.title || event.action_key}</p><p className="text-xs font-bold text-slate-9500">{event.summary || 'Audit event'} · {safeDate(event.created_at)}</p></div>
                ))}
              </Panel>
            </div>
          </section>
...
function Panel({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
      <p className="text-xs font-black uppercase tracking-[0.25em] text-slate-9500">{title}</p>
