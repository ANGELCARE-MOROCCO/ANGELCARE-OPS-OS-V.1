...
function Board({title,items,empty}:{title:string;items:any[];empty:string}){return <div style={panelStyle}><h2 style={titleStyle}>{title}</h2>{items.length?items.slice(0,10).map((c:any)=><Link key={c.id} href={`/caregivers/${c.id}`} style={itemStyle}><strong>{text(c.full_name)}</strong><span>{text(c.city)} • {text(c.zone)} • Score {text(c.reliability_score,0)}</span></Link>):<div style={emptyStyle}>{empty}</div>}</div>}
