...
import type { StaffPortalMemo } from '@/lib/staff-portal-os/phase1-data'

function getAcknowledged() {
  if (typeof window === 'undefined') return new Set<string>()
  try {
    const raw = window.localStorage.getItem('angelcare_staff_portal_ack_memos')
...
function saveAcknowledged(values: Set<string>) {
  if (typeof window === 'undefined') return
  window.localStorage.setItem('angelcare_staff_portal_ack_memos', JSON.stringify(Array.from(values)))
}

export default function StaffPortalMemoPanel({ memos }: { memos: StaffPortalMemo[] }) {
  const initial = useMemo(() => getAcknowledged(), [])
  const [acknowledged, setAcknowledged] = useState<Set<string>>(initial)
  const [activeId, setActiveId] = useState<string>(memos[0]?.id || '')
  const [expanded, setExpanded] = useState(false)

  const activeMemo = memos.find((memo) => memo.id === activeId) || memos[0]
  const unackedCount = memos.filter((memo) => !acknowledged.has(memo.id)).length
          ...
          {unackedCount > 0 ? (
            <span
              style={{
                ...
                borderRadius: 999,
                background: '#ef4444',
                boxShadow: '0 0 0 8px rgba(239,68,68,.18), 0 0 30px rgba(239,68,68,.8)',
              }}
            />
          ) : (
            <span
              style={{
                ...
                borderRadius: 999,
                background: '#22c55e',
                boxShadow: '0 0 0 8px rgba(34,197,94,.18)',
              }}
            />
          )}
          <strong style={{ color: unackedCount ? '#fecaca' : '#bbf7d0' }}>
            {unackedCount ? `${unackedCount} UNACK` : 'CLEAR'}
          </strong>
        </div>
      </div>
        ...
        {!expanded ? (
          <div style={{ display: 'grid', gap: 8, alignContent: 'start' }}>
            {memos.map((memo) => {
              ...
              const isAck = acknowledged.has(memo.id)
              return (
                <button
                  key={memo.id}
                  type="button"
          ...
          {activeMemo ? (
            <>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 18 }}>
                <button
                  type="button"
                  onClick={() => acknowledge(activeMemo.id)}
                  style={{
                    border: '1px solid #22c55e',
                    background: acknowledged.has(activeMemo.id) ? '#14532d' : '#22c55e',
                    color: acknowledged.has(activeMemo.id) ? '#bbf7d0' : '#022c22',
                    borderRadius: 999,
                    padding: '10px 14px',
                    fontWeight: 950,
                    cursor: 'pointer',
                ...
                >
                  {acknowledged.has(activeMemo.id) ? 'Receipt acknowledged' : 'Acknowledge receipt'}
                </button>
              </div>
            </>
          ) : (
