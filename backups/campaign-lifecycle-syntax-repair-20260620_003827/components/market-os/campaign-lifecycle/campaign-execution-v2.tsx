...
function CampaignLifecycleClassicSidebarCSS() {
  return (
    <style
...
function CampaignLifecycleClassicSidebar({ selectedId }: { selectedId?: string | null }) {
  ...
  return (
    <aside className="sticky top-28 h-fit rounded-[26px] border border-slate-200 bg-white p-3 shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
      <div className="px-3 py-3">
        <p className="text-[10px] font-black uppercase tracking-[0.20em] text-sky-700">
          ANGELCARE
        </p>
        ...
        <p className="mt-2 text-xs font-bold leading-5 text-slate-500">
          Select a campaign to unlock launch, budget, risk, task and performance controls.
        </p>
      </div>
        ...
        {menu.map((item) => {
          const content = (
            <>
              ...
              {!item.enabled ? (
                <span className="ml-auto rounded-full bg-amber-50 px-2 py-1 text-[9px] font-black uppercase tracking-[0.10em] text-amber-700">
                  Select
                </span>
              ) : null}
            </>
      ...
      <div className="mt-3 rounded-2xl border border-emerald-100 bg-emerald-50 px-3 py-3">
        ...
        <p className="mt-1 text-xs font-bold text-emerald-800">
          {selectedId ? "Campaign controls ready" : "Board mode active"}
        </p>
      </div>
    </aside>
  )
...
function StatCard({ label, value, note, tone }: { label: string; value: string | number; note: string; tone: string }) {
  return (
    <div className={`rounded-3xl p-5 text-slate-950 shadow-lg ${tone}`} data-old-campaign-action-strip="true">
...
export default function CampaignExecutionV2() {
  ...
  return (
    <main data-market-os-root className="min-h-screen bg-slate-50 text-slate-950 selection:bg-emerald-200 selection:text-slate-950">
      <CampaignLifecycleClassicSidebarCSS />
      <CampaignLifecycleMegaHero />

      <div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
        <CampaignLifecycleClassicSidebar selectedId={selected?.id} />
