...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={`w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-950 outline-none transition placeholder:text-slate-9500 focus:border-slate-400 ${props.className ?? ""}`} />
}
...
export function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...props} rows={props.rows ?? 5} className={`w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold leading-6 text-slate-950 outline-none transition placeholder:text-slate-9500 focus:border-slate-400 ${props.className ?? ""}`} />
}
...
export function Metric({ label, value, sub }: { label: string; value: string; sub: string }) {
  return <Panel className="p-5"><p className="text-xs font-black uppercase tracking-wider text-slate-9500">{label}</p><p className="mt-2 text-3xl font-black">{value}</p><p className="mt-1 text-xs font-bold text-slate-9500">{sub}</p></Panel>
}
...
export function ContentRow({ item, tasks, onAdvance, onArchive, onDelete }: { item: ContentItem; tasks: ContentTask[]; onAdvance: () => void; onArchive: () => void; onDelete: () => void }) {
  ...
  return <div className="grid gap-4 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-[1.5fr_.8fr_.8fr_.8fr_1.3fr] lg:items-center">
    ...
    <div><p className="text-xs font-black uppercase text-slate-9500">Owner</p><p className="mt-1 text-sm font-bold">{item.owner}</p></div>
    <div><p className="text-xs font-black uppercase text-slate-9500">Due</p><p className="mt-1 text-sm font-bold">{item.dueDate}</p></div>
    <div><p className="text-xs font-black uppercase text-slate-9500">Tasks</p><p className="mt-1 text-sm font-bold">{completed}/{itemTasks.length}</p></div>
