  ...
  return (
    <main data-market-os-root className="min-h-screen bg-slate-50 text-slate-950 selection:bg-emerald-200 selection:text-slate-950">
      ...
      {/* Restored dashboard hero removed permanently.
        ...
        <section className="rounded-[34px] border border-slate-200 bg-white p-5 shadow-[0_22px_70px_rgba(15,23,42,0.07)]">
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-5">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.24em] text-sky-700">
                ANGELCARE Campaign Lifecycle
              </p>
              <h2 className="mt-1 text-3xl font-black tracking-[-0.05em] text-slate-950">
                Live command dashboard
              </h2>
            </div>

            <div className="rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-[11px] font-black uppercase tracking-[0.14em] text-emerald-700">
              Production active
            </div>
          </div>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
            {dashboardSignals.map((signal) => (
              <div key={signal.label} className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`grid h-10 w-10 place-items-center rounded-2xl text-xs font-black ${signal.tone}`}>
                    {signal.icon}
                  </span>
                </div>
          ...
          <div className="mt-5 grid gap-4 xl:grid-cols-3">
            <div className="rounded-[26px] border border-slate-200 bg-slate-50 p-5">
              ...
              <div className="mt-4 grid gap-2">
                {stages.slice(0, 5).map((stage) => {
                  ...
                  return (
                    <div key={stage} className="flex items-center justify-between rounded-2xl bg-white px-4 py-3">
                      <span className="text-sm font-black text-slate-700">{label(stage)}</span>
            ...
            <div className="rounded-[26px] border border-slate-200 bg-slate-50 p-5">
              ...
              <div className="mt-4 grid gap-2">
                <div className="flex justify-between rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-700">
                  <span>Readiness</span>
                  <span>{selected ? `${selected.readiness}%` : "—"}</span>
                </div>
                <div className="flex justify-between rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-700">
                  <span>Risk</span>
                  <span>{selected ? label(selected.risk) : "—"}</span>
                </div>
                <div className="flex justify-between rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-700">
                  <span>Leads</span>
                  <span>{selected ? selected.leads : "—"}</span>
                </div>
              </div>
            </div>
