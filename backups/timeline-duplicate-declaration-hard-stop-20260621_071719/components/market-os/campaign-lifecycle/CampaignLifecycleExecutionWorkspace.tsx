...
export default function CampaignLifecycleExecutionWorkspace() {
  ...
  function taskOperationalDate(task: Task) {
                                const rowTimelineStartMs = new Date(campaign.startDate || isoToday()).getTime()
                            const rowTimelineEndMs = new Date(campaign.endDate || isoToday()).getTime()
                            const rowTimelineNowMs = Date.now()
                            const rowTimelineTotalDays = Math.max(1, Math.ceil((rowTimelineEndMs - rowTimelineStartMs) / 86400000))
                            const rowTimelineDaysLeft = Math.max(0, Math.ceil((rowTimelineEndMs - rowTimelineNowMs) / 86400000))
                            const rowTimelineProgressPct = Math.max(
                              0,
                              Math.min(100, Math.round(((rowTimelineNowMs - rowTimelineStartMs) / Math.max(1, rowTimelineEndMs - rowTimelineStartMs)) * 100))
                            )
...
return (task.startDateTime || task.dueDate || task.createdAt || isoToday()).slice(0, 10)
  ...
  function timelineRowTone(daysLeft: number, progress: number) {
    ...
    if (progress >= 50 || rowTimelineDaysLeft <= 20) return {
                      ...
                      <div className="max-h-[620px] overflow-auto pr-2">
                        <div className="space-y-3">
                          {state.campaigns.length ? state.campaigns.map((campaign) => {
                            ...
                            const remaining = remainingCampaignDays(campaign)
                            const timelineProgressPct = campaignElapsedPct(campaign)
                            const selectedRow = selectedDatesCampaign?.id === campaign.id

                            const timelineRowStartMs = new Date(campaign.startDate || isoToday()).getTime()
                            const timelineRowEndMs = new Date(campaign.endDate || isoToday()).getTime()
                            const timelineRowNowMs = Date.now()
                            const timelineRowTotalDays = Math.max(1, Math.ceil((timelineRowEndMs - timelineRowStartMs) / 86400000))
                            const timelineDaysLeft = Math.max(0, Math.ceil((timelineRowEndMs - timelineRowNowMs) / 86400000))
                            const timelineProgressPct = Math.max(0, Math.min(100, Math.round(((timelineRowNowMs - timelineRowStartMs) / Math.max(1, timelineRowEndMs - timelineRowStartMs)) * 100)))
                          ...
                          className={`group relative grid w-full cursor-pointer grid-cols-1 items-center gap-5 overflow-hidden rounded-[30px] border p-5 text-left shadow-[0_18px_50px_rgba(15,23,42,0.08)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_30px_80px_rgba(15,23,42,0.13)] xl:grid-cols-[88px_minmax(260px,1.35fr)_190px_minmax(260px,1fr)_150px_64px] ${
                            timelineRowTone(rowTimelineDaysLeft, rowTimelineProgressPct).shell
                          }`}
                        >
                          ...
                          <div className={`relative grid h-[72px] w-[72px] place-items-center rounded-[26px] border text-3xl shadow-[0_18px_42px_rgba(15,23,42,0.10)] ${
                            timelineRowTone(rowTimelineDaysLeft, rowTimelineProgressPct).icon
                          }`}>
                            ▣
                          </div>
                          ...
                          <div className="relative flex flex-wrap items-center gap-2">
                            ...
                            <span className={`rounded-full border px-4 py-2 text-[11px] font-black uppercase tracking-[0.16em] shadow-sm ${
                              timelineRowTone(rowTimelineDaysLeft, rowTimelineProgressPct).badge
                            }`}>
                              {timelineRowTone(rowTimelineDaysLeft, rowTimelineProgressPct).label}
                            </span>
                          </div>
                          ...
                          <div className="relative rounded-[24px] border border-white/70 bg-white/65 p-4 shadow-sm backdrop-blur">
                            <div className="flex items-center justify-between gap-3">
                              ...
                              <p className="text-xs font-black text-slate-600">{rowTimelineProgressPct}%</p>
                            </div>
                            <div className="mt-3 h-2.5 overflow-hidden rounded-full bg-slate-200">
                              <div
                                className={`h-full rounded-full ${timelineRowTone(rowTimelineDaysLeft, rowTimelineProgressPct).bar}`}
                                style={{ width: `${Math.max(0, Math.min(100, rowTimelineProgressPct))}%` }}
                              />
                            </div>
                            <div className="mt-3 flex items-center justify-between text-[11px] font-black text-slate-500">
                              ...
                              <span>{rowTimelineProgressPct}% elapsed</span>
                          ...
                          <div className="relative flex items-center justify-center gap-2 rounded-[24px] border border-white/70 bg-white/70 p-4 shadow-sm backdrop-blur">
                            <span className={`h-3 w-3 rounded-full ${timelineRowTone(rowTimelineDaysLeft, rowTimelineProgressPct).dot} ${rowTimelineDaysLeft <= 20 ? "animate-pulse" : ""}`} />
                            <div>
                              <p className="text-5xl font-black tracking-[-0.08em] text-slate-950">
                                {rowTimelineDaysLeft}
                              </p>
                          ...
                          <div className="relative grid h-14 w-14 place-items-center rounded-[22px] border border-white/80 bg-white/80 text-2xl font-black text-slate-500 shadow-sm">
                            ⋯
                                ...
                                <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100">
                                  <div className={`h-full rounded-full ${tone.bar}`} style={{ width: `${rowTimelineProgressPct}%` }} />
                                </div>
                              </button>
                            )
                          }) : (
