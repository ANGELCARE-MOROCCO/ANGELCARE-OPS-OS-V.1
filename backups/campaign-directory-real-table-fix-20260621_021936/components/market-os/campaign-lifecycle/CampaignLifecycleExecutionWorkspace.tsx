              ...
              <section id="campaign-management-workspace" className="space-y-5">
                ...
                <section className="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_520px]">
                  <Card className="overflow-hidden p-0">
                    ...
                    <div className="max-h-[880px] overflow-auto px-6 py-5">
                      {directoryView === "columns" ? (
                        <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-slate-50/70 shadow-sm">
                          <div className="grid min-w-[1320px] grid-cols-[44px_2.2fr_1.15fr_1.15fr_1.1fr_1.15fr_.95fr_.95fr_1.25fr_1fr_.85fr_.8fr_1fr] gap-3 border-b border-slate-200 px-5 py-4 text-[10px] font-black uppercase tracking-[0.20em] text-slate-400">
                            <div />
                            <div>Campaign</div>
                            <div>Channel</div>
                            <div>Type</div>
                          ...
                          <div className="min-w-[1320px] space-y-2 p-3">
                            {managementCampaigns.length ? managementCampaigns.map((campaign) => {
                              ...
                              return (
                                <button
                                  key={campaign.id}
                                  type="button"
                                  data-campaign-directory-row="true"
                                  onClick={() => setState({ ...state, selectedId: campaign.id })}
                                  className={`grid w-full grid-cols-[44px_2.2fr_1.15fr_1.15fr_1.1fr_1.15fr_.95fr_.95fr_1.25fr_1fr_.85fr_.8fr_1fr] items-center gap-3 rounded-[24px] border border-slate-200 bg-white px-5 py-4 text-left shadow-sm transition hover:-translate-y-[1px] hover:shadow-[0_16px_38px_rgba(15,23,42,0.09)] ${
                                    selectedRow ? "border-blue-200 ring-4 ring-blue-50" : ""
                                ...
                                >
                                  ...
                                  <div className="min-w-0">
                                    <p className="truncate text-base font-black tracking-[-0.03em] text-slate-950">{campaign.name}</p>
                                    <p className="mt-1 truncate text-xs font-bold text-slate-500">{campaign.objective || "No objective written yet."}</p>
                                  </div>

                                  <div className="flex items-center gap-2">
                                    <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl border border-slate-200 bg-white text-sm font-black text-blue-700 shadow-sm">
                                      {directoryChannelIcon(campaign)}
                                    </span>
                                    <span className="truncate text-xs font-black text-slate-700">{directoryChannelLabel(campaign)}</span>
                                  </div>

                                  <div className="truncate text-xs font-black text-slate-700">{directoryTypeLabel(campaign)}</div>
                                  <div className="text-xs font-black text-slate-700">{managementAudienceType(campaign)}</div>
                                  <div className="truncate text-xs font-black text-slate-700">{campaign.owner || "Unassigned"}</div>

                                  <div>
                                    <span className={`inline-flex rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.13em] ${directoryChipClass(stageLabel[campaign.stage])}`}>
                                      {stageLabel[campaign.stage]}
                                    </span>
                                  </div>

                                  <div>
                                    <span className={`inline-flex rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.13em] ${directoryChipClass(status)}`}>
                                      {status}
                                    </span>
                                  </div>

                                  <div className="flex flex-col gap-1.5">
                                    <span className={`inline-flex w-fit rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.13em] ${directoryChipClass(riskLabel[campaign.risk])}`}>
                                  ...
                                  <div className="text-xs font-black text-slate-700">{campaign.endDate || "—"}</div>
                                  <div className="text-xs font-black text-slate-950">{mad(campaign.budgetMad)}</div>
                                  <div className="text-xs font-black text-slate-950">{campaign.leads || 0}</div>
                          ...
                          {managementCampaigns.length ? managementCampaigns.map((campaign) => {
                            ...
                            return (
                              <button
                                key={campaign.id}
                                type="button"
                                data-campaign-directory-card="true"
                                onClick={() => setState({ ...state, selectedId: campaign.id })}
                                className={`rounded-[30px] border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-[2px] hover:shadow-[0_22px_48px_rgba(15,23,42,0.10)] ${
                                  selectedCard ? "border-blue-200 ring-4 ring-blue-50" : ""
                                } ${expiry.row} ${expiry.glow}`}
                              >
                                <div className="flex items-start justify-between gap-4">
                                  <div className="flex items-center gap-3">
                                    <span className="grid h-14 w-14 place-items-center rounded-2xl border border-slate-200 bg-slate-50 text-xl font-black text-blue-700 shadow-sm">
                                      {directoryChannelIcon(campaign)}
                                    </span>
                                    <div className="min-w-0">
                                      <p className="truncate text-xl font-black tracking-[-0.04em] text-slate-950">{campaign.name}</p>
                                      <p className="mt-1 text-sm font-bold text-slate-500">{campaign.objective || "No objective written yet."}</p>
                                    </div>
                                  </div>
                                ...
                                <div className="mt-5 grid grid-cols-2 gap-3">
                                  ...
                                  <div className="rounded-[20px] border border-slate-200 bg-slate-50 p-4">
                                    ...
                                    <p className="mt-2 text-sm font-black text-slate-700">{directoryChannelLabel(campaign)}</p>
                                  </div>
                                  <div className="rounded-[20px] border border-slate-200 bg-slate-50 p-4">
