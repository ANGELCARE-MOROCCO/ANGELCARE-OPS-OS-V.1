...
export default function CampaignLifecycleExecutionWorkspace() {
  ...
  function taskOperationalDate(task: Task) {
                                var rowTimelineStartMs = new Date(campaign.startDate || isoToday()).getTime()
                            var rowTimelineEndMs = new Date(campaign.endDate || isoToday()).getTime()
                            var rowTimelineNowMs = Date.now()
                            var rowTimelineTotalDays = Math.max(1, Math.ceil((rowTimelineEndMs - rowTimelineStartMs) / 86400000))
                            var rowTimelineDaysLeft = Math.max(0, Math.ceil((rowTimelineEndMs - rowTimelineNowMs) / 86400000))
                            var rowTimelineProgressPct = Math.max(
                              0,
                      ...
                      <div className="max-h-[620px] overflow-auto pr-2">
                        <div className="space-y-3">
                          {state.campaigns.length ? state.campaigns.map((campaign) => {
                            ...
                            const remaining = remainingCampaignDays(campaign)
                            var timelineProgressPct = campaignElapsedPct(campaign)
                            const selectedRow = selectedDatesCampaign?.id === campaign.id

                            var timelineRowStartMs = new Date(campaign.startDate || isoToday()).getTime()
                            var timelineRowEndMs = new Date(campaign.endDate || isoToday()).getTime()
                            var timelineRowNowMs = Date.now()
                            var timelineRowTotalDays = Math.max(1, Math.ceil((timelineRowEndMs - timelineRowStartMs) / 86400000))
                            var timelineDaysLeft = Math.max(0, Math.ceil((timelineRowEndMs - timelineRowNowMs) / 86400000))
                            var timelineProgressPct = Math.max(0, Math.min(100, Math.round(((timelineRowNowMs - timelineRowStartMs) / Math.max(1, timelineRowEndMs - timelineRowStartMs)) * 100)))
