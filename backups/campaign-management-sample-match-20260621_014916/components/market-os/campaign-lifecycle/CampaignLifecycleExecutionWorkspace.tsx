          ...
          <div className="space-y-5 p-5">
            ...
            {activePanel === "management" ? (
              <section id="campaign-management-workspace" className="space-y-5">
                <div className="flex flex-wrap items-end justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-4xl font-black tracking-[-0.06em] text-slate-950">Campaign Management</h2>
                      <span className="grid h-9 w-9 place-items-center rounded-2xl border border-slate-200 bg-white text-sm font-black text-slate-500 shadow-sm">☆</span>
                    </div>
                    <p className="mt-2 text-sm font-bold text-slate-500">Central directory and management workspace for all ANGELCARE campaigns.</p>
                  </div>
                ...
                <Card className="p-4">
                  <div className="grid gap-3 xl:grid-cols-[minmax(260px,1fr)_140px_160px_130px_130px_130px_130px_auto]">
                    <input
                  ...
                  <Card className="overflow-hidden p-0">
                    ...
                    <div className="overflow-auto">
                      <div className="min-w-[1180px]">
                        <div className="grid grid-cols-[44px_1.45fr_90px_120px_120px_120px_110px_110px_110px_110px_85px_85px_95px] border-b border-slate-100 bg-slate-50 px-4 py-3 text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">
                          <div />
                          <div>Campaign</div>
                          <div>Channel</div>
                          <div>Type</div>
                        ...
                        {managementVisibleCampaigns.length ? managementVisibleCampaigns.map((campaign) => {
                          ...
                          const status = campaignLifecycleStatus(campaign)
                          const audienceType = managementAudienceType(campaign)
                          const campaignTasks = state.tasks.filter((task) => task.campaignId === campaign.id)
                          const campaignApprovals = state.approvals.filter((approval) => approval.campaignId === campaign.id)
                          ...
                          return (
                            <button
                              ...
                              className={`grid w-full grid-cols-[44px_1.45fr_90px_120px_120px_120px_110px_110px_110px_110px_85px_85px_95px] items-center border-b border-slate-100 px-4 py-4 text-left transition hover:bg-blue-50/40 ${
                            ...
                            >
                              ...
                              <div className="text-lg">{managementChannelIcon(campaign)}</div>
                              <div className="text-xs font-black text-slate-700">{campaign.campaignType || "—"}</div>
                              <div className="text-xs font-black text-slate-700">{audienceType}</div>
                              <div className="truncate text-xs font-black text-slate-700">{campaign.owner || "Unassigned"}</div>
                              <div><Pill tone="blue">{stageLabel[campaign.stage]}</Pill></div>
                              <div><Pill tone={status === "active" ? "emerald" : status === "paused" ? "amber" : status === "archived" ? "slate" : "blue"}>{status}</Pill></div>
                              <div><Pill tone={riskTone(campaign.risk)}>{riskLabel[campaign.risk]}</Pill></div>
                              <div className="text-xs font-black text-slate-700">{campaign.endDate || "—"}</div>
                              <div className="text-xs font-black text-slate-700">{mad(campaign.budgetMad)}</div>
                              <div className="text-xs font-black text-slate-700">{campaign.leads}</div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-black text-slate-400">{campaignApprovals.length}/{campaignTasks.length}</span>
                              </div>
                            </button>
                          )
                        }) : (
