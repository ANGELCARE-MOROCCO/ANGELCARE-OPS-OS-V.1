...
export default function CampaignLifecycleExecutionWorkspace() {
  ...
  function campaignTimelineDaysLeft(campaign: any) {
    const endMs = new Date(campaign?.endDate || isoToday()).getTime()
    return Math.max(0, Math.ceil((endMs - Date.now()) / 86400000))
  }

  function campaignTimelineProgress(campaign: any) {
    const startMs = new Date(campaign?.startDate || isoToday()).getTime()
    const endMs = new Date(campaign?.endDate || isoToday()).getTime()
    return Math.max(
      0,
      Math.min(100, Math.round(((Date.now() - startMs) / Math.max(1, endMs - startMs)) * 100))
    )
  }
                    ...
                    <div className="grid gap-4 p-5 xl:grid-cols-[minmax(0,1fr)_360px]">
                      <div className="max-h-[620px] overflow-auto pr-2">
                        <div className="space-y-3">
                          ...
                          className={`group relative grid cursor-pointer grid-cols-1 content-between gap-5 overflow-hidden rounded-[34px] border p-7 text-left shadow-[0_18px_50px_rgba(15,23,42,0.08)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_30px_80px_rgba(15,23,42,0.13)] ${
                          ...
                          <div className="relative grid h-14 w-14 place-items-center rounded-[22px] border border-white/80 bg-white/80 text-2xl font-black text-slate-500 shadow-sm">
                            ⋯
                                ...
                                <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100">
                                  <div className={`h-full rounded-full ${tone.bar}`} style={{ width: `${campaignTimelineProgress(campaign)}%` }} />
                                </div>
                              </button>
                            )
                          }) : (
