              ...
              <section id="execution-workspace-panel" className="space-y-5">
                ...
                <section className="grid gap-5 2xl:grid-cols-[1.15fr_0.85fr]">
                  <Card className="overflow-hidden p-0">
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-white p-5">
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="text-2xl font-black tracking-[-0.04em] text-slate-950">Channel Activation Mindmap</h3>
                          <span className="grid h-6 w-6 place-items-center rounded-full bg-slate-100 text-xs font-black text-slate-500">i</span>
                        </div>
                        <p className="mt-1 text-sm font-bold text-slate-500">Click a channel node to inspect live campaigns and execution data.</p>
                      </div>

                      <div className="flex items-center gap-2">
                        <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">View</span>
                        <select value={selectedChannel} onChange={(event) => setSelectedChannel(event.target.value as ChannelNodeName)} className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-black text-slate-700">
                          {channelNodes.map((channel) => <option key={channel.name} value={channel.name}>{channel.name}</option>)}
                        </select>
                      </div>
                    </div>

                    <div className="relative min-h-[600px] overflow-hidden bg-gradient-to-br from-white via-blue-50/20 to-white p-6">
                      <div className="absolute left-1/2 top-1/2 h-[330px] w-[330px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-blue-100 bg-blue-50/30" />
                      <div className="absolute left-1/2 top-1/2 h-[235px] w-[235px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-blue-200 bg-white shadow-[0_24px_70px_rgba(37,99,235,0.12)]" />
                      <div className="absolute left-1/2 top-1/2 grid h-[190px] w-[190px] -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-white text-center">
                        <div>
                          <div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br from-blue-600 to-emerald-500 text-white shadow-lg">AC</div>
                          <p className="text-2xl font-black tracking-[-0.05em] text-slate-950">ANGELCARE</p>
                          <p className="text-sm font-black text-slate-600">Campaign Network</p>
                        </div>
                      </div>

                      <div className="relative z-10 grid min-h-[560px] grid-cols-1 gap-4 lg:grid-cols-[1fr_260px_1fr]">
                        <div className="grid content-center gap-4">
                          {channelStats.slice(0, 5).map((channel) => {
                            const selectedNode = selectedChannel === channel.name
                            return (
                              <button
                                key={channel.name}
                                onClick={() => setSelectedChannel(channel.name)}
                                className={`flex items-center gap-3 rounded-[24px] border px-4 py-3 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg ${channelToneClasses(channel.tone, selectedNode)}`}
                              >
                                <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white text-lg font-black shadow-sm">{channel.icon}</span>
                                ...
                                <span className="rounded-full bg-white px-2 py-1 text-xs font-black">{channel.activeCampaigns.length}</span>
                              </button>
                            )
                          })}
                        </div>
                        ...
                        <div className="grid content-center gap-4">
                          {channelStats.slice(5).map((channel) => {
                            const selectedNode = selectedChannel === channel.name
                            return (
                              <button
                                key={channel.name}
                      ...
                      <div className="mt-4 flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-3">
                        {["Paid Media", "Owned Channels", "Earned Channels", "Partnerships", "Offline", "CRM & Data"].map((item) => (
