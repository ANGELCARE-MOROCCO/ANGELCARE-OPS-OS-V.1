...
import { logAuditAction } from '@refferq/lib/audit';
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get currency symbol
    const { getCurrencySymbol } = await import('@refferq/lib/currency');
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate with Zod
    const { success, data, error: validationError } = await import('@refferq/lib/validations').then(m => m.payoutSchema.safeParse(body));
    ...
    try {
      ...
      if (affiliateUser?.email) {
        const { emailService } = await import('@refferq/lib/email');
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Validate with Zod
    const { success, data, error: validationError } = await import('@refferq/lib/validations').then(m => m.payoutUpdateSchema.safeParse(body));
