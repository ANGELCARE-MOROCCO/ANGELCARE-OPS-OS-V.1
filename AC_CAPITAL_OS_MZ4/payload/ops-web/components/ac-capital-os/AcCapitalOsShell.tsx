...
export function AcCapitalOsShell({
  children,
  activeWorkspaceKey = 'qualification-engine',
  zipLabel = 'Mega ZIP 04',
      ...
      <aside className={styles.sidebar} aria-label="AC CAPITAL OS workspaces">
        ...
        <nav className={styles.workspaceNav}>
          {AC_CAPITAL_OS_WORKSPACES.map((workspace) => {
            ...
            const isReady = workspace.status === 'foundation-ready' || workspace.status === 'activated-mz2' || workspace.status === 'activated-mz3' || workspace.status === 'activated-mz4';
            return (
              <a
                key={workspace.key}
