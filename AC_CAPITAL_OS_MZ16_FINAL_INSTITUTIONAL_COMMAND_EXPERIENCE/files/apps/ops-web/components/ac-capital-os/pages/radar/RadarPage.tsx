...
export function RadarPage({ actor }: { actor: CapitalActor }) {
  ...
  async function runResearch() {
    ...
    await action.execute(async () => postEnvelope("/api/ac-capital-os/capital-radar/research/run", research), "Governed dry research completed without a live provider call.");
    await workspace.refresh();
  }
  ...
  const insights = [
    ...
    { label: "Research policy", value: "Dry-run only until live provider research is explicitly enabled" },
  ];
  ...
  return <AcCapitalShell actor={actor} workspaceKey="opportunity-radar" title="Opportunity Intelligence Radar" subtitle="A live, evidence-first capital signal room for Moroccan banks, grants, VC, angels, strategic partners, women-founder programs, impact funding and international routes." envelope={workspace.envelope} insights={insights} primaryAction="Create Opportunity" onPrimaryAction={() => setModal("create")}>
    {workspace.loading ? <LoadingState label="Scanning Capital Radar records…" /> : workspace.error ? <ErrorState message={workspace.error} onRetry={() => void workspace.refresh()} /> : <>
      <section className={styles.radarDeck}>
        ...
        <div className={styles.radarCommand}><span><RadioTower size={15} /> Controlled opportunity scan</span><h2>See the capital market as a field of evidence—not a list of promises.</h2><p>Each signal carries source confidence, deadline heat, region, capital type, eligibility clues and a controlled handoff state.</p><div className={styles.radarActions}><PrimaryButton onClick={() => setModal("research")}>Run Dry Research</PrimaryButton><SecondaryButton onClick={() => setModal("create")}>Manual Opportunity</SecondaryButton></div><div className={styles.coverage}><div><Globe2 size={18} /><strong>Morocco</strong><span>Banks · public finance · impact</span></div><div><Waves size={18} /><strong>Africa / MENA</strong><span>VC · grants · strategic partners</span></div><div><Sparkles size={18} /><strong>International</strong><span>Education · child quality · SaaS</span></div></div></div>
      </section>
    ...
    <Dialog open={modal === "research"} title="Run Governed Research Dry Test" eyebrow="AI Research Safety" onClose={() => { setModal(null); action.reset(); }} footer={<><SecondaryButton onClick={() => setModal(null)}>Cancel</SecondaryButton><PrimaryButton onClick={() => void runResearch()} disabled={action.state.phase === "submitting"}>Run dry test</PrimaryButton></>}><div className={styles.researchGuard}><AlertOctagon size={22} /><div><strong>No live provider call</strong><p>This validates the research workflow, source requirements and audit trail. It does not invent funders, sources or deadlines.</p></div></div><Field label="Research query"><textarea value={research.query} onChange={(event) => setResearch({ query: event.target.value })} /></Field><ActionFeedback phase={action.state.phase} message={action.state.message} /></Dialog>
