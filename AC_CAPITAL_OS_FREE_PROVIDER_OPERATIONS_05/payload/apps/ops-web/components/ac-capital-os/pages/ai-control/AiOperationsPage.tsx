...
export function AiOperationsPage({ actor }: { actor: CapitalActor }) {
  ...
  return <AcCapitalShell actor={actor} workspaceKey="ai-operations" title="AC Capital AI Operations" subtitle="Writable control plane for Tavily external research, OpenRouter free analysis, outbound agent frequencies, quotas, permissions, profiles, execution evidence and internal automation." envelope={envelope} insights={insights} primaryAction="Run Due Agents" onPrimaryAction={() => void runDueAgents()}>
    ...
    {snapshot && tab === "activity" ? <section className={styles.activitySection}>
      ...
      <div className={styles.panel}><h3>Execution runs</h3><div className={styles.tableWrap}><table><thead><tr><th>Started</th><th>Agent</th><th>Status</th><th>Providers</th><th>Actual model</th><th>Evidence</th><th>Error</th></tr></thead><tbody>{runs.slice(0,100).map((run) => <tr key={text(run.id)}><td>{formatDate(run.started_at || run.created_at)}</td><td>{text(run.agent_key)}</td><td><b className={statusTone(run.status)}>{text(run.status)}</b><small>{text(run.phase)}</small></td><td>Tavily → OpenRouter</td><td>{text(run.selected_analysis_model || "—")}</td><td>{formatNumber(run.sources_persisted || run.sources_returned)} sources · {formatNumber(run.opportunities_created)} opportunities · {formatNumber(run.duplicates_detected)} duplicates</td><td>{text(run.error_message || "—")}</td></tr>)}</tbody></table></div></div>
