...
import { executeAcCapitalAiControlAction, loadAcCapitalAiControlSnapshot } from "@/lib/ac-capital-os/server/free-provider-control";
...
export async function GET() {
  try {
    ...
    return Response.json(success(await loadAcCapitalAiControlSnapshot()));
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    if (!action) throw Object.assign(new Error("ACTION_REQUIRED"), { status: 400 });
    const data = await executeAcCapitalAiControlAction(action, body.payload || {}, actor);
