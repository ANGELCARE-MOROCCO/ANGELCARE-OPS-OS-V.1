...
export async function runDueAcCapitalAgents(actor: Actor) {
  ...
  const agentsResult = await supabase.from("ac_capital_ai_agents").select("*").eq("status", "active").in("trigger_mode", ["scheduled", "both"]).order("next_run_at", { ascending: true, nullsFirst: true }).limit(100);
  if (agentsResult.error) throw new Error(agentsResult.error.message);
