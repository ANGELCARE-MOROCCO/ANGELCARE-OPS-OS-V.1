...
export function AcCapitalOsShell({
  children,
  activeWorkspaceKey = 'doctrine-vault',
  zipLabel = 'Mega ZIP 06',
      ...
      <aside className={styles.sidebar} aria-label="AC CAPITAL OS workspaces">
        ...
        <nav className={styles.workspaceNav}>
          {AC_CAPITAL_OS_WORKSPACES.map((workspace) => {
            ...
            const isReady = workspace.status === 'foundation-ready' || workspace.status === 'activated-mz2' || workspace.status === 'activated-mz3' || workspace.status === 'activated-mz4' || workspace.status === 'activated-mz5' || workspace.status === 'activated-mz6';
            return (
              <a
                key={workspace.key}
