...
export async function GET() {
  try {
    ...
    const [opportunities, sources, researchRuns, handoffQueue, opportunityTags, rejections] = await Promise.all([
      ...
      readTable("ac_capital_radar_rejections", 200),
    ]);
    return Response.json(envelope({ opportunities, sources, researchRuns, handoffQueue, opportunityTags, rejections }));
