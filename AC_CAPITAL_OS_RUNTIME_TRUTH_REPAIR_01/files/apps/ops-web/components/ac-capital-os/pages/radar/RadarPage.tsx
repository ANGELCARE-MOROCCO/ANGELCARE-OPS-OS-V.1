...
export function RadarPage({ actor }: { actor: CapitalActor }) {
  ...
  async function runResearch() {
    ...
    await action.execute(async () => postEnvelope("/api/ac-capital-os/capital-radar/research/run", research), "Live governed Google-grounded research completed and persisted for human review.", { title: "Live grounded capital research", workspaceKey: "opportunity-radar", stage: "provider-grounding" });
    await workspace.refresh();
  }
  ...
  const insights = [
    ...
    { label: "Research policy", value: "Live Gemini Google Search · grounded sources · human review" },
  ...
  return <AcCapitalShell actor={actor} workspaceKey="opportunity-radar" title="Opportunity Intelligence Radar" subtitle="A live, evidence-first capital signal room for Moroccan banks, grants, VC, angels, strategic partners, women-founder programs, impact funding and international routes." envelope={workspace.envelope} insights={insights} primaryAction="Create Opportunity" onPrimaryAction={() => setModal("create")}>
    ...
    <Dialog open={modal === "research"} title="Run Live Governed Capital Research" eyebrow="Gemini Google Search Grounding" onClose={() => { setModal(null); action.reset(); }} footer={<><SecondaryButton onClick={() => setModal(null)}>Cancel</SecondaryButton><PrimaryButton onClick={() => void runResearch()} disabled={action.state.phase === "submitting"}>Run live research</PrimaryButton></>}><div className={styles.researchGuard}><AlertOctagon size={22} /><div><strong>Live provider call required</strong><p>The command succeeds only after a governed Gemini request returns Google Search grounding metadata. Source records, opportunity candidates and rejection reasons are persisted; qualification and submission remain human-controlled.</p></div></div><Field label="Research query"><textarea value={research.query} onChange={(event) => setResearch({ query: event.target.value })} /></Field><ActionFeedback phase={action.state.phase} message={action.state.message} /></Dialog>
