import { ArrowRight, CheckCircle2, Clock3, LockKeyhole, ShieldAlert } from "lucide-react";
...
export function SectionHeading({ eyebrow, title, copy, action }: { eyebrow: string; title: string; copy?: string; action?: React.ReactNode }) {
  return <div className={styles.sectionHeading}><div><span>{eyebrow}</span><h2>{title}</h2>{copy ? <p>{copy}</p> : null}</div>{action ? <div>{action}</div> : null}</div>;
}
