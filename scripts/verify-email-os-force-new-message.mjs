...
for (const marker of ["New Message", "Generate with AI", "EnterpriseComposeModal", "InboxActionToolbar", "setComposeOpen(true)"]) { const ok = all.includes(marker); console.log(`${ok ? "✓" : "✗"} ${marker}`); if (!ok) failed = true }
if (all.includes("Enterprise Compose")) { console.log("✗ old Enterprise Compose string still exists"); failed = true }
