...
select
  t.*,
  coalesce(p.name, p.company, rp.organization, rp.name, t.entity_id::text) as linked_entity_name,
  coalesce(p.city, rp.city, 'Unassigned') as entity_city,
  coalesce(p.priority, rp.status, 'medium') as entity_priority,
  coalesce(p.stage, rp.stage, 'unknown') as entity_stage,
  coalesce(p.value_mad, rp.value_mad, rp.potential_value_mad, 0) as entity_value_mad,
  coalesce(p.contact_name, rp.contact_name) as entity_contact,
  coalesce(p.phone, rp.phone) as entity_phone,
  coalesce(p.email, rp.email) as entity_email
from public.revenue_tasks t
left join public.revenue_prospects p
  on p.id::text = coalesce(t.prospect_id::text, case when t.entity_type = 'prospect' then t.entity_id::text end)
left join public.revenue_partnerships rp
  on rp.id::text = coalesce(t.partnership_id::text, case when t.entity_type = 'partnership' then t.entity_id::text end);

create or replace view public.revenue_appointment_command_view as
select
  a.*,
  coalesce(p.name, p.company, rp.organization, rp.name, a.entity_id::text) as linked_entity_name,
  coalesce(p.city, rp.city, 'Unassigned') as entity_city,
  coalesce(p.priority, rp.status, 'medium') as entity_priority,
  coalesce(p.stage, rp.stage, 'unknown') as entity_stage,
  coalesce(p.value_mad, rp.value_mad, rp.potential_value_mad, 0) as entity_value_mad,
  ...
  coalesce(p.phone, rp.phone) as entity_phone,
  coalesce(p.email, rp.email) as entity_email,
  (select count(*) from public.revenue_tasks t where t.entity_id::text = a.entity_id::text and t.entity_type = a.entity_type) as linked_task_count
from public.revenue_appointments a
left join public.revenue_prospects p
  on p.id::text = coalesce(a.prospect_id::text, case when a.entity_type = 'prospect' then a.entity_id::text end)
left join public.revenue_partnerships rp
  on rp.id::text = coalesce(a.partnership_id::text, case when a.entity_type = 'partnership' then a.entity_id::text end);

create or replace view public.revenue_activity_events as
select
  id,
  entity_type,
  entity_id::text as entity_id,
  prospect_id::text as prospect_id,
  partnership_id::text as partnership_id,
  event_type,
  coalesce(title, event_title, 'Revenue activity') as title,
  coalesce(event_title, title, 'Revenue activity') as event_title,
  coalesce(body, event_body) as body,
  coalesce(event_body, body) as event_body,
  actor,
  severity,
  metadata,
  created_at
from public.revenue_activities;

create or replace view public.revenue_command_center_live_counts as
select
  (select count(*) from public.revenue_prospects where coalesce(status,'active') <> 'archived' and archived_at is null) as total_prospects,
  (select count(*) from public.revenue_prospects where coalesce(status,'active') not in ('archived','closed_lost','lost') and archived_at is null) as active_prospects,
  (select count(*) from public.revenue_partnerships where coalesce(status,'active') <> 'archived' and archived_at is null) as active_partners,
  (select count(*) from public.revenue_tasks where coalesce(status,'open') not in ('done','completed','cancelled','canceled','archived') and archived_at is null) as open_tasks,
  (select count(*) from public.revenue_tasks where coalesce(status,'open') not in ('done','completed','cancelled','canceled','archived') and due_date < now() and archived_at is null) as overdue_tasks,
  (select count(*) from public.revenue_appointments where appointment_at::date = current_date and coalesce(status,'scheduled') not in ('cancelled','canceled','archived')) as today_appointments,
  (select count(*) from public.revenue_follow_ups where coalesce(status,'pending') in ('pending','open','scheduled') and archived_at is null) as pending_follow_ups,
  (select coalesce(sum(value_mad),0) from public.revenue_prospects where coalesce(status,'active') <> 'archived' and archived_at is null) as pipeline_value_mad,
  now() as generated_at;
